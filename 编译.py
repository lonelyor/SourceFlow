...
def has_broken_node_modules_symlinks(modules_dir: Path) -> bool:
    """Detect dangling symlinks under a node_modules directory.

    pnpm's virtual store links may be absolute. When the project directory is
    moved or copied elsewhere, those absolute links break even though the
    package.json fingerprint (content-based, path-independent) is unchanged.
    Catching dangling links here lets the build self-heal by reinstalling
    instead of failing later with a cryptic "Cannot find module" error.
    """
    if not modules_dir.is_dir():
        return False
    max_check = 4000
    checked = 0
    for root, dirs, files in os.walk(modules_dir, followlinks=False):
...
def app_dependencies_look_current() -> bool:
    ...
    if has_broken_node_modules_symlinks(APP_DIR / "node_modules"):
        print(
            "Dangling symlinks detected under app/node_modules; the project "
            "directory was likely moved or copied. Reinstalling dependencies.",
            flush=True,
        )
        return False
