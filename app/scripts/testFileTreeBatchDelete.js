...
// 批量分支必须同时收集 notebookIds 与 paths，而非仅按 data-path !== "/" 过滤
assert(
    deleteFile.includes("const notebookIds: string[] = [];"),
    "deleteFiles batch branch must collect notebookIds separately from paths"
);
...
assert(
    deleteFile.includes("const totalCount = paths.length + notebookIds.length;") &&
...
// doc 删除仅在 paths 非空时调用，避免空数组请求
const removeDocsIdx = deleteFile.indexOf('fetchPost("/api/filetree/removeDocs"');
const removeDocsGuardIdx = deleteFile.indexOf("if (paths.length > 0)");
...
assert(
    ...
    "removeDocs call must be guarded by paths.length > 0"
);
