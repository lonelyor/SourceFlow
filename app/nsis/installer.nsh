...
!macro preInit
    ${IfNot} ${AtLeastWin10}
        MessageBox MB_ICONEXCLAMATION "非常抱歉，源流无法在低于 Windows 10 的系统上进行安装$\n$\n\
            Sorry, SourceFlow cannot be installed on systems below Windows 10$\n"
        Quit
    ${EndIf}

    nsExec::Exec 'TASKKILL /F /IM "SourceFlow.exe"'
    nsExec::Exec 'TASKKILL /F /IM "SourceFlow-Kernel.exe"'
!macroend
...
!macro customInstall
    RMDir /r "$PROFILE\AppData\Local\sourceflow-updater"
!macroend

!macro customUnInstall
    ${IfNot} ${isUpdated}
        MessageBox MB_YESNO "是否需要彻底删除全局配置（$PROFILE\.config\sourceflow\）？$\n$\n\
            Do you want to delete the global configuration ($PROFILE\.config\sourceflow\)?$\n" \
            /SD IDYES IDYES AcceptedRMConf IDNO SkippedRMConf
            AcceptedRMConf:
                RMDir /r "$PROFILE\.config\sourceflow\"
            SkippedRMConf:
    ${EndIf}

    ${IfNot} ${isUpdated}
        MessageBox MB_YESNO "是否需要彻底删除默认工作空间（$PROFILE\SourceFlow\）？$\n$\n\
            Do you want to completely delete the default workspace ($PROFILE\SourceFlow\)?$\n" \
            /SD IDNO IDYES AcceptedRMWorkspace IDNO SkippedRMWrokspace
            AcceptedRMWorkspace:
                RMDir /r "$PROFILE\SourceFlow\"
            SkippedRMWrokspace:
    ${EndIf}
