  ...
  updateUIState(state, previous, matchesCount) {
    ...
    switch (state) {
      ...
      case FindState.NOT_FOUND:
        // NOTE
        findMsgId = window.sourceflow.languages.find_not_found
        // findMsgId = "pdfjs-find-not-found";
        status = "notFound";
        break;
      case FindState.WRAPPED:
        // NOTE
        findMsgId = window.sourceflow.languages.find_not_found[`find_reached_${previous ? 'top' : 'bottom'}`]
        // findMsgId = previous
  ...
  updateResultsCount({ current = 0, total = 0 } = {}) {
    ...
    if (total > 0) {
      ...
      findResultsCount.textContent = total > limit ?
          window.sourceflow.languages.find_match_count_limit.replace('{{limit}}', limit) :
          window.sourceflow.languages.find_match_count.replace('{{current}}', current).replace('{{total}}', total);
