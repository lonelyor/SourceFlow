...
export const initAnno = (element: HTMLElement, pdf: any) => {
    ...
    pdfConfig.mainContainer.addEventListener("mousedown", (event: MouseEvent) => {
        ...
        documentSelf.onmouseup = () => {
            ...
            const coords = getHightlightCoordsByRect(pdf, window.sourceflow.storage[Constants.LOCAL_PDFTHEME].annoColor || "var(--b3-pdf-background1)", rectResizeElement,
    ...
    element.firstElementChild.addEventListener("click", (event: MouseEvent) => {
        ...
        if (typeof event.detail === "string") {
            window.sourceflow.storage[Constants.LOCAL_PDFTHEME].annoColor = event.detail === "0" ?
                (window.sourceflow.storage[Constants.LOCAL_PDFTHEME].annoColor || "var(--b3-pdf-background1)")
            ...
            const coords = getHightlightCoordsByRange(pdf, window.sourceflow.storage[Constants.LOCAL_PDFTHEME].annoColor);
            if (coords) {
                coords.forEach((item, index) => {
                    const newElement = showHighlight(item, pdf);
                    if (index === 0) {
        ...
        while (target && !target.classList.contains("pdf__outer")) {
            ...
            if (target.classList.contains("color__square")) {
                ...
                window.sourceflow.storage[Constants.LOCAL_PDFTHEME].annoColor = color;
                setStorageVal(Constants.LOCAL_PDFTHEME, window.sourceflow.storage[Constants.LOCAL_PDFTHEME]);
                if (rectElement) {
...
const getRelationHTML = (ids: string[]) => {
    if (!ids) {
        return `<li class="b3-list--empty">${window.sourceflow.languages.emptyContent}</li>`;
    }
    let html = "";
    ...
    <span data-type="clear" class="b3-tooltips b3-tooltips__w b3-list-item__action" aria-label="${window.sourceflow.languages.delete}">
...
const setRelation = (pdf: any) => {
    ...
    const dialog = new Dialog({
        title: window.sourceflow.languages.relation,
        content: `<div class="b3-dialog__content">
    <div class="fn__flex">
        <input class="b3-text-field fn__flex-1" placeholder="${window.sourceflow.languages.fileAnnoRefPlaceholder}">
        <div class="fn__space"></div>
        <button class="b3-button b3-button--text" data-type="add">${window.sourceflow.languages.addAttr}</button>
    </div>
    ...
    const addRelation = () => {
        if (/\d{14}-\w{7}/.test(inputElement.value)) {
            ...
            showMessage("ID " + window.sourceflow.languages.invalid);
        }
    };
...
const getHightlightCoordsByRange = (pdf: any, color: string) => {
    ...
    // https://github.com/lonelyor/SourceFlow/issues/5213
        ...
        if (item.tagName === "BR" && item.previousElementSibling && item.nextElementSibling) {
            ...
            if (/^[A-Za-z]$/.test(previousText.substring(previousText.length - 2, previousText.length - 1)) &&
                ...
                if (previousText.endsWith("-")) {
                    ...
                    // 中文情况不能添加 https://github.com/lonelyor/SourceFlow/issues/8152
...
const showHighlight = (selected: IPdfAnno, pdf: any, hl?: boolean) => {
    ...
    const viewport = page.viewport.clone({rotation: 0}); // rotation https://github.com/lonelyor/SourceFlow/issues/9831
            ...
            getRectImgData(pdf).then((imageData) => {
                fetch(imageData.url).then((response) => {
                    ...
                    if (Constants.SIZE_UPLOAD_TIP_SIZE <= blob.size) {
                        msg = window.sourceflow.languages.uploadFileTooLarge.replace("${x}", content + ".png").replace("${y}", filesize(blob.size, {standard: "iec"}));
                    }
