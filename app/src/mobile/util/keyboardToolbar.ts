...
import {MIND_ELIXIR_SLASH_VALUE} from "../../protyle/render/mindmapConstants";
...
export const renderTextMenu = (protyle: IProtyle, toolbarElement: Element) => {
    ...
    <span class="keyboard__slash-text">${window.sourceflow.languages.colorFont} ${item ? index + 1 : window.sourceflow.languages.default}</span>
</button>`;
    });
    let bgHTML = "";
    ...
    <span class="keyboard__slash-text">${window.sourceflow.languages.colorPrimary} ${item ? index + 1 : window.sourceflow.languages.default}</span>
</button>`;
    });
    ...
    let lastColorHTML = "";
    const lastFonts = window.sourceflow.storage[Constants.LOCAL_FONTSTYLES];
    if (lastFonts.length > 0) {
        lastColorHTML = `<div data-id="lastUsed" class="keyboard__slash-title">
    ${window.sourceflow.languages.lastUsed}
</div>
<div data-id="lastUsedWrap" class="keyboard__slash-block">`;
    ...
    <span class="keyboard__slash-text">${window.sourceflow.languages.colorFont} ${lastFontStatus[1] ? parseInt(lastFontStatus[1].replace("var(--b3-font-color", "")) + 1 : window.sourceflow.languages.default}</span>
</button>`;
                    break;
                case "backgroundColor":
                    lastColorHTML += `<button class="keyboard__slash-item" data-type="${lastFontStatus[0]}">
    <span class="keyboard__slash-icon" ${lastFontStatus[1] ? `style="background-color:${lastFontStatus[1]}"` : ""}>A</span>
    <span class="keyboard__slash-text">${window.sourceflow.languages.colorPrimary} ${lastFontStatus[1] ? parseInt(lastFontStatus[1].replace("var(--b3-font-background", "")) + 1 : window.sourceflow.languages.default}</span>
</button>`;
                    break;
                case "style2":
                    lastColorHTML += `<button class="keyboard__slash-item" data-type="${lastFontStatus[0]}">
    <span class="keyboard__slash-text" style="-webkit-text-stroke: 0.2px var(--b3-theme-on-background);-webkit-text-fill-color : transparent;">${window.sourceflow.languages.hollow}</span>
</button>`;
                    break;
                case "style4":
                    lastColorHTML += `<button class="keyboard__slash-item" data-type="${lastFontStatus[0]}">
    <span class="keyboard__slash-text" style="text-shadow: 1px 1px var(--b3-theme-surface-lighter), 2px 2px var(--b3-theme-surface-lighter), 3px 3px var(--b3-theme-surface-lighter), 4px 4px var(--b3-theme-surface-lighter)">${window.sourceflow.languages.shadow}</span>
</button>`;
                    break;
                case "fontSize":
                    if (!disableFont) {
    ...
    <span class="keyboard__slash-text">${window.sourceflow.languages[lastFontStatus[2].replace("var(--b3-card-", "").replace("-color)", "") + "Style"]}</span>
</button>`;
                    } else {
                        lastColorHTML += `<button class="keyboard__slash-item" data-type="${lastFontStatus[0]}">
    <span class="keyboard__slash-icon">A</span>
    <span class="keyboard__slash-text">${window.sourceflow.languages.color} ${window.sourceflow.languages.default}</span>
</button>`;
                    }
                    break;
                case "clear":
                    lastColorHTML += `<button class="keyboard__slash-item" data-type="${lastFontStatus[0]}">
    <span class="keyboard__slash-text">${window.sourceflow.languages.clearFontStyle}</span>
</button>`;
                    break;
            }
        });
...
<div data-id="color" class="keyboard__slash-title">${window.sourceflow.languages.color}</div>
<div data-id="colorWrap" class="keyboard__slash-block">
    ...
    <button class="keyboard__slash-item" data-type="style1">
        ...
        <span class="keyboard__slash-text">${window.sourceflow.languages.errorStyle}</span>
    </button>
    <button class="keyboard__slash-item" data-type="style1">
        ...
        <span class="keyboard__slash-text">${window.sourceflow.languages.warningStyle}</span>
    </button>
    <button class="keyboard__slash-item" data-type="style1">
        <span class="keyboard__slash-icon" style="color: var(--b3-card-info-color);background-color: var(--b3-card-info-background);">A</span>
        <span class="keyboard__slash-text">${window.sourceflow.languages.infoStyle}</span>
    </button>
    <button class="keyboard__slash-item" data-type="style1">
        ...
        <span class="keyboard__slash-text">${window.sourceflow.languages.successStyle}</span>
    </button>
</div>
<div data-id="colorFont" class="keyboard__slash-title">${window.sourceflow.languages.colorFont}</div>
<div data-id="colorFontWrap" class="keyboard__slash-block">
    ${colorHTML}
</div>
<div data-id="colorPrimary" class="keyboard__slash-title">${window.sourceflow.languages.colorPrimary}</div>
<div data-id="colorPrimaryWrap" class="keyboard__slash-block">
    ${bgHTML}
</div>
<div data-id="fontStyle" class="keyboard__slash-title">${window.sourceflow.languages.fontStyle}</div>
...
<div data-id="fontSize" class="keyboard__slash-title${disableFont ? " fn__none" : ""}">${window.sourceflow.languages.fontSize}</div>
...
<div class="keyboard__slash-block">
    ${getSlashItem(Constants.ZWSP, "iconMarkdown", window.sourceflow.languages.template)}
    ${getSlashItem(Constants.ZWSP + 1, "iconBoth", window.sourceflow.languages.widget)}
    ${getSlashItem(Constants.ZWSP + 2, "iconImage", window.sourceflow.languages.assets)}
    ${getSlashItem("((", "iconRef", window.sourceflow.languages.ref, "true")}
    ${getSlashItem("{{", "iconSQL", window.sourceflow.languages.blockEmbed, "true")}
    ${getSlashItem(Constants.ZWSP + 5, "iconSparkles", window.sourceflow.languages.aiWriting)}
    ${getSlashItem('<div data-type="NodeAttributeView" data-av-type="table"></div>', "iconDatabase", window.sourceflow.languages.database, "true")}
    ${getSlashItem(Constants.ZWSP + 4, "iconFile", window.sourceflow.languages.newSubDocRef)}
</div>
<div class="keyboard__slash-title"></div>
<div class="keyboard__slash-block">
    ${getSlashItem(Constants.ZWSP + 3, "iconDownload", window.sourceflow.languages.insertAsset + '<input class="b3-form__upload" type="file"' + (protyle.options.upload.accept ? (' multiple="' + protyle.options.upload.accept + '"') : "") + "/>", "true")}
    ${isInAndroid() ? getSlashItem(Constants.ZWSP + 3, "iconCamera", window.sourceflow.languages.insertPhoto + '<input class="b3-form__upload" capture="user" type="file"' + (protyle.options.upload.accept ? (' multiple="' + protyle.options.upload.accept + '"') : "") + "/>", "true") : ""}
    ${getSlashItem('<iframe sandbox="allow-forms allow-presentation allow-same-origin allow-scripts allow-modals allow-popups" src="" border="0" frameborder="no" framespacing="0" allowfullscreen="true"></iframe>', "iconLanguage", window.sourceflow.languages.insertIframeURL, "true")}
    ${getSlashItem("![]()", "iconImage", window.sourceflow.languages.insertImgURL, "true")}
    ${getSlashItem('<video controls="controls" src=""></video>', "iconVideo", window.sourceflow.languages.insertVideoURL, "true")}
    ${getSlashItem('<audio controls="controls" src=""></audio>', "iconRecord", window.sourceflow.languages.insertAudioURL, "true")}
    ${getSlashItem("emoji", "iconEmoji", window.sourceflow.languages.emoji, "true")}
</div>
<div class="keyboard__slash-title"></div>
<div class="keyboard__slash-block">
    ${getSlashItem("# " + Lute.Caret, "iconH1", window.sourceflow.languages.heading1, "true")}
    ${getSlashItem("## " + Lute.Caret, "iconH2", window.sourceflow.languages.heading2, "true")}
    ${getSlashItem("### " + Lute.Caret, "iconH3", window.sourceflow.languages.heading3, "true")}
    ${getSlashItem("#### " + Lute.Caret, "iconH4", window.sourceflow.languages.heading4, "true")}
    ${getSlashItem("##### " + Lute.Caret, "iconH5", window.sourceflow.languages.heading5, "true")}
    ${getSlashItem("###### " + Lute.Caret, "iconH6", window.sourceflow.languages.heading6, "true")}
    ${getSlashItem("- " + Lute.Caret, "iconList", window.sourceflow.languages.list, "true")}
    ${getSlashItem("1. " + Lute.Caret, "iconOrderedList", window.sourceflow.languages["ordered-list"], "true")}
    ${getSlashItem("- [ ] " + Lute.Caret, "iconCheck", window.sourceflow.languages.check, "true")}
    ${getSlashItem("> " + Lute.Caret, "iconQuote", window.sourceflow.languages.quote, "true")}
    ${getSlashItem(`> [!NOTE]\n> ${Lute.Caret}`, '<span class="keyboard__slash-icon">✏️</span>', `${window.sourceflow.languages.callout} - <span style="color: var(--b3-callout-note)">Note</span>`, "true")}
    ${getSlashItem(`> [!TIP]\n> ${Lute.Caret}`, '<span class="keyboard__slash-icon">💡</span>', `${window.sourceflow.languages.callout} - <span style="color: var(--b3-callout-tip)">Tip</span>`, "true")}
    ${getSlashItem(`> [!IMPORTANT]\n> ${Lute.Caret}`, '<span class="keyboard__slash-icon">❗</span>', `${window.sourceflow.languages.callout} - <span style="color: var(--b3-callout-important)">Important</span>`, "true")}
    ${getSlashItem(`> [!WARNING]\n> ${Lute.Caret}`, '<span class="keyboard__slash-icon">⚠️</span>', `${window.sourceflow.languages.callout} - <span style="color: var(--b3-callout-warning)">Warning</span>`, "true")}
    ${getSlashItem(`> [!CAUTION]\n> ${Lute.Caret}`, '<span class="keyboard__slash-icon">🚨</span>', `${window.sourceflow.languages.callout} - <span style="color: var(--b3-callout-caution)">Caution</span>`, "true")}
    ${getSlashItem("```", "iconCode", window.sourceflow.languages.code, "true")}
    ${getSlashItem(`| ${Lute.Caret} |  |  |\n| --- | --- | --- |\n|  |  |  |\n|  |  |  |`, "iconTable", window.sourceflow.languages.table, "true")}
    ${getSlashItem("---", "iconLine", window.sourceflow.languages.line, "true")}
    ${getSlashItem("$$", "iconMath", window.sourceflow.languages.math)}
    ${getSlashItem("<div>", "iconHTML5", "HTML")}
</div>
<div class="keyboard__slash-title"></div>
<div class="keyboard__slash-block">
    ${getSlashItem("```abc\n```", "", window.sourceflow.languages.staff, "true")}
    ${getSlashItem("```echarts\n```", "", window.sourceflow.languages.chart, "true")}
    ${getSlashItem("```flowchart\n```", "", "Flow Chart", "true")}
    ${getSlashItem("```graphviz\n```", "", "Graph", "true")}
    ${getSlashItem("```mermaid\n```", "", "Mermaid", "true")}
    ${getSlashItem(MIND_ELIXIR_SLASH_VALUE, "", `${window.sourceflow.languages.mindmap} · Mind Elixir`, "true")}
    ${getSlashItem("```plantuml\n```", "", "UML", "true")}
</div>
<div class="keyboard__slash-title"></div>
<div class="keyboard__slash-block">
    ${getSlashItem(`style${Constants.ZWSP}color: var(--b3-card-info-color);background-color: var(--b3-card-info-background);`, '<div style="color: var(--b3-card-info-color);background-color: var(--b3-card-info-background);" class="keyboard__slash-icon">A</div>', window.sourceflow.languages.infoStyle, "true")}
    ${getSlashItem(`style${Constants.ZWSP}color: var(--b3-card-success-color);background-color: var(--b3-card-success-background);`, '<div style="color: var(--b3-card-success-color);background-color: var(--b3-card-success-background);" class="keyboard__slash-icon">A</div>', window.sourceflow.languages.successStyle, "true")}
    ${getSlashItem(`style${Constants.ZWSP}color: var(--b3-card-warning-color);background-color: var(--b3-card-warning-background);`, '<div style="color: var(--b3-card-warning-color);background-color: var(--b3-card-warning-background);" class="keyboard__slash-icon">A</div>', window.sourceflow.languages.warningStyle, "true")}
    ${getSlashItem(`style${Constants.ZWSP}color: var(--b3-card-error-color);background-color: var(--b3-card-error-background);`, '<div style="color: var(--b3-card-error-color);background-color: var(--b3-card-error-background);" class="keyboard__slash-icon">A</div>', window.sourceflow.languages.errorStyle, "true")}
    ${getSlashItem(`style${Constants.ZWSP}`, '<div class="keyboard__slash-icon">A</div>', window.sourceflow.languages.clearFontStyle, "true")}
</div>${pluginHTML}`;
...
export const showKeyboardToolbarUtil = (oldScrollTop: number) => {
    ...
    if (window.sourceflow.mobile.size.isLandscape) {
        if (window.sourceflow.mobile.size.landscape.height1 !== window.sourceflow.mobile.size.landscape.height2) {
            keyboardHeight = window.sourceflow.mobile.size.landscape.height1 - window.sourceflow.mobile.size.landscape.height2 + toolHeight;
        }
    } else {
        if (window.sourceflow.mobile.size.portrait.height1 !== window.sourceflow.mobile.size.portrait.height2) {
            keyboardHeight = window.sourceflow.mobile.size.portrait.height1 - window.sourceflow.mobile.size.portrait.height2 + toolHeight;
        }
    }
...
export const showKeyboardToolbar = () => {
    ...
    toolbarElement.style.zIndex = (++window.sourceflow.zIndex).toString();
...
export const initKeyboardToolbar = () => {
    ...
    window.sourceflow.mobile.size.isLandscape = window.matchMedia && window.matchMedia("(orientation: landscape)").matches;
    if (window.sourceflow.mobile.size.isLandscape) {
        window.sourceflow.mobile.size.landscape = {
            height1: window.innerHeight,
            height2: window.innerHeight,
        };
    } else {
        window.sourceflow.mobile.size.portrait = {
            height1: window.innerHeight,
            height2: window.innerHeight,
        };
    }
    if (!isInEdge()) {
        window.addEventListener("resize", () => {
            ...
            window.sourceflow.mobile.size.isLandscape = window.matchMedia && window.matchMedia("(orientation: landscape)").matches;
            if (window.sourceflow.mobile.size.isLandscape) {
                if (!window.sourceflow.mobile.size.landscape) {
                    window.sourceflow.mobile.size.landscape = {
                        height1: window.innerHeight,
                        height2: window.innerHeight,
                    };
                }
                if (window.innerHeight < window.sourceflow.mobile.size.landscape.height1 - 100) {
                    window.sourceflow.mobile.size.landscape.height2 = window.innerHeight;
                }
                if (window.innerHeight > window.sourceflow.mobile.size.landscape.height1) {
                    window.sourceflow.mobile.size.landscape.height1 = window.innerHeight;
                }
                if (window.sourceflow.mobile.size.landscape.height2 < window.innerHeight) {
                    activeBlur();
                } else if (!preventRender) {
                    renderKeyboardToolbar();
                }
            } else {
                if (!window.sourceflow.mobile.size.portrait) {
                    window.sourceflow.mobile.size.portrait = {
                        height1: window.innerHeight,
                        height2: window.innerHeight,
                    };
                }
                if (window.innerHeight < window.sourceflow.mobile.size.portrait.height1 - 100) {
                    window.sourceflow.mobile.size.portrait.height2 = window.innerHeight;
                }
                if (window.innerHeight > window.sourceflow.mobile.size.portrait.height1) {
                    window.sourceflow.mobile.size.portrait.height1 = window.innerHeight;
                }
                if (window.sourceflow.mobile.size.portrait.height2 < window.innerHeight) {
                    activeBlur();
                } else if (!preventRender) {
                    renderKeyboardToolbar();
                }
...
<div class="keyboard__util"></div>`;
    ...
    toolbarElement.addEventListener(isInAndroid() || isInHarmony() ? "touchend" : "click", (event) => {
        ...
        if (window.sourceflow.config.readonly || !protyle || protyle.disabled) {
            return;
        }
        if (type === "undo") {
