...
export const mobileKeydown = (app: App, event: KeyboardEvent) => {
    ...
    const matchGeneral = Object.keys(window.sourceflow.config.keymap.general).find((key) => {
        if (matchHotKey(window.sourceflow.config.keymap.general[key].custom, event)) {
