...
import {getRecentUpdatedBlocksCached} from "../../search/cache";
            ...
            menu.addItem({
                ...
                click() {
                    if (custom?.type === dockId) {
                        ...
                        window.sourceflow.mobile.docks[dockId] = custom;
                    }
                }
            });
        });
...
export const initFramework = (app: App, isStart: boolean) => {
        ...
        firstToolbarElement.querySelectorAll(".toolbar__icon").forEach(item => {
            ...
            if (itemType === type) {
                if (type === "sidebar-outline-tab") {
                    if (!window.sourceflow.mobile.docks.outline) {
                        window.sourceflow.mobile.docks.outline = new MobileOutline({
                            app,
                            blockId: window.sourceflow.mobile.editor?.protyle.block.rootID,
                            isPreview: window.sourceflow.mobile.editor ? !window.sourceflow.mobile.editor.protyle.preview.element.classList.contains("fn__none") : false
                        });
                    } else {
                        fetchPost("/api/outline/getDocOutline", {
                            id: window.sourceflow.mobile.editor.protyle.block.rootID,
                            preview: window.sourceflow.mobile.editor.protyle.preview.element.classList.contains("fn__none")
                        }, response => {
                            window.sourceflow.mobile.docks.outline.update(response);
                        });
                    }
                } else if (type === "sidebar-backlink-tab") {
                    if (!window.sourceflow.mobile.docks.backlink) {
                        window.sourceflow.mobile.docks.backlink = new MobileBacklinks(app);
                    } else {
                        window.sourceflow.mobile.docks.backlink.update();
                    }
                } else if (type === "sidebar-bookmark-tab") {
                    if (!window.sourceflow.mobile.docks.bookmark) {
                        window.sourceflow.mobile.docks.bookmark = new MobileBookmarks(app);
                    } else {
                        window.sourceflow.mobile.docks.bookmark.update();
                    }
                } else if (type === "sidebar-tag-tab") {
                    if (!window.sourceflow.mobile.docks.tag) {
                        window.sourceflow.mobile.docks.tag = new MobileTags(app);
                    } else {
                        window.sourceflow.mobile.docks.tag.update();
                    }
                } else if (type === "sidebar-plugin-tab") {
                    if (!custom) {
                        tabPanelElement.innerHTML = `<div class="b3-list--empty">${window.sourceflow.languages.emptyContent}</div>`;
                        openDockMenu(app);
    ...
    window.sourceflow.mobile.docks.file = new MobileFiles(app);
    ...
    if (getOpenNotebookCount() > 0) {
        ...
        if (window.sourceflow.config.fileTree.closeTabsOnStart && isStart) {
            setEmpty(app);
            return;
        }
        const localDoc = window.sourceflow.storage[Constants.LOCAL_DOCINFO];
...
const initEditorName = () => {
    ...
    inputElement.setAttribute("placeholder", window.sourceflow.languages._kernel[16]);
    inputElement.addEventListener("blur", () => {
        ...
        fetchPost("/api/filetree/renameDoc", {
            notebook: window.sourceflow.mobile.editor.protyle.notebookId,
            path: window.sourceflow.mobile.editor.protyle.path,
