...
export const goBack = () => {
    ...
    if (window.sourceflow.menus.menu.element.classList.contains("b3-menu--fullscreen") &&
        !window.sourceflow.menus.menu.element.classList.contains("fn__none")) {
        window.sourceflow.menus.menu.element.dispatchEvent(new CustomEvent("click", {detail: "back"}));
        return;
        ...
        return;
    } else if (window.sourceflow.viewer && !window.sourceflow.viewer.destroyed) {
        ...
        closePanel();
        return;
    } else if (window.sourceflow.dialogs.length !== 0) {
        hideElements(["dialog"]);
        closePanel();
        return;
    }
    if ((window.JSAndroid || window.JSHarmony) && window.sourceflow.backStack.length < 1) {
        if (document.querySelector('#message [data-id="exitTip"]')) {
            ...
            showMessage(window.sourceflow.languages.returnDesktop, 3000, "info", "exitTip");
        }
        return;
    }
    ...
    const item = window.sourceflow.backStack.pop();
