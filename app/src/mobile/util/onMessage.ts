        ...
        switch (data.cmd) {
            ...
            case "backgroundtask":
                if (!document.querySelector("#keyboardToolbar").classList.contains("fn__none") ||
                    window.sourceflow.config.appearance.hideStatusBar) {
                    return;
                }
...
const setPublish = () => {
    const accessElement = window.sourceflow.mobile.docks.file.element.previousElementSibling.querySelector('[data-type="publish-access"]');
    if (!window.sourceflow.config.publish.enable) {
        ...
        window.sourceflow.mobile.docks.file.element.querySelectorAll(".b3-list-item__icon").forEach(item => {
