...
export const handleTouchEnd = (event: TouchEvent, app: App) => {
    ...
    if (typeof yDiff === "undefined" && window.sourceflow.mobile.editor.protyle.options.render.gutter) {
        ...
        if (nodeElement) {
            ...
            if (embedElement) {
                window.sourceflow.mobile.editor.protyle.gutter.render(window.sourceflow.mobile.editor.protyle, embedElement);
                return;
            }
            window.sourceflow.mobile.editor.protyle.gutter.render(window.sourceflow.mobile.editor.protyle, nodeElement, target);
        }
    }
    isFirstMove = true;
    if (!clientY || typeof yDiff === "undefined" ||
        ...
        (window.sourceflow.mobile.editor && !window.sourceflow.mobile.editor.protyle.toolbar.subElement.classList.contains("fn__none")) ||
    ...
    if (window.sourceflow.mobile.editor) {
        window.sourceflow.mobile.editor.protyle.contentElement.style.overflow = "";
    }
...
export const handleTouchMove = (event: TouchEvent) => {
    ...
    if (getSelection().rangeCount > 0) {
        ...
        if (range.toString() !== "" && window.sourceflow.mobile.editor.protyle.wysiwyg.element.contains(range.startContainer)) {
            return;
        }
    }
    ...
    if (Math.abs(xDiff) > Math.abs(yDiff)) {
        ...
        if (isFirstMove) {
            sideMaskElement.style.zIndex = (++window.sourceflow.zIndex).toString();
            document.getElementById("sidebar").style.zIndex = (++window.sourceflow.zIndex).toString();
            document.getElementById("menu").style.zIndex = (++window.sourceflow.zIndex).toString();
            isFirstMove = false;
        }
        ...
        activeBlur();
        if (window.sourceflow.mobile.editor) {
            window.sourceflow.mobile.editor.protyle.contentElement.style.overflow = "hidden";
        }
    }
};
