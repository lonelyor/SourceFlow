...
export class MobileBacklinks {
    ...
    <div class="toolbar__text">
        ${window.sourceflow.languages.backlinks}
    </div>
...
<div class="toolbar">
    <div class="fn__space"></div>
    <div class="toolbar__text">
        ${window.sourceflow.languages.mentions}
    </div>
...
<div class="backlinkMList fn__flex-1"></div>`;
                        ...
                        case "layout":
                            if (this.mTree.element.style.flex) {
                                if (this.mTree.element.style.height === "0px") {
                                    ...
                                    target.setAttribute("aria-label", window.sourceflow.languages.up);
                                    target.querySelector("use").setAttribute("xlink:href", "#iconUp");
                                } else {
                                    this.mTree.element.removeAttribute("style");
                                    target.setAttribute("aria-label", window.sourceflow.languages.down);
                                    target.querySelector("use").setAttribute("xlink:href", "#iconDown");
                                }
                            } else {
                                if (target.getAttribute("aria-label") === window.sourceflow.languages.down) {
    ...
    public update() {
        fetchPost("/api/ref/getBacklink", {
            id: window.sourceflow.mobile.editor.protyle.block.id,
