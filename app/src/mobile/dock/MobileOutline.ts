...
export class MobileOutline extends Model {
    ...
    <div class="toolbar__text">
        ${window.sourceflow.languages.outline}
    </div>
    ...
    <svg data-type="keepCurrentExpand" class="toolbar__icon${window.sourceflow.storage[Constants.LOCAL_OUTLINE].keepCurrentExpand ? " toolbar__icon--active" : ""}"><use xlink:href="#iconFocus"></use></svg>
...
<div class="fn__flex-1" style="padding: 3px 0 8px"></div>`;
        ...
        this.element.querySelector('[data-type="keepCurrentExpand"]').addEventListener("click", (event: MouseEvent & {
            ...
            if (iconElement.classList.contains("toolbar__icon--active")) {
                ...
                let focusElement;
                const blockElement = hasClosestBlock(window.sourceflow.mobile.editor.protyle.toolbar.range?.startContainer);
                if (blockElement) {
    ...
    private showExpandLevelMenu() {
        ...
        window.sourceflow.menus.menu.fullscreen("bottom");
    ...
    private getProtyleAndBlockElement(element: HTMLElement) {
        const id = element.getAttribute("data-node-id");
        if (!window.sourceflow.mobile.editor?.protyle) {
            return;
        }
        const blockElement = window.sourceflow.mobile.editor.protyle.wysiwyg.element.querySelector(`[data-node-id="${id}"]`);
        if (!blockElement) {
            return;
        }
        return {
            protyle: window.sourceflow.mobile.editor.protyle, blockElement
        };
    }
    ...
    private genHeadingTransform(id: string, level: number) {
        return {
            ...
            click: () => {
                const protyle = window.sourceflow.mobile.editor?.protyle;
                if (!protyle) {
                    return;
                }
