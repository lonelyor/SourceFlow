...
export class MobileFiles extends Model {
    ...
    <div class="toolbar__text">${window.sourceflow.languages.fileTree}</div>
    ...
    <svg data-type="publish-access" class="toolbar__icon${window.sourceflow.config.readonly || !window.sourceflow.config.publish.enable ? " fn__none" : ""}"><use xlink:href="#iconEye"></use></svg>
    <svg data-type="sort" class="toolbar__icon${window.sourceflow.config.readonly ? " fn__none" : ""}"><use xlink:href="#iconSort"></use></svg>
</div>
...
<ul class="b3-list b3-list--background fn__flex-column" style="min-height: auto;height:42px;transition: height .2s cubic-bezier(0, 0, .2, 1) 0ms">
            ...
            while (target && !target.isEqualNode(this.actionsElement)) {
                ...
                if (type === "refresh") {
                    ...
                    if (window.sourceflow.mobile.editor) {
                        this.selectItem(window.sourceflow.mobile.editor.protyle.notebookId, window.sourceflow.mobile.editor.protyle.path);
                    }
                    event.preventDefault();
                    break;
    ...
    private genSort() {
        ...
        subMenu.forEach((item) => {
            window.sourceflow.menus.menu.append(new MenuItem(item).element);
        });
        window.sourceflow.menus.menu.fullscreen("bottom");
    }
    ...
    <span data-type="open" data-url="${item.id}" class="b3-list-item__action${(window.sourceflow.config.readonly) ? " fn__none" : ""}">
...
<li class="b3-list-item" data-type="navigation-root" data-path="/">
    ...
    <span data-type="more-root" class="b3-list-item__action${(window.sourceflow.config.readonly || item.closed) ? " fn__none" : ""}">
        <svg><use xlink:href="#iconMore"></use></svg>
    </span>
    <span data-type="new" class="b3-list-item__action${(window.sourceflow.config.readonly || item.closed) ? " fn__none" : ""}">
    ...
    public init(init = true) {
        ...
        window.sourceflow.storage[Constants.LOCAL_FILESPATHS].forEach((item: IFilesPath) => {
    ...
    private onMove(data: {
        ...
        if (sourceElement) {
            ...
            const parentElement = this.element.querySelector(`ul[data-url="${data.fromNotebook}"] li[data-path="${pathPosix().dirname(data.fromPath)}.sf"]`) as HTMLElement;
...
class="b3-list-item" data-path="${item.path}">
    ...
    <span class="b3-list-item__icon"${editingPublishAccess ? " fn__none" : ""}>${unicode2Emoji(item.icon || (item.subFileCount === 0 ? window.sourceflow.storage[Constants.LOCAL_IMAGES].file : window.sourceflow.storage[Constants.LOCAL_IMAGES].folder))}</span>
