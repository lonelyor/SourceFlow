...
<label class="fn__flex b3-label">
    <div class="fn__flex-1">
        ${window.sourceflow.languages.publishService}
        <div class="b3-label__text">${window.sourceflow.languages.publishServiceTip}</div>
    </div>
    <span class="fn__space"></span>
    <input class="b3-switch fn__flex-center" id="publishEnable" type="checkbox"${window.sourceflow.config.publish.enable ? " checked" : ""}/>
</label>
<div class="b3-label">
    ${(() => {
            if (mobile) {
                return `
${window.sourceflow.languages.publishServicePort}
<span class="fn__hr"></span>
<input class="b3-text-field fn__block" id="publishPort" type="number" min="0" max="65535" value="${window.sourceflow.config.publish.port}">
<div class="b3-label__text">${window.sourceflow.languages.publishServicePortTip}</div>`;
            } else {
                return `
<div class="fn__flex">
    <div class="fn__flex-1">
    ...
    <span class="fn__space"></span>
    <input class="b3-text-field fn__flex-center fn__size200" id="publishPort" type="number" min="0" max="65535" value="${window.sourceflow.config.publish.port}">
</div>`;
            }
        })()}
</div>
<div class="b3-label">
    <div class="fn__flex">
        <div class="fn__flex-1">
            ${window.sourceflow.languages.publishServiceAddresses}
            <div class="b3-label__text">${window.sourceflow.languages.publishServiceAddressesTip}</div>
        </div>
...
<div class="b3-label">
    <label class="fn__flex">
        <div class="fn__flex-1">
            ${window.sourceflow.languages.publishServiceAuth}
            <div class="b3-label__text">${window.sourceflow.languages.publishServiceAuthTip}</div>
        </div>
        <span class="fn__space"></span>
        <input class="b3-switch fn__flex-center" id="publishAuthEnable" type="checkbox"${window.sourceflow.config.publish.auth.enable ? " checked" : ""}/>
    </label>
</div>
<div class="b3-label">
    ${(() => {
            if (mobile) {
                return `
${window.sourceflow.languages.publishServiceAuthAccounts}
<div class="b3-label__text">${window.sourceflow.languages.publishServiceAuthAccountsTip}</div>
<div class="b3-label b3-label--inner fn__flex">
    <span class="fn__flex-1"></span>
    <button class="b3-button b3-button--outline fn__size200 fn__flex-center" id="publishAuthAccountAdd">
        <svg><use xlink:href="#iconAdd"></use></svg>${window.sourceflow.languages.publishServiceAuthAccountAdd}
    </button>
</div>`;
            } else {
                return `
...
`;
    ...
    bindEvent: () => {
        ...
        publishAuthAccountAdd.addEventListener("click", () => {
            window.sourceflow.config.publish.auth.accounts.push({
                username: "",
                password: "",
                memo: "",
            });
    ...
    _savePublish: (reloadAccounts = true) => {
        ...
        fetchPost("/api/setting/setPublish", {
            ...
            auth: {
                ...
                accounts: window.sourceflow.config.publish.auth.accounts,
            },
    ...
    updatePublishConfig: (
        ...
        if (response.code === 0) {
            window.sourceflow.config.publish = response.data.publish;
            if (reloadAccounts) {
                publish._renderPublishAuthAccounts(publish.element);
            }
    ...
    _renderPublishAuthAccounts: (
        element: Element,
        accounts: Config.IPublishAuthAccount[] = window.sourceflow.config.publish.auth.accounts,
    ) => {
        const mobile = isMobile();
...
<li class="b3-label b3-label--inner fn__flex" data-index="${index}">
    <input class="b3-text-field fn__block" data-name="username" value="${account.username}" placeholder="${window.sourceflow.languages.userName}">
    <span class="fn__space"></span>
    <div class="b3-form__icona fn__block">
        <input class="b3-text-field fn__block b3-form__icona-input" type="password" data-name="password" value="${account.password}" placeholder="${window.sourceflow.languages.password}">
        <svg class="b3-form__icona-icon" data-action="togglePassword"><use xlink:href="#iconEye"></use></svg>
    </div>
    <span class="fn__space"></span>
    <input class="b3-text-field fn__block" data-name="memo" value="${account.memo}" placeholder="${window.sourceflow.languages.memo}">
    <span class="fn__space"></span>
    ${(() => {
                    if (mobile) {
                        return `
<button class="b3-button b3-button--outline fn__block" data-action="remove">
    <svg><use xlink:href="#iconTrashcan"></use></svg>${window.sourceflow.languages.delete}
</button>`;
                    } else {
                        return `
...
`)
            ...
            .forEach(input => {
                input.addEventListener("change", () => {
                    ...
                    if (li) {
                        ...
                        if (name in window.sourceflow.config.publish.auth.accounts[index]) {
                            window.sourceflow.config.publish.auth.accounts[index][name] = input.value;
        ...
        publishAuthAccounts
            ...
            .forEach(remove => {
                remove.addEventListener("click", () => {
                    ...
                    if (li) {
                        const index = parseInt(li.dataset.index);
                        window.sourceflow.config.publish.auth.accounts.splice(index, 1);
                        publish._savePublish();
                    }
                });
            });
    ...
    _renderPublishAddressList: (
        ...
        if (port === 0) {
            publishAddresses.innerText = window.sourceflow.languages.publishServiceNotStarted;
        } else {
            publishAddresses.innerHTML = `<div class="b3-label__text">${
                window.sourceflow.config.serverAddrs
                    .map(serverAddr => {
