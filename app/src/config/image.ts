...
export const image = {
    ...
    <div class="layout-tab-bar fn__flex">
        <div class="item item--full item--focus" data-type="remove">
            ...
            <span class="item__text">${window.sourceflow.languages.unreferencedAssets}</span>
            <span class="fn__flex-1"></span>
        </div>
        <div class="item item--full" data-type="removeAV">
            <span class="fn__flex-1"></span>
                <span class="item__text">${window.sourceflow.languages.unreferencedAV}</span>
            <span class="fn__flex-1"></span>
        </div>
        <div class="item item--full" data-type="missing">
            <span class="fn__flex-1"></span>
            <span class="item__text">${window.sourceflow.languages.missingAssets}</span>
            <span class="fn__flex-1"></span>
        </div>
    </div>
        ...
        <div class="config-assets${isM ? " b3-list--mobile" : ""}" data-type="remove" data-init="true">
            ...
            <div class="fn__flex">
                ...
                <button id="removeAll" class="b3-button b3-button--outline fn__flex-center fn__size200">
                    ...
                    ${window.sourceflow.languages.delete}
                </button>
            </div>
        ...
        image.element.addEventListener("click", (event) => {
            ...
            while (target && !target.isEqualNode(image.element)) {
                ...
                if (target.id === "removeAll") {
                    confirmDialog(window.sourceflow.languages.deleteOpConfirm, `${window.sourceflow.languages.clearAll}`, () => {
                        fetchPost("/api/asset/removeUnusedAssets", {}, response => {
                            ...
                            /// #endif
                            assetsListElement.innerHTML = `<li class="b3-list--empty">${window.sourceflow.languages.emptyContent}</li>`;
                    ...
                    confirmDialog(window.sourceflow.languages.deleteOpConfirm, `${window.sourceflow.languages.clearAllAV}`, () => {
                        fetchPost("/api/av/removeUnusedAttributeViews", {}, () => {
                            avListElement.innerHTML = `<li class="b3-list--empty">${window.sourceflow.languages.emptyContent}</li>`;
                    ...
                    /// #if !BROWSER
                    if (target.parentElement.getAttribute("data-tab-type") === "unRefAV") {
                        openBy(path.join(window.sourceflow.config.system.dataDir, "storage", "av", target.parentElement.dataset.item) + ".json", "folder");
                    } else {
                    ...
                    confirmDialog(window.sourceflow.languages.deleteOpConfirm, `${window.sourceflow.languages.delete} <b>${liElement.querySelector(".b3-list-item__text").textContent}</b>`, () => {
                        if (liElement.getAttribute("data-tab-type") === "unRefAV") {
                            ...
                            fetchPost("/api/av/removeUnusedAttributeView", {
                                ...
                                if (liElement.parentElement.querySelectorAll("li").length === 1) {
                                    liElement.parentElement.innerHTML = `<li class="b3-list--empty">${window.sourceflow.languages.emptyContent}</li>`;
                                } else {
                                    liElement.remove();
                                }
    ...
    _renderList: (data: {
        ...
        if (!isBrowser() && type !== "lostAssets") {
            boxOpenHTML = `<span data-type="open" class="ariaLabel b3-list-item__action" aria-label="${window.sourceflow.languages.showInFolder}">
    <svg><use xlink:href="#iconFolder"></use></svg>
</span>`;
        }
        let boxClearHTML = "";
        if (type !== "lostAssets") {
            boxClearHTML = `<span data-type="clear" class="ariaLabel b3-list-item__action" aria-label="${window.sourceflow.languages.delete}">
    ...
    <span data-type="copy" class="ariaLabel b3-list-item__action" aria-label="${type === "unRefAV" ? window.sourceflow.languages.copyMirror : window.sourceflow.languages.copy}">
    ...
    ${boxClearHTML}
        ...
        element.innerHTML = html || `<li class="b3-list--empty">${window.sourceflow.languages.emptyContent}</li>`;
    }
};
