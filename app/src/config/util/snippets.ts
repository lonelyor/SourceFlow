...
export const renderSnippet = () => {
    fetchPost("/api/snippet/getSnippet", {type: "all", enabled: 2}, (response) => {
        response.data.snippets.forEach((item: ISnippet) => {
            ...
            if ((!window.sourceflow.config.snippet.enabledCSS && item.type === "css") ||
                (!window.sourceflow.config.snippet.enabledJS && item.type === "js")) {
                if (exitElement) {
                    exitElement.remove();
                }
                return;
    ...
    <div>
        <div class="fn__flex">
            <div class="b3-form__icon fn__flex-1">
                ...
                <input data-type="css" data-action="search" type="text" placeholder="${window.sourceflow.languages.search}" class="b3-text-field b3-form__icon-input fn__block">
            </div>
            <div class="fn__space"></div>
            <span aria-label="${window.sourceflow.languages.addAttr} CSS" id="addCodeSnippetCSS" class="b3-tooltips b3-tooltips__sw block__icon block__icon--show">
                <svg><use xlink:href="#iconAdd"></use></svg>
            </span>
            <div class="fn__space"></div>
            <input data-action="toggleCSS" class="b3-switch fn__flex-center" type="checkbox"${window.sourceflow.config.snippet.enabledCSS ? " checked" : ""}>
        </div>
        ${cssHTML}
    </div>
    <div class="fn__none">
        <div class="fn__flex">
             <div class="b3-form__icon fn__flex-1">
                <svg class="b3-form__icon-icon"><use xlink:href="#iconSearch"></use></svg>
                <input data-type="js" data-action="search" type="text" placeholder="${window.sourceflow.languages.search}" class="b3-text-field b3-form__icon-input fn__block">
            </div>
            <div class="fn__space"></div>
            <span aria-label="${window.sourceflow.languages.addAttr} JS" id="addCodeSnippetJS" class="b3-tooltips b3-tooltips__sw block__icon block__icon--show">
                <svg><use xlink:href="#iconAdd"></use></svg>
            </span>
            <div class="fn__space"></div>
            <input data-action="toggleJS" class="b3-switch fn__flex-center" type="checkbox"${window.sourceflow.config.snippet.enabledJS ? " checked" : ""}>
        </div>
        ${jsHTML}
    </div>
</div>
...
const genSnippet = (options: ISnippet) => {
    ...
    <div class="fn__flex">
        <input type="text" class="fn__size200 b3-text-field" placeholder="${window.sourceflow.languages.title}">
        <div class="fn__space"></div>
        <label class="fn__flex${window.sourceflow.config.publish.enable ? "" : " fn__none"}">
            <input data-type="disabledInPublish" type="checkbox" class="b3-switch fn__flex-center" ${options.disabledInPublish ? "" : " checked"}>
            <div class="fn__space"></div>
            <span class="fn__flex-center">${window.sourceflow.languages.publishService}</span>
        </label>
        <div class="fn__flex-1"></div>
        <div class="fn__space"></div>
        <span aria-label="${window.sourceflow.languages.remove}" data-action="remove" class="b3-tooltips b3-tooltips__sw block__icon block__icon--show">
    ...
    <textarea class="fn__block b3-text-field" placeholder="${window.sourceflow.languages.codeSnippet}" style="resize: vertical;font-family:var(--b3-font-family-code)" spellcheck="false"></textarea>
    <div class="fn__hr--b"></div>
</div>`;
};
...
const setSnippetPost = (dialog: Dialog, snippets: ISnippet[], removeIds: string[]) => {
    fetchPost("/api/snippet/setSnippet", {snippets}, () => {
        ...
        window.sourceflow.config.snippet.enabledCSS = (dialog.element.querySelector('.b3-switch[data-action="toggleCSS"]') as HTMLInputElement).checked;
        window.sourceflow.config.snippet.enabledJS = (dialog.element.querySelector('.b3-switch[data-action="toggleJS"]') as HTMLInputElement).checked;
        fetchPost("/api/setting/setSnippet", window.sourceflow.config.snippet);
        dialog.destroy({cancel: "true"});
    });
};
...
const setSnippet = (dialog: Dialog, oldSnippets: ISnippet[], removeIds: string[], confirm = false) => {
    ...
    if (objEquals(oldSnippets, snippets) &&
        ...
        if (confirm) {
            confirmDialog(window.sourceflow.languages.save, window.sourceflow.languages.snippetsTip, () => {
