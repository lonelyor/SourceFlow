...
export const globalClickHideMenu = (element: HTMLElement) => {
    if (!window.sourceflow.menus.menu.element.contains(element) && !hasClosestByAttribute(element, "data-menu", "true")) {
        if (getSelection().rangeCount > 0 && window.sourceflow.menus.menu.element.contains(getSelection().getRangeAt(0).startContainer) &&
            window.sourceflow.menus.menu.element.contains(document.activeElement)) {
            //
        } else {
...
export const globalClick = (event: MouseEvent & { target: HTMLElement }) => {
    ...
    if (!isWindow() && window.sourceflow.layout.leftDock &&
