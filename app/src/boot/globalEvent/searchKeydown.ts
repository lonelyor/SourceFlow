...
export const searchKeydown = (app: App, event: KeyboardEvent) => {
    ...
    if (searchType === "doc" && matchHotKey(window.sourceflow.config.keymap.general.newFile.custom, event)) {
        if (config.method === 0) {
    ...
    const assetLocal = window.sourceflow.storage[Constants.LOCAL_SEARCHASSET] as ISearchAssetOption;
    ...
    if (searchType !== "asset") {
        if (matchHotKey(window.sourceflow.config.keymap.editor.general.insertRight.custom, event)) {
            openSearchEditor({
        ...
        if (matchHotKey(window.sourceflow.config.keymap.editor.general.copyBlockRef.custom, event)) {
            fetchPost("/api/block/getRefText", {id}, (response) => {
                writeText(`((${id} '${response.data}'))`);
            });
            return true;
        }
        if (matchHotKey(window.sourceflow.config.keymap.editor.general.copyBlockEmbed.custom, event)) {
            writeText(`{{select * from blocks where id='${id}'}}`);
            return true;
        }
        if (matchHotKey(window.sourceflow.config.keymap.editor.general.copyProtocol.custom, event)) {
            writeText(`sf://blocks/${id}`);
            return true;
        }
        if (matchHotKey(window.sourceflow.config.keymap.editor.general.copyProtocolInMd.custom, event)) {
            fetchPost("/api/block/getRefText", {id}, (response) => {
                writeText(`[${response.data}](sf://blocks/${id})`);
            });
            return true;
        }
        if (matchHotKey(window.sourceflow.config.keymap.editor.general.copyHPath.custom, event)) {
        ...
        if (matchHotKey(window.sourceflow.config.keymap.editor.general.copyID.custom, event)) {
            writeText(id);
            return true;
        }
    }
    ...
    if (event.key === "Enter") {
        if (searchType !== "asset") {
            ...
            /// #if !BROWSER
            useShell("showItemInFolder", path.join(window.sourceflow.config.system.dataDir, currentList.lastElementChild.getAttribute("aria-label")));
            /// #endif
        }
        return true;
    }
