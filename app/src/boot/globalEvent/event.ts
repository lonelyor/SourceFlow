...
export const initWindowEvent = (app: App) => {
    ...
    window.addEventListener("dragover", (event: DragEvent & { target: HTMLElement }) => {
        ...
        const fileElement = hasClosestByClassName(event.target, "sf__file");
        ...
        if (scrollTarget && protyleElement && (
            scrollTarget.classList.contains("sf__file") || protyleElement !== scrollTarget
        )) {
        ...
        if (hasClosestByClassName(event.target, "layout-tab-container__drag") ||
            event.dataTransfer.types.includes(Constants.SOURCEFLOW_DROP_TAB)) {
            stopScrollAnimation();
            return;
        }
        let scrollElement;
        if (scrollTarget && scrollTarget.classList.contains("sf__file")) {
        ...
        if (scrollTarget && scrollElement) {
            if ((event.dataTransfer.types.includes(Constants.SOURCEFLOW_DROP_FILE) && hasClosestByClassName(event.target, "layout-tab-bar")) ||
                (event.dataTransfer.types.includes("Files") && scrollTarget.classList.contains("sf__file")) ||
    ...
    document.addEventListener("touchstart", (event) => {
        ...
        // https://github.com/lonelyor/SourceFlow/issues/6328
    ...
    document.addEventListener("touchend", (event) => {
        if (isIPad()) {
            // https://github.com/lonelyor/SourceFlow/issues/9113
