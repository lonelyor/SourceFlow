...
export const windowMouseMove = (event: MouseEvent, mouseIsEnter: boolean) => {
    ...
    // https://github.com/lonelyor/SourceFlow/pull/8793
    const coordinates = window.sourceflow.coordinates ?? (window.sourceflow.coordinates = {
        pageX: 0,
        pageY: 0,
        clientX: 0,
        clientY: 0,
    ...
    // breadcrumb
    if (window.sourceflow.hideBreadcrumb) {
        window.sourceflow.hideBreadcrumb = false;
    ...
    if (!mouseIsEnter &&
        ...
        window.sourceflow.layout.bottomDock &&
        !isWindow()) {
        const unifiedActivityBar = document.body.classList.contains("body--activitybar-unified");
        if (event.clientX < Math.max(document.getElementById("dockLeft").clientWidth + 1, 16)) {
            if (!window.sourceflow.layout.leftDock.pin && window.sourceflow.layout.leftDock.layout.element.clientWidth > 0 &&
                // 隐藏停靠栏会导致点击两侧内容触发浮动面板弹出，因此需减小鼠标范围
                (window.sourceflow.layout.leftDock.element.clientWidth > 0 || (window.sourceflow.layout.leftDock.element.clientWidth === 0 && event.clientX < 8))) {
                if (event.clientY > document.getElementById("toolbar").clientHeight &&
                    ...
                    if (!hasClosestByClassName(target, "b3-menu") &&
                        ...
                        window.sourceflow.layout.leftDock.showDock();
                    }
                } else {
                    window.sourceflow.layout.leftDock.hideDock();
                }
            }
        } else if (!unifiedActivityBar && event.clientX > window.innerWidth - Math.max(document.getElementById("dockRight").clientWidth - 2, 16)) {
            if (!window.sourceflow.layout.rightDock.pin && window.sourceflow.layout.rightDock.layout.element.clientWidth > 0 &&
                (window.sourceflow.layout.rightDock.element.clientWidth > 0 || (window.sourceflow.layout.rightDock.element.clientWidth === 0 && event.clientX > window.innerWidth - 8))) {
                if (event.clientY > document.getElementById("toolbar").clientHeight &&
                    event.clientY < window.innerHeight - document.getElementById("status").clientHeight - document.getElementById("dockBottom").clientHeight) {
                    if (!hasClosestByClassName(target, "b3-menu") &&
                        !hasClosestByClassName(target, "layout--float") &&
                        ...
                        !hasClosestByClassName(target, "protyle-util") &&
                        !hasClosestByClassName(target, "b3-dialog", true)) {
                        window.sourceflow.layout.rightDock.showDock();
                    }
                } else {
        ...
        if (!unifiedActivityBar && event.clientY > Math.min(window.innerHeight - 10, window.innerHeight - (window.sourceflow.config.uiLayout.hideDock ? 0 : document.getElementById("dockBottom").clientHeight) - document.querySelector("#status").clientHeight)) {
            window.sourceflow.layout.bottomDock.showDock();
        }
    }
    ...
    if (eventPath0 && eventPath0.nodeType !== 3 && eventPath0.classList.contains("av")) {
        // 数据库居中时光标在数据库侧边 https://github.com/lonelyor/SourceFlow/issues/13853
