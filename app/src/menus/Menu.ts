...
export class Menu {
    ...
    constructor() {
        ...
        this.element.querySelector(".b3-menu__title .b3-menu__label").innerHTML = window.sourceflow.languages.back;
    ...
    public remove(isKeyEvent = false) {
        if (isKeyEvent) {
            const subElements = window.sourceflow.menus.menu.element.querySelectorAll(".b3-menu__item--show");
        ...
        if (window.sourceflow.menus.menu.removeCB) {
            window.sourceflow.menus.menu.removeCB();
            window.sourceflow.menus.menu.removeCB = undefined;
        }
...
const getActionMenu = (element: Element, next: boolean) => {
    ...
    while (actionMenuElement &&
        (actionMenuElement.classList.contains("b3-menu__separator") ||
            ...
            // https://github.com/lonelyor/SourceFlow/issues/12518
...
export const bindMenuKeydown = (event: KeyboardEvent) => {
    ...
    if (window.sourceflow.menus.menu.element.contains(target) && ["INPUT", "TEXTAREA"].includes(target.tagName)) {
        return false;
    }
    const eventCode = Constants.KEYCODELIST[event.keyCode];
    if (eventCode === "↓" || eventCode === "↑") {
        const currentElement = window.sourceflow.menus.menu.element.querySelector(".b3-menu__item--current");
        let actionMenuElement;
        if (!currentElement) {
            if (eventCode === "↑") {
                actionMenuElement = getActionMenu(window.sourceflow.menus.menu.element.lastElementChild.lastElementChild, false);
            } else {
                actionMenuElement = getActionMenu(window.sourceflow.menus.menu.element.lastElementChild.firstElementChild, true);
            }
        } else {
        ...
        window.sourceflow.menus.menu.showSubMenu(subMenuElement);
        return true;
    } else if (eventCode === "←") {
        const currentElement = window.sourceflow.menus.menu.element.querySelector(".b3-menu__submenu .b3-menu__item--current");
        if (!currentElement) {
            return true;
        }
        ...
        if (!currentElement) {
            ...
            if (window.sourceflow.menus.menu.element.contains(currentElement)) {
