    ...
    constructor(app: App) {
        ...
        window.addEventListener("contextmenu", (event) => {
            ...
            if (target.classList.contains("b3-text-field") || (target.tagName === "INPUT" && (target as HTMLInputElement).type === "text")) {
                /// #if !BROWSER
                ipcRenderer.send(Constants.SOURCEFLOW_CONTEXT_MENU, {
                    undo: window.sourceflow.languages.undo,
                    redo: window.sourceflow.languages.redo,
                    copy: window.sourceflow.languages.copy,
                    cut: window.sourceflow.languages.cut,
                    delete: window.sourceflow.languages.delete,
                    paste: window.sourceflow.languages.paste,
                    pasteAsPlainText: window.sourceflow.languages.pasteAsPlainText,
                    selectAll: window.sourceflow.languages.selectAll,
                });
                /// #endif
                event.stopPropagation();
            } else {
            ...
            && !target.parentElement.isEqualNode(document.querySelector("body"))) {
                ...
                if (dataType === "tab-header") {
                    ...
                    event.stopPropagation();
                    break;
                } else if (dataType === "navigation-root" && !window.sourceflow.config.readonly) {
                    ...
                    setPanelFocus(hasClosestByClassName(target, "sf__file") as HTMLElement);
                    event.stopPropagation();
                    break;
