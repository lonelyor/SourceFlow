...
export const openBookmarkMenu = (element: HTMLElement, event: MouseEvent, bookmarkObj: Bookmark | MobileBookmarks) => {
    if (!window.sourceflow.menus.menu.element.classList.contains("fn__none") &&
        window.sourceflow.menus.menu.element.getAttribute("data-name") === Constants.MENU_BOOKMARK) {
        window.sourceflow.menus.menu.remove();
        return;
    }
    window.sourceflow.menus.menu.remove();
    const id = element.getAttribute("data-node-id");
    if (!id && !window.sourceflow.config.readonly) {
...
<div class="b3-dialog__action">
    ...
    if (!window.sourceflow.config.readonly) {
        window.sourceflow.menus.menu.append(new MenuItem({
            ...
            click: () => {
                ...
                confirmDialog(window.sourceflow.languages.deleteOpConfirm, window.sourceflow.languages.removeBookmark.replace("${x}", `<b>${escapeHtml(bookmarkText)}</b>`), () => {
                    if (id) {
    ...
    window.sourceflow.menus.menu.element.setAttribute("data-name", Constants.MENU_BOOKMARK);
