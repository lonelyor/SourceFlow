...
const validateFile = (protyle: IProtyle, files: File[]) => {
    ...
    for (let iMax = files.length, i = 0; i < iMax; i++) {
        ...
        if (!file.name) {
            errorTip += `<li>${window.sourceflow.languages.nameEmpty}</li>`;
            validate = false;
        }

        if (file.size > protyle.options.upload.max) {
            errorTip += `<li>${file.name} ${window.sourceflow.languages.over} ${protyle.options.upload.max / 1024 / 1024}M</li>`;
            validate = false;
        }
        ...
        if (protyle.options.upload.accept) {
            ...
            if (!isAccept) {
                errorTip += `<li>${file.name} ${window.sourceflow.languages.fileTypeError}</li>`;
                validate = false;
            }
        }
        ...
        if (validate) {
            ...
            uploadingStr += `<li>${filename} ${window.sourceflow.languages.uploading}</li>`;
        }
    }
    let msgId;
...
const genUploadedLabel = async (responseText: string, protyle: IProtyle) => {
    ...
    if (response.data.errFiles && response.data.errFiles.length > 0) {
        ...
        response.data.errFiles.forEach((data: string) => {
            ...
            errorTip += `<li>${filename} ${window.sourceflow.languages.uploadError}</li>`;
        });
        errorTip += "</ul>";
    }
    ...
    // https://github.com/lonelyor/SourceFlow/issues/7624
    ...
    let successFileText = "";
    // 插入多个资源文件时按文件名自然升序排列 Use natural ascending order when inserting multiple assets https://github.com/lonelyor/SourceFlow/issues/14643
    ...
    keys.forEach((key, index) => {
        ...
        hasImage = Constants.SOURCEFLOW_ASSETS_IMAGE.includes(type);
        avAssets.push({
        ...
        if (!Constants.SOURCEFLOW_ASSETS_AUDIO.includes(type) && !Constants.SOURCEFLOW_ASSETS_VIDEO.includes(type) &&
            keys.length - 1 !== index) {
    ...
    // 避免插入代码块中，其次因为都要独立成块 https://github.com/lonelyor/SourceFlow/issues/7607
    insertHTML(successFileText, protyle, insertBlock);
    // 粘贴图片后定位不准确 https://github.com/lonelyor/SourceFlow/issues/13336
    setTimeout(() => {
...
export const uploadFiles = (protyle: IProtyle, files: FileList | DataTransferItemList | File[], element?: HTMLInputElement, successCB?: (res: string) => void) => {
    ...
    for (let i = 0, iMax = validateResult.files.length; i < iMax; i++) {
        ...
        if (Constants.SIZE_UPLOAD_TIP_SIZE <= validateResult.files[i].size) {
            msg += window.sourceflow.languages.uploadFileTooLarge.replace("${x}", validateResult.files[i].name).replace("${y}", filesize(validateResult.files[i].size, {standard: "iec"})) + "<br>";
        }
    }
        ...
        xhr.onreadystatechange = () => {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                ...
                if (xhr.status === 200) {
                    ...
                    showMessage(window.sourceflow.languages.fileTypeError);
                } else {
