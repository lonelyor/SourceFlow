...
export const openMenuPanel = (options: {
    ...
    <div class="b3-menu" ${["select", "date", "asset", "relation", "rollup"].includes(options.type) ? `style="${["select", "asset", "relation"].includes(options.type) ? "max-height: calc(100vh - 32px);display: flex;flex-direction: column;" : ""}min-width: 200px;${isMobile() ? "max-width: 90vw;" : "max-width: 50vw;"}"` : ""}>${html}</div>
        ...
        if (["select", "date", "asset", "relation", "rollup"].includes(options.type)) {
            ...
            if (!options.blockElement.contains(lastElement)) {
                // https://github.com/lonelyor/SourceFlow/issues/15839
    ...
    ${isTwoWay ? window.sourceflow.languages.confirmRemoveRelationField
                                    .replace("${x}", menuElement.querySelector("input").value || window.sourceflow.languages._kernel[272])
                                    .replace("${y}", menuElement.querySelector('.b3-menu__item[data-type="goSearchAV"] .b3-menu__accelerator').textContent)
                                    .replace("${z}", (menuElement.querySelector('input[data-type="colName"]') as HTMLInputElement).value || window.sourceflow.languages._kernel[272])
                                : window.sourceflow.languages.removeCol.replace("${x}", menuElement.querySelector("input").value || window.sourceflow.languages._kernel[272])}
    ...
    <button class="fn__block b3-button b3-button--cancel">${window.sourceflow.languages.cancel}</button>
                    ...
                    if (target.parentElement.dataset.type === "image") {
                        previewAttrViewImages(assetLink, avID, getAVViewAttr(options.blockElement),
                    ...
                    menuElement.querySelector('[data-type="hideGroups"]').innerHTML = `${window.sourceflow.languages[showCount === 0 ? "showAll" : "hideAll"]}
