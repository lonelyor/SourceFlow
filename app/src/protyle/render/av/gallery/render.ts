...
const getGalleryHTML = (data: IAVGallery) => {
    ...
    data.cards.forEach((item: IAVGalleryItem, rowIndex: number) => {
        ...
        item.values.forEach((cell, fieldsIndex) => {
            ...
            // NOTE: innerHTML 中不能换行否则 https://github.com/lonelyor/SourceFlow/issues/15132
...
${cell.color ? `color:${cell.color};` : ""}">${renderCell(cell.value, rowIndex, data.showIcon, "gallery")}</div>`;
    ...
    <div class="av__gallery-tip">
        ${data.fields[fieldsIndex].icon ? unicode2Emoji(data.fields[fieldsIndex].icon, undefined, true) : `<svg><use xlink:href="#${getColIconByType(data.fields[fieldsIndex].type)}"></use></svg>`}${window.sourceflow.languages.edit} ${Lute.EscapeHTMLStr(data.fields[fieldsIndex].name)}
    </div>
    ${cellHTML}
</div>`;
            }
    ...
    <div class="av__gallery-actions">
        <span class="protyle-icon protyle-icon--first ariaLabel" data-position="4north" aria-label="${window.sourceflow.languages.displayEmptyFields}" data-type="av-gallery-edit"><svg><use xlink:href="#iconEdit"></use></svg></span>
        <span class="protyle-icon protyle-icon--last ariaLabel" data-position="4north" aria-label="${window.sourceflow.languages.more}" data-type="av-gallery-more"><svg><use xlink:href="#iconMore"></use></svg></span>
    </div>
</div>`;
    });
    galleryHTML += `<div class="av__gallery-add" data-type="av-add-bottom"><svg class="svg"><use xlink:href="#iconAdd"></use></svg><span class="fn__space"></span>${window.sourceflow.languages.newRow}</div>`;
...
export const afterRenderGallery = (options: ITableOptions) => {
    ...
    options.resetData.editIds.find(selectId => {
        ...
        if (itemElement) {
            ...
            itemElement.querySelector('.protyle-icon[data-type="av-gallery-edit"]').setAttribute("aria-label", window.sourceflow.languages.hideEmptyFields);
        }
    });
