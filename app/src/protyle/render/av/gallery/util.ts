...
export const setGalleryCover = (options: {
    ...
    menu.addItem({
        ...
        label: window.sourceflow.languages.calcOperatorNone,
        click() {
            ...
            targetNameElement.textContent = window.sourceflow.languages.calcOperatorNone;
        }
    });
    menu.addItem({
        iconHTML: "",
        checked: options.view.coverFrom === 3,
        label: window.sourceflow.languages.contentBlock,
        click() {
            transaction(options.protyle, [{
                action: "setAttrViewCoverFrom",
                avID,
            ...
            targetNameElement.textContent = window.sourceflow.languages.contentBlock;
        }
    });
    menu.addItem({
        iconHTML: "",
        checked: options.view.coverFrom === 1,
        label: window.sourceflow.languages.contentImage,
        click() {
            transaction(options.protyle, [{
                action: "setAttrViewCoverFrom",
                avID,
            ...
            targetNameElement.textContent = window.sourceflow.languages.contentImage;
        }
    });
...
export const setGallerySize = (options: {
    ...
    const avID = options.nodeElement.getAttribute("data-av-id");
    const blockID = options.nodeElement.getAttribute("data-node-id");
    const viewID = getAVViewAttr(options.nodeElement);
    const targetNameElement = options.target.querySelector(".b3-menu__accelerator");
    menu.addItem({
        iconHTML: "",
        checked: options.view.cardSize === 0,
        label: window.sourceflow.languages.small,
        click() {
            transaction(options.protyle, [{
            ...
            targetNameElement.textContent = window.sourceflow.languages.small;
        }
    });
    menu.addItem({
        iconHTML: "",
        checked: options.view.cardSize === 1,
        label: window.sourceflow.languages.medium,
        click() {
            transaction(options.protyle, [{
                action: "setAttrViewCardSize",
                avID,
            ...
            targetNameElement.textContent = window.sourceflow.languages.medium;
        }
    });
    menu.addItem({
        iconHTML: "",
        checked: options.view.cardSize === 2,
        label: window.sourceflow.languages.large,
        click() {
            transaction(options.protyle, [{
                action: "setAttrViewCardSize",
                avID,
            ...
            targetNameElement.textContent = window.sourceflow.languages.large;
        }
    });
    const rect = options.target.getBoundingClientRect();
    menu.open({x: rect.left, y: rect.bottom});
...
export const editGalleryItem = (target: Element) => {
    ...
    if (itemElement) {
        ...
        if (fieldsElement) {
            target.setAttribute("aria-label", window.sourceflow.languages[fieldsElement.classList.contains("av__gallery-fields--edit") ? "displayEmptyFields" : "hideEmptyFields"]);
