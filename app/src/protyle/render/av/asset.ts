        ...
        uploadFiles(options.protyle, event.target.files, event.target, (res) => {
            ...
            Object.keys(resData.data.succMap).forEach((key) => {
                value.push({
                    ...
                    type: Constants.SOURCEFLOW_ASSETS_IMAGE.includes(pathPosix().extname(resData.data.succMap[key]).toLowerCase()) ? "image" : "file"
                });
            });
            updateAssetCell({
...
<svg class="b3-menu__action" data-type="editAssetItem"><use xlink:href="#iconEdit"></use></svg>
    ...
    <button data-type="addAssetExist" class="b3-menu__item b3-menu__item--current">
        ...
        <span class="b3-menu__label">${window.sourceflow.languages.assets}</span>
    </button>
    <button class="b3-menu__item">
        <svg class="b3-menu__icon"><use xlink:href="#iconDownload"></use></svg>
        <span class="b3-menu__label">${window.sourceflow.languages.insertAsset}</span> 
    ...
    <button data-type="addAssetLink" class="b3-menu__item">
        ...
        <span class="b3-menu__label">${window.sourceflow.languages.link}</span>
    </button>
</div>`;
};
...
export const editAssetItem = (options: {
    ...
    const openSubMenu = openMenu(options.protyle ? options.protyle.app : window.sourceflow.ws.app, linkAddress, true, false);
    ...
    if (type !== "file") {
        menu.addItem({
            id: "cardPreview",
            icon: "iconPreview",
            label: window.sourceflow.languages.cardPreview,
            click() {
                previewAttrViewImages(
                    linkAddress,
                    options.blockElement.getAttribute("data-av-id"),
                    getAVViewAttr(options.blockElement),
...
${window.sourceflow.languages.title}
