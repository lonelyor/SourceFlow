...
export const blockRender = (protyle: IProtyle, element: Element, top?: number) => {
    ...
    blockElements.forEach((item: HTMLElement) => {
        ...
        if (breadcrumb) {
            ...
            breadcrumb = window.sourceflow.config.editor.embedBlockBreadcrumb;
        }
...
const renderEmbed = (blocks: {
    ...
    if (blocks.length > 0) {
        ...
        item.firstElementChild.insertAdjacentHTML("afterend", `<div class="protyle-wysiwyg__embed ft__smaller ft__secondary b3-form__space--small" contenteditable="false">${errorTip || window.sourceflow.languages.refExpired}</div>`);
    }
