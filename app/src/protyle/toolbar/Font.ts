...
export const appearanceMenu = (protyle: IProtyle, nodeElements?: Element[]) => {
    ...
    ["", "var(--b3-font-color1)", "var(--b3-font-color2)", "var(--b3-font-color3)", "var(--b3-font-color4)",
        ...
        colorHTML += `<button ${item ? `class="color__square" style="color:${item}"` : `class="color__square ariaLabel" data-position="3south" aria-label="${window.sourceflow.languages.default}"`} data-type="color">A</button>`;
    });
    let bgHTML = "";
    ["", "var(--b3-font-background1)", "var(--b3-font-background2)", "var(--b3-font-background3)", "var(--b3-font-background4)",
        ...
        bgHTML += `<button ${item ? `class="color__square" style="background-color:${item}"` : `class="color__square ariaLabel" data-position="3south" aria-label="${window.sourceflow.languages.default}"`} data-type="backgroundColor"></button>`;
    });
    ...
    <kbd class="fn__kbd fn__flex-center${window.sourceflow.config.keymap.editor.insert.lastUsed.custom ? "" : " fn__none"}">${updateHotkeyTip(window.sourceflow.config.keymap.editor.insert.lastUsed.custom)}</kbd>
</div>
...
<div data-id="lastUsedWrap" class="fn__flex fn__flex-wrap" style="align-items: center">`;
        lastFonts.forEach((item: string) => {
            ...
            switch (lastFontStatus[0]) {
                case "color":
                    lastColorHTML += `<button class="color__square ariaLabel" data-position="3south" aria-label="${window.sourceflow.languages.colorFont}${lastFontStatus[1] ? "" : " " + window.sourceflow.languages.default}" ${lastFontStatus[1] ? `style="color:${lastFontStatus[1]}"` : ""} data-type="${lastFontStatus[0]}">A</button>`;
                    break;
                case "backgroundColor":
                    lastColorHTML += `<button class="color__square ariaLabel" data-position="3south" aria-label="${window.sourceflow.languages.colorPrimary}${lastFontStatus[1] ? "" : " " + window.sourceflow.languages.default}" ${lastFontStatus[1] ? `style="background-color:${lastFontStatus[1]}"` : ""} data-type="${lastFontStatus[0]}"></button>`;
                    break;
                case "style2":
                    lastColorHTML += `<button data-type="${lastFontStatus[0]}" class="protyle-font__style" style="-webkit-text-stroke: 0.2px var(--b3-theme-on-background);-webkit-text-fill-color : transparent;">${window.sourceflow.languages.hollow}</button>`;
                    break;
                case "style4":
                    lastColorHTML += `<button data-type="${lastFontStatus[0]}" class="protyle-font__style" style="text-shadow: 1px 1px var(--b3-theme-surface-lighter), 2px 2px var(--b3-theme-surface-lighter), 3px 3px var(--b3-theme-surface-lighter), 4px 4px var(--b3-theme-surface-lighter)">${window.sourceflow.languages.shadow}</button>`;
                    break;
                case "fontSize":
                    if (!disableFont) {
                    ...
                    break;
                case "style1":
                    lastColorHTML += `<button class="color__square ariaLabel" data-position="3south" aria-label="${window.sourceflow.languages.color}${lastFontStatus[1] ? "" : " " + window.sourceflow.languages.default}" ${lastFontStatus[1] ? `style="background-color:${lastFontStatus[1]};color:${lastFontStatus[2]}"` : ""} data-type="${lastFontStatus[0]}">A</button>`;
                    break;
                case "clear":
                    lastColorHTML += `<button style="height: 26px;display: flex;align-items: center;padding: 0 5px;" data-type="${lastFontStatus[0]}" class="protyle-font__style ariaLabel" aria-label="${window.sourceflow.languages.clearFontStyle}"><svg class="svg--mid"><use xlink:href="#iconTrashcan"></use></svg></button>`;
                    break;
            }
        });
        lastColorHTML += "</div>";
    ...
    let fontSize = window.sourceflow.config.editor.fontSize + "px";
    ...
    if (textElement) {
        fontSize = textElement.style.fontSize || window.sourceflow.config.editor.fontSize + "px";
    }
    element.innerHTML = `${lastColorHTML}
<div class="fn__hr"></div>
<div data-id="color">${window.sourceflow.languages.color}</div>
<div class="fn__hr--small"></div>
<div data-id="colorWrap" class="fn__flex fn__flex-wrap">
    <button class="color__square ariaLabel" data-position="3south" data-type="style1" aria-label="${window.sourceflow.languages.default}">A</button>
...
<div class="fn__hr"></div>
<div data-id="colorFont">${window.sourceflow.languages.colorFont}</div>
<div class="fn__hr--small"></div>
<div data-id="colorFontWrap" class="fn__flex fn__flex-wrap">
    ${colorHTML}
</div>
<div class="fn__hr"></div>
<div data-id="colorPrimary">${window.sourceflow.languages.colorPrimary}</div>
<div class="fn__hr--small"></div>
<div data-id="colorPrimaryWrap" class="fn__flex fn__flex-wrap">
    ${bgHTML}
</div>
<div class="fn__hr"></div>
<div data-id="fontStyle">${window.sourceflow.languages.fontStyle}</div>
<div class="fn__hr--small"></div>
<div data-id="fontStyleWrap" class="fn__flex">
    <button data-type="style2" class="protyle-font__style" style="-webkit-text-stroke: 0.2px var(--b3-theme-on-background);-webkit-text-fill-color : transparent;">${window.sourceflow.languages.hollow}</button>
    <button data-type="style4" class="protyle-font__style" style="text-shadow: 1px 1px var(--b3-theme-surface-lighter), 2px 2px var(--b3-theme-surface-lighter), 3px 3px var(--b3-theme-surface-lighter), 4px 4px var(--b3-theme-surface-lighter)">${window.sourceflow.languages.shadow}</button>
</div>
...
<div data-id="fontSize" class="fn__flex${disableFont ? " fn__none" : ""}">
    ...
    <label class="fn__flex">
        ${window.sourceflow.languages.relativeFontSize}
        <span class="fn__space"></span>
...
<div data-id="clearFontStyle" class="fn__flex">
    ...
    <button class="b3-button b3-button--remove fn__block" data-type="clear">
        <svg><use xlink:href="#iconTrashcan"></use></svg>${window.sourceflow.languages.clearFontStyle}
    </button>
    <div class="fn__space--small"></div>
</div>`;
...
export const fontEvent = (protyle: IProtyle, nodeElements: Element[], type?: string, color?: string) => {
    let localFontStyles = window.sourceflow.storage[Constants.LOCAL_FONTSTYLES];
    if (type) {
        ...
        window.sourceflow.storage[Constants.LOCAL_FONTSTYLES] = localFontStyles;
        setStorageVal(Constants.LOCAL_FONTSTYLES, window.sourceflow.storage[Constants.LOCAL_FONTSTYLES]);
    } else {
...
export const setFontStyle = (textElement: HTMLElement, textOption: ITextOption) => {
    const setBlockRef = (blockRefOption: string) => {
        ...
        // 标签等元素中包含 ZWSP，需移除后拼接 https://github.com/lonelyor/SourceFlow/issues/6466
    ...
    if (textOption) {
        switch (textOption.type) {
            ...
            case "style5":
                ...
                textElement.style.opacity = "0.18";
                textElement.style.userSelect = "none";
                textElement.style.pointerEvents = "none";
                textElement.setAttribute("data-inline-hidden", "true");
                break;
            case "id":
...
export const hasSameTextStyle = (currentElement: HTMLElement, sideElement: HTMLElement, textObj?: ITextOption) => {
    ...
    if (textObj) {
        ...
        if (textObj.type === "style5") {
            return sideElement.getAttribute("data-inline-hidden") === "true" &&
                "blur(6px)" === sideElement.style.filter &&
                "0.18" === sideElement.style.opacity;
        }
