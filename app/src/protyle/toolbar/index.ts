...
- baz`;
        ...
        const isPin = this.subElement.querySelector('[data-type="pin"]')?.getAttribute("aria-label") === window.sourceflow.languages.unpin;
...
<textarea ${protyle.disabled ? " readonly" : ""} spellcheck="false" class="b3-text-field b3-text-field--text fn__block" placeholder="${placeholder}" style="${isMobile() ? "" : "width:" + Math.max(480, renderElement.clientWidth * 0.7) + "px"};max-height:calc(80vh - 44px);min-height: 48px;min-width: 268px;border-radius: 0 0 var(--b3-border-radius-b) var(--b3-border-radius-b);font-family: var(--b3-font-family-code);"></textarea></div>`;
        ...
        headerElement.addEventListener("click", (event: MouseEvent) => {
            ...
            switch (btnElement.getAttribute("data-type")) {
                case "close":
                    this.subElement.querySelector('[data-type="pin"]').setAttribute("aria-label", window.sourceflow.languages.pin);
        ...
        textElement.addEventListener("keydown", (event: KeyboardEvent) => {
            event.stopPropagation();
            // 阻止 ctrl+m 缩小窗口 https://github.com/lonelyor/SourceFlow/issues/5541
            ...
            if (event.key === "Escape" || matchHotKey("⌘↩", event)) {
                ...
                // https://github.com/lonelyor/SourceFlow/issues/5270
        ...
        this.subElementCloseCB = () => {
            ...
            if (types.includes("NodeHTMLBlock") && !noChange) {
                ...
                if (htmlText) {
                    // 需移除首尾的空白字符与连续的换行 (空行) https://github.com/lonelyor/SourceFlow/issues/7921
                    htmlText = htmlText.trim().replace(/\n+/g, "\n");
                    // 需一对 div 标签包裹，否则行内元素会解析错误 https://github.com/lonelyor/SourceFlow/issues/6764
                ...
                // HTML 块中包含多个 <pre> 时只能保存第一个 https://github.com/lonelyor/SourceFlow/issues/5732
                ...
                inlineMemoElements.forEach((item, index) => {
                    if (!textElement.value) {
                        // https://github.com/lonelyor/SourceFlow/issues/1046
                ...
                // 行内数学公式不允许换行 https://github.com/lonelyor/SourceFlow/issues/2187
                if (textElement.value) {
                    renderElement.setAttribute("data-content", Lute.EscapeHTMLStr(textElement.value));
                    renderElement.removeAttribute("data-render");
                    processRender(renderElement);
        ...
        this.subElement.style.zIndex = (++window.sourceflow.zIndex).toString();
...
<button class="keyboard__action" data-action="select"><svg><use xlink:href="#iconSelect"></use></svg></button>`;
        ...
        this.subElement.lastElementChild.addEventListener("click", async (event) => {
            ...
            if (action === "copy") {
                ...
                this.subElement.lastElementChild.innerHTML = `<button class="keyboard__action${hasCopy ? "" : " fn__none"}" data-action="copyPlainText"><span>${window.sourceflow.languages.copyPlainText}</span></button>
