...
export const initUI = (protyle: IProtyle) => {
    ...
    protyle.contentElement.addEventListener("mousewheel", (event: WheelEvent) => {
        if (!window.sourceflow.config.editor.fontSizeScrollZoom || (isMacOS && !event.metaKey) || (!isMacOS && !event.ctrlKey) || event.deltaX !== 0) {
            return;
        }
        event.stopPropagation();
        if (event.deltaY < 0) {
            if (window.sourceflow.config.editor.fontSize < 72) {
                window.sourceflow.config.editor.fontSize++;
            } else {
                return;
            }
        } else if (event.deltaY > 0) {
            if (window.sourceflow.config.editor.fontSize > 9) {
                window.sourceflow.config.editor.fontSize--;
            } else {
                return;
            }
        ...
        showMessage(`${window.sourceflow.languages.fontSize} ${window.sourceflow.config.editor.fontSize}px<span class="fn__space"></span>
<button class="b3-button b3-button--white">${window.sourceflow.languages.reset} 16px</button>`, undefined, undefined, wheelId);
        wheelTimeout = window.setTimeout(() => {
            ...
            document.querySelector(`#message [data-id="${wheelId}"] button`)?.addEventListener("click", () => {
                window.sourceflow.config.editor.fontSize = 16;
                setInlineStyle();
                fetchPost("/api/setting/setEditor", window.sourceflow.config.editor);
                hideMessage(wheelId);
                protyle.wysiwyg.element.querySelectorAll(".code-block .protyle-linenumber__rows").forEach((block: HTMLElement) => {
    ...
    protyle.contentElement.addEventListener("click", (event: MouseEvent & { target: HTMLElement }) => {
        ...
        // wysiwyg 元素下方点击无效果 https://github.com/lonelyor/SourceFlow/issues/12009
        if (protyle.disabled ||
            // 选中块时，禁止添加空块 https://github.com/lonelyor/SourceFlow/issues/13905
        ...
        // https://github.com/lonelyor/SourceFlow/issues/14190 选中最后一个块末尾点击底部时，range 会有值，需使用 setTimeout，最新测试无需 setTimeout 了，且会影响移动端键盘弹起故移除
        // 选中文本禁止添加空块 https://github.com/lonelyor/SourceFlow/issues/13905
...
export const setPadding = (protyle: IProtyle) => {
    ...
    // https://github.com/lonelyor/SourceFlow/issues/15021
...
export const getPadding = (protyle: IProtyle) => {
    ...
    if (!isMobile()) {
        let isFullWidth = getFullWidthAttr(protyle.wysiwyg.element);
        if (!isFullWidth) {
