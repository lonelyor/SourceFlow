...
export const hideElements = (panels: string[], protyle?: IProtyle, focusHide = false) => {
    if (!protyle) {
        if (panels.includes("dialog")) {
            const dialogLength = window.sourceflow.dialogs.length;
            for (let i = 0; i < dialogLength; i++) {
                window.sourceflow.dialogs[i].destroy();
            }
        }
        return;
    }
    ...
    if (protyle.toolbar && panels.includes("util")) {
        ...
        if (focusHide || !pinElement || (pinElement && pinElement.getAttribute("aria-label") === window.sourceflow.languages.pin)) {
    ...
    if (types.includes("util")) {
        getAllEditor().forEach(item => {
            if (item.protyle.toolbar) {
                ...
                if (!pinElement || (pinElement && pinElement.getAttribute("aria-label") === window.sourceflow.languages.pin)) {
