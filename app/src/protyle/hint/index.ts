    ...
    constructor(protyle: IProtyle) {
        ...
        this.element.addEventListener("click", (event) => {
            ...
            if (btnElement && !btnElement.classList.contains("emojis__item") && !btnElement.classList.contains("emojis__type")) {
                ...
                event.stopPropagation(); // https://github.com/lonelyor/SourceFlow/issues/3710
                return;
            }
...
${unicode2Emoji(emoji.unicode)}</button>`;
    ...
    public render(protyle: IProtyle) {
        ...
        // 粘贴后 range.startContainer 为空 https://github.com/lonelyor/SourceFlow/issues/7360
        ...
        // https://github.com/lonelyor/SourceFlow/issues/7933
        if (this.splitChar === "#") {
        ...
        // https://github.com/lonelyor/SourceFlow/issues/5083
    ...
    private getHTMLByData(data: IHintData[]) {
        ...
        data.forEach((hintData, i) => {
            // https://github.com/lonelyor/SourceFlow/issues/1229 提示时，新建文件不应默认选中
            let focusClass = "";
            if ((i === 1 && data[i].focus) ||
    ...
    public genHTML(data: IHintData[], protyle: IProtyle, hide = false, source: THintSource) {
        ...
        this.element.classList.remove("fn__none");
        // https://github.com/lonelyor/SourceFlow/issues/4575
        if (data[0].filter) {
...
<span class="b3-list-item__text">${window.sourceflow.languages.newFile} <mark>${response.data.k}</mark></span></div></button>`;
            }
            response.data.blocks.forEach((item: IBlock, index: number) => {
                let blockRefHTML;
                if (source === "av") {
                    // av 搜索时需要获取值 https://github.com/lonelyor/SourceFlow/issues/12020
...
${genHintItemHTML(item)}
            ...
            if (searchHTML === "") {
                searchHTML = `<button style="width: calc(100% - 16px)" class="b3-list-item b3-list-item--two" data-value="">${window.sourceflow.languages.emptyContent}</button>`;
            }
...
<div class="fn__flex${value ? " fn__none" : ""}">
    ...
    public fill(value: string, protyle: IProtyle, updateRange = true, refIsS = false) {
        ...
        if (value === MIND_ELIXIR_SLASH_VALUE) {
            range.deleteContents();
            this.fixImageCursor(range);
            const mindElixirID = Lute.NewNodeID();
            void import("../render/mindmapData").then(({buildMindElixirHTMLBlockDOM, createDefaultMindElixirData}) => {
                insertHTML(buildMindElixirHTMLBlockDOM(createDefaultMindElixirData(), {
                    id: mindElixirID,
                }), protyle, true, false, true);
                processRender(protyle.wysiwyg.element);
                const mindElixirElement = protyle.wysiwyg.element.querySelector(`[data-node-id="${mindElixirID}"]`) as HTMLElement;
                if (mindElixirElement) {
                    protyle.toolbar.showRender(protyle, mindElixirElement);
                }
                hideElements(["hint"], protyle);
            });
            return;
        }
        // 自顶向下法新建文档后光标定位问题 https://github.com/lonelyor/SourceFlow/issues/299
        // QQ 拼音输入法自动补全需移除补全内容 https://github.com/lonelyor/SourceFlow/issues/320
        // 前后有标记符的情况 https://github.com/lonelyor/SourceFlow/issues/2511
        ...
        if (Constants.BLOCK_HINT_KEYS.includes(this.splitChar) && value.startsWith("((newFile ") && value.endsWith(`${Lute.Caret}'))`)) {
            ...
            getSavePath(protyle.path, protyle.notebookId, (pathString, targetNotebookId) => {
                fetchPost("/api/filetree/createDocWithMd", {
                    ...
                    // https://github.com/lonelyor/SourceFlow/issues/10133
                    protyle.toolbar.range = range;
                    const refElement = protyle.toolbar.setInlineMark(protyle, "block-ref", "range", {
                        type: "id",
                        color: `${response.data}${Constants.ZWSP}${refIsS ? "s" : "d"}${Constants.ZWSP}${(refIsS ? fileNames[0] : realFileName).substring(0, window.sourceflow.config.editor.blockRefDynamicAnchorTextMaxLen)}`
                    });
                    if (refElement[0]) {
        ...
        if (Constants.BLOCK_HINT_KEYS.includes(this.splitChar)) {
            ...
            if (value === "((" || value === "{{") {
                ...
                fetchPost("/api/filetree/createDoc", {
                    ...
                    path: pathPosix().join(getDisplayName(protyle.path, false, true), newSubDocId + ".sf"),
                    title: window.sourceflow.languages.untitled,
                    md: ""
                }, () => {
                    insertHTML(`<span data-type="block-ref" data-id="${newSubDocId}" data-subtype="d">${window.sourceflow.languages.untitled}</span>`, protyle);
                    /// #if MOBILE
                ...
                if (value === "```") {
                    textContent = value + (Constants.SOURCEFLOW_RENDER_CODE_LANGUAGES.includes(window.sourceflow.storage[Constants.LOCAL_CODELANG]) ? "" : window.sourceflow.storage[Constants.LOCAL_CODELANG]) + Lute.Caret + "\n```";
                }
                const editableElement = getContenteditableElement(nodeElement);
                if (value === "![]()") { // https://github.com/lonelyor/SourceFlow/issues/4586 1
                    let newHTML = "";
                    ...
                    if (imgElement && imgElement.nodeType !== 3 && imgElement.classList.contains("img")) {
                        ...
                        // https://github.com/lonelyor/SourceFlow/issues/7540
                    ...
                    nodeElement.outerHTML = newHTML;
                    nodeElement = protyle.wysiwyg.element.querySelector(`[data-node-id="${id}"]`);
                    // https://github.com/lonelyor/SourceFlow/issues/6864
                ...
                if (value === "<div>" || value === "$$" || (value.indexOf("```") > -1 && (value.length > 3 || nodeElement.classList.contains("render-node")))) {
                    ...
                    const itemElement = window.sourceflow.menus.menu.element.querySelector('[data-id="assetVideo"], [data-id="assetAudio"], [data-id="assetIFrame"]');
                    itemElement.classList.add("b3-menu__item--show");
                    window.sourceflow.menus.menu.showSubMenu(itemElement.querySelector(".b3-menu__submenu"));
                    window.sourceflow.menus.menu.element.querySelector("textarea").focus();
                } else if (value === "---") {
    ...
    public select(event: KeyboardEvent, protyle: IProtyle) {
        ...
        if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
            hideElements(["hint"], protyle);
            // 不需要 preventDefault https://github.com/lonelyor/SourceFlow/issues/11846
            return true;
        }
        return false;
    }
    ...
    private getKey(currentLineValue: string, extend: IHintExtend[]) {
        ...
        // 上一次提示没有结束时不能被其余提示干扰 https://github.com/lonelyor/SourceFlow/issues/14324
        ...
        if (lineArray.length > 1 &&
            // https://github.com/lonelyor/SourceFlow/issues/10637
            lastItem.trimStart() === lastItem &&
            lastItem.length < Constants.SIZE_TITLE) {
            // 输入法自动补全 https://github.com/lonelyor/SourceFlow/issues/100
