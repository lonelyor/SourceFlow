...
export const reloadProtyle = (protyle: IProtyle, focus: boolean, updateReadonly?: boolean) => {
    ...
    protyle.lute.SetProtyleMarkNetImg(window.sourceflow.config.editor.displayNetImgMark);
    protyle.lute.SetSpellcheck(window.sourceflow.config.editor.spellcheck);
    restoreLuteMarkdownSyntax(protyle);
    protyle.lute.SetGFMStrikethrough1(false);
    addLoading(protyle);
    if (protyle.options.backlinkData) {
        const isMention = protyle.element.getAttribute("data-ismention") === "true";
        const tabElement = hasClosestByClassName(protyle.element, "sf__backlink");
        if (tabElement) {
