...
export const onGet = (options: {
    ...
    if (options.data.code === 1) {
        // 其他报错
        if (!options.action.includes(Constants.CB_GET_APPEND)) {    // 向下加载时块可能还没有创建 https://github.com/lonelyor/SourceFlow/issues/10851
            if (options.protyle.model) {
                options.protyle.model.parent.parent.removeTab(options.protyle.model.parent.id);
            } else {
                options.protyle.element.innerHTML = `<div class="ft__smaller ft__secondary b3-form__space--small" contenteditable="false">${window.sourceflow.languages.refExpired}</div>`;
            }
        }
        return;
    }
...
const setHTML = (options: {
    ...
    /// #endif
    // https://github.com/lonelyor/SourceFlow/issues/10528
    if (!protyle.block.showAll && protyle.wysiwyg.element.childElementCount === 1 && protyle.wysiwyg.element.firstElementChild.classList.contains("p")) {
        ...
        if (editElement && editElement.textContent === "") {
            ...
            /// #if MOBILE
            editElement.setAttribute("placeholder", window.sourceflow.languages.emptyMobilePlaceholder);
            /// #else
            editElement.setAttribute("placeholder", window.sourceflow.languages.emptyPlaceholder);
            /// #endif
        }
    }
    ...
    if (options.isSyncing) {
        ...
        if (options.action.includes(Constants.CB_GET_OPENNEW) && window.sourceflow.config.editor.readOnly && !window.sourceflow.config.readonly) {
    ...
    if (options.action.includes(Constants.CB_GET_SETID)) {
        // 点击大纲后，如果需要动态加载，在定位后，需要重置 block.id https://github.com/lonelyor/SourceFlow/issues/4487
    ...
    // 屏幕太高的页签 https://github.com/lonelyor/SourceFlow/issues/5018
    if (options.scrollAttr && !protyle.scroll.element.classList.contains("fn__none") &&
        !protyle.element.classList.contains("block__edit") &&   // 不能为浮窗，否则悬浮为根文档无法打开整个文档 https://github.com/lonelyor/SourceFlow/issues/9082
        protyle.wysiwyg.element.lastElementChild.getAttribute("data-eof") !== "2" &&
        protyle.contentElement.scrollHeight > 0 && // 没有激活的页签 https://github.com/lonelyor/SourceFlow/issues/5255
        !options.action.includes(Constants.CB_GET_FOCUSFIRST) && // 防止 eof 为true https://github.com/lonelyor/SourceFlow/issues/5291
    ...
    if (options.scrollAttr && !protyle.scroll.element.classList.contains("fn__none") && !protyle.element.classList.contains("fn__none")) {
        ...
        protyle.scroll.updateIndex(protyle, options.scrollAttr.startId, (index) => {
            // https://github.com/lonelyor/SourceFlow/issues/8224
            // https://github.com/lonelyor/SourceFlow/issues/10716
            if (index > 1 && protyle.block.blockCount > 1 && protyle.contentElement.scrollHeight <= protyle.contentElement.clientHeight) {
                showMessage(window.sourceflow.languages.scrollGetMore);
            }
        });
...
export const disabledForeverProtyle = (protyle: IProtyle) => {
    ...
    if (protyle.breadcrumb && !isMobile()) {
        protyle.breadcrumb.element.nextElementSibling.textContent = window.sourceflow.languages["_kernel"][81];
    } else {
        showMessage(window.sourceflow.languages["_kernel"][81]);
    }
...
export const disabledProtyle = (protyle: IProtyle) => {
    ...
    if (protyle.breadcrumb) {
        ...
        readonlyButton.setAttribute("aria-label", window.sourceflow.config.editor.readOnly ? window.sourceflow.languages.tempUnlock : window.sourceflow.languages.unlockEdit);
...
export const enableProtyle = (protyle: IProtyle) => {
    ...
    if (protyle.breadcrumb) {
        ...
        readonlyButton.setAttribute("aria-label", window.sourceflow.config.editor.readOnly ? window.sourceflow.languages.cancelTempUnlock : window.sourceflow.languages.lockEdit);
...
export const setReadonlyByConfig = (protyle: IProtyle, updateReadonly: boolean) => {
    let readOnly = window.sourceflow.config.readonly ? "true" : "false";
    if (!updateReadonly) {
        readOnly = protyle.disabled ? "true" : "false";
    } else if (readOnly === "false") {
        readOnly = window.sourceflow.config.editor.readOnly ? "true" : "false";
        if (readOnly === "false") {
            readOnly = getReadonlyAttr(protyle.wysiwyg.element);
        }
    }
    if (readOnly === "true") {
        disabledProtyle(protyle);
