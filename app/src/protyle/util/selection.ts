    ...
    if (range.toString() !== "" && tableElement && range.commonAncestorContainer.nodeType !== 3) {
        ...
        if (parentTag !== "TH" && parentTag !== "TD") {
            ...
            if (!startCellElement && !endCellElement) {
                ...
                // 不能包含自身元素，否则对 cell 中的部分文字两次高亮后就会选中整个 cell。 https://github.com/lonelyor/SourceFlow/issues/3649 第二点
...
// https://github.com/lonelyor/SourceFlow/issues/8196
    ...
    if (getSelection().rangeCount > 0) {
        ...
        if (element === range.startContainer || element.contains(range.startContainer)) {
            if (range.toString() === "" && range.startContainer.nodeType === 1) {
                ...
                // 移动端获取有偏差 https://github.com/lonelyor/SourceFlow/issues/15998
...
export const getSelectionPosition = (nodeElement: Element, range?: Range, useDirect = false) => {
    ...
    if (range.getClientRects().length === 0) {
        ...
        const rects = range.getClientRects(); // 由于长度过长折行，光标在行首时有多个 rects https://github.com/lonelyor/SourceFlow/issues/6156
        if (range.toString()) {
            if (useDirect) {
                ...
                return {    // 选中多行不应遮挡第一行 https://github.com/lonelyor/SourceFlow/issues/7541
                    left: rects[rects.length - 1].left,
                    top: rects[0].top
                };
            }
        } else {
            return {    // 代码块首 https://github.com/lonelyor/SourceFlow/issues/13113
                left: rects[rects.length - 1].left,
...
export const setLastNodeRange = (editElement: Element, range: Range, setStart = true) => {
    ...
    while (lastNode && lastNode.nodeType !== 3) {
        // https://github.com/lonelyor/SourceFlow/issues/12792
    ...
    // https://github.com/lonelyor/SourceFlow/issues/12753
    if (!lastNode) {
...
export const focusBlock = (element: Element, parentElement?: HTMLElement, toStart = true): false | Range => {
    ...
    // hr、嵌入块、数学公式、iframe、音频、视频、图表渲染块等，删除段落块后，光标位置矫正 https://github.com/lonelyor/SourceFlow/issues/4143
    ...
    if (cursorElement) {
        ...
        let range;
        if (toStart) {
            // 需要定位到第一个 child https://github.com/lonelyor/SourceFlow/issues/5930
            range = setFirstNodeRange(cursorElement, getEditorRange(cursorElement));
            range.collapse(true);
        } else {
            let focusHljs = false;
            // 定位到末尾 https://github.com/lonelyor/SourceFlow/issues/5982
            if (element.getAttribute("data-type") === "NodeCodeBlock") {
                // 代码块末尾定位需在 /n 之前 https://github.com/lonelyor/SourceFlow/issues/9141，https://github.com/lonelyor/SourceFlow/issues/9189
                ...
                if (lastNode && lastNode.textContent.endsWith("\n")) {
                    // https://github.com/lonelyor/SourceFlow/issues/11362
