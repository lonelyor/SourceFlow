...
export const insertRowAbove = (protyle: IProtyle, range: Range, cellElement: HTMLElement, nodeElement: Element, count = 1) => {
    ...
    for (let m = 0; m < cellElement.parentElement.childElementCount; m++) {
        ...
        // 不需要空格，否则列宽调整后在空格后插入图片会换行 https://github.com/lonelyor/SourceFlow/issues/7631
        if (cellElement.tagName === "TH") {
...
export const fixTable = (protyle: IProtyle, event: KeyboardEvent, range: Range) => {
    ...
    if (event.key === "Enter" && event.shiftKey && isNotCtrl(event) && !event.altKey) {
        ...
        if (types.includes("code") && range.startContainer.nodeType !== 3) {
            // https://github.com/lonelyor/SourceFlow/issues/4169
    ...
    if (!nodeElement.classList.contains("protyle-wysiwyg--select") && !hasClosestByClassName(nodeElement, "protyle-wysiwyg--select")) {
        ...
        if (isNotCtrl(event) && !event.shiftKey && !event.altKey && event.key === "Backspace"
            ...
            // 空换行无法删除 https://github.com/lonelyor/SourceFlow/issues/2732
    ...
    if (matchHotKey(window.sourceflow.config.keymap.editor.table.moveToUp.custom, event)) {
    ...
    if (matchHotKey(window.sourceflow.config.keymap.editor.table.moveToDown.custom, event)) {
        if ((!hasNone || (hasNone && !hasRowSpan && hasColSpan)) &&
    ...
    if (matchHotKey(window.sourceflow.config.keymap.editor.table.moveToLeft.custom, event)) {
    ...
    if (matchHotKey(window.sourceflow.config.keymap.editor.table.moveToRight.custom, event)) {
    ...
    // 上方新添加一行
    if (matchHotKey(window.sourceflow.config.keymap.editor.table.insertRowAbove.custom, event)) {
    ...
    if (matchHotKey(window.sourceflow.config.keymap.editor.table.insertRowBelow.custom, event)) {
    ...
    // 左方新添加一列
    if (matchHotKey(window.sourceflow.config.keymap.editor.table.insertColumnLeft.custom, event)) {
        if (colIsPure || previousColIsPure) {
    ...
    // 后方新添加一列
    if (matchHotKey(window.sourceflow.config.keymap.editor.table.insertColumnRight.custom, event)) {
        if (colIsPure || nextColIsPure) {
    ...
    // 删除当前行
    if (matchHotKey(window.sourceflow.config.keymap.editor.table["delete-row"].custom, event)) {
        if ((!hasNone && !hasRowSpan) || //https://github.com/lonelyor/SourceFlow/issues/5045
    ...
    // 删除当前列
    if (matchHotKey(window.sourceflow.config.keymap.editor.table["delete-column"].custom, event)) {
        if (colIsPure) {
...
export const updateTableTitle = (protyle: IProtyle, nodeElement: Element) => {
    ...
    const dialog = new Dialog({
        title: window.sourceflow.languages.table,
        width: isMobile() ? "92vw" : "520px",
        content: `<div class="b3-dialog__content">
    <label>
        <div>${window.sourceflow.languages.title}</div>
        <div class="fn__hr"></div>
        <input class="b3-text-field fn__block">
    </label>
    <div class="fn__hr--b"></div>
    <label>
        <div>${window.sourceflow.languages.position}</div>
        <div class="fn__hr"></div>
        <select class="b3-select fn__block">
            <option value="top">${window.sourceflow.languages.up}</option>
            <option value="bottom" ${captionElement?.style.captionSide === "bottom" ? "selected" : ""}>${window.sourceflow.languages.down}</option>
        </select>
    </label>
</div>
