...
export const matchHotKey = (hotKey: string, event: KeyboardEvent) => {
    ...
    // https://github.com/lonelyor/SourceFlow/issues/9770
    if (hotKey.startsWith("⌃") && !isMac()) {
        if (hotKey === "⌃D") {
            // https://github.com/lonelyor/SourceFlow/issues/9841
            return false;
        }
...
export const isIncludesHotKey = (hotKey: string) => {
    let isInclude = false;
    Object.keys(window.sourceflow.config.keymap).find(key => {
        const item = window.sourceflow.config.keymap[key as "editor"];
...
export const updateControlAlt = () => {
    if (!window.sourceflow.config.keymap.general) {
        return;
    }
    Object.keys(window.sourceflow.config.keymap.general).forEach(key => {
        if (["fileTree", "outline", "bookmark", "tag", "dailyNote", "backlinks",
            "graphView", "globalGraph", "riffCard"].includes(key)) {
            if (navigator.platform.toUpperCase().indexOf("MAC") > -1) {
                window.sourceflow.config.keymap.general[key].default = window.sourceflow.config.keymap.general[key].default.replace("⌥", "⌃");
                if (window.sourceflow.config.keymap.general[key].default === window.sourceflow.config.keymap.general[key].custom) {
                    window.sourceflow.config.keymap.general[key].custom = window.sourceflow.config.keymap.general[key].default.replace("⌥", "⌃");
                }
            } else {
                window.sourceflow.config.keymap.general[key].default = window.sourceflow.config.keymap.general[key].default.replace("⌃", "⌥");
                if (window.sourceflow.config.keymap.general[key].default === window.sourceflow.config.keymap.general[key].custom) {
