...
import {updateAttrViewCellAnimation, updateCellsValue} from "../render/av/cell";
...
export const insertHTML = (html: string, protyle: IProtyle, isBlock = false,
    ...
    if (hasClosestByAttribute(range.startContainer, "data-type", "NodeTable") && !isBlock) {
        if (hasClosestByTag(range.startContainer, "TABLE")) {
            ...
            // https://github.com/lonelyor/SourceFlow/issues/9411
            isBlock = true;
        }
    }
    ...
    if (!isBlock &&
        ...
        range.deleteContents();
        // 代码块需保持至少一个 \n https://github.com/lonelyor/SourceFlow/pull/13271#issuecomment-2502672155
        ...
        range.insertNode(document.createElement("wbr"));
        if (codeBlockIsEmpty) {
            // 代码块为空添加的 \n 需放在最后 https://github.com/lonelyor/SourceFlow/issues/15399
            range.collapse(false);
    ...
    if (range.toString() !== "") {
        ...
        if (inlineMathElement) {
            ...
            // 选中 ref**bbb** 后 alt+[
            range.deleteContents();
            // https://github.com/lonelyor/SourceFlow/issues/14035
    ...
    // https://github.com/lonelyor/SourceFlow/issues/14162 & https://github.com/lonelyor/SourceFlow/issues/14965
    if (/^\s*&gt;|\*|-|\+|\d*.|\[ \]|[x]/.test(html) &&
        editableElement.textContent.replace(Constants.ZWSP, "") !== "") {
        unSpinHTML = html;
    }

    let innerHTML = unSpinHTML || // 在 table 中插入需要使用转换好的行内元素 https://github.com/lonelyor/SourceFlow/issues/9358
        html;   // 空格会被 Spin 不再，需要使用原文
    // 粘贴纯文本时会进行内部转义，这里需要进行反转义 https://github.com/lonelyor/SourceFlow/issues/10620
    ...
    if ((
        ...
        // https://github.com/lonelyor/SourceFlow/issues/14114
        isBlock = false;
        block2text = true;
    }
    ...
    if (!isBlock) {
        if (tempElement.content.firstChild.nodeType === 3 || block2text ||
            ...
            if (spanElement.tagName === "SPAN" && spanElement === (range.endContainer.nodeType === 3 ? range.endContainer.parentElement : range.endContainer) &&
                // 粘贴纯文本不需切割
                // emoji 图片需要切割 https://github.com/lonelyor/SourceFlow/issues/9370
    ...
    // https://github.com/lonelyor/SourceFlow/issues/15768
    ...
    (insertBefore ? Array.from(tempElement.content.children) : Array.from(tempElement.content.children).reverse()).find((item) => {
        ...
        if (addId === id) {
            ...
            if (item.classList.contains("li") && !blockElement.parentElement.classList.contains("list")) {
                // https://github.com/lonelyor/SourceFlow/issues/6534
    ...
    if (lastElement) {
        // https://github.com/lonelyor/SourceFlow/issues/5591
