...
const dragSb = async (protyle: IProtyle, sourceElements: Element[], targetElement: Element, isBottom: boolean,
    ...
    // 把列表块中的唯一一个列表项块拖拽到列表块的左侧 https://github.com/lonelyor/SourceFlow/issues/16315
    ...
    // 横向超级块A内两个元素拖拽成纵向超级块B，取消超级块A会导致 targetElement 被删除，需先移动再删除 https://github.com/lonelyor/SourceFlow/issues/16292
    ...
    doOperations.find((item, index) => {
        ...
        // 超级块内有两个块，拖拽其中一个到超级块外 https://github.com/lonelyor/SourceFlow/issues/16292#issuecomment-3523600155
...
const dragSame = async (protyle: IProtyle, sourceElements: Element[], targetElement: Element, isBottom: boolean, isCopy: boolean) => {
    ...
    if (targetElement.getAttribute("data-type") === "NodeListItem" &&
        ...
        // https://github.com/lonelyor/SourceFlow/issues/536
...
export const dropEvent = (protyle: IProtyle, editorElement: HTMLElement) => {
    editorElement.addEventListener("dragstart", (event) => {
        ...
        if (target.classList) {
            if (hasClosestByClassName(target, "protyle-wysiwyg__embed")) {
                ...
                window.sourceflow.dragElement = target;
                target.style.width = target.clientWidth + "px";
                target.style.opacity = ".36";
                event.dataTransfer.setData(`${Constants.SOURCEFLOW_DROP_GUTTER}NodeAttributeView${Constants.ZWSP}ViewTab${Constants.ZWSP}${[target.previousElementSibling?.getAttribute("data-id")]}`,
                    target.outerHTML);
                return;
                ...
                window.sourceflow.dragElement = protyle.wysiwyg.element;
                event.dataTransfer.setData(`${Constants.SOURCEFLOW_DROP_GUTTER}NodeListItem${Constants.ZWSP}${target.parentElement.getAttribute("data-subtype")}${Constants.ZWSP}${[target.parentElement.getAttribute("data-node-id")]}`,
                    protyle.wysiwyg.element.innerHTML);
                return;
            } else if (target.classList.contains("av__cell--header")) {
                window.sourceflow.dragElement = target;
                event.dataTransfer.setData(`${Constants.SOURCEFLOW_DROP_GUTTER}NodeAttributeView${Constants.ZWSP}Col${Constants.ZWSP}${[target.getAttribute("data-col-id")]}`,
                    target.outerHTML);
                return;
                ...
                if (blockElement) {
                    ...
                    event.dataTransfer.setData(`${Constants.SOURCEFLOW_DROP_GUTTER}NodeAttributeView${Constants.ZWSP}GalleryItem${Constants.ZWSP}${selectIds}`,
        ...
        // 选中编辑器中的文字进行拖拽
        event.dataTransfer.setData(Constants.SOURCEFLOW_DROP_EDITOR, Constants.SOURCEFLOW_DROP_EDITOR);
    ...
    editorElement.addEventListener("drop", async (event: DragEvent & { target: HTMLElement }) => {
        counter = 0;
        if (protyle.disabled || event.dataTransfer.getData(Constants.SOURCEFLOW_DROP_EDITOR)) {
            // 只读模式/编辑器内选中文字拖拽
            event.preventDefault();
            event.stopPropagation();
            return;
        ...
        if (gutterType.startsWith(`${Constants.SOURCEFLOW_DROP_GUTTER}NodeAttributeView${Constants.ZWSP}ViewTab${Constants.ZWSP}`.toLowerCase())) {
            const blockElement = hasClosestBlock(window.sourceflow.dragElement);
            if (blockElement) {
                const avID = blockElement.getAttribute("data-av-id");
                const blockID = blockElement.getAttribute("data-node-id");
                const id = window.sourceflow.dragElement.getAttribute("data-id");
                transaction(protyle, [{
                    action: "sortAttrViewView",
                    avID,
                    blockID,
                    id,
                    previousID: window.sourceflow.dragElement.previousElementSibling?.getAttribute("data-id"),
        ...
        if (gutterType) {
            ...
            if (event.altKey) {
                ...
                if (window.sourceflow.dragElement) {
                    window.sourceflow.dragElement.querySelectorAll(queryClass.substring(0, queryClass.length - 1)).forEach(elementItem => {
                        if (!isInEmbedBlock(elementItem)) {
                            sourceElements.push(elementItem);
                        }
                    });
                } else if (window.sourceflow.config.system.workspaceDir.toLowerCase() === gutterTypes[3]) {
                    // 跨窗口拖拽
                    // 不能跨工作区域拖拽 https://github.com/lonelyor/SourceFlow/issues/13582
                ...
                if (targetElement.classList.contains("av__cell")) {
                    ...
                    // https://github.com/lonelyor/SourceFlow/issues/10528#issuecomment-2205165824
                    editorElement.querySelectorAll(".protyle-wysiwyg--empty").forEach(item => {
                        item.classList.remove("protyle-wysiwyg--empty");
                    });

                    // 需重新渲染 https://github.com/lonelyor/SourceFlow/issues/7574
        ...
        } else if (event.dataTransfer.getData(Constants.SOURCEFLOW_DROP_FILE)?.split("-").length > 1) {
            // 文件树拖拽
            const ids = event.dataTransfer.getData(Constants.SOURCEFLOW_DROP_FILE).split(",");
            if (!event.altKey && (!targetElement || (
                ...
                if (targetElement.classList.contains("av__row") ||
                    ...
                    fetchPost("/api/filetree/getDoc", {
                        ...
                        /// #endif
                        // 文档标题互转后，编辑区会跳转到开头 https://github.com/lonelyor/SourceFlow/issues/2939
                        setTimeout(() => {
        ...
        } else if (!window.sourceflow.dragElement && (event.dataTransfer.types[0] === "Files" || event.dataTransfer.types.includes("text/html"))) {
            event.preventDefault();
            // 外部文件拖入编辑器中或者编辑器内选中文字拖拽
            // https://github.com/lonelyor/SourceFlow/issues/9544
    ...
    editorElement.addEventListener("dragover", (event: DragEvent & { target: HTMLElement }) => {
        if (protyle.disabled || event.dataTransfer.types.includes(Constants.SOURCEFLOW_DROP_EDITOR)) {
            event.preventDefault();
            event.stopPropagation();
        ...
        if (event.dataTransfer.types.includes("Files")) {
            ...
            // 使用 event.preventDefault(); 会导致无光标 https://github.com/lonelyor/SourceFlow/issues/12857
            return;
        }

        if (!gutterType && !window.sourceflow.dragElement) {
            // https://github.com/lonelyor/SourceFlow/issues/6436
            event.preventDefault();
            return;
        }
        const gutterTypes = gutterType ? gutterType.replace(Constants.SOURCEFLOW_DROP_GUTTER, "").split(Constants.ZWSP) : [];
        const fileTreeIds = (event.dataTransfer.types.includes(Constants.SOURCEFLOW_DROP_FILE) && window.sourceflow.dragElement) ? window.sourceflow.dragElement.innerText : "";
        if (event.shiftKey || (event.altKey && fileTreeIds.indexOf("-") === -1)) {
            ...
            if (targetAssetElement) {
                ...
                // https://github.com/lonelyor/SourceFlow/issues/14177
        ...
        // 编辑器内文字拖拽或资源文件拖拽或按住 alt/shift 拖拽反链图标进入编辑器时不能运行 event.preventDefault()， 否则无光标; 需放在 !window.sourceflow.dragElement 之后
        event.preventDefault();
        ...
        if (gutterType && gutterType.startsWith(`${Constants.SOURCEFLOW_DROP_GUTTER}NodeAttributeView${Constants.ZWSP}Col${Constants.ZWSP}`.toLowerCase())) {
            // 表头只能拖拽到当前 av 的表头中
            targetElement = hasClosestByClassName(event.target, "av__cell");
            if (targetElement) {
                const targetRowElement = hasClosestByClassName(targetElement, "av__row--header");
                const dragRowElement = hasClosestByClassName(window.sourceflow.dragElement, "av__row--header");
                if (targetElement === window.sourceflow.dragElement || !targetRowElement || !dragRowElement ||
                    (targetRowElement && dragRowElement && targetRowElement !== dragRowElement)
                ) {
                    targetElement = false;
                }
            }
        } else if (targetElement && gutterType && gutterType.startsWith(`${Constants.SOURCEFLOW_DROP_GUTTER}NodeAttributeViewRowMenu${Constants.ZWSP}`.toLowerCase())) {
            if ((!targetElement.classList.contains("av__row") && !targetElement.classList.contains("av__row--util")) ||
                (window.sourceflow.dragElement && !window.sourceflow.dragElement.contains(targetElement))) {
                // 行只能拖拽当前 av 中
                targetElement = false;
            } else {
                const bodyElement = hasClosestByClassName(targetElement, "av__body");
                if (bodyElement) {
                    ...
                    const groupID = bodyElement.getAttribute("data-group-id");
                    // 模板、创建时间、更新时间 字段作为分组方式时不允许跨分组拖拽 https://github.com/lonelyor/SourceFlow/issues/15553
        ...
        } else if (targetElement && gutterType && gutterType.startsWith(`${Constants.SOURCEFLOW_DROP_GUTTER}NodeAttributeView${Constants.ZWSP}GalleryItem${Constants.ZWSP}`.toLowerCase())) {
            ...
            if (targetElement.classList.contains("av") || !containerElement ||
                !containerElement.contains(window.sourceflow.dragElement) || targetElement === window.sourceflow.dragElement) {
        ...
        if (targetElement && dragoverElement && targetElement === dragoverElement) {
            ...
            // 忘记为什么要限定文档树的拖拽了，先放开 https://github.com/lonelyor/SourceFlow/pull/13284#issuecomment-2503853135
            if (targetElement.getAttribute("data-type") === "NodeListItem") {
            ...
            if (targetElement.classList.contains("av__cell")) {
                if (event.clientX < nodeRect.left + nodeRect.width / 2 && event.clientX > nodeRect.left &&
                    !targetElement.classList.contains("av__row") && targetElement.previousElementSibling !== window.sourceflow.dragElement) {
                    ...
                    !targetElement.classList.contains("av__row") && targetElement !== window.sourceflow.dragElement.previousElementSibling) {
                    if (window.sourceflow.dragElement.previousElementSibling.classList.contains("av__colsticky") &&
                        targetElement === window.sourceflow.dragElement.previousElementSibling.lastElementChild) {
                        // 拖拽到固定列的最后一个元素
                    } else {
            ...
            // 减小两个列表之间左侧间距，以便拖拽到其中 https://github.com/lonelyor/SourceFlow/issues/15672
        ...
        if (gutterType) {
            ...
            if (gutterTypes[0] === "nodelistitem" && "NodeListItem" === targetElement.getAttribute("data-type")) {
                ...
                // 选中非列表不能拖拽到列表中 https://github.com/lonelyor/SourceFlow/issues/13822
            ...
            if (gutterTypes[0] === "nodelistitem" &&
                ...
                // https://github.com/lonelyor/SourceFlow/issues/15672
...
// https://github.com/lonelyor/SourceFlow/issues/12651
