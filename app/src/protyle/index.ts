...
export class Protyle {
    ...
    constructor(app: App, id: HTMLElement, options?: IProtyleOptions) {
        this.version = Constants.SOURCEFLOW_VERSION;
  ...
  .protyle-content::highlight(search-mark-hl-${styleId}) {color: var(--b3-highlight-color);background-color: var(--b3-highlight-current-background)}`;
        ...
        if (!mergedOptions.action.includes(Constants.CB_GET_HISTORY)) {
                    ...
                    switch (data.cmd) {
                        ...
                        case "rename":
                            if (this.protyle.path === data.data.path) {
                                ...
                                if (window.sourceflow.config.export.addTitle &&
                            ...
                            if (this.protyle.options.render.title && this.protyle.block.parentID === data.data.id) {
                                if (!document.body.classList.contains("body--blur") && getSelection().rangeCount > 0 &&
                                    ...
                                    // 标题编辑中的不用更新 https://github.com/lonelyor/SourceFlow/issues/6565
                                } else {
                                    this.protyle.title.setTitle(data.data.title, data.data.empty);
                                }
                                if (data.data.empty) {
                                    this.protyle.wysiwyg.element.setAttribute(Constants.CUSTOM_SF_TITLE_EMPTY, "true");
                                } else {
                                    this.protyle.wysiwyg.element.removeAttribute(Constants.CUSTOM_SF_TITLE_EMPTY);
                                }
                            }
                            // update ref
                            this.protyle.wysiwyg.element.querySelectorAll(`[data-type~="block-ref"][data-id="${data.data.id}"]`).forEach(item => {
                                if (item.getAttribute("data-subtype") === "d") {
                                    // 同 updateRef 一样处理 https://github.com/lonelyor/SourceFlow/issues/10458
                        ...
                        case "removeDoc":
                            if (data.data.ids.includes(this.protyle.block.rootID)) {
                                ...
                                /// #endif
                                delete window.sourceflow.storage[Constants.LOCAL_FILEPOSITION][this.protyle.block.rootID];
            ...
            if (options.backlinkData) {
                ...
                // 为了满足 eventPath0.style.paddingLeft 从而显示块标 https://github.com/lonelyor/SourceFlow/issues/11578
            ...
            if (this.protyle.options.mode !== "preview" &&
                options.rootId && window.sourceflow.storage[Constants.LOCAL_FILEPOSITION][options.rootId] &&
                (
                ...
                getDocByScroll({
                    protyle: this.protyle,
                    scrollAttr: window.sourceflow.storage[Constants.LOCAL_FILEPOSITION][options.rootId],
                    mergedOptions,
                    cb: () => {
    ...
    private getDoc(mergedOptions: IProtyleOptions) {
        fetchPost("/api/filetree/getDoc", {
            ...
            size: mergedOptions.action?.includes(Constants.CB_GET_ALL) ? Constants.SIZE_GET_MAX : window.sourceflow.config.editor.dynamicLoadBlocks,
        }, getResponse => {
            onGet({
                data: getResponse,
                protyle: this.protyle,
    ...
    private afterOnGet(mergedOptions: IProtyleOptions) {
        ...
        // 需等渲染完后再回调，用于定位搜索字段 https://github.com/lonelyor/SourceFlow/issues/3171
