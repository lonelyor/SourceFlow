...
export const commonHotkey = (protyle: IProtyle, event: KeyboardEvent, nodeElement?: HTMLElement) => {
    if (matchHotKey(window.sourceflow.config.keymap.editor.general.netImg2LocalAsset.custom, event)) {
    ...
    if (matchHotKey(window.sourceflow.config.keymap.editor.general.netAssets2LocalAssets.custom, event)) {
    ...
    if (matchHotKey(window.sourceflow.config.keymap.editor.general.optimizeTypography.custom, event)) {
...
export const upSelect = (options: {
    ...
    if (selectElements.length > 0) {
        ...
        const isExpandUp = matchHotKey(window.sourceflow.config.keymap.editor.general.expandUp.custom, options.event);
        if (!isMac() && isExpandUp) {
...
export const downSelect = (options: {
    ...
    if (selectElements.length > 0) {
        ...
        const innerText = nodeEditableElement.innerText;
        const isExpandDown = matchHotKey(window.sourceflow.config.keymap.editor.general.expandDown.custom, options.event);
