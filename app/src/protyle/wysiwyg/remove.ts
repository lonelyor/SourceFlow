...
export const removeBlock = async (protyle: IProtyle, blockElement: Element, range: Range, type: "Delete" | "Backspace" | "remove") => {
    ...
    if (selectElements?.length > 0) {
        ...
        for (let i = 0; i < selectElements.length; i++) {
            ...
            if (topElement.getAttribute("data-type") === "NodeHeading" && topElement.getAttribute("fold") === "1") {
                ...
                // https://github.com/lonelyor/SourceFlow/issues/4422
                ...
                // https://github.com/lonelyor/SourceFlow/issues/12327
        ...
        if (sideElement) {
            if (protyle.block.showAll && sideElement.classList.contains("protyle-wysiwyg") && protyle.wysiwyg.element.childElementCount === 0) {
                ...
                // https://github.com/lonelyor/SourceFlow/issues/5485
                // https://github.com/lonelyor/SourceFlow/issues/10389
                // https://github.com/lonelyor/SourceFlow/issues/10899
        ...
        /// #if !MOBILE
        if (!sideElement) {
            const backlinkElement = hasClosestByClassName(protyle.element, "sf__backlink", true);
            if (backlinkElement) {
                const backLinkTab = getInstanceById(backlinkElement.getAttribute("data-id"), window.sourceflow.layout.layout);
        ...
        /// #endif
        // https://github.com/lonelyor/SourceFlow/issues/16767
        setTimeout(() => {
            if (!document.contains(protyle.element)) {
                return;
            }
    ...
    // 设置 bq 和代码块光标
    // 需放在列表处理后 https://github.com/lonelyor/SourceFlow/issues/11606
    if (["NodeCodeBlock", "NodeTable", "NodeAttributeView"].includes(blockType)) {
        if (previousElement) {
            if (previousElement.classList.contains("p") && getContenteditableElement(previousElement).textContent === "") {
                // 空块向后删除时移除改块 https://github.com/lonelyor/SourceFlow/issues/11732
    ...
    if (isSelectNode) {
        ...
        if (editableElement.textContent !== "" ||
            // https://github.com/lonelyor/SourceFlow/issues/10207
    ...
    if (isSelectNode) {
        ...
        focusBlock(previousLastElement, undefined, false);
        // https://github.com/lonelyor/SourceFlow/issues/13254
        ...
        if (editableElement && (editableElement.textContent !== "" || editableElement.querySelector(".emoji"))) {
            ...
            // 数学公式回车后再删除 https://github.com/lonelyor/SourceFlow/issues/3850
        ...
        // https://github.com/lonelyor/SourceFlow/issues/14807
        ...
        // https://github.com/lonelyor/SourceFlow/issues/15554
...
// https://github.com/lonelyor/SourceFlow/issues/10393
