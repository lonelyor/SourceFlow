...
export const enter = (blockElement: HTMLElement, range: Range, protyle: IProtyle) => {
    ...
    // https://github.com/lonelyor/SourceFlow/issues/5471
    if (disableElement ||
        // https://github.com/lonelyor/SourceFlow/issues/10633
    ...
    if (trimStartHTML.startsWith("```") || trimStartHTML.startsWith("···") || trimStartHTML.startsWith("~~~") ||
        ...
        if (trimStartHTML.indexOf("\n") === -1 && trimStartHTML.replace(/·|~/g, "`").replace(/^`{3,}/g, "").indexOf("`") > -1) {
            // ```test` 不处理，正常渲染为段落块
        } else if (blockElement.classList.contains("p")) { // https://github.com/lonelyor/SourceFlow/issues/6953
            range.insertNode(document.createElement("wbr"));
            const oldHTML = blockElement.outerHTML;
            // https://github.com/lonelyor/SourceFlow/issues/16744
            ...
            if (languageElement) {
                if (window.sourceflow.storage[Constants.LOCAL_CODELANG] && languageElement.textContent === "") {
                    languageElement.textContent = window.sourceflow.storage[Constants.LOCAL_CODELANG];
                } else if (!Constants.SOURCEFLOW_RENDER_CODE_LANGUAGES.includes(languageElement.textContent)) {
                    window.sourceflow.storage[Constants.LOCAL_CODELANG] = languageElement.textContent;
                    setStorageVal(Constants.LOCAL_CODELANG, window.sourceflow.storage[Constants.LOCAL_CODELANG]);
                }
                if (Constants.SOURCEFLOW_RENDER_CODE_LANGUAGES.includes(languageElement.textContent)) {
    ...
    if (range.toString() !== "") {
        // 选中数学公式后回车取消选中 https://github.com/lonelyor/SourceFlow/issues/12637#issuecomment-2381106949
    ...
    // https://github.com/lonelyor/SourceFlow/issues/10759
    ...
    // https://github.com/lonelyor/SourceFlow/issues/480
    newElement.innerHTML = protyle.lute.SpinBlockDOM(newElement.innerHTML);

    // https://github.com/lonelyor/SourceFlow/issues/3850
    // https://github.com/lonelyor/SourceFlow/issues/6018
    // https://github.com/lonelyor/SourceFlow/issues/9682
    // 图片后的零宽空格前回车 https://github.com/lonelyor/SourceFlow/issues/5690
...
const listEnter = (protyle: IProtyle, blockElement: HTMLElement, range: Range) => {
    ...
    if (// \n 是因为 https://github.com/lonelyor/SourceFlow/issues/3846
    ...
    if (range.toString() === "" && position.start === 0 &&
        ...
        // https://github.com/lonelyor/SourceFlow/issues/8935
        const wbrElement = document.createElement("wbr");
        range.insertNode(wbrElement);
    ...
    if (subListElement && listItemElement.getAttribute("fold") !== "1" &&
        ...
        if (position.end >= editableElement.textContent.length -
            // 数学公式结尾会有 zwsp https://github.com/lonelyor/SourceFlow/issues/6679
            ...
            // https://github.com/lonelyor/SourceFlow/issues/13016
    ...
    // 回车移除空元素 https://github.com/lonelyor/SourceFlow/issues/480
    // https://github.com/lonelyor/SourceFlow/issues/12273
...
export const softEnter = (range: Range, nodeElement: HTMLElement, protyle: IProtyle) => {
    ...
    if (nextSibling && nextSibling.nodeType !== 3) {
        ...
        if (textPosition.end === range.endContainer.textContent.length) {
            // 图片之前软换行 || 数学公式之前软换行 https://github.com/lonelyor/SourceFlow/issues/13621
    ...
    // 行内元素末尾软换行 https://github.com/lonelyor/SourceFlow/issues/886
