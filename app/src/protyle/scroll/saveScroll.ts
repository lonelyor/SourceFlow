...
export const saveScroll = (protyle: IProtyle, getObject = false) => {
    if (!protyle.wysiwyg.element.firstElementChild || window.sourceflow.config.readonly) {
        // 报错或者空白页面
        return undefined;
    }
    ...
    window.sourceflow.storage[Constants.LOCAL_FILEPOSITION][protyle.block.rootID] = attr;
