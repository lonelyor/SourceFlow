...
export class Preview {
    ...
    constructor(protyle: IProtyle) {
        ...
        for (let i = 0; i < actions.length; i++) {
            ...
            switch (action) {
                case "desktop":
                    actionHtml.push(`<button type="button" class="protyle-preview__action--current" data-type="desktop">${window.sourceflow.languages.desktop}</button>`);
                    break;
                case "tablet":
                    actionHtml.push(`<button type="button" data-type="tablet">${window.sourceflow.languages.tablet}</button>`);
                    break;
                case "mobile":
                    actionHtml.push(`<button type="button" data-type="mobile">${window.sourceflow.languages.mobile}</button>`);
                    break;
                case "mp-wechat":
                    actionHtml.push(`<button type="button" data-type="mp-wechat" class="b3-tooltips b3-tooltips__w" aria-label="${window.sourceflow.languages.copyToWechatMP}"><svg><use xlink:href="#iconMp"></use></svg></button>`);
                    break;
                case "zhihu":
                    actionHtml.push(`<button type="button" data-type="zhihu" class="b3-tooltips b3-tooltips__w" aria-label="${window.sourceflow.languages.copyToZhihu}"><svg><use xlink:href="#iconZhihu"></use></svg></button>`);
                    break;
                case "yuque":
                    actionHtml.push(`<button type="button" data-type="yuque" class="b3-tooltips b3-tooltips__w" aria-label="${window.sourceflow.languages.copyToYuque}"><svg><use xlink:href="#iconYuque"></use></svg></button>`);
                    break;
            }
        }
        ...
        this.element.addEventListener("click", (event) => {
            ...
            while (target && !target.isEqualNode(this.element)) {
                if (target.tagName === "A") {
                    ...
                    if (isLocalPath(linkAddress)) {
                        ...
                        } else if (Constants.SOURCEFLOW_ASSETS_EXTS.includes(pathPosix().extname((linkAddress).split("?")[0]))) {
            ...
            if (nodeElement) {
                ...
                /// #else
                window.sourceflow.mobile.docks.outline?.setCurrentByPreview(nodeElement);
                /// #endif
            }
        });
    ...
    <img width="48px" src="/stage/loading-pure.svg">
        ...
        this.mdTimeoutId = window.setTimeout(() => {
            fetchPost("/api/export/preview", {
                ...
                speechRender(protyle.preview.previewElement, window.sourceflow.config.appearance.lang);
    ...
    private async copyToX(copyElement: HTMLElement, protyle: IProtyle, type?: string) {
        ...
        if (type === "mp-wechat") {
            ...
            // 列表嵌套 https://github.com/lonelyor/SourceFlow/issues/11276
            ...
            copyElement.querySelectorAll('[data-subtype="math"]').forEach((item: HTMLElement) => {
                // https://github.com/lonelyor/SourceFlow/issues/10015
            ...
            fetchPost("/api/lute/copyStdMarkdown", {
                ...
                showMessage(`${window.sourceflow.languages.pasteToYuque}`);
            });
            return;
        }
        ...
        if (type) {
            showMessage(`${type === "zhihu" ? window.sourceflow.languages.pasteToZhihu : window.sourceflow.languages.pasteToWechatMP}`);
        }
    }
