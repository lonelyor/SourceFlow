...
export class Title {
    ...
    constructor(protyle: IProtyle) {
        ...
        if (protyle.options.render?.titleShowTop) {
            ...
            this.element.innerHTML = `<span aria-label="${isMac() ? window.sourceflow.languages.gutterTip2 : window.sourceflow.languages.gutterTip2.replace("⇧", "Shift+")}" data-position="west" class="protyle-title__icon ariaLabel"><svg><use xlink:href="#iconFile"></use></svg></span>
<div contenteditable="true" spellcheck="${window.sourceflow.config.editor.spellcheck}" class="protyle-title__input" data-tip="${window.sourceflow.languages._kernel[16]}"> </div><div class="protyle-attr"></div>`;
            ...
            this.editElement.addEventListener("paste", (event: ClipboardEvent) => {
                ...
                let text = event.clipboardData.getData(Constants.SOURCEFLOW_HTML_CLIPBOARD_MIME);
                if (text) {
                    try {
                        JSON.parse(text);
            ...
            this.editElement.addEventListener("keydown", async (event: KeyboardEvent) => {
                ...
                if (event.key === "ArrowDown") {
                    ...
                    // https://github.com/lonelyor/SourceFlow/issues/11729
                    if (rects.length === 0 // 标题为空时时
                        ...
                        // https://github.com/lonelyor/SourceFlow/issues/4923
                    ...
                    event.preventDefault();
                    event.stopPropagation();
                } else if (matchHotKey(window.sourceflow.config.keymap.editor.general.attr.custom, event)) {
    ...
    public render(protyle: IProtyle, response: IWebSocketData) {
        ...
        this.setTitle(response.data.ial.title, isTitleEmptyAttr(response.data.ial));
        let nodeAttrHTML = "";
