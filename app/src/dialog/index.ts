...
export class Dialog {
    ...
    private ensureDialogPositionStorage() {
        if (!window.sourceflow.storage) {
            window.sourceflow.storage = {} as NonNullable<ISourceFlow["storage"]>;
        }
        if (!window.sourceflow.storage[Constants.LOCAL_DIALOGPOSITION]) {
            window.sourceflow.storage[Constants.LOCAL_DIALOGPOSITION] = {};
        }
        return window.sourceflow.storage[Constants.LOCAL_DIALOGPOSITION];
    }
    ...
    constructor(options: {
        ...
        this.id = genUUID();
        window.sourceflow.dialogs.push(this);
        this.destroyCallback = options.destroyCallback;
        this.element = document.createElement("div") as HTMLElement;
        let left;
        let top;
        if (!isMobile() && options.positionId) {
            const dialogPosition = this.ensureDialogPositionStorage()[options.positionId];
            if (dialogPosition) {
        ...
        this.element.innerHTML = `<div class="b3-dialog" style="z-index: ${++window.sourceflow.zIndex};${typeof left === "string" ? "display:block" : ""}">
  ...
  <div class="resize__rd"></div><div class="resize__ld"></div><div class="resize__lt"></div><div class="resize__rt"></div><div class="resize__r"></div><div class="resize__d"></div><div class="resize__t"></div><div class="resize__l"></div>
    ...
    public destroy(options?: IObject) {
        ...
        setTimeout(() => {
            ...
            if ((this.element.querySelector(".b3-dialog") as HTMLElement).style.zIndex < window.sourceflow.menus.menu.element.style.zIndex) {
                // https://github.com/lonelyor/SourceFlow/issues/6783
            ...
            window.sourceflow.dialogs.find((item, index) => {
                if (item.id === this.id) {
                    window.sourceflow.dialogs.splice(index, 1);
                    return true;
                }
            });
            // https://github.com/lonelyor/SourceFlow/issues/10475
