...
const ensureDialogPositionStorage = () => {
...
export const moveResize = (element: HTMLElement, afterCB?: (type: string) => void) => {
    element.addEventListener("mousedown", (event: MouseEvent & { target: HTMLElement }) => {
        // https://github.com/lonelyor/SourceFlow/issues/8746
        ...
        documentSelf.onmouseup = () => {
            ...
            if (dialogElement) {
                ...
                if (dialogId && element.offsetWidth) {
                    const dialogPositionStorage = ensureDialogPositionStorage();
                    dialogPositionStorage[dialogId] = {
                        width: element.offsetWidth,
                        height: element.offsetHeight,
                        left: parseInt(element.style.left),
                        top: parseInt(element.style.top),
                    };
                    setStorageVal(Constants.LOCAL_DIALOGPOSITION, dialogPositionStorage);
                }
            }
            if (hasMove && afterCB) {
                afterCB(type);
