...
export const getRandomEmoji = () => {
    const emojis = window.sourceflow.emojis[getRandom(0, window.sourceflow.emojis.length - 1)];
...
export const unicode2Emoji = (unicode: string, className = "", needSpan = false, lazy = false) => {
    ...
    if (unicode.startsWith("api/icon/getDynamicIcon")) {
        ...
        try {
            ...
            // 自定义表情搜索报错 https://github.com/lonelyor/SourceFlow/issues/5883
            // 这里忽略错误不做处理
        }
    }
    return emoji;
    ...
    const emojiIntersectionObserver = new IntersectionObserver((entries) => {
        entries.forEach((entrie: IntersectionObserverEntry & { target: HTMLImageElement }) => {
            ...
            if ((typeof entrie.isIntersecting === "undefined" ? entrie.intersectionRatio !== 0 : entrie.isIntersecting) && index) {
                let html = "";
                window.sourceflow.emojis[parseInt(index)].items.forEach(emoji => {
...
export const filterEmoji = (key = "", max?: number, hideCustom = false) => {
    ...
    if (key) {
        html = `<div class="emojis__title">${window.sourceflow.languages.emoji}</div><div class="emojis__content">`;
    }
    let maxCount = 0;
    let keyHTML = "";
    const customStore: IEmojiItem[] = [];
    window.sourceflow.emojis.forEach((category, index) => {
        ...
        if (category.items.length === 0 && index === 0 && !key) {
            html += `<div style="margin-left: 4px">${window.sourceflow.languages.setEmojiTip}</div>`;
        }

        category.items.forEach(emoji => {
            if (key) {
                if (window.sourceflow.config.editor.emoji.includes(emoji.unicode) &&
...
${unicode2Emoji(item.unicode, undefined, false, true)}</button>`;
    ...
    if (recentEmojis.length > 0) {
        recentHTML = `<div class="emojis__title" data-type="0">${window.sourceflow.languages.recentEmoji}</div><div class="emojis__content">`;
        window.sourceflow.config.editor.emoji.forEach(emojiUnicode => {
...
${unicode2Emoji(emoji[0].unicode, undefined, false, true)}
    ...
    if (recentHTML + html === "") {
        return `<div class="emojis__title">${window.sourceflow.languages.emptyContent}</div>`;
    }
    return recentHTML + html;
};

export const addEmoji = (unicode: string) => {
    window.sourceflow.config.editor.emoji.unshift(unicode);
    if (window.sourceflow.config.editor.emoji.length > Constants.SIZE_UNDO) {
        window.sourceflow.config.editor.emoji.pop();
    }
    window.sourceflow.config.editor.emoji = Array.from(new Set(window.sourceflow.config.editor.emoji));

    fetchPost("/api/setting/setEmoji", {emoji: window.sourceflow.config.editor.emoji});
};
...
const genWeekdayOptions = (lang: string, weekdayType: string) => {
    ...
    let currentLang = 0;
    if (lang === "") {
        lang = window.sourceflow.config.lang;
    }
    if (lang === "zh_CN") {
        currentLang = 1;
    }
...
const renderEmojiContent = (previousIndex: string, previousContentElement: Element) => {
    ...
    let html = "";
    window.sourceflow.emojis[parseInt(previousIndex)].items.forEach(emoji => {
...
export const openEmojiPanel = (
    ...
    if (type !== "av") {
        ...
        window.sourceflow.menus.menu.removeScrollEvent();
    }
    ...
    if (dynamicImgElement && dynamicImgElement.getAttribute("src").startsWith(dynamicURL)) {
        ...
        dynamicCurrentObj.content = dynamicCurrentUrl.get("content") || "SourceFlow";
    }
    ...
    <div class="emojis__tabheader">
        <div data-type="tab-emoji" class="ariaLabel block__icon block__icon--show" aria-label="${window.sourceflow.languages.emoji}"><svg><use xlink:href="#iconEmoji"></use></svg></div>
        <div class="fn__space"></div>
        <div data-type="tab-dynamic" class="ariaLabel block__icon block__icon--show${hide?.dynamic ? " fn__none" : ""}" aria-label="${window.sourceflow.languages.dynamicIcon}"><svg><use xlink:href="#iconCalendar"></use></svg></div>
        <div class="fn__flex-1"></div>
        <span class="block__icon block__icon--show fn__flex-center ariaLabel" data-action="remove" aria-label="${window.sourceflow.languages.remove}"><svg><use xlink:href="#iconTrashcan"></use></svg></span>
    </div>
    <div class="emojis__tabbody">
        <div class="fn__none" data-type="tab-emoji">
            ...
            <div class="fn__flex">
                ...
                <label class="b3-form__icon fn__flex-1" style="overflow:initial;">
                    ...
                    <input class="b3-form__icon-input b3-text-field fn__block" placeholder="${window.sourceflow.languages.search}">
                </label>
                <span class="fn__space"></span>
                <span class="block__icon block__icon--show fn__flex-center ariaLabel" data-action="random" aria-label="${window.sourceflow.languages.random}"><svg><use xlink:href="#iconRefresh"></use></svg></span>
                <span class="fn__space"></span>
            </div>
            <div class="emojis__panel">${filterEmoji("", null, hide?.custom)}</div>
            <div class="fn__flex">
                ${[
            ["2b50", window.sourceflow.languages.recentEmoji],
        ...
        <div class="fn__none" data-type="tab-dynamic">
            ...
            <div class="fn__flex">
                <span class="fn__space"></span>
                <span class="fn__flex-center ft__on-surface" style="width: 89px">${window.sourceflow.languages.language}</span>
                <span class="fn__space--small"></span>
                <select class="b3-select fn__flex-1">
                    <option value="" ${dynamicCurrentObj.lang === "" ? " selected" : ""}>${window.sourceflow.languages.themeOS}</option>
            ...
            <div class="fn__flex">
                <span class="fn__space"></span>
                <span class="fn__flex-center ft__on-surface" style="width: 89px">${window.sourceflow.languages.date}</span>
                <span class="fn__space--small"></span>
                <input type="date" max="9999-12-31" class="b3-text-field fn__flex-1" value="${dynamicCurrentObj.date}"/>
                <span class="fn__space--small"></span>
                <span data-action="clearDate" class="ariaLabel block__icon block__icon--show" aria-label="${window.sourceflow.languages.dynamicIconDateEmptyInfo}"><svg><use xlink:href="#iconTrashcan"></use></svg></span>
                <span class="fn__space"></span>
            </div>
            <div class="fn__hr"></div>
            <div class="fn__flex">
                <span class="fn__space"></span>
                <span class="fn__flex-center ft__on-surface" style="width: 89px">${window.sourceflow.languages.format}</span>
                <span class="fn__space--small"></span>
                <select class="b3-select fn__flex-1">
            ...
            <div class="fn__flex">
                <span class="fn__space"></span>
                <span class="fn__flex-center ft__on-surface" style="width: 89px">${window.sourceflow.languages.custom}</span>
                <span class="fn__space--small"></span>
    ...
    const currentTab = window.sourceflow.storage[Constants.LOCAL_EMOJIS].currentTab;
    ...
    lazyLoadEmoji(dialog.element);
    lazyLoadEmojiImg(dialog.element);
    // 不能使用 getEventName 否则 https://github.com/lonelyor/SourceFlow/issues/5472
    dialog.element.addEventListener("click", (event) => {
        ...
        while (target && target !== dialog.element) {
            if (target.classList.contains("emojis__type")) {
                ...
                window.sourceflow.storage[Constants.LOCAL_EMOJIS].currentTab = target.dataset.type.replace("tab-", "");
                setStorageVal(Constants.LOCAL_EMOJIS, window.sourceflow.storage[Constants.LOCAL_EMOJIS]);
                break;
...
export const updateOutlineEmoji = (unicode: string, id: string) => {
    ...
    getAllModels().outline.forEach(model => {
        if (model.blockId === id) {
            model.headerElement.nextElementSibling.firstElementChild.outerHTML = unicode2Emoji(unicode || window.sourceflow.storage[Constants.LOCAL_IMAGES].file, "b3-list-item__graphic", true);
        }
    });
    /// #endif
};
...
export const updateFileTreeEmoji = (unicode: string, id: string, icon = "iconFile") => {
    ...
    /// #endif
    if (emojiElement) {
        emojiElement.innerHTML = unicode2Emoji(unicode || (icon === "iconFile" ? (emojiElement.previousElementSibling.classList.contains("fn__hidden") ? window.sourceflow.storage[Constants.LOCAL_IMAGES].file : window.sourceflow.storage[Constants.LOCAL_IMAGES].folder) : window.sourceflow.storage[Constants.LOCAL_IMAGES].note));
    }
    if (icon !== "iconFile") {
        setNoteBook();
    }
...
export const getEmojiTitle = (index: number) => {
    if (window.sourceflow.config.lang === "zh_CN") {
        return window.sourceflow.emojis[index].title_zh_cn;
    }
    return window.sourceflow.emojis[index].title;
};

const putEmojis = (protyle: IProtyle) => {
    if (window.sourceflow.emojis[0].items.length > 0) {
        const emojis: IObject = {};
        window.sourceflow.emojis[0].items.forEach(emojiITem => {
...
export const applyEmojiConf = (emojis: IEmoji[]) => {
    if (!Array.isArray(emojis) || emojis.length === 0) {
        return;
    }
    window.sourceflow.emojis = emojis;
...
export const reloadEmoji = () => {
    fetchPost("/api/system/getEmojiConf", {}, response => {
        applyEmojiConf(response.data as IEmoji[]);
    });
};
