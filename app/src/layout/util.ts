...
import {syncActivityBarState} from "./activityBar";
...
const dockToJSON = (dock: Dock) => {
    ...
    if (data0.length > 0 || data2.length > 0) {
        // https://github.com/lonelyor/SourceFlow/issues/5641
        json.push(data0);
    }
    if (data2.length > 0) {
        json.push(data2);
...
const getDockHiddenState = () => {
    return !!window.sourceflow.config.uiLayout?.hideDock;
};

const createDesktopLayoutJSON = () => {
    return {
        hideDock: getDockHiddenState(),
        layout: {},
        bottom: dockToJSON(window.sourceflow.layout.bottomDock),
        left: dockToJSON(window.sourceflow.layout.leftDock),
        right: dockToJSON(window.sourceflow.layout.rightDock),
    };
};
...
export const resetLayout = () => {
    if (window.sourceflow.config.readonly) {
        ...
        fetchPost("/api/system/setUILayout", {layout: {}}, () => {
            ...
            window.sourceflow.storage[Constants.LOCAL_DIALOGPOSITION] = {};
            setStorageVal(Constants.LOCAL_DIALOGPOSITION, window.sourceflow.storage[Constants.LOCAL_DIALOGPOSITION]);
            window.location.reload();
        });
    }
};
...
export const saveLayout = () => {
    ...
    if (isWindow()) {
        ...
        layoutToJSON(window.sourceflow.layout.layout, layoutJSON.layout, breakObj);
    } else {
        layoutJSON = createDesktopLayoutJSON();
        layoutToJSON(window.sourceflow.layout.layout, layoutJSON.layout, breakObj);
        window.sourceflow.config.uiLayout = layoutJSON;
    }
...
export const exportLayout = async (options: {
    ...
    if (isWindow()) {
        ...
        layoutToJSON(window.sourceflow.layout.layout, layoutJSON.layout);
        sessionStorage.setItem("layout", JSON.stringify(layoutJSON));
        options.cb();
        return;
    }
    const layoutJSON: any = createDesktopLayoutJSON();
    layoutToJSON(window.sourceflow.layout.layout, layoutJSON.layout);
    if (window.sourceflow.config.readonly) {
        options.cb();
    } else {
...
const initInternalDock = (dockItem: Config.IUILayoutDockTab[]) => {
    dockItem.forEach((existSubItem) => {
        if (existSubItem.hotkeyLangId) {
            existSubItem.title = window.sourceflow.languages[existSubItem.hotkeyLangId];
            existSubItem.hotkey = window.sourceflow.config.keymap.general[existSubItem.hotkeyLangId].custom;
        }
    });
};

const BUILTIN_DOCK_DEFAULTS: Record<string, Config.IUILayoutDockTab> = {
    file: {
        type: "file",
        size: {width: 184, height: 0},
        show: true,
        icon: "iconFiles",
...
const ensureDockGroups = (dock: { pin?: boolean, data?: Config.IUILayoutDockTab[][] }) => {
    if (!Array.isArray(dock.data)) {
        dock.data = [];
    }
    if (!dock.data[0]) {
        dock.data[0] = [];
...
const normalizeBuiltinDockItem = (type: keyof typeof BUILTIN_DOCK_DEFAULTS, existing?: Config.IUILayoutDockTab, forceShow = false) => {
    const defaults = BUILTIN_DOCK_DEFAULTS[type];
    return {
        ...defaults,
        ...existing,
        type,
        icon: defaults.icon,
        hotkeyLangId: defaults.hotkeyLangId,
        size: {
            ...defaults.size,
            ...(existing?.size || {}),
        },
        show: forceShow ? true : (existing?.show ?? defaults.show),
    };
};

const normalizeDockPlacement = (json: any) => {
    ensureDockGroups(json.left);
    ensureDockGroups(json.right);
    ensureDockGroups(json.bottom);
    const builtinTypes = Object.keys(BUILTIN_DOCK_DEFAULTS);
    const collected: Partial<Record<keyof typeof BUILTIN_DOCK_DEFAULTS, Config.IUILayoutDockTab>> = {};
    const stripBuiltinItems = (groups: Config.IUILayoutDockTab[][]) => {
        groups.forEach((group) => {
            for (let i = group.length - 1; i >= 0; i--) {
                const item = group[i];
                if (!builtinTypes.includes(item.type)) {
                    continue;
                }
                if (!collected[item.type as keyof typeof BUILTIN_DOCK_DEFAULTS]) {
                    collected[item.type as keyof typeof BUILTIN_DOCK_DEFAULTS] = item;
                }
                group.splice(i, 1);
            }
        });
    };
    stripBuiltinItems(json.left.data);
    stripBuiltinItems(json.right.data);
    json.left.data[0].unshift(normalizeBuiltinDockItem("file", collected.file, true));
    json.right.data[0].unshift(
        normalizeBuiltinDockItem("outline", collected.outline),
        normalizeBuiltinDockItem("bookmark", collected.bookmark),
        normalizeBuiltinDockItem("tag", collected.tag),
    );
    json.right.data[1].unshift(
        normalizeBuiltinDockItem("graph", collected.graph),
        normalizeBuiltinDockItem("globalGraph", collected.globalGraph),
        normalizeBuiltinDockItem("backlink", collected.backlink),
    );
};

const JSONToDock = (json: any, app: App) => {
    normalizeDockPlacement(json);
    ...
    window.sourceflow.layout.centerLayout = window.sourceflow.layout.layout.children[0].children[1] as Layout;
    window.sourceflow.layout.leftDock = new Dock({position: "Left", data: json.left, app});
    window.sourceflow.layout.rightDock = new Dock({position: "Right", data: json.right, app});
    window.sourceflow.layout.bottomDock = new Dock({position: "Bottom", data: json.bottom, app});
};
...
export const JSONToCenter = (
    ...
    if (json.instance === "Layout") {
        // TabA 向右分屏后向下分屏，依次关闭右侧、上侧分屏无法移除 layout 嵌套，故在此解决 https://github.com/lonelyor/SourceFlow/issues/12196
        ...
        if (!layout) {
            window.sourceflow.layout.layout = new Layout({
        ...
        if (!json.title) {
            ...
            if (json.lang) {
                title = window.sourceflow.languages[json.lang];
            }
            child = new Tab({
                icon: json.icon,
    ...
    if ("children" in json) {
        if (Array.isArray(json.children)) {
            json.children.forEach((item: any) => {
                JSONToCenter(app, item, layout ? child : window.sourceflow.layout.layout);
            });
...
export const JSONToLayout = (app: App, isStart: boolean) => {
    JSONToCenter(app, window.sourceflow.config.uiLayout.layout, undefined);
    JSONToDock(window.sourceflow.config.uiLayout, app);
    // 启动时不打开页签，需要移除没有钉住的页签
    if (window.sourceflow.config.fileTree.closeTabsOnStart) {
        /// #if BROWSER
    ...
    document.querySelectorAll('li[data-type="tab-header"]').forEach((item: HTMLElement) => {
        ...
        if (initData) {
            ...
            if (initDataObj.instance === "Custom" && initDataObj.customModelType !== "sourceflow-card") {
                let hasPlugin = false;
    ...
    if (idZoomIn.id) {
        ...
        // 移除没有数据的页签 https://github.com/lonelyor/SourceFlow/issues/13390
...
export const resizeTopBar = () => {
    ...
    if (document.body.classList.contains("body--activitybar-unified")) {
        dragElement.style.padding = "";
        toolbarElement.querySelectorAll('[data-static-hide="true"]').forEach((item: HTMLElement) => {
    ...
    window.sourceflow.storage[Constants.LOCAL_PLUGINTOPUNPIN].forEach((id: string) => {
...
export const newModelByInitData = (app: App, tab: Tab, json: any) => {
    ...
    if (json.instance === "Custom") {
        if (json.customModelType === "sourceflow-card") {
            model = newCardModel({
                app,
                tab: tab,
...
export const pdfIsLoading = (element: HTMLElement) => {
    ...
    if (isLoading) {
        showMessage(window.sourceflow.languages.pdfIsLoading);
    }
    return isLoading;
};

export const getInstanceById = (id: string, layout = window.sourceflow.layout.centerLayout) => {
...
export const addResize = (obj: Layout | Wnd) => {
    ...
    const getMinSize = (element: HTMLElement) => {
        ...
        Array.from(element.querySelectorAll(".file-tree")).find((item) => {
            if (item.classList.contains("sf__backlink") || item.classList.contains("sf__graph")
                || item.classList.contains("sf__globalGraph")) {
    ...
    const resizeWnd = (resizeElement: HTMLElement, direction: string) => {
        ...
        resizeElement.addEventListener("mousedown", (event: MouseEvent) => {
            ...
            nextElement.style.overflow = "auto"; // 拖动时 layout__resize 会出现 https://github.com/lonelyor/SourceFlow/issues/6221
            ...
            documentSelf.ondragstart = () => {
                // 文件树拖拽会产生透明效果
                document.querySelectorAll(".sf__file .b3-list-item").forEach((item: HTMLElement) => {
            ...
            documentSelf.onmousemove = (moveEvent: MouseEvent) => {
                ...
                if (window.sourceflow.layout.leftDock && window.sourceflow.layout.leftDock.layout.element === previousElement &&
                    previousNowSize < getMinSize(previousElement) &&
                    // https://github.com/lonelyor/SourceFlow/issues/10506
                    previousNowSize < previousSize) {
                    return;
                }
                if (window.sourceflow.layout.rightDock && window.sourceflow.layout.rightDock.layout.element === nextElement &&
                    nextNowSize < getMinSize(nextElement) && nextNowSize < nextSize) {
                    return;
                }
                if (window.sourceflow.layout.bottomDock && window.sourceflow.layout.bottomDock.layout.element === nextElement &&
            ...
            documentSelf.onmouseup = () => {
                ...
                adjustLayout(isWindow() ? window.sourceflow.layout.centerLayout : undefined);
                resizeTabs();
                if (!isWindow()) {
                    window.sourceflow.layout.leftDock.setSize();
                    window.sourceflow.layout.bottomDock.setSize();
                    window.sourceflow.layout.rightDock.setSize();
                }
                if (range) {
                    focusByRange(range);
                }
        ...
        if (previousElement && nextElement) {
            ...
            if (resizeElement.classList.contains("layout__resize--lr")) {
                ...
                if (nextElement.classList.contains("layout__dockb")) {
                    nextElement.style.height = "184px";
                    window.sourceflow.layout.bottomDock.setSize();
                } else {
...
export const adjustLayout = (layout: Layout = window.sourceflow.layout.centerLayout.parent) => {
