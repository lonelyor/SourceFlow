...
<span class="item__close"><svg><use xlink:href="#iconClose"></use></svg></span>`;
            ...
            this.headElement.addEventListener("dragstart", (event: DragEvent & { target: HTMLElement }) => {
                ...
                if (tabElement) {
                    ...
                    event.dataTransfer.setData(Constants.SOURCEFLOW_DROP_TAB, JSON.stringify(modeJSON));
                    event.dataTransfer.dropEffect = "move";
                    tabElement.style.opacity = "0.38";
                    window.sourceflow.dragElement = this.headElement;
                }
                /// #if !BROWSER
                ipcRenderer.send(Constants.SOURCEFLOW_SEND_WINDOWS, {cmd: "resetTabsStyle", data: "removeRegionStyle"});
                /// #endif
            });
            this.headElement.addEventListener("dragend", (event: DragEvent & { target: HTMLElement }) => {
                ...
                /// #if !BROWSER
                ipcRenderer.send(Constants.SOURCEFLOW_SEND_WINDOWS, {cmd: "resetTabsStyle", data: "addRegionStyle"});
                /// #endif
            });
        }
