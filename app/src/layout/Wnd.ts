...
export class Wnd {
    ...
    <div class="fn__flex fn__none">
        ...
        <ul class="layout-tab-bar layout-tab-bar--readonly fn__flex-1">
            <li class="item item--readonly">
                <span data-type="new" class="block__icon block__icon--show ariaLabel${window.sourceflow.config.readonly ? " fn__none" : ""}" aria-label="${window.sourceflow.languages.newFile}"><svg><use xlink:href="#iconAdd"></use></svg></span>
                <span class="fn__flex-1"></span>
                <span data-type="more" data-menu="true" class="block__icon block__icon--show ariaLabel" aria-label="${window.sourceflow.languages.switchTab}"><svg><use xlink:href="#iconDown"></use></svg></span>
            </li>
        </ul>
    </div>
    <div class="layout-tab-container fn__flex-1">${dragHTML}</div>
        ...
        this.headersElement.addEventListener("mousedown", (event) => {
            ...
            while (target && !target.isEqualNode(this.headersElement)) {
                if (target.tagName === "LI") {
                    ...
                    const frontend = getFrontend() as string;
                    if ((["desktop", "desktop-window"].includes(frontend) && window.sourceflow.config.system.os === "linux") ||
                        ...
                        // TODO 保持原有焦点？https://github.com/lonelyor/SourceFlow/pull/13395/files#r1877004077
        ...
        this.headersElement.parentElement.addEventListener("dblclick", (event) => {
            let target = event.target as HTMLElement;
            while (target && !target.isEqualNode(this.headersElement)) {
                if (window.sourceflow.config.fileTree.openFilesUseCurrentTab && target.getAttribute("data-type") === "tab-header") {
        ...
        this.headersElement.parentElement.addEventListener("dragover", function (event: DragEvent & {
            ...
            if (!window.sourceflow.currentDragOverTabHeadersElement) {
                window.sourceflow.currentDragOverTabHeadersElement = it;
            } else {
                if (window.sourceflow.currentDragOverTabHeadersElement !== it) {
                    window.sourceflow.currentDragOverTabHeadersElement.classList.remove("layout-tab-bars--drag");
                    window.sourceflow.currentDragOverTabHeadersElement.querySelectorAll(".layout-tab-bar li[data-clone='true']").forEach(item => {
                        item.remove();
                    });
                    window.sourceflow.currentDragOverTabHeadersElement = it;
                }
            }
            if (event.dataTransfer.types.includes(Constants.SOURCEFLOW_DROP_FILE)) {
                event.preventDefault();
                it.classList.add("layout-tab-bars--drag");
                return;
            }
            // 不能使用 !window.sourceflow.dragElement，因为移动页签到新窗口后，再把主窗口页签拖拽新窗口页签上时，该值为空
            if (!event.dataTransfer.types.includes(Constants.SOURCEFLOW_DROP_TAB)) {
                return;
            }
            event.preventDefault();
            let oldTabHeaderElement = window.sourceflow.dragElement;
            let exitDrag = false;
        ...
        this.headersElement.parentElement.addEventListener("drop", function (event: DragEvent & {
            ...
            if (event.dataTransfer.types.includes(Constants.SOURCEFLOW_DROP_FILE)) {
                ...
                event.dataTransfer.getData(Constants.SOURCEFLOW_DROP_FILE).split(",").forEach(item => {
                    if (item) {
                        openFileById({
                            app,
                            id: item,
            ...
            const tabData = JSON.parse(event.dataTransfer.getData(Constants.SOURCEFLOW_DROP_TAB));
            ...
            if (!oldTab) { // 从主窗口拖拽到页签新窗口
                if (wnd instanceof Wnd) {
                    ...
                    ipcRenderer.send(Constants.SOURCEFLOW_SEND_WINDOWS, {cmd: "closetab", data: tabData.id});
                    it.querySelector("li[data-clone='true']").remove();
                    wnd.switchTab(oldTab.headElement);
                    ipcRenderer.send(Constants.SOURCEFLOW_CMD, "focus");
                }
            }
            /// #endif
            if (!oldTab) {
        ...
        //  dragElement dragleave 后还会触发 dragenter https://github.com/lonelyor/SourceFlow/issues/13753
    ...
    public switchTab(target: HTMLElement, pushBack = false, update = true, resize = true, isSaveLayout = true) {
        ...
        this.children.forEach((item) => {
            if (target === item.headElement) {
                if (item.headElement && item.headElement.classList.contains("fn__none")) {
                    // https://github.com/lonelyor/SourceFlow/issues/267
                } else {
                    if (item.headElement) {
        ...
        if (currentTab && target === currentTab.headElement) {
            if (currentTab.model instanceof Graph) {
                ...
                // https://github.com/lonelyor/SourceFlow/issues/5655
        ...
        if (currentTab && currentTab.model instanceof Editor) {
            ...
            if (window.sourceflow.editorIsFullscreen && !currentTab.model.editor.protyle.element.className.includes("fullscreen")) {
    ...
    public addTab(tab: Tab, keepCursor = false, isSaveLayout = true, activeTime?: string) {
        ...
        } else if (this.children.length > window.sourceflow.config.fileTree.maxOpenTabCount) {
    ...
    private renderTabList(target: HTMLElement) {
        if (!window.sourceflow.menus.menu.element.classList.contains("fn__none") &&
            window.sourceflow.menus.menu.element.getAttribute("data-name") === Constants.MENU_TAB_LIST) {
            window.sourceflow.menus.menu.remove();
            return;
        }
        window.sourceflow.menus.menu.remove();
        ...
        Array.from(this.headersElement.children).forEach((item: HTMLElement) => {
            ...
            if (iconElement) {
                ...
                // 没有图标的文档
                iconHTML = unicode2Emoji(window.sourceflow.storage[Constants.LOCAL_IMAGES].file, "b3-menu__icon", true);
            }
                    ...
                    element.addEventListener("click", (itemEvent) => {
                        if (hasClosestByClassName(itemEvent.target as Element, "b3-menu__action")) {
                            ...
                            if (element.previousElementSibling || element.nextElementSibling) {
                                element.remove();
                                setPosition(window.sourceflow.menus.menu.element, rect.left + rect.width - window.sourceflow.menus.menu.element.clientWidth, rect.top + rect.height);
                            } else {
                                window.sourceflow.menus.menu.remove();
                            }
                        } else {
        ...
        window.sourceflow.menus.menu.element.setAttribute("data-name", Constants.MENU_TAB_LIST);
    ...
    private removeOverCounter(isSaveLayout = false) {
        ...
        if (removeCount > 0 && this.children.length > window.sourceflow.config.fileTree.maxOpenTabCount) {
            this.removeOverCounter(isSaveLayout);
        }
    }
    ...
    private removeTabAction = (id: string, isBatchClose = false, animate = true, isSaveLayout = true) => {
        ...
        this.children.find((item, index) => {
            if (item.id === id) {
                if (window.sourceflow.storage[Constants.LOCAL_CLOSED_TABS].length > Constants.SIZE_UNDO) {
                    window.sourceflow.storage[Constants.LOCAL_CLOSED_TABS].pop();
                }
                if (item.headElement) {
                    const tabJSON = {};
                    layoutToJSON(item, tabJSON);
                    window.sourceflow.storage[Constants.LOCAL_CLOSED_TABS].push(tabJSON);
                    ...
                    if (item.headElement.classList.contains("item--focus")) {
                        ...
                        Array.from(item.headElement.parentElement.children).forEach((headItem: HTMLElement) => {
                            if (headItem !== item.headElement &&
                                headItem.style.maxWidth !== "0px"   // 不对比已移除但还在动画效果中的元素 https://github.com/lonelyor/SourceFlow/issues/7878
                            ) {
        ...
        if (window.sourceflow.layout.centerLayout) {
            const wnd = getWndByLayout(window.sourceflow.layout.centerLayout);
            if (!wnd) {
                /// #if !BROWSER
                if (isWindow()) {
                ...
                window.sourceflow.layout.centerLayout.addWnd(wnd);
        ...
        /// #if !BROWSER
        webFrame.clearCache();
        ipcRenderer.send(Constants.SOURCEFLOW_CMD, "clearCache");
        setTabPosition();
        setModelsHash();
        /// #endif
    };
            ...
            if (item.id === id) {
                if ((item.model instanceof Editor) && item.model.editor?.protyle) {
                    if (item.model.editor.protyle.upload.isUploading) {
                        showMessage(window.sourceflow.languages.uploading);
                        return;
                    }
    ...
    public moveTab(tab: Tab, nextId?: string) {
        ...
        if (nextId) {
            // 只能用 find https://github.com/lonelyor/SourceFlow/issues/3455
            this.children.find((item, index) => {
                if (item.id === nextId) {
        ...
        if (this.children.length > window.sourceflow.config.fileTree.maxOpenTabCount) {
        ...
        // https://github.com/lonelyor/SourceFlow/issues/13551
        ...
        layout.children.find((item, index) => {
            if (item.id === id) {
                if (layout.children.length > 1) {
                    ...
                    // https://github.com/lonelyor/SourceFlow/issues/5844
