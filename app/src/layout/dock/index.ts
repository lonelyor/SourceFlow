...
import {refreshActivityBar, syncActivityBarState} from "../activityBar";

const TYPES = ["file", "outline", "bookmark", "tag", "graph", "globalGraph", "backlink"];

const getDockMinWidth = (element: Element) => {
    if ((element as HTMLElement).classList.contains("sf__backlink") ||
        (element as HTMLElement).classList.contains("sf__graph") ||
        (element as HTMLElement).classList.contains("sf__globalGraph")) {
        return 240;
    }
    if ((element as HTMLElement).classList.contains("sf__terminal")) {
        return 168;
    }
    if ((element as HTMLElement).classList.contains("sf__ai")) {
        return 144;
    }
    return 184;
};

const getDockDefaultWidth = (type: string | null) => {
    if (type === "terminal") {
        return 360;
    }
    if (type === "ai") {
    ...
    if (type === "graph" || type === "globalGraph" || type === "backlink") {
        return 240;
    }
    return 184;
};
...
export class Dock {
    ...
    constructor(options: {
        ...
        switch (options.position) {
            case "Left":
                this.layout = window.sourceflow.layout.layout.children[0].children[0] as Layout;
                ...
                break;
            case "Right":
                this.layout = window.sourceflow.layout.layout.children[0].children[2] as Layout;
                ...
                break;
            case "Bottom":
                this.layout = window.sourceflow.layout.layout.children[1] as Layout;
                this.resizeElement = this.layout.element.previousElementSibling as HTMLElement;
        ...
        if (!showDock) {
            this.element.firstElementChild.innerHTML = `<span class="dock__item dock__item--pin ariaLabel" aria-label="${this.pin ? window.sourceflow.languages.unpin : window.sourceflow.languages.pin}">
    <svg><use xlink:href="#icon${this.pin ? "Unpin" : "Pin"}"></use></svg>
        ...
        this.element.addEventListener("click", (event) => {
            ...
            while (target && !target.isEqualNode(this.element)) {
                ...
                if (type) {
                    ...
                    this.togglePin();
                    target.setAttribute("aria-label", this.pin ? window.sourceflow.languages.unpin : window.sourceflow.languages.pin);
            ...
            documentSelf.onmouseup = () => {
                ...
                if (!moveItem.classList.contains("fn__none")) {
                    ...
                    if (moveItem.parentElement.parentElement.id === "dockBottom") {
                        dock = window.sourceflow.layout.bottomDock;
                    } else if (moveItem.parentElement.parentElement.id === "dockLeft") {
                        dock = window.sourceflow.layout.leftDock;
                    } else if (moveItem.parentElement.parentElement.id === "dockRight") {
                        dock = window.sourceflow.layout.rightDock;
                    }
        ...
        this.layout.element.querySelector(".layout__dockresize").addEventListener("mousedown", (event: MouseEvent) => {
            ...
            documentSelf.onmousemove = (moveEvent: MouseEvent) => {
                ...
                let minSize = 184;
                Array.from(this.layout.element.querySelectorAll(".dockPanel")).find((item) => {
                    if (!(item instanceof HTMLElement) || item.classList.contains("fn__none") || hasClosestByClassName(item, "fn__none")) {
                        return false;
                    }
                    minSize = getDockMinWidth(item);
                    return true;
                });
        ...
        if (window.sourceflow.config.uiLayout.hideDock) {
            this.element.classList.add("fn__none");
        }
        if (!this.pin) {
            setTimeout(() => {
    ...
    public showDock(reset = false) {
        ...
        if ((
            ...
            window.sourceflow.layout.leftDock?.layout.element.style.opacity === "1" ||
            window.sourceflow.layout.rightDock?.layout.element.style.opacity === "1" ||
            window.sourceflow.layout.bottomDock?.layout.element.style.opacity === "1"
        )) {
            return;
        }
        ...
        this.layout.element.style.transform = "";
        this.layout.element.style.zIndex = (++window.sourceflow.zIndex).toString();
        if (this.position === "Left") {
    ...
    public hideDock(reset = false) {
        ...
        // 关系图全屏不应该退出 & https://github.com/lonelyor/SourceFlow/issues/11775
        ...
        // https://github.com/lonelyor/SourceFlow/issues/7504
    ...
    public add(index: number, sourceElement: Element, previousType?: string) {
        ...
        refreshActivityBar();
        // 保存布局需等待动画完毕 https://github.com/lonelyor/SourceFlow/issues/13507
        setTimeout(() => {
            saveLayout();
    ...
    private getMaxSize() {
        ...
        this.element.querySelectorAll(".dock__item--active").forEach((item) => {
            let size;
            if (this.position === "Left" || this.position === "Right") {
                size = parseInt(item.getAttribute("data-width")) || getDockDefaultWidth(item.getAttribute("data-type"));
            } else {
                size = parseInt(item.getAttribute("data-height")) || 184;
            }
            if (size > max) {
                max = size;
            }
    ...
    public genButton(data: Config.IUILayoutDockTab[], index: number, tabIndex?: number) {
        ...
        data.forEach(item => {
            ...
            html += `<span data-height="${item.size.height}" data-width="${item.size.width}" data-type="${item.type}" data-index="${index}" data-hotkey="${item.hotkey || ""}" data-hotkeylangid="${item.hotkeyLangId || ""}" data-title="${item.title}" class="dock__item${item.show ? " dock__item--active" : ""} ariaLabel" aria-label="<span style='white-space:pre'>${item.title} ${item.hotkey ? updateHotkeyTip(item.hotkey) : ""}${window.sourceflow.languages.dockTip}</span>">
    <svg><use xlink:href="#${item.icon}"></use></svg>
        ...
        if (index === 0) {
            if (typeof tabIndex === "number") {
                ...
                this.element.firstElementChild.innerHTML = `${html}<span class="dock__item dock__item--pin ariaLabel" aria-label="${this.pin ? window.sourceflow.languages.unpin : window.sourceflow.languages.pin}">
    <svg><use xlink:href="#icon${this.pin ? "Unpin" : "Pin"}"></use></svg>
</span>`;
            }
        } else {
        ...
        if (typeof tabIndex === "number") {
            // https://github.com/lonelyor/SourceFlow/issues/8614
            if (!window.sourceflow.config.uiLayout.hideDock) {
                this.element.classList.remove("fn__none");
            }
            if (data[0].show) {
