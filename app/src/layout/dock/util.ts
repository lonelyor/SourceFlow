...
export const openBacklink = async (options: {
    ...
    if (!options.rootId) {
        ...
        options.title = response.data.name || window.sourceflow.languages.untitled;
...
export const resetFloatDockSize = () => {
    if (!window.sourceflow.layout.leftDock.pin && window.sourceflow.layout.leftDock.layout.element.style.opacity === "1") {
        window.sourceflow.layout.leftDock.showDock(true);
    }
    if (!window.sourceflow.layout.rightDock.pin && window.sourceflow.layout.rightDock.layout.element.style.opacity === "1") {
        window.sourceflow.layout.rightDock.showDock(true);
    }
    if (!window.sourceflow.layout.bottomDock.pin && window.sourceflow.layout.bottomDock.layout.element.style.opacity === "1") {
        window.sourceflow.layout.bottomDock.showDock(true);
    }
};

export const toggleDockBar = (useElement?: Element | null) => {
    const hideDock = !window.sourceflow.config.uiLayout.hideDock;
    if (useElement) {
        useElement.setAttribute("xlink:href", hideDock ? "#iconDock" : "#iconHideDock");
    }
    window.sourceflow.config.uiLayout.hideDock = hideDock;
