...
export class Outline extends Model {
    ...
    constructor(options: {
        ...
        options.tab.panelElement.classList.add("fn__flex-column", "file-tree", "sf__outline", "dockPanel");
        options.tab.panelElement.innerHTML = `<div class="block__icons fn__hidescrollbar">
    <div class="block__logo">
        <svg class="block__logoicon"><use xlink:href="#iconAlignCenter"></use></svg>${window.sourceflow.languages.outline}
    </div>
    <span class="fn__flex-1 fn__space"></span>
    <input class="b3-text-field search__label fn__none fn__size200" placeholder="${window.sourceflow.languages.filterKeywordEnter}" />
    <span data-type="search" class="block__icon ariaLabel" aria-label="${window.sourceflow.languages.filter}">
        <svg><use xlink:href='#iconFilter'></use></svg>
    </span>
    <span class="fn__space"></span>
    <span data-type="keepCurrentExpand" class="block__icon ariaLabel${window.sourceflow.storage[Constants.LOCAL_OUTLINE].keepCurrentExpand ? " block__icon--active" : ""}" aria-label="${window.sourceflow.languages.outlineKeepCurrentExpand}">
        <svg><use xlink:href="#iconFocus"></use></svg>
    </span>
    <span class="fn__space"></span>
    <span data-type="expandLevel" class="block__icon ariaLabel" aria-label="${window.sourceflow.languages.expandLevel}">
        <svg><use xlink:href="#iconList"></use></svg>
    </span>
    <span class="fn__space"></span>
    <span data-type="expand" class="block__icon ariaLabel" aria-label="${window.sourceflow.languages.expandAll}${updateHotkeyAfterTip(window.sourceflow.config.keymap.editor.general.expand.custom)}">
        <svg><use xlink:href="#iconExpand"></use></svg>
    </span>
    <span class="fn__space"></span>
    <span data-type="collapse" class="block__icon ariaLabel" aria-label="${window.sourceflow.languages.foldAll}${updateHotkeyAfterTip(window.sourceflow.config.keymap.editor.general.collapse.custom)}">
        <svg><use xlink:href="#iconContract"></use></svg>
    </span>
    <span class="${this.type === "local" ? "fn__none " : ""}fn__space"></span>
    <span data-type="min" class="${this.type === "local" ? "fn__none " : ""}block__icon ariaLabel" aria-label="${window.sourceflow.languages.min}${updateHotkeyAfterTip(window.sourceflow.config.keymap.general.closeTab.custom)}">
...
<div class="fn__flex-1" style="padding: 3px 0 8px"></div>`;
        ...
        inputElement.addEventListener("blur", () => {
            ...
            if (value) {
                ...
                filterIconElement.setAttribute("aria-label", window.sourceflow.languages.filter + " " + escapeAttr(value));
            } else {
                filterIconElement.classList.remove("block__icon--active");
                filterIconElement.setAttribute("aria-label", window.sourceflow.languages.filter);
            }
                ...
                if (this.isPreview) {
                    ...
                    checkFold(id, (zoomIn) => {
                        openFileById({
                            ...
                            action: zoomIn ? [Constants.CB_GET_HL, Constants.CB_GET_FOCUS, Constants.CB_GET_ALL, Constants.CB_GET_HTML, Constants.CB_GET_OUTLINE] : [Constants.CB_GET_HL, Constants.CB_GET_FOCUS, Constants.CB_GET_OUTLINE, Constants.CB_GET_SETID, Constants.CB_GET_CONTEXT, Constants.CB_GET_HTML],
                        });
                    });
                }
            },
        ...
        options.tab.panelElement.querySelector('[data-type="keepCurrentExpand"]').addEventListener("click", (event: MouseEvent & {
            ...
            if (iconElement.classList.contains("block__icon--active")) {
                ...
                window.sourceflow.storage[Constants.LOCAL_OUTLINE].keepCurrentExpand = false;
            } else {
                iconElement.classList.add("block__icon--active");
                window.sourceflow.storage[Constants.LOCAL_OUTLINE].keepCurrentExpand = true;
                let focusElement;
            ...
            setStorageVal(Constants.LOCAL_OUTLINE, window.sourceflow.storage[Constants.LOCAL_OUTLINE]);
        });
            ...
            documentSelf.onmouseup = () => {
                ...
                if (selectItem && editor &&
                    ...
                    if (hasChange) {
                        ...
                        // https://github.com/lonelyor/SourceFlow/issues/10828#issuecomment-2044099675
    ...
    public updateDocTitle(ial?: IObject, count?: number) {
        ...
        if (this.type === "pin") {
            ...
            if (ial) {
                let iconHTML = `${unicode2Emoji(ial.icon || window.sourceflow.storage[Constants.LOCAL_IMAGES].file, "b3-list-item__graphic", true)}`;
    ...
    private onTransaction(data: IWebSocketData) {
        ...
        if (needReload) {
            fetchPost("/api/outline/getDocOutline", {
                ...
                // 文档切换后不再更新原有推送 https://github.com/lonelyor/SourceFlow/issues/13409
                if (data.data.rootID !== this.blockId) {
                    return;
                }
                this.update(response);
                this.updateDocTitle(null, response.data?.length || 0);
                // https://github.com/lonelyor/SourceFlow/issues/8372
    ...
    private setCurrentById(id: string) {
        ...
        if (window.sourceflow.storage[Constants.LOCAL_OUTLINE].keepCurrentExpand) {
    ...
    private showExpandLevelMenu(target: HTMLElement) {
        ...
        window.sourceflow.menus.menu.element.setAttribute("data-name", Constants.MENU_OUTLINE_EXPAND_LEVEL);
        for (let i = 1; i <= 6; i++) {
            window.sourceflow.menus.menu.append(new MenuItem({
                id: `heading${i}`,
                icon: `iconH${i}`,
                label: window.sourceflow.languages[`heading${i}`],
    ...
    private showContextMenu(element: HTMLElement, event: MouseEvent) {
        ...
        window.sourceflow.menus.menu.remove();
        window.sourceflow.menus.menu.element.setAttribute("data-name", Constants.MENU_OUTLINE_CONTEXT);
        const id = element.getAttribute("data-node-id");
        if (!window.sourceflow.config.readonly) {
            ...
            if (currentLevel > 1) {
                window.sourceflow.menus.menu.append(new MenuItem({
                    id: "upgrade",
                    icon: "iconUp",
                    label: window.sourceflow.languages.upgrade,
                    click: () => {
            ...
            if (currentLevel < 6) {
                window.sourceflow.menus.menu.append(new MenuItem({
                    id: "downgrade",
                    icon: "iconDown",
                    label: window.sourceflow.languages.downgrade,
                    click: () => {
                        const data = this.getProtyleAndBlockElement(element);
                        if (data) {
                            turnsIntoTransaction({
            ...
            window.sourceflow.menus.menu.append(new MenuItem({
                ...
                icon: "iconBefore",
                label: window.sourceflow.languages.insertSameLevelHeadingBefore,
                click: () => {
                    const data = this.getProtyleAndBlockElement(element);
            ...
            window.sourceflow.menus.menu.append(new MenuItem({
                ...
                icon: "iconAfter",
                label: window.sourceflow.languages.insertSameLevelHeadingAfter,
                click: () => {
            ...
            if (currentLevel < 6) { // 只有当前级别小于6时才能添加子标题
                window.sourceflow.menus.menu.append(new MenuItem({
                    id: "addChildHeading",
                    icon: "iconAdd",
                    label: window.sourceflow.languages.addChildHeading,
                    click: () => {
                        fetchPost("/api/block/getHeadingDeleteTransaction", {
                            id,
                        }, (deleteResponse) => {
        ...
        window.sourceflow.menus.menu.append(new MenuItem({
            ...
            icon: "iconExpand",
            label: window.sourceflow.languages.expandChildHeading,
            accelerator: "⌘" + window.sourceflow.languages.clickArrow,
        ...
        window.sourceflow.menus.menu.append(new MenuItem({
            id: "foldChildHeading",
            icon: "iconContract",
            label: window.sourceflow.languages.foldChildHeading,
            accelerator: "⌘" + window.sourceflow.languages.clickArrow,
        ...
        window.sourceflow.menus.menu.append(new MenuItem({
            ...
            icon: "iconExpand",
            label: window.sourceflow.languages.expandSameLevelHeading,
            accelerator: "⌥" + window.sourceflow.languages.clickArrow,
        ...
        window.sourceflow.menus.menu.append(new MenuItem({
            ...
            icon: "iconContract",
            label: window.sourceflow.languages.foldSameLevelHeading,
            accelerator: "⌥" + window.sourceflow.languages.clickArrow,
        ...
        window.sourceflow.menus.menu.append(new MenuItem({
            id: "expandAll",
            icon: "iconExpand",
            label: window.sourceflow.languages.expandAll,
            click: () => {
                this.tree.expandAll();
                this.saveExpendIds();
            }
        ...
        window.sourceflow.menus.menu.append(new MenuItem({
            id: "foldAll",
            icon: "iconContract",
            label: window.sourceflow.languages.foldAll,
            click: () => {
                this.tree.collapseAll();
                this.saveExpendIds();
            }
