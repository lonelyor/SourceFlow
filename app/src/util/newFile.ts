...
export const getNewFilePath = (useSavePath: boolean) => {
    ...
    /// #else
    if (window.sourceflow.mobile.editor && document.getElementById("empty").classList.contains("fn__none")) {
        notebookId = window.sourceflow.mobile.editor.protyle.notebookId;
        if (useSavePath) {
            currentPath = window.sourceflow.mobile.editor.protyle.path;
        } else {
            currentPath = pathPosix().dirname(window.sourceflow.mobile.editor.protyle.path);
        }
    }
    /// #endif
    if (!notebookId) {
    ...
    fetchPost("/api/filetree/getDocCreateSavePath", {notebook: optios.notebookId}, (data) => {
        ...
        if ((data.data.path.indexOf("/") > -1 && optios.useSavePath) || optios.name) {
            if (data.data.path.startsWith("/") || optios.currentPath === "/") {
                const createPath = pathPosix().join(data.data.path, optios.name || (data.data.path.endsWith("/") ? window.sourceflow.languages.untitled : ""));
                ...
                fetchPost("/api/filetree/getHPathByPath", {
                    notebook: data.data.box,
                    path: optios.notebookId === data.data.box ? (optios.currentPath.endsWith(".sf") ? optios.currentPath : optios.currentPath + ".sf") : (data.data.path || "/")
                }, (responseHPath) => {
                    const createPath = pathPosix().join(responseHPath.data, data.data.path, optios.name || (data.data.path.endsWith("/") ? window.sourceflow.languages.untitled : ""));
                    fetchPost("/api/filetree/createDocWithMd", {
                        notebook: data.data.box,
                        path: createPath,
            ...
            const title = pathPosix().basename(data.data.path || window.sourceflow.languages.untitled);
            ...
            const newPath = (pathPosix().join(getDisplayName(optios.currentPath, false, true), id + ".sf"));
            if (optios.paths) {
...
export const newFileByName = (app: App, value: string) => {
    ...
    newFile({
        app,
        useSavePath: true,
        name: replaceFileName(value.trim()) || window.sourceflow.languages.untitled
    });
};
