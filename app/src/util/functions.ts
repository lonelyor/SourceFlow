...
export const isKernelInContainer = (): boolean => {
    return CONTAINER_BACKEND_SET.has(window.sourceflow.config.system.container);
};
...
export const getBackend = () => {
    if (isKernelInContainer()) {
        return window.sourceflow.config.system.container;
    } else {
        return window.sourceflow.config.system.os;
    }
};
...
export const getFrontend = () => {
    /// #if MOBILE
    if (isSourceFlowDesktopUserAgent(window.navigator.userAgent)) {
        return "mobile";
    } else {
