...
export const getIdZoomInByPath = () => {
    ...
    if (/^web\+sf:\/\/blocks\/\d{14}-\w{7}/.test(PWAURL)) {
        // PWA 捕获 web+sf://blocks/20221031001313-rk7sd0e?focus=1
        data.id = getIdFromSYProtocol(PWAURL);
        data.isZoomIn = getSearch("focus", PWAURL) === "1";
        window.sourceflow.editorIsFullscreen = getSearch("fullscreen", PWAURL) === "1";
        ...
        window.sourceflow.editorIsFullscreen = getSearch("fullscreen", SYURL) === "1";
    } else {
        // 支持通过 URL 查询字符串参数 `id` 和 `focus` 跳转到 Web 端指定块 https://github.com/lonelyor/SourceFlow/pull/7086
        data.id = searchParams.get("id");
        data.isZoomIn = searchParams.get("focus") === "1";
        window.sourceflow.editorIsFullscreen = searchParams.get("fullscreen") === "1";
    }
    return data;
};

export const isSYProtocol = (url: string) => {
    return /^sf:\/\/blocks\/\d{14}-\w{7}/.test(url);
};

export const getIdFromSYProtocol = (url: string) => {
    const match = /^(?:web\+)?sf:\/\/blocks\/(\d{14}-\w{7})/.exec(url || "");
    ...
    liElements.forEach((item: HTMLElement) => {
        if (item.getAttribute("data-type") !== "navigation-root") {
            ...
            const isChild = fromPaths.find(item => {
                if (dataPath.startsWith(item.replace(".sf", ""))) {
                    return true;
                }
            });
            if (!isChild) {
...
export const movePathTo = (options: {
    ...
    ${options.title || window.sourceflow.languages.move}
    ...
    <input class="b3-text-field fn__block" style="padding-left: 42px;" value="" placeholder="${window.sourceflow.languages.search}">
</div>
        ...
        notebooks.forEach((item) => {
            if (!item.closed) {
                ...
                if (options.flashcard) {
                    countHTML = `<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.sourceflow.languages.flashcardNewCard}">${item.newFlashcardCount}</span>
<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.sourceflow.languages.flashcardDueCard}">${item.dueFlashcardCount}</span>
<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.sourceflow.languages.flashcardCard}">${item.flashcardCount}</span>`;
                }
...
<li class="b3-list-item${html === "" ? " b3-list-item--focus" : ""}" data-path="/" data-box="${item.id}">
    ...
    ${unicode2Emoji(item.icon || window.sourceflow.storage[Constants.LOCAL_IMAGES].note, "b3-list-item__graphic", true)}
    ...
    inputElement.value = window.sourceflow.storage[Constants.LOCAL_MOVE_PATH].k;
    /// #if !MOBILE
    inputElement.select();
    /// #endif
...
<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.sourceflow.languages.flashcardNewCard}">${item.newFlashcardCount}</span>
<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.sourceflow.languages.flashcardDueCard}">${item.dueFlashcardCount}</span>
<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.sourceflow.languages.flashcardCard}">${item.flashcardCount}</span>`;
                }
                fileHTML += `<li class="b3-list-item${fileHTML === "" ? " b3-list-item--focus" : ""}" data-path="${item.path}" data-box="${item.box}">
    ${unicode2Emoji(item.boxIcon || window.sourceflow.storage[Constants.LOCAL_IMAGES].note, "b3-list-item__graphic", true)}
    ...
    const toggleMovePathHistory = () => {
        const keys = window.sourceflow.storage[Constants.LOCAL_MOVE_PATH].keys;
        ...
        menu.addItem({
            iconHTML: "",
            label: window.sourceflow.languages.clearHistory,
            click() {
                window.sourceflow.storage[Constants.LOCAL_MOVE_PATH].keys = [];
                setStorageVal(Constants.LOCAL_MOVE_PATH, window.sourceflow.storage[Constants.LOCAL_MOVE_PATH]);
            }
        });
                    ...
                    bind(element) {
                        element.addEventListener("click", (itemEvent) => {
                            if (hasClosestByClassName(itemEvent.target as Element, "b3-menu__action")) {
                                ...
                                window.sourceflow.storage[Constants.LOCAL_MOVE_PATH].keys = keys;
                                setStorageVal(Constants.LOCAL_MOVE_PATH, window.sourceflow.storage[Constants.LOCAL_MOVE_PATH]);
    ...
    inputElement.addEventListener("blur", () => {
        if (inputElement.value) {
            let list: string[] = window.sourceflow.storage[Constants.LOCAL_MOVE_PATH].keys;
            list.splice(0, 0, inputElement.value);
            list = Array.from(new Set(list));
            if (list.length > window.sourceflow.config.search.limit) {
                list.splice(window.sourceflow.config.search.limit, list.length - window.sourceflow.config.search.limit);
            }
            window.sourceflow.storage[Constants.LOCAL_MOVE_PATH].keys = list;
        }
        window.sourceflow.storage[Constants.LOCAL_MOVE_PATH].k = inputElement.value;
        setStorageVal(Constants.LOCAL_MOVE_PATH, window.sourceflow.storage[Constants.LOCAL_MOVE_PATH]);
    });
    const lineHeight = 28;
    inputElement.addEventListener("keydown", (event: KeyboardEvent) => {
        ...
        if (window.sourceflow.menus.menu.element.getAttribute("data-name") === Constants.MENU_MOVE_PATH_HISTORY) {
            return;
        }
    ...
    dialog.element.addEventListener("click", (event) => {
        ...
        while (target && !target.isEqualNode(dialog.element)) {
            if (target.classList.contains("b3-list-item__toggle")) {
                ...
                if (options.title === window.sourceflow.languages.specifyPath && isOnlyMeta(event)) {
...
<span class="counter counter--right b3-tooltips b3-tooltips__w" aria-label="${window.sourceflow.languages.flashcardCard}">${item.flashcardCount}</span>`;
                ...
                countHTML = `<span class="popover__block counter b3-tooltips b3-tooltips__w" aria-label="${window.sourceflow.languages.ref}">${item.count}</span>`;
            }
    ...
    ${unicode2Emoji(item.icon || (item.subFileCount === 0 ? window.sourceflow.storage[Constants.LOCAL_IMAGES].file : window.sourceflow.storage[Constants.LOCAL_IMAGES].folder), "b3-list-item__graphic", true)}
    <span class="b3-list-item__text ariaLabel" data-position="parentE" aria-label="${getDisplayName(item.name, true, true)} <small class='ft__on-surface'>${item.hSize}</small>${item.bookmark ? "<br>" + window.sourceflow.languages.bookmark + " " + item.bookmark : ""}${item.name1 ? "<br>" + window.sourceflow.languages.name + " " + item.name1 : ""}${item.alias ? "<br>" + window.sourceflow.languages.alias + " " + item.alias : ""}${item.memo ? "<br>" + window.sourceflow.languages.memo + " " + item.memo : ""}${item.subFileCount !== 0 ? window.sourceflow.languages.includeSubFile.replace("x", item.subFileCount) : ""}<br>${window.sourceflow.languages.modifiedAt} ${item.hMtime}<br>${window.sourceflow.languages.createdAt} ${item.hCtime}">${getDisplayName(item.name, true, true)}</span>
    ${countHTML}
</li>`;
        });
        if (fileHTML === "") {
...
export const getNotebookName = (id: string) => {
    let rootPath = "";
    window.sourceflow.notebooks.find((item) => {
        if (item.id === id) {
            rootPath = item.name;
            return true;
        }
...
export const setNoteBook = (cb?: (notebook: INotebook[]) => void, flashcard = false) => {
    fetchPost("/api/notebook/lsNotebooks", {
        ...
        if (!flashcard) {
            window.sourceflow.notebooks = response.data.notebooks;
        }
        if (cb) {
