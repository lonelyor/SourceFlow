...
const focusStack = async (app: App, stack: IBackStack) => {
        ...
        if (wnd) {
            ...
            const tab = new Tab({
                ...
                callback(tab) {
                    ...
                    window.sourceflow.storage[Constants.LOCAL_FILEPOSITION][stack.protyle.block.rootID] = scrollAttr;
            ...
            window.sourceflow.backStack.forEach(item => {
                if (!document.contains(item.protyle.element) && item.protyle.block.rootID === info.data.rootID) {
                    item.protyle = protyle;
                }
            });
    ...
    if (stack.protyle.element.parentElement) {
        ...
        // 动态加载导致内容移除 https://github.com/lonelyor/SourceFlow/issues/10692
...
export const goBack = async (app: App) => {
    if (window.sourceflow.backStack.length === 0) {
    ...
    if (!previousIsBack &&
        ...
        document.contains(window.sourceflow.backStack[window.sourceflow.backStack.length - 1].protyle.element)) {
        forwardStack.push(window.sourceflow.backStack.pop());
    }
    let stack = window.sourceflow.backStack.pop();
    while (stack) {
        ...
        if (isFocus) {
            ...
            stack = window.sourceflow.backStack.pop();
        }
    }
    previousIsBack = true;
    if (window.sourceflow.backStack.length === 0) {
...
export const goForward = async (app: App) => {
    if (forwardStack.length === 0) {
        if (window.sourceflow.backStack.length > 0) {
            await focusStack(app, window.sourceflow.backStack[window.sourceflow.backStack.length - 1]);
        }
        return;
    }
    document.querySelector("#barBack")?.classList.remove("toolbar__item--disabled");
    if (previousIsBack) {
        window.sourceflow.backStack.push(forwardStack.pop());
    }
    ...
    while (stack) {
        const isFocus = await focusStack(app, stack);
        if (isFocus) {
            window.sourceflow.backStack.push(stack);
            break;
        } else {
...
export const pushBack = (protyle: IProtyle, range?: Range, blockElement?: Element) => {
    ...
    if (editElement) {
        ...
        const lastStack = window.sourceflow.backStack[window.sourceflow.backStack.length - 1];
        if (lastStack && lastStack.id === id && (
            ...
            window.sourceflow.backStack.push({
                position,
                id,
                protyle,
                zoomId: protyle.block.showAll ? protyle.block.id : undefined,
            });
            if (window.sourceflow.backStack.length > Constants.SIZE_UNDO) {
                window.sourceflow.backStack.shift();
            }
            previousIsBack = false;
        }
        if (window.sourceflow.backStack.length > 1) {
            document.querySelector("#barBack")?.classList.remove("toolbar__item--disabled");
        }
    }
};
