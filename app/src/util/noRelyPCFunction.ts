...
<div class="b3-dialog__action">
    ...
    <div class="fn__flex-1">${item}</div>
            ...
            if (!hasKey && response.data.k) {
                searchHTML = `<div data-type="new" class="b3-list-item${searchHTML ? "" : " b3-list-item--focus"}"><div class="fn__flex-1">${window.sourceflow.languages.new} <mark>${escapeHtml(response.data.k)}</mark></div></div>` + searchHTML;
            }
...
export const getWorkspaceName = () => {
    return pathPosix().basename(window.sourceflow.config.system.workspaceDir.replace(/\\/g, "/"));
};
...
export const setLocalShorthandCount = () => {
    let fileElement;
    /// #if MOBILE
    fileElement = window.sourceflow.mobile.docks.file.element;
    /// #else
