...
export const scrollCenter = (
    ...
    if (!protyle.disabled && !nodeElement && getSelection().rangeCount > 0) {
        ...
        if (blockElement) {
            // https://github.com/lonelyor/SourceFlow/issues/10769
            ...
            // 撤销时 br 插入删除会导致 rang 被修改 https://github.com/lonelyor/SourceFlow/issues/12679
    ...
    if (!nodeElement &&
        // https://github.com/lonelyor/SourceFlow/issues/11175
    ...
    if (position === "start") {
        protyle.contentElement.scroll({
            top: protyle.contentElement.scrollTop + elementRect.top - contentRect.top - (window.sourceflow.config.editor.fontSize * 1.625 * 2 + 24),
            behavior
        });
        return;
    }
