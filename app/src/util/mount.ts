...
export const newDailyNote = (app: App) => {
    ...
    const localNotebookId = window.sourceflow.storage[Constants.LOCAL_DAILYNOTEID];
    const localNotebookIsOpen = window.sourceflow.notebooks.find((item) => {
    ...
    if (localNotebookId && localNotebookIsOpen && !isMobile()) {
        ...
        const dialog = new Dialog({
            ...
            title: window.sourceflow.languages.plsChoose,
...
<div class="b3-dialog__action">
    ...
    <button class="b3-button b3-button--text">${window.sourceflow.languages.confirm}</button>
        ...
        btnsElement[1].addEventListener("click", () => {
            ...
            window.sourceflow.storage[Constants.LOCAL_DAILYNOTEID] = notebook;
...
export const mountHelp = () => {
    const notebookId = Constants.HELP_PATH[window.sourceflow.config.appearance.lang as "zh_CN" | "en_US"];
...
export const newNotebook = () => {
    const dialog = new Dialog({
        title: window.sourceflow.languages.newNotebook,
        content: `<div class="b3-dialog__content">
    <input placeholder="${window.sourceflow.languages.notebookName}" class="b3-text-field fn__block">
</div>
<div class="b3-dialog__action">
    <button class="b3-button b3-button--cancel">${window.sourceflow.languages.cancel}</button><div class="fn__space"></div>
    <button class="b3-button b3-button--text">${window.sourceflow.languages.confirm}</button>
