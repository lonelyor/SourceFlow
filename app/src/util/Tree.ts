...
export class Tree {
    ...
    public updateData(data: IBlockTree[]) {
        ...
        if (!this.data || this.data.length === 0) {
            this.element.innerHTML = `<ul class="b3-list b3-list--background"><li class="b3-list--empty">${window.sourceflow.languages.emptyContent}</li></ul>`;
        } else {
    ...
    private genHTML(data: (IBlockTree & { folded?: boolean })[]) {
        ...
        data.forEach((item) => {
            ...
            // data-id 需要添加 item.id，否则大纲更新时 name 不一致导致 https://github.com/lonelyor/SourceFlow/issues/11843
        ...
        data.forEach((item: IBlock & {
            ...
            if (type === "outline") {
                ...
                if (item.type === "NodeDocument") {
                    iconHTML = `<span data-showref="true" class="b3-list-item__graphic popover__block" data-id="${item.id}">${unicode2Emoji(item.ial.icon || window.sourceflow.storage[Constants.LOCAL_IMAGES].file)}</span>`;
                } else {
    ...
    private bindEvent() {
        ...
        this.element.addEventListener("click", (event: MouseEvent & { target: HTMLElement }) => {
            ...
            while (target && !target.isEqualNode(this.element)) {
                if (target.classList.contains("b3-list-item__toggle") &&
                    !target.classList.contains("fn__hidden") && !window.sourceflow.ctrlIsPressed && !window.sourceflow.altIsPressed) {
                ...
                if (target.classList.contains("b3-list-item__action") && this.click) {
                    ...
                    if (target.getAttribute("data-node-id") || target.getAttribute("data-treetype") === "tag") {
                        if (this.ctrlClick && window.sourceflow.ctrlIsPressed) {
                            this.ctrlClick(target, event);
                        } else if (this.altClick && window.sourceflow.altIsPressed) {
                            this.altClick(target, event);
                        } else if (this.shiftClick && window.sourceflow.shiftIsPressed) {
        ...
        this.element.addEventListener("dragstart", (event: DragEvent & { target: HTMLElement }) => {
            ...
            if (liElement) {
                ...
                window.sourceflow.dragElement = liElement;
            }
        });
