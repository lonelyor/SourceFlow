...
const renderRecentDocsContent = async (data: {
    ...
    ${unicode2Emoji(item.icon || window.sourceflow.storage[Constants.LOCAL_IMAGES].file, "b3-list-item__graphic", true)}
    ...
    if (!isWindow()) {
        let docIndex = 0;
        if (!key || window.sourceflow.languages.riffCard.toLowerCase().includes(key.toLowerCase())) {
    ...
    <span class="b3-list-item__meta">${updateHotkeyTip(window.sourceflow.config.keymap.general.riffCard.custom)}</span>
            ...
            docIndex++;
            if (!switchPath) {
                switchPath = window.sourceflow.languages.riffCard;
            }
        }
...
export const openRecentDocs = () => {
    ...
    const sortBy = window.sourceflow.storage[Constants.LOCAL_RECENT_DOCS].type as TRecentDocsSort;
...
<div class="fn__flex-center">${window.sourceflow.languages.recentDocs}</div>
<div class="fn__flex-1"></div>
<div class="b3-form__icon fn__size200">
    <svg class="b3-form__icon-icon"><use xlink:href="#iconSearch"></use></svg>
    <input placeholder="${window.sourceflow.languages.search}" class="b3-text-field fn__block b3-form__icon-input">
</div>
<span class="fn__space"></span>
<div class="fn__flex-center">
    <select class="b3-select" id="recentDocsSort">
        <option value="viewedAt">${window.sourceflow.languages.recentViewed}</option>
        <option value="updated">${window.sourceflow.languages.recentModified}</option>
        <option value="openAt">${window.sourceflow.languages.recentOpened}</option>
        <option value="closedAt">${window.sourceflow.languages.recentClosed}</option>
    </select>
</div>
</div>`,
    ...
    <div class="switch-doc__path"></div>
        ...
        sortSelect.addEventListener("change", () => {
            ...
            window.sourceflow.storage[Constants.LOCAL_RECENT_DOCS].type = sortSelect.value;
            setStorageVal(Constants.LOCAL_RECENT_DOCS, window.sourceflow.storage[Constants.LOCAL_RECENT_DOCS]);
        });
