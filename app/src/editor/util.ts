...
export const openAsset = (app: App, assetPath: string, page: number | string, position?: string) => {
    ...
    if (!Constants.SOURCEFLOW_ASSETS_EXTS.includes(suffix)) {
        return;
    }
    openFile({
        app,
...
export const openFile = async (options: IOpenFileOptions) => {
    ...
    // https://github.com/lonelyor/SourceFlow/issues/10168
        ...
        allModels.editor.find((item) => {
            if (item.editor.protyle.block.rootID === options.rootID) {
                ...
                if (!editor || item.headElement.getAttribute("data-activetime") > editor.headElement.getAttribute("data-activetime")) {
                    // https://github.com/lonelyor/SourceFlow/issues/11981#issuecomment-2351939812
                    editor = item;
                }
            }
            if (activeEditor) {
    ...
    /// #if !BROWSER
    // https://github.com/lonelyor/SourceFlow/issues/7491
    if (!options.position || (options.position === "right" && options.assetPath)) {
        ...
        hasMatch = await ipcRenderer.invoke(Constants.SOURCEFLOW_GET, {
            cmd: Constants.SOURCEFLOW_OPEN_FILE,
    ...
    if (wnd) {
        ...
        if ((options.position === "right" || options.position === "bottom") && wnd.children[0].headElement) {
            ...
            if (targetWnd) {
                ...
                // 在右侧/下侧打开已有页签将进行页签切换 https://github.com/lonelyor/SourceFlow/issues/5366
        ...
        if (options.keepCursor && wnd.children[0].headElement) {
            ...
            let unUpdateTab: Tab;
            // 不能 reverse, 找到也不能提前退出循环，否则 https://github.com/lonelyor/SourceFlow/issues/3271
...
const switchEditor = (editor: Editor, options: IOpenFileOptions, allModels: IModels) => {
    ...
    if (options.mode !== "preview" && !editor.editor.protyle.preview.element.classList.contains("fn__none")) {
        // TODO https://github.com/lonelyor/SourceFlow/issues/3059
        return true;
    }
    if (options.zoomIn) {
        ...
        if (options.action?.includes(Constants.CB_GET_FOCUS)) {
            if (nodeElement) {
                ...
                if (options.action.includes(Constants.CB_GET_SEARCH)) {
                    const scrollAttr = window.sourceflow.storage[Constants.LOCAL_FILEPOSITION][editor.editor.protyle.block.rootID];
                ...
                // 由于 https://github.com/lonelyor/SourceFlow/issues/5420，移除定位
    ...
    // https://github.com/lonelyor/SourceFlow/issues/16445
...
const newTab = (options: IOpenFileOptions) => {
    ...
    if (options.assetPath) {
        ...
        if (Constants.SOURCEFLOW_ASSETS_EXTS.includes(suffix)) {
            let icon = "iconPDF";
            if (Constants.SOURCEFLOW_ASSETS_IMAGE.includes(suffix)) {
                icon = "iconImage";
            } else if (Constants.SOURCEFLOW_ASSETS_AUDIO.includes(suffix)) {
                icon = "iconRecord";
            } else if (Constants.SOURCEFLOW_ASSETS_VIDEO.includes(suffix)) {
                icon = "iconVideo";
            }
            tab = new Tab({
                icon,
        ...
        tab = new Tab({
            ...
            callback(tab) {
                if (options.custom.id) {
                    if (options.custom.id === "sourceflow-card") {
        ...
        tab = new Tab({
            icon: "iconSearch",
            title: window.sourceflow.languages.search,
            callback(tab) {
    ...
    if (options.protyle && options.protyle.path) {
        ...
        if (options.protyle.element.classList.contains("fn__none") ||
            (!hasClosestByClassName(options.protyle.element, "layout__wnd--active") &&
                document.querySelector(".layout__wnd--active")  // https://github.com/lonelyor/SourceFlow/issues/4414
            )
        ) {
            return;
        }
...
export const updateBacklinkGraph = (models: IModels, protyle: IProtyle) => {
    ...
    models.backlink.forEach(item => {
        ...
        fetchPost("/api/ref/getBacklink2", {
            sort: item.status[blockId] ? item.status[blockId].sort.toString() : window.sourceflow.config.editor.backlinkSort.toString(),
            mSort: item.status[blockId] ? item.status[blockId].mSort.toString() : window.sourceflow.config.editor.backmentionSort.toString(),
            id: blockId || "",
...
export const openBy = (url: string, type: "folder" | "app") => {
    ...
    if ("windows" === window.sourceflow.config.system.os) {
        // `file://` 协议兼容 Window 平台使用 `/` 作为目录分割线 https://github.com/lonelyor/SourceFlow/issues/5681
        address = url.replace("file:///", "").replace("file://\\", "").replace("file://", "").replace(/\//g, "\\");
    } else {
        address = url.replace("file://", "");
    }

    // 拖入文件名包含 `)` 、`(` 的文件以 `file://` 插入后链接解析错误 https://github.com/lonelyor/SourceFlow/issues/5786
    address = address.replace(/\\\)/g, ")").replace(/\\\(/g, "(");
    if (type === "app") {
        useShell("openPath", address);
    } else if (type === "folder") {
        if ("windows" === window.sourceflow.config.system.os) {
            if (!address.startsWith("\\\\")) { // \\ 开头的路径是 Windows 网络共享路径 https://github.com/lonelyor/SourceFlow/issues/5980
                // Windows 端打开本地文件所在位置失效 https://github.com/lonelyor/SourceFlow/issues/5808
