...
export const validateName = (name: string, targetElement?: HTMLElement) => {
    if (/\r\n|\r|\n|\u2028|\u2029|\t/.test(name)) {
        if (targetElement) {
            showTooltip(window.sourceflow.languages.fileNameRule, targetElement, "error");
        } else {
            showMessage(window.sourceflow.languages.fileNameRule);
        }
        return false;
    }
    if (name.length > Constants.SIZE_TITLE) {
        if (targetElement) {
            showTooltip(window.sourceflow.languages["_kernel"]["106"], targetElement, "error");
        } else {
...
<div class="b3-dialog__action">
    ...
    btnsElement[1].addEventListener("click", () => {
        ...
        if (options.type === "notebook") {
            if (!name) {
                name = window.sourceflow.languages.untitled;
            }
...
export const newFileContentBySelect = (protyle: IProtyle) => {
    ...
    fetchPost("/api/filetree/createDoc", {
        ...
        path: pathPosix().join(getDisplayName(protyle.path, false, true), Lute.NewNodeID() + ".sf"),
        title: fileNameShort,
