...
import {showDocHistoryCompare} from "./docCompare";
...
const renderDoc = (element: HTMLElement, currentPage: number, id: string) => {
    ...
    fetchPost("/api/history/searchHistory", {
        ...
        pageInfoElement.textContent = window.sourceflow.languages.pageCountAndHistoryCount.replace("${x}", response.data.pageCount).replace("${y}", response.data.totalCount);
    ...
    <span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="compare" aria-label="${window.sourceflow.languages.compare}">
...
export const openDocHistory = (options: {
    ...
    <div class="block__icons">
        ...
        <select data-type="opselect" class="b3-select">
            <option value="all" selected>${window.sourceflow.languages.allOp}</option>
            <option value="update">${window.sourceflow.languages.historyUpdate}</option>
            <option value="format">${window.sourceflow.languages.historyFormat}</option>
            <option value="sync">${window.sourceflow.languages.historySync}</option>
            <option value="replace">${window.sourceflow.languages.historyReplace}</option>
            <option value="outline">${window.sourceflow.languages.historyOutline}</option>
        </select>
    </div>
</div>
<div class="fn__flex fn__flex-1 history__panel">
    <ul class="b3-list b3-list--background history__side" ${isMobile() ? "" : `style="width: ${window.sourceflow.storage[Constants.LOCAL_HISTORY].sideDocWidth}"`}>
        ...
        while (target && !target.isEqualNode(dialog.element)) {
            ...
            if (type === "rollback" && !isLoading) {
                getHistoryPath(target.parentElement, opElement.value, options.id, (item) => {
                    ...
                    isLoading = false;
                    const confirmTip = window.sourceflow.languages.rollbackConfirm.replace("${name}", escapeHtml(item.title))
                        .replace("${time}", target.previousElementSibling.previousElementSibling.textContent.trim());
                    confirmDialog("⚠️ " + window.sourceflow.languages.rollback, confirmTip, () => {
                ...
                break;
            } else if (type === "compare" && !isLoading) {
                getHistoryPath(target.parentElement, opElement.value, options.id, (item) => {
                    isLoading = false;
                    showDocHistoryCompare(options.app, {
                        id: options.id,
                        notebookId: options.notebookId,
                        pathString: options.pathString,
                        historyPath: item.path,
                        created: target.parentElement.getAttribute("data-created"),
                    });
                });
                event.stopPropagation();
                event.preventDefault();
