...
export const openSearchAsset = (element: HTMLElement, isStick: boolean) => {
    ...
    let enterTip = "";
    /// #if !BROWSER
    enterTip = `<kbd>${window.sourceflow.languages.enterKey}/${window.sourceflow.languages.doubleClick}</kbd> ${window.sourceflow.languages.showInFolder}`;
    /// #endif
    element.innerHTML = `<div class="block__icons">
    <span data-type="assetPrevious" class="block__icon block__icon--show ariaLabel" data-position="9south" disabled="disabled" aria-label="${window.sourceflow.languages.previousLabel}"><svg><use xlink:href='#iconLeft'></use></svg></span>
    <span class="fn__space"></span>
    <span data-type="assetNext" class="block__icon block__icon--show ariaLabel" data-position="9south" disabled="disabled" aria-label="${window.sourceflow.languages.nextLabel}"><svg><use xlink:href='#iconRight'></use></svg></span>
    <span class="fn__space"></span>
    <span id="searchAssetResult" class="ft__selectnone"></span>
    <span class="fn__flex-1${!isStick ? " resize__move" : ""}" style="min-height: 100%"></span>
    <span class="fn__space"></span>
    <span id="assetMore" aria-label="${window.sourceflow.languages.more}" class="block__icon block__icon--show ariaLabel" data-position="9south">
        <svg><use xlink:href="#iconMore"></use></svg>
    </span>
    <span class="fn__space"></span>
    <span id="searchAssetClose" aria-label="${isStick ? window.sourceflow.languages.stickSearch : window.sourceflow.languages.globalSearch}" class="block__icon block__icon--show ariaLabel" data-position="9south">
...
<div class="b3-form__icon search__header">
    <div class="fn__flex-1" style="position: relative">
        ...
        <input id="searchAssetInput" value="${localSearch.k}" class="b3-text-field b3-text-field--text" placeholder="${window.sourceflow.languages.keyword}">
    </div>
    <div class="block__icons">
        <span data-type="assetRefresh" aria-label="${window.sourceflow.languages.refresh}" class="block__icon ariaLabel" data-position="9south">
            <svg><use xlink:href="#iconRefresh"></use></svg>
        </span>
        <span class="fn__space"></span>
        ${genQueryHTML(localSearch.method, "assetSyntaxCheck")}
        <span class="fn__space"></span>
        <span id="assetFilter" aria-label="${window.sourceflow.languages.type}" class="block__icon ariaLabel" data-position="9south">
...
<div class="search__tip${isStick ? " fn__none" : ""}">
    ...
    dragElement.addEventListener("mousedown", (event: MouseEvent) => {
        ...
        documentSelf.onmouseup = () => {
            ...
            window.sourceflow.storage[Constants.LOCAL_SEARCHASSET][direction === "lr" ? "col" : "row"] = previewElement[direction === "lr" ? "offsetWidth" : "offsetHeight"] + "px";
            setStorageVal(Constants.LOCAL_SEARCHASSET, window.sourceflow.storage[Constants.LOCAL_SEARCHASSET]);
        };
    });
    dragElement.addEventListener("dblclick", () => {
        ...
        const direction = localSearch.layout === 1 ? "lr" : "tb";
        window.sourceflow.storage[Constants.LOCAL_SEARCHASSET][direction === "lr" ? "col" : "row"] = "";
        setStorageVal(Constants.LOCAL_SEARCHASSET, window.sourceflow.storage[Constants.LOCAL_SEARCHASSET]);
    });
    /// #endif
};
...
export const assetInputEvent = (element: Element, localSearch?: ISearchAssetOption, page = 1) => {
    ...
    inputTimeout = window.setTimeout(() => {
        if (!localSearch) {
            localSearch = window.sourceflow.storage[Constants.LOCAL_SEARCHASSET] as ISearchAssetOption;
        }
...
<span class="ft__on-surface">${window.sourceflow.languages.total} ${response.data.matchedAssetCount}</span>`;
...
export const assetMethodMenu = (target: HTMLElement, cb: () => void) => {
    const method = window.sourceflow.storage[Constants.LOCAL_SEARCHASSET].method;
    if (!window.sourceflow.menus.menu.element.classList.contains("fn__none") &&
        window.sourceflow.menus.menu.element.getAttribute("data-name") === Constants.MENU_SEARCH_ASSET_METHOD) {
        window.sourceflow.menus.menu.remove();
        return;
    }
    window.sourceflow.menus.menu.remove();
    window.sourceflow.menus.menu.element.setAttribute("data-name", Constants.MENU_SEARCH_ASSET_METHOD);
    window.sourceflow.menus.menu.append(new MenuItem({
        icon: "iconExact",
        label: window.sourceflow.languages.keyword,
        current: method === 0,
        click() {
            window.sourceflow.storage[Constants.LOCAL_SEARCHASSET].method = 0;
            cb();
        }
    }).element);
    window.sourceflow.menus.menu.append(new MenuItem({
        ...
        current: method === 1,
        click() {
            window.sourceflow.storage[Constants.LOCAL_SEARCHASSET].method = 1;
            cb();
        }
    }).element);
    window.sourceflow.menus.menu.append(new MenuItem({
        ...
        current: method === 3,
        click() {
            window.sourceflow.storage[Constants.LOCAL_SEARCHASSET].method = 3;
            cb();
        }
    }).element);
    /// #if MOBILE
...
const filterTypesHTML = (types: IObject) => {
    let html = "";
    Constants.SOURCEFLOW_ASSETS_SEARCH.sort((a: string, b: string) => {
...
export const assetFilterMenu = (assetsElement: Element) => {
    const localData = window.sourceflow.storage[Constants.LOCAL_SEARCHASSET].types;
    const filterDialog = new Dialog({
        title: window.sourceflow.languages.type,
...
export const assetMoreMenu = (target: Element, element: Element, cb: () => void) => {
    if (!window.sourceflow.menus.menu.element.classList.contains("fn__none") &&
        window.sourceflow.menus.menu.element.getAttribute("data-name") === Constants.MENU_SEARCH_ASSET_MORE) {
        window.sourceflow.menus.menu.remove();
        return;
    }
    window.sourceflow.menus.menu.remove();
    window.sourceflow.menus.menu.element.setAttribute("data-name", Constants.MENU_SEARCH_ASSET_MORE);
    const localData = window.sourceflow.storage[Constants.LOCAL_SEARCHASSET];
    const sortMenu = [{
        iconHTML: "",
