...
type TEventBus = "ws-main" | "sync-start" | "sync-end" | "sync-fail" |
    ...
    "open-sourceflow-url-plugin" | "open-sourceflow-url-block" |
    "opened-notebook" |
    "closed-notebook" |
    "paste" |
    "input-search" |
...
type TEditorHTMLPasteMode = "smart" | "html" | "image";
...
interface VideoFrame {
    readonly __sourceflowVideoFrameBrand?: unique symbol;
}
...
interface Window {
    ...
    JSAndroid: {
        ...
        writeSourceFlowHTMLClipboard?(text: string, html: string, sourceflowHTML: string): void
        writeImageClipboard(uri: string): void
        readClipboard(): string
        readHTMLClipboard(): string
        readSourceFlowHTMLClipboard?(): string
        getBlockURL(): string
        hideKeyboard(): void
        showKeyboard(): void
...
interface ISourceFlow {
    ...
    user?: {
        ...
        userIntro: string
        userNickname: string
        userSourceFlowOneTimePayStatus: number  // 0 未付费；1 已付费
        userSourceFlowProExpireTime: number // -1 终身会员；0 普通用户；> 0 过期时间
        userSourceFlowSubscriptionPlan: number // 0 年付订阅/终生；1 教育优惠；2 订阅试用
        userSourceFlowSubscriptionType: number // 0 年付；1 终生；2 月付
        userSourceFlowSubscriptionStatus: number // -1：未订阅，0：订阅可用，1：订阅封禁，2：订阅过期
        userToken: string
        userTitles: {
            name: string,
            icon: string,
...
interface IAVCellValue {
    ...
    checkbox?: {
        checked: boolean,
        content?: string, // gallery 中显示 https://github.com/lonelyor/SourceFlow/issues/15389
    }
