...
export const setTabPosition = () => {
    ...
    wndsTemp.forEach(async item => {
        ...
        // 显示器缩放后像素存在小数点偏差 https://github.com/lonelyor/SourceFlow/issues/7355
        if (rect.top <= 0 && rect.right + 8 >= window.innerWidth) {
            headersLastElement.style.paddingRight = (42 * ("darwin" === window.sourceflow.config.system.os ? 1 : 4)) + "px";
        } else {
            headersLastElement.style.paddingRight = "";
        }
    });
