...
export const initBlockPopover = (app: App) => {
    ...
    document.addEventListener("mouseover", (event: MouseEvent & { target: HTMLElement, path: HTMLElement[] }) => {
        if (!window.sourceflow.config || !window.sourceflow.menus ||
            // 拖拽时禁止
            window.sourceflow.dragElement || document.onmousemove) {
            hideTooltip();
            return;
        }
        ...
        if (aElement) {
            ...
            if (!tip && aElement.getAttribute("data-type")?.includes("inline-memo")) {
                ...
                tooltipClass = "memo"; // 为行级备注添加 class https://github.com/lonelyor/SourceFlow/issues/6161
                tooltipSpace = 0; // tooltip 和备注元素之间不能有空隙 https://github.com/lonelyor/SourceFlow/issues/14796#issuecomment-3649757267
            }
            if (!tip) {
                if (aElement.getAttribute("data-type")?.includes("a")) {
                    tooltipClass = "href"; // 为超链接添加 class https://github.com/lonelyor/SourceFlow/issues/11440#issuecomment-2119080691
                    tooltipSpace = 0;
                }
                const href = aElement.getAttribute("data-href") || "";
                // 链接地址强制换行 https://github.com/lonelyor/SourceFlow/issues/11539
                if (href) {
                ...
                if (tip && isLocalPath(href) && !aElement.classList.contains("b3-tooltips")) {
                    ...
                    fetchPost("/api/asset/statAsset", {path: href}, (response) => {
                        if (response.code === 1) {
                            ...
                            assetTip += ` ${response.data.hSize}${title ? '<div class="fn__hr"></div><span>' + title + "</span>" : ""}<br>${window.sourceflow.languages.modifiedAt} ${response.data.hUpdated}<br>${window.sourceflow.languages.createdAt} ${response.data.hCreated}`;
                        }
                        try {
            ...
            if (notebookItemElement && notebookItemElement.parentElement.getAttribute("data-type") === "navigation-root") {
                fetchPost("/api/notebook/getNotebookInfo", {notebook: notebookItemElement.parentElement.parentElement.getAttribute("data-url")}, (response) => {
                    ...
                    const tip = `${boxData.name} <small class='ft__on-surface'>${boxData.hSize}</small>${boxData.docCount !== 0 ? window.sourceflow.languages.includeSubFile.replace("x", boxData.docCount) : ""}<br>${window.sourceflow.languages.modifiedAt} ${boxData.hMtime}<br>${window.sourceflow.languages.createdAt} ${boxData.hCtime}`;
            ...
            if (tip && !aElement.classList.contains("b3-tooltips")) {
                // https://github.com/lonelyor/SourceFlow/issues/11294
                try {
        ...
        if (window.sourceflow.config.editor.floatWindowMode === 1 || window.sourceflow.shiftIsPressed) {
            ...
            // https://github.com/lonelyor/SourceFlow/issues/9007
            if (event.relatedTarget && !document.contains(event.relatedTarget as Node)) {
                return;
            }
            if (window.sourceflow.ctrlIsPressed) {
                clearTimeout(timeoutHide);
                showPopover(app);
            } else if (window.sourceflow.shiftIsPressed) {
                clearTimeout(timeoutHide);
        ...
        timeout = window.setTimeout(() => {
            ...
            clearTimeout(timeoutHide);
            showPopover(app);
        }, window.sourceflow.config.editor.floatWindowDelay);
    });
};

export const deferBlockPopover = (app: App) => {
    const run = () => initBlockPopover(app);
...
const hidePopover = (event: MouseEvent & { path: HTMLElement[] }) => {
    ...
    if (!popoverTargetElement && linkElement && /^sf:\/\/blocks/.test(linkElement.getAttribute("data-href") || "")) {
        popoverTargetElement = linkElement;
    }
    if (!popoverTargetElement || (popoverTargetElement && window.sourceflow.menus.menu.data && window.sourceflow.menus.menu.data === popoverTargetElement)) {
        ...
        const menuLevel = parseInt(window.sourceflow.menus.menu.element.dataset.from);
        if (blockElement) {
            for (let i = window.sourceflow.blockPanels.length - 1; i >= 0; i--) {
            ...
            for (let i = window.sourceflow.blockPanels.length - 1; i >= 0; i--) {
                ...
                if ((item.targetElement || typeof item.x === "number") && item.element.getAttribute("data-pin") === "false") {
                    if (menuLevel && menuLevel >= itemLevel) {
                        ...
                        // 点击嵌入块后浮窗消失后再快速点击嵌入块无法弹出浮窗 https://github.com/lonelyor/SourceFlow/issues/12511
                        break;
                    } else {
                        const hasToolbar = item.editors.find(editItem => {
                            if (!editItem.protyle.toolbar.subElement.classList.contains("fn__none")) {
...
const getTarget = (event: MouseEvent & { target: HTMLElement }, aElement: false | HTMLElement) => {
    if (window.sourceflow.config.editor.floatWindowMode === 2 || hasClosestByClassName(event.target, "history__repo", true)) {
        return false;
    }
    ...
    if (!popoverTargetElement && aElement) {
        if (/^sf:\/\/blocks/.test(aElement.getAttribute("data-href") || "") && aElement.getAttribute("prevent-popover") !== "true") {
            ...
            if (textElement && textElement.dataset.type === "url" && /^sf:\/\/blocks/.test(textElement.dataset.href || "")) {
                popoverTargetElement = textElement;
            }
        }
    }
    if (!popoverTargetElement || window.sourceflow.altIsPressed ||
        (window.sourceflow.config.editor.floatWindowMode === 0 && window.sourceflow.ctrlIsPressed) ||
        (popoverTargetElement && popoverTargetElement.getAttribute("prevent-popover") === "true")) {
        return false;
    }
    // https://github.com/lonelyor/SourceFlow/issues/4314
...
export const showPopover = async (app: App, showRef = false) => {
    if (!popoverTargetElement || (window.sourceflow.menus.menu.data && window.sourceflow.menus.menu.data === popoverTargetElement)) {
        return;
    }
    ...
    if (dataId) {
        ...
        // 在 database 的 url 列中以 SF 协议开头的链接
