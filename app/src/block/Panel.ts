...
export class BlockPanel {
    ...
    constructor(options: {
        ...
        this.element.addEventListener("click", (event) => {
            if (this.element && window.sourceflow.blockPanels.length > 1) {
                this.element.style.zIndex = (++window.sourceflow.zIndex).toString();
            }
            ...
            while (target && !target.isEqualNode(this.element)) {
                if (target.classList.contains("block__icon") || target.classList.contains("block__logo")) {
                    ...
                    if (type === "close") {
                        ...
                        if (this.element.getAttribute("data-pin") === "true") {
                            target.setAttribute("aria-label", window.sourceflow.languages.pin);
                            target.querySelector("use").setAttribute("xlink:href", "#iconPin");
                            this.element.setAttribute("data-pin", "false");
                        } else {
                            target.setAttribute("aria-label", window.sourceflow.languages.unpin);
                        ...
                        checkFold(this.refDefs[0].refID, (zoomIn, action) => {
                            /// #if MOBILE
                            openMobileFileById(options.app, this.refDefs[0].refID, action);
                            void zoomIn;
                            /// #else
                            openFileById({
                                app: options.app,
    ...
    private initProtyle(editorElement: HTMLElement, afterCB?: () => void) {
        ...
        fetchPost("/api/block/getBlockInfo", {id: this.refDefs[index].refID}, (response) => {
            ...
            if (response.data.rootID !== this.refDefs[index].refID) {
                ...
                // 不需要高亮 https://github.com/lonelyor/SourceFlow/issues/11160#issuecomment-2084652764
            }
    ...
    public destroy() {
        ...
        window.sourceflow.blockPanels.find((item, index) => {
            if (item.id === this.id) {
                window.sourceflow.blockPanels.splice(index, 1);
                return true;
            }
        });
        if (this.editors.length > 0) {
            this.editors.forEach(item => {
                // https://github.com/lonelyor/SourceFlow/issues/8199
        ...
        if (menuLevel && menuLevel >= level && window.sourceflow.menus.menu.element.dataset.from?.includes("popover")) {
            // https://github.com/lonelyor/SourceFlow/issues/9854 右键菜单不是从浮窗中弹出的则不进行移除
    ...
    private render() {
        ...
        if (this.refDefs.length === 1) {
            openHTML = `<span data-type="stickTab" class="block__icon block__icon--show b3-tooltips b3-tooltips__sw" aria-label="${window.sourceflow.languages.openInNewTab}${updateHotkeyAfterTip(window.sourceflow.config.keymap.editor.general.openInNewTab.custom)}"><svg><use xlink:href="#iconOpen"></use></svg></span>
<span class="fn__space"></span>`;
            /// #if !BROWSER
            openHTML += `<span data-type="open" class="block__icon block__icon--show b3-tooltips b3-tooltips__sw" aria-label="${window.sourceflow.languages.openByNewWindow}"><svg><use xlink:href="#iconOpenWindow"></use></svg></span>
<span class="fn__space"></span>`;
            /// #endif
        }
        let html = `<div class="block__icons block__icons--menu">
    <span class="fn__space fn__flex-1 resize__move"></span>${openHTML}
    <span data-type="pin" class="block__icon block__icon--show b3-tooltips b3-tooltips__sw" aria-label="${window.sourceflow.languages.pin}"><svg><use xlink:href="#iconPin"></use></svg></span>
    <span class="fn__space"></span>
    <span data-type="close" class="block__icon block__icon--show b3-tooltips b3-tooltips__sw" aria-label="${window.sourceflow.languages.close}${updateHotkeyAfterTip(window.sourceflow.config.keymap.general.closeTab.custom)}"><svg style="width: 12px;margin: 0 1px;"><use xlink:href="#iconClose"></use></svg></span>
</div>
<div class="block__content">`;
        if (this.refDefs.length === 0) {
            html += `<div class="ft__smaller ft__smaller ft__secondary b3-form__space--small" contenteditable="false">${window.sourceflow.languages.refExpired}</div>`;
        } else {
            ...
            if (index < 5) {
                this.initProtyle(item, index === 0 ? () => {
                    ...
                    if (this.targetElement && this.targetElement.classList.contains("protyle-wysiwyg__embed")) {
                        ...
                        // 靠边不宜拖拽 https://github.com/lonelyor/SourceFlow/issues/2937
