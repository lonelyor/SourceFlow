...
export const insertEmptyBlock = (protyle: IProtyle, position: InsertPosition, id?: string) => {
    ...
    if (id) {
        ...
        if (selectElements.length > 0) {
            ...
            // https://github.com/lonelyor/SourceFlow/issues/14720#issuecomment-2840665326
...
export const genEmptyBlock = (zwsp = true, wbr = true, string?: string) => {
    ...
    return `<div data-node-id="${Lute.NewNodeID()}" data-type="NodeParagraph" class="p"><div contenteditable="true" spellcheck="${window.sourceflow.config.editor.spellcheck}">${html}</div><div contenteditable="false" class="protyle-attr">${Constants.ZWSP}</div></div>`;
};
...
export const genEmptyElement = (zwsp = true, wbr = true, id?: string) => {
    ...
    element.innerHTML = `<div contenteditable="true" spellcheck="${window.sourceflow.config.editor.spellcheck}">${zwsp ? Constants.ZWSP : ""}${wbr ? "<wbr>" : ""}</div><div class="protyle-attr" contenteditable="false">${Constants.ZWSP}</div>`;
    return element;
};
...
export const getLangByType = (type: string) => {
    ...
    switch (type) {
        ...
        case "NodeAttributeView":
            lang = window.sourceflow.languages.database;
            break;
        case "NodeThematicBreak":
            lang = window.sourceflow.languages.line;
            break;
        case "NodeWidget":
            lang = window.sourceflow.languages.widget;
            break;
        case "NodeVideo":
            lang = window.sourceflow.languages.video;
            break;
        case "NodeAudio":
            lang = window.sourceflow.languages.audio;
            break;
        case "NodeBlockQueryEmbed":
            lang = window.sourceflow.languages.blockEmbed;
            break;
    }
    return lang;
};
