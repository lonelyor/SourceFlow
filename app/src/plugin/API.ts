...
import {lockScreen, exitSourceFlow} from "../dialog/processSystem";
...
let openTab: (options: TPluginOpenTabOptions) => unknown;
let openWindow: (options: TPluginOpenWindowOptions) => void;
/// #if MOBILE
openTab = () => {
    // TODO: Mobile
};
...
const getModelByDockType = (type: TDock | string) => {
    /// #if MOBILE
    return window.sourceflow.mobile.docks[type];
    /// #else
...
const saveLayout = (cb: () => void) => {
    ...
    if (window.sourceflow.mobile.editor) {
        const result = saveScroll(window.sourceflow.mobile.editor.protyle);
...
const getActiveEditor = (wndActive = true) => {
    ...
    /// #else
    editor = window.sourceflow.mobile.popEditor || window.sourceflow.mobile.editor;
...
const hasPluginPermission = (manifest: IPluginManifest, permission: string) => {
    return Array.isArray(manifest?.permissions) && manifest.permissions.includes(permission);
};

const getPermissionDeniedMessage = (pluginName: string, permission: string) => {
    const isZh = `${window.sourceflow?.config?.lang || navigator.language || ""}`.toLowerCase().startsWith("zh");
    return isZh
        ? `插件 ${pluginName} 未声明权限 ${permission}，该能力已被拒绝。`
        : `Plugin ${pluginName} does not declare permission ${permission}. The capability is denied.`;
};

const createPermissionDeniedError = (manifest: IPluginManifest, permission: string) => {
    return new Error(getPermissionDeniedMessage(manifest?.name || "unknown", permission));
};

const withPermission = <T extends (...args: any[]) => any>(manifest: IPluginManifest, permission: string, callback: T): T => {
    return ((...args: any[]) => {
        if (!hasPluginPermission(manifest, permission)) {
            showMessage(getPermissionDeniedMessage(manifest?.name || "unknown", permission), 5000, "error");
            return;
        }
...
const withAnyPermission = <T extends (...args: any[]) => any>(manifest: IPluginManifest, permissions: string[], callback: T): T => {
    return ((...args: any[]) => {
        const granted = permissions.find((permission) => hasPluginPermission(manifest, permission));
        if (!granted) {
            showMessage(getPermissionDeniedMessage(manifest?.name || "unknown", permissions.join(" | ")), 5000, "error");
            return;
        }
        return callback(...args);
    }) as T;
...
const createGuardedDialogClass = (manifest: IPluginManifest): typeof Dialog => {
    return class SourceFlowPluginDialog extends Dialog {
        constructor(options: ConstructorParameters<typeof Dialog>[0]) {
            if (!hasPluginPermission(manifest, "ui.dialog")) {
                throw createPermissionDeniedError(manifest, "ui.dialog");
            }
            super(options);
        }
    };
...
const createGuardedMenuClass = (manifest: IPluginManifest): typeof Menu => {
    return class SourceFlowPluginMenu extends Menu {
        constructor(...args: ConstructorParameters<typeof Menu>) {
            if (!hasPluginPermission(manifest, "ui.dialog")) {
                throw createPermissionDeniedError(manifest, "ui.dialog");
            }
            super(...args);
...
const createGuardedSettingClass = (manifest: IPluginManifest): typeof Setting => {
    return class SourceFlowPluginSetting extends Setting {
        constructor(options: ConstructorParameters<typeof Setting>[0]) {
            if (!hasPluginPermission(manifest, "ui.setting")) {
                throw createPermissionDeniedError(manifest, "ui.setting");
            }
            super(options);
        }
...
const createPluginPlatformUtils = (manifest: IPluginManifest) => {
    return {
        ...platformUtils,
        sendNotification: withPermission(manifest, "ui.notification", platformUtils.sendNotification),
        cancelNotification: withPermission(manifest, "ui.notification", platformUtils.cancelNotification),
    };
};
