...
type EditorCursorSavedImage struct {
	ID        string `json:"id"`        // 本地光标 ID
	Name      string `json:"name"`      // 本地光标名称
	Source    string `json:"source"`    // 本地光标数据 URL
	CreatedAt int64  `json:"createdAt"` // 本地光标保存时间
}

const (
	EditorAssetUploadProviderLocal = "local"
	EditorAssetUploadProviderPicGo = "picgo"
	DefaultEditorPicGoServerURL    = "http://127.0.0.1:36677/upload"
	EditorHTMLPasteModeSmart       = "smart"
	EditorHTMLPasteModeHTML        = "html"
	EditorHTMLPasteModeImage       = "image"
)
...
type Editor struct {
	...
	CursorPreset                    string                   `json:"cursorPreset"`                    // 编辑器光标样式
	CursorColor                     string                   `json:"cursorColor"`                     // 编辑器光标颜色
	CursorImage                     string                   `json:"cursorImage"`                     // 自定义光标图片（data URL 或远程地址）
	CursorImageName                 string                   `json:"cursorImageName"`                 // 自定义光标图片名称
	CursorImageTint                 bool                     `json:"cursorImageTint"`                 // 是否对 SVG 光标应用颜色着色
	CursorImageWidthPercent         int                      `json:"cursorImageWidthPercent"`         // 自定义光标图片宽度，按字号百分比
	CursorImageHeightPercent        int                      `json:"cursorImageHeightPercent"`        // 自定义光标图片高度，按字号百分比
	CursorImageOffsetX              int                      `json:"cursorImageOffsetX"`              // 自定义光标图片水平偏移，单位：px
	CursorImageOffsetY              int                      `json:"cursorImageOffsetY"`              // 自定义光标图片垂直偏移，单位：px
	CursorSavedImages               []EditorCursorSavedImage `json:"cursorSavedImages"`               // 本地保存的光标图片库
	CursorBlink                     bool                     `json:"cursorBlink"`                     // 编辑器光标是否闪烁
	CursorBlinkEffect               string                   `json:"cursorBlinkEffect"`               // 编辑器光标闪烁特效
	HiddenBlockColor                string                   `json:"hiddenBlockColor"`                // 隐藏块遮罩颜色，为空时跟随主题色
	NoteBackgroundImage             string                   `json:"noteBackgroundImage"`             // 笔记编辑区背景图（data URL 或远程地址）
	NoteBackgroundOpacity           int                      `json:"noteBackgroundOpacity"`           // 笔记编辑区背景图透明度，单位：%
	NoteBackgroundBlur              int                      `json:"noteBackgroundBlur"`              // 笔记编辑区背景图模糊度，单位：px
	AssetUploadProvider             string                   `json:"assetUploadProvider"`             // 图片上传方式，local：保存到本地 assets，picgo：调用 PicGo Server
	PicGoServerURL                  string                   `json:"picgoServerURL"`                  // PicGo Server 地址
	...
	HTMLPasteMode                   string                   `json:"htmlPasteMode"`                   // HTML 粘贴模式，smart/html/image
...
func NewEditor() *Editor {
	return &Editor{
		...
		CursorImageWidthPercent:         118,
		CursorImageHeightPercent:        118,
		CursorImageOffsetX:              0,
		CursorImageOffsetY:              0,
		CursorSavedImages:               []EditorCursorSavedImage{},
		CursorBlink:                     true,
		CursorBlinkEffect:               "fade",
		HiddenBlockColor:                "",
		NoteBackgroundImage:             "",
		NoteBackgroundOpacity:           28,
		NoteBackgroundBlur:              0,
		AssetUploadProvider:             EditorAssetUploadProviderLocal,
		PicGoServerURL:                  DefaultEditorPicGoServerURL,
		...
		HTMLPasteMode:                   EditorHTMLPasteModeSmart,
