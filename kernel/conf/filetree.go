...
type FileTree struct {
	...
	UseSingleLineSave        bool   `json:"useSingleLineSave"`        // 使用单行保存文档 .sf 和属性视图 .json
	LargeFileWarningSize     int    `json:"largeFileWarningSize"`     // 大文件警告大小（单位：MB）
	CreateDocAtTop           *bool  `json:"createDocAtTop"`           // 在顶部创建新文档 https://github.com/lonelyor/SourceFlow/issues/16327
	Sort                     int    `json:"sort"`                     // 排序方式
	RecentDocsMaxListCount   int    `json:"recentDocsMaxListCount"`   // 最近的文档最大列出数量
	NoSplitScreenWhenOpenTab bool   `json:"noSplitScreenWhenOpenTab"` // 打开页签时不分屏 https://github.com/lonelyor/SourceFlow/issues/16833
}
