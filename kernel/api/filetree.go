...
func moveLocalShorthands(c *gin.Context) {
	...
	// https://github.com/lonelyor/SourceFlow/issues/14414
...
func listDocTree(c *gin.Context) {
	// Add kernel API `/api/filetree/listDocTree` https://github.com/lonelyor/SourceFlow/issues/10482
...
func moveDocs(c *gin.Context) {
	...
	if 1 < len(fromPaths) {
		model.TryCreateProtectionSnapshot("batch-move")
	}
...
func createDailyNote(c *gin.Context) {
	...
	if !existed {
		...
		// Creating a dailynote existed no longer expands the doc tree https://github.com/lonelyor/SourceFlow/issues/9959
		appArg := arg["app"]
		app := ""
		if nil != appArg {
			app = appArg.(string)
...
func listDocsByPath(c *gin.Context) {
	...
	if arg["maxListCount"] != nil {
		// API `listDocsByPath` add an optional parameter `maxListCount` https://github.com/lonelyor/SourceFlow/issues/7993
	...
	if maxListCount < totals {
		// API `listDocsByPath` add an optional parameter `ignoreMaxListHint` https://github.com/lonelyor/SourceFlow/issues/10290
...
func getDoc(c *gin.Context) {
	...
	// 判断是否正在同步中 https://github.com/lonelyor/SourceFlow/issues/6290
