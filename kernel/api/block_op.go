...
func moveBlock(c *gin.Context) {
	...
	if nil != arg["previousID"] {
		...
		// Check the validity of the API `moveBlock` parameter `previousID` https://github.com/lonelyor/SourceFlow/issues/8007
...
func updateBlock(c *gin.Context) {
	...
	if "NodeDocument" == block.Type {
		...
		if "NodeListItem" == block.Type && ast.NodeList == tree.Root.FirstChild.Type {
			// 使用 API `api/block/updateBlock` 更新列表项时渲染错误 https://github.com/lonelyor/SourceFlow/issues/4658
