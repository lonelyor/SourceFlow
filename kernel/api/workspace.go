...
func setWorkspaceDir(c *gin.Context) {
	...
	if gulu.OS.IsWindows() && !util.IsPortableMode() {
		// 改进判断工作空间路径实现 https://github.com/lonelyor/SourceFlow/issues/7569
...
func isInvalidWorkspacePath(absPath string) bool {
	...
	if 32 < utf8.RuneCountInString(name) {
		// Adjust workspace name length limit to 32 runes https://github.com/lonelyor/SourceFlow/issues/9440
		return true
	}
