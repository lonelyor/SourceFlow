...
func saveTemplate(c *gin.Context) {
	...
	path, err := model.SaveTemplate(name, content)
	if err != nil {
		ret.Code = -1
		ret.Msg = err.Error()
		return
...
func getEmbedBlock(c *gin.Context) {
	// Query embed block supports executing JavaScript https://github.com/lonelyor/SourceFlow/issues/9648
	ret := gulu.Ret.NewResult()
	defer c.JSON(http.StatusOK, ret)
...
func fullTextSearchBlock(c *gin.Context) {
	...
	var docMode bool
	cacheable := !model.IsReadOnlyRoleContext(c) && method != 2
	if cacheable {
		cacheKey := buildFullTextSearchBlockCacheKey(query, boxes, paths, types, method, orderBy, groupBy, page, pageSize)
		if cached, ok := getCachedFullTextSearchBlock(cacheKey); ok {
			blocks = cached.blocks
			matchedBlockCount = cached.matchedBlockCount
			matchedRootCount = cached.matchedRootCount
			pageCount = cached.pageCount
			docMode = cached.docMode
		} else {
			...
			setCachedFullTextSearchBlock(cacheKey, fullTextSearchBlockCacheEntry{
				blocks:            blocks,
				matchedBlockCount: matchedBlockCount,
				matchedRootCount:  matchedRootCount,
				pageCount:         pageCount,
