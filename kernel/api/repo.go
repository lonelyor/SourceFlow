...
func getRepoFile(c *gin.Context) {
	// Add internal kernel API `/api/repo/getRepoFile` https://github.com/lonelyor/SourceFlow/issues/10101
...
func getSnapshotProtectionStat(c *gin.Context) {
	ret := gulu.Ret.NewResult()
	defer c.JSON(http.StatusOK, ret)

	stat, err := model.GetSnapshotProtectionStat()
	if nil != err {
		ret.Code = -1
		ret.Msg = err.Error()
		return
...
func maintainSnapshotProtection(c *gin.Context) {
	ret := gulu.Ret.NewResult()
	defer c.JSON(http.StatusOK, ret)

	stat, err := model.MaintainSnapshotProtection()
	if nil != err {
		ret.Code = -1
		ret.Msg = err.Error()
		return
