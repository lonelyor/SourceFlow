...
func exportSyncProviderWebDAV(c *gin.Context) {
	...
	name := "sourceflow-webdav-" + time.Now().Format("20060102150405") + ".json"
...
func importSyncBackupProfile(c *gin.Context) {
	ret := gulu.Ret.NewResult()
	defer c.JSON(http.StatusOK, ret)

	data, err := readImportJSONFile(c, "sync-backup-profile")
	if nil != err {
		logging.LogErrorf("import sync backup profile failed: %s", err)
		ret.Code = -1
		ret.Msg = err.Error()
		return
	}

	info, err := model.ImportSyncBackupProfile(data)
	if nil != err {
		logging.LogErrorf("import sync backup profile failed: %s", err)
		ret.Code = -1
		ret.Msg = err.Error()
...
func exportSyncProviderS3(c *gin.Context) {
	...
	name := "sourceflow-s3-" + time.Now().Format("20060102150405") + ".json"
	tmpDir := filepath.Join(util.TempDir, "export")
	if err := os.MkdirAll(tmpDir, 0755); err != nil {
...
func getSyncDiagnostics(c *gin.Context) {
	ret := gulu.Ret.NewResult()
	defer c.JSON(http.StatusOK, ret)
	...
	ret.Data = map[string]interface{}{
		"enabled":        model.Conf.Sync.Enabled,
		"provider":       model.Conf.Sync.Provider,
		"providerName":   conf.ProviderToStr(model.Conf.Sync.Provider),
		"synced":         model.Conf.Sync.Synced,
		"stat":           stat,
		"bootSyncFailed": model.IsBootSyncReadonlyGuardActive(),
		"bootSyncStatus": model.BootSyncSucc,
		"bootSyncGuard":  model.GetBootSyncGuardInfo(),
		"kernels":        model.GetOnlineKernels(),
		"kernel":         model.KernelID,
		"perception":     model.GetSyncPerceptionStatus(),
		"recent":         model.GetSyncDiagnostics(20),
	}
}
...
func getBootSync(c *gin.Context) {
	...
	if model.IsBootSyncReadonlyGuardActive() {
		guardInfo := model.GetBootSyncGuardInfo()
		ret.Code = 1
		if nil != guardInfo {
			ret.Msg = guardInfo.Summary
			ret.Data = guardInfo
		} else {
...
func performSync(c *gin.Context) {
	...
	// Android 端前后台切换时自动触发同步 https://github.com/lonelyor/SourceFlow/issues/7122
	...
	// 云端同步模式支持 `完全手动同步` 模式 https://github.com/lonelyor/SourceFlow/issues/7295
...
func performSyncDownload(c *gin.Context) {
	ret := gulu.Ret.NewResult()
	defer c.JSON(http.StatusOK, ret)
	if err := model.CreateProtectionSnapshot("restore"); nil != err {
		ret.Code = -1
		ret.Msg = err.Error()
		return
	}
...
func readImportJSONFile(c *gin.Context, name string) (data []byte, err error) {
	form, err := c.MultipartForm()
	if nil != err {
		return
	}
	...
	files := form.File["file"]
	if 1 != len(files) {
		err = errors.New("invalid upload file")
		return
	}
	...
	if err = os.WriteFile(writePath, data, 0644); nil != err {
		return
	}

	tmpDir := filepath.Join(importDir, name)
	os.RemoveAll(tmpDir)
	lowerWritePath := strings.ToLower(writePath)
	if strings.HasSuffix(lowerWritePath, ".zip") {
		if err = gulu.Zip.Unzip(writePath, tmpDir); nil != err {
			return
		}
	} else if strings.HasSuffix(lowerWritePath, ".json") {
		if err = gulu.File.CopyFile(writePath, filepath.Join(tmpDir, f.Filename)); nil != err {
			return
		}
	} else {
		err = errors.New("invalid upload file")
