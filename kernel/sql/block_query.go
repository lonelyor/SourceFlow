...
func QueryRecentRootBlocks(limit int) (ret []*Block) {
	if 1 > limit {
		limit = 256
	}
	sqlStmt := "SELECT * FROM blocks WHERE type = 'd' ORDER BY updated DESC LIMIT " + strconv.Itoa(limit)
...
func QueryRecentBlocksByIALFragments(limit int, fragments ...string) (ret []*Block) {
	if 1 > limit {
		limit = 256
	}

	conditions := make([]string, 0, len(fragments))
	params := make([]interface{}, 0, len(fragments))
	for _, fragment := range fragments {
		fragment = strings.TrimSpace(fragment)
		if "" == fragment {
			continue
		}
		conditions = append(conditions, "ial LIKE ?")
		params = append(params, "%"+fragment+"%")
	}
	if 1 > len(conditions) {
		return
	}

	sqlStmt := "SELECT * FROM blocks WHERE type != 'd' AND (" + strings.Join(conditions, " OR ") + ") ORDER BY updated DESC LIMIT " + strconv.Itoa(limit)
	rows, err := query(sqlStmt, params...)
	if err != nil {
		logging.LogErrorf("sql query [%s] failed: %s", sqlStmt, err)
		return
	}
...
func Query(stmt string, limit int) (ret []map[string]interface{}, err error) {
	originalStmt := stmt
	// Kernel API `/api/query/sql` support `||` operator https://github.com/lonelyor/SourceFlow/issues/9662
	// 这里为了支持 || 操作符，使用了另一个 sql 解析器，但是这个解析器无法处理 UNION https://github.com/lonelyor/SourceFlow/issues/8226
	...
	if err != nil {
		if !strings.Contains(stmt, "||") {
			...
			case *sqlparser.Union:
				// Kernel API `/api/query/sql` support `UNION` statement https://github.com/lonelyor/SourceFlow/issues/8226
				limitClause := getLimitClause(parsedStmt, limit)
