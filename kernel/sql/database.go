...
import (
	...
	"github.com/lonelyor/sourceflow/third_party/go/go-sqlite3"
...
func initDBTables() {
	...
	_, err = db.Exec(fmt.Sprintf("CREATE VIRTUAL TABLE blocks_fts USING fts5(id UNINDEXED, parent_id UNINDEXED, root_id UNINDEXED, hash UNINDEXED, box UNINDEXED, path UNINDEXED, hpath, name, alias, memo, tag, content, fcontent, markdown UNINDEXED, length UNINDEXED, type UNINDEXED, subtype UNINDEXED, ial, sort UNINDEXED, created UNINDEXED, updated UNINDEXED, tokenize=%q)", util.FTSTokenizerName()))
	if err != nil {
	...
	_, err = db.Exec(fmt.Sprintf("CREATE VIRTUAL TABLE blocks_fts_case_insensitive USING fts5(id UNINDEXED, parent_id UNINDEXED, root_id UNINDEXED, hash UNINDEXED, box UNINDEXED, path UNINDEXED, hpath, name, alias, memo, tag, content, fcontent, markdown UNINDEXED, length UNINDEXED, type UNINDEXED, subtype UNINDEXED, ial, sort UNINDEXED, created UNINDEXED, updated UNINDEXED, tokenize=%q)", util.FTSTokenizerCaseInsensitive()))
	if err != nil {
...
func InitHistoryDatabase(forceRebuild bool) {
	...
	historyDBExists := gulu.File.IsExist(util.HistoryDBPath)
	initHistoryDBConnection()

	if !forceRebuild && historyDBExists && sqliteTableExists(historyDB, "histories_fts_case_insensitive") {
		return
	}
	if !forceRebuild && historyDBExists {
		logging.LogInfof("history database schema is missing, rebuilding [%s]", util.HistoryDBPath)
	}
...
func initHistoryDBTables() {
	...
	_, err := historyDB.Exec(fmt.Sprintf("CREATE VIRTUAL TABLE histories_fts_case_insensitive USING fts5(id UNINDEXED, type UNINDEXED, op UNINDEXED, title, content, path UNINDEXED, created UNINDEXED, tokenize=%q)", util.FTSTokenizerCaseInsensitive()))
	if err != nil {
...
func InitAssetContentDatabase(forceRebuild bool) {
	...
	assetContentDBExists := gulu.File.IsExist(util.AssetContentDBPath)
	initAssetContentDBConnection()

	if !forceRebuild && assetContentDBExists && sqliteTableExists(assetContentDB, "asset_contents_fts_case_insensitive") {
		return
	}
	if !forceRebuild && assetContentDBExists {
		logging.LogInfof("asset content database schema is missing, rebuilding [%s]", util.AssetContentDBPath)
	}
...
func initAssetContentDBTables() {
	...
	_, err := assetContentDB.Exec(fmt.Sprintf("CREATE VIRTUAL TABLE asset_contents_fts_case_insensitive USING fts5(id UNINDEXED, name, ext, path, size UNINDEXED, updated UNINDEXED, content, tokenize=%q)", util.FTSTokenizerCaseInsensitive()))
	if err != nil {
...
func isRepeatedRef(refs []*Ref, ref *Ref) bool {
	// Repeated references to the same block within a block only count as one reference https://github.com/lonelyor/SourceFlow/issues/9670
	for _, r := range refs {
...
func buildSpanFromNode(n *ast.Node, tree *parse.Tree, rootID, boxID, p string) (blocks []*Block, spans []*Span, assets []*Asset, attributes []*Attribute, walkStatus ast.WalkStatus) {
	...
	case ast.NodeInlineHTML, ast.NodeHTMLBlock, ast.NodeIFrame, ast.NodeWidget, ast.NodeAudio, ast.NodeVideo:
		...
		if 1 > len(nodes) &&
			ast.NodeHTMLBlock != n.Type { // HTML 块若内容为空时无法在数据库中查询到 https://github.com/lonelyor/SourceFlow/issues/4691
			walkStatus = ast.WalkContinue
			return
		}
...
func buildBlockFromNode(n *ast.Node, tree *parse.Tree) (block *Block, attributes []*Attribute) {
	...
	// 剔除零宽空格 Database index content/markdown values no longer contain zero-width spaces https://github.com/lonelyor/SourceFlow/issues/15204
	fcontent = strings.ReplaceAll(fcontent, editor.Zwsp, "")
	content = strings.ReplaceAll(content, editor.Zwsp, "")
	markdown = strings.ReplaceAll(markdown, editor.Zwsp, "")

	// 剔除标签结尾处的零宽连字符 Improve search for emojis in tags https://github.com/lonelyor/SourceFlow/issues/15391
...
func ensureHistoryDatabase() {
	if nil != historyDB {
		return
	}
	InitHistoryDatabase(false)
}

func ensureAssetContentDatabase() {
	if nil != assetContentDB {
		return
	}
	InitAssetContentDatabase(false)
}

func sqliteTableExists(database *sql.DB, table string) bool {
	if nil == database {
		return false
	}
	...
	var name string
	err := database.QueryRow("SELECT name FROM sqlite_master WHERE type='table' AND name = ? LIMIT 1", table).Scan(&name)
	if nil == err {
	...
	logging.LogWarnf("check sqlite table [%s] failed: %s", table, err)
	return false
}
...
func beginHistoryTx() (tx *sql.Tx, err error) {
	...
	if nil == historyDB {
		return nil, errors.New("history database is nil")
	}
...
func beginAssetContentTx() (tx *sql.Tx, err error) {
	ensureAssetContentDatabase()
	if nil == assetContentDB {
		return nil, errors.New("asset content database is nil")
	}
...
func prepareExecInsertTx(tx *sql.Tx, stmtSQL string, args []interface{}) (err error) {
	...
	if _, err = stmt.Exec(args...); err != nil {
		...
		if strings.Contains(err.Error(), "database disk image is malformed") {
			...
			initDatabase(true)
			logging.LogFatalf(logging.ExitCodeUnavailableDatabase, "database disk image [%s] is malformed, please restart SourceFlow kernel to rebuild it\n\t%s\n\t%v", util.DBPath, stmtSQL, args)
		}
		return
	}
	return
