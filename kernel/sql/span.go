...
func SelectSpansRawStmtWithArgs(stmt string, args []any, limit int) (ret []*Span) {
	parsedStmt, err := sqlparser.Parse(stmt)
	if err != nil {
		return
	}
...
func QueryTagSpansByLabel(label string) (ret []*Span) {
	stmt := "SELECT * FROM spans WHERE type LIKE '%tag%' AND content LIKE ? GROUP BY block_id"
	rows, err := query(stmt, "%"+label+"%")
	if err != nil {
...
func QueryTagSpansByKeyword(keyword string, limit int) (ret []*Span) {
	...
	stmt := "SELECT * FROM spans WHERE type LIKE '%tag%' AND (" + strings.Join(conds, " AND ") + ") GROUP BY markdown LIMIT " + strconv.Itoa(limit)
	rows, err := query(stmt, args...)
	if err != nil {
		logging.LogErrorf("sql query failed: %s", err)
		return
...
func QueryTagSpans(p string) (ret []*Span) {
	if "" != p {
		stmt := "SELECT * FROM spans WHERE type LIKE '%tag%' AND path = ?"
		rows, err := query(stmt, p)
		if err != nil {
			logging.LogErrorf("sql query failed: %s", err)
			return
		}
