...
func renderBlockDOMByNode(node *ast.Node, luteEngine *lute.Lute) string {
	...
	ast.Walk(node, func(n *ast.Node, entering bool) ast.WalkStatus {
		if entering {
			if n.IsBlock() {
				// 内容图中不需要渲染数据库角标 https://github.com/lonelyor/SourceFlow/issues/15057
				ial := parse.IAL2Map(n.KramdownIAL)
				delete(ial, av.NodeAttrNameAvs)

				// 重置 data-node-id 的值，避免触发前端绑定的事件 https://github.com/lonelyor/SourceFlow/issues/15088
				newID := ast.NewNodeID()
				resetIDs[newID] = n.ID
				...
				if ast.NodeIFrame == n.Type || ast.NodeAudio == n.Type || ast.NodeVideo == n.Type {
					// 禁止自动播放 Disable automatic video playback in database card view https://github.com/lonelyor/SourceFlow/issues/15212
