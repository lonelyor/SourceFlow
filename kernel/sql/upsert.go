...
func insertTree0(tx *sql.Tx, tree *parse.Tree, context map[string]interface{},
	...
	if ignoreLines := getIndexIgnoreLines(); 0 < len(ignoreLines) {
		// Support ignore index https://github.com/lonelyor/SourceFlow/issues/9198
	...
	if 0 < len(spans) {
		// 移除文档标签，否则会重复添加 https://github.com/lonelyor/SourceFlow/issues/3723
...
func getIndexIgnoreLines() (ret []string) {
	...
	indexIgnorePath := util.HiddenDataPath(util.DataDir, "indexignore")
