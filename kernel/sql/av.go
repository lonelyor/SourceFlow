...
func renderTemplateField(ial map[string]string, keyValues []*av.KeyValues, tplContent string) (ret string, err error) {
	...
	for k, v := range ial {
		...
		// Database template column supports `created` and `updated` built-in variables https://github.com/lonelyor/SourceFlow/issues/9364
		createdStr := ial["id"]
		if "" != createdStr {
	...
	for _, keyValue := range keyValues {
		...
		// Database template fields support access to the raw value https://github.com/lonelyor/SourceFlow/issues/14903
		dataModel[keyValue.Key.Name+"_raw"] = v

		// Database template fields support access by ID https://github.com/lonelyor/SourceFlow/issues/11237
...
func fillAttributeViewAutoGeneratedValues(attrView *av.AttributeView, collection av.Collection, ials map[string]map[string]string, depth *int, cachedAttrViews map[string]*av.AttributeView) {
	...
	for _, item := range collection.GetItems() {
		for _, value := range item.GetValues() {
			...
			switch value.Type {
			case av.KeyTypeBlock: // 对于主键可能需要填充静态锚文本 Database-bound block primary key supports setting static anchor text https://github.com/lonelyor/SourceFlow/issues/10049
				if nil != value.Block {
					if content := av.GetNodeAttrViewStaticTextFromAttrs(ials[value.Block.ID], attrView.ID); "" != content {
...
func filterByQuery(query string, collection av.Collection) {
	...
	if "" != query {
		...
		// 使用 AND 逻辑 https://github.com/lonelyor/SourceFlow/issues/11535
...
func GetTemplateKeyRelevantKeys(attrView *av.AttributeView, templateKey *av.Key) (ret []*av.Key) {
	...
	if 1 > len(ret) {
		// 没有相关字段情况下直接尝试解析模板，如果能解析成功则返回模板字段本身 https://github.com/lonelyor/SourceFlow/issues/15560#issuecomment-3182691193
		goTpl := template.New("").Delims(".action{", "}")
		tplFuncMap := filesys.BuiltInTemplateFuncs()
		SQLTemplateFuncs(&tplFuncMap)
		goTpl = goTpl.Funcs(tplFuncMap)
