...
func NodeStaticContent(node *ast.Node, excludeTypes []string, includeTextMarkATitleURL, includeAssetPath, fullAttrView bool) string {
	...
	ast.Walk(node, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		case ast.NodeTableCell:
			// 表格块写入数据库表时在单元格之间添加空格 https://github.com/lonelyor/SourceFlow/issues/7654
		...
		case ast.NodeText, ast.NodeCodeBlockCode, ast.NodeMathBlockContent, ast.NodeHTMLBlock:
			...
			if treenode.IsChartCodeBlockCode(n) {
				// 图表块的内容在数据库 `blocks` 表 `content` 字段中被转义 https://github.com/lonelyor/SourceFlow/issues/6326
		...
		case ast.NodeTextMark:
			...
			if n.IsTextMarkType("a") && includeTextMarkATitleURL {
				// 搜索不到超链接元素的 URL 和标题 https://github.com/lonelyor/SourceFlow/issues/7352
				if "" != n.TextMarkATitle {
	...
	// Improve search and replace for spaces https://github.com/lonelyor/SourceFlow/issues/10231
	return buf.String()
}
