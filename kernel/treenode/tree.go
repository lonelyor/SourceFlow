...
func IALStr(n *ast.Node) string {
	...
	// 所以 Some symbols should not be escaped to avoid inaccurate searches https://github.com/lonelyor/SourceFlow/issues/10185 无法被修复了
...
func RootChildIDs(rootID string) (ret []string) {
	...
	subFolder := filepath.Join(boxLocalPath, strings.TrimSuffix(root.Path, ".sf"))
