...
func IsBlockLink(n *ast.Node) bool {
	...
	return ast.NodeTextMark == n.Type && n.IsTextMarkType("a") && util.IsBlockProtocolLink(n.TextMarkAHref)
}
...
func FormatNode(node *ast.Node, luteEngine *lute.Lute) string {
	...
	if err != nil {
		root := TreeRoot(node)
		logging.LogErrorf("format node [%s] in tree [%s] failed: %s", node.ID, root.ID, err)
		return ""
	}
	return markdown
}
...
func ExportNodeStdMd(node *ast.Node, luteEngine *lute.Lute) string {
	...
	if err != nil {
		root := TreeRoot(node)
		logging.LogErrorf("export markdown for node [%s] in tree [%s] failed: %s", node.ID, root.ID, err)
		return ""
	}
	return markdown
}
...
func ParentNodesWithHeadings(node *ast.Node) (parents []*ast.Node) {
	...
	for n := node; nil != n; n = n.Parent {
		...
		// 标题下方块编辑后刷新标题块更新时间
		// The heading block update time is refreshed after editing the blocks under the heading https://github.com/lonelyor/SourceFlow/issues/11374
