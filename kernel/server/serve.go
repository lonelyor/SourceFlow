...
import (
	...
	"github.com/lonelyor/sourceflow/kernel/api"
	"github.com/lonelyor/sourceflow/kernel/cmd"
...
func Serve(fastMode bool, cookieKey string) {
	...
	ginServer.MaxMultipartMemory = 1024 * 1024 * 32 // 插入较大的资源文件时内存占用较大 https://github.com/lonelyor/SourceFlow/issues/5023
	ginServer.Use(
		model.ControlConcurrency, // 请求串行化 Concurrency control when requesting the kernel API https://github.com/lonelyor/SourceFlow/issues/9939
		model.Timing,
		model.Recover,
		corsMiddleware(), // 后端服务支持 CORS 预检请求验证 https://github.com/lonelyor/SourceFlow/pull/5593
		jwtMiddleware,    // 解析 JWT https://github.com/lonelyor/SourceFlow/issues/11364
	...
	ginServer.Use(sessions.Sessions("sourceflow", sessionStore))
...
func serveExport(ginServer *gin.Engine) {
	// Potential data export disclosure security vulnerability https://github.com/lonelyor/SourceFlow/issues/12213
...
func serveAppearance(ginServer *gin.Engine) {
	sourceflow := ginServer.Group("", model.CheckAuth)

	sourceflow.Handle("GET", "/", func(c *gin.Context) {
		...
		if strings.Contains(userAgentHeader, "Electron") {
			...
			// Improve detecting Pad device, treat it as desktop device https://github.com/lonelyor/SourceFlow/issues/8435 https://github.com/lonelyor/SourceFlow/issues/8497
	...
	sourceflow.GET("/appearance/*filepath", func(c *gin.Context) {
		...
		if strings.HasSuffix(c.Request.URL.Path, "/theme.js") {
			...
			if "zh_CN" != lang && "en_US" != lang {
				// 多语言配置缺失项使用对应英文配置项补齐 https://github.com/lonelyor/SourceFlow/issues/5322
	...
	sourceflow.Static("/stage", filepath.Join(util.WorkingDir, "stage"))
}
...
func serveAssets(ginServer *gin.Engine) {
	...
	ginServer.GET("/assets/*path", model.CheckAuth, func(context *gin.Context) {
		...
		if "/" == requestPath || "" == requestPath {
			// 禁止访问根目录 Disable HTTP access to the /assets/ path https://github.com/lonelyor/SourceFlow/issues/15257
		...
		if err != nil {
			if strings.Contains(strings.TrimPrefix(requestPath, "/"), "/") {
				// 再使用编码过的路径解析一次 https://github.com/lonelyor/SourceFlow/issues/11823
...
func serveDebug(ginServer *gin.Engine) {
	if "prod" == util.Mode {
		// The production environment will no longer register `/debug/pprof/` https://github.com/lonelyor/SourceFlow/issues/10152
		return
	}
...
func serveWebSocket(ginServer *gin.Engine) {
	...
	util.WebSocketServer.HandleConnect(func(s *melody.Session) {
		...
		if "" != model.Conf.AccessAuthCode {
			session, err := sessionStore.Get(s.Request, "sourceflow")
			if err != nil {
				authOk = false
	...
	ginGroup.Match(WebDavMethods, "/*path", func(c *gin.Context) {
		if util.ReadOnly {
			...
			case http.MethodPost,
				...
				MethodUnlock,
				MethodPropPatch:
				c.AbortWithError(http.StatusForbidden, fmt.Errorf("%s", model.Conf.Language(34)))
				return
			}
		}
