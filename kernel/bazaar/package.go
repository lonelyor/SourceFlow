...
//   - 集市包新增元数据字段时，需要同步更新集市工作流和对应的元数据解析逻辑。
type Package struct {
...
func packageContainsKeyword(pkg *Package, kw string) bool {
	if strings.Contains(strings.ToLower(pkg.Name), kw) || // https://github.com/lonelyor/SourceFlow/issues/10515
		strings.Contains(strings.ToLower(pkg.Author), kw) { // https://github.com/lonelyor/SourceFlow/issues/11673
		return true
	}
