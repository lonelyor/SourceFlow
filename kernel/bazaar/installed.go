...
func SetInstalledPackageMetadata(pkg *Package, installPath, baseURLPath, pkgType string, bazaarPackagesMap map[string]*Package) bool {
	...
	cacheKey := pkg.RepoURL
	if "" == cacheKey {
		cacheKey = pkgType + ":" + pkg.Name
	}
	// TODO 本地安装大小的缓存改成 1 分钟有效，打开集市包 README 的时候才遍历集市包文件夹进行统计，异步返回结果到前端显示 https://github.com/lonelyor/SourceFlow/issues/16983
	// 目前优先使用在线 stage 数据：不耗时，但可能不准确，比如本地旧版本与云端最新版本的安装大小可能不一致；其次使用本地目录大小：耗时，但准确
	// 需要分离本地安装大小和在线 stage 数据的安装大小
	bazaarMemMu.RLock()
	cachedSize, hit := installSizeCache[cacheKey]
	bazaarMemMu.RUnlock()
	if hit {
		...
		bazaarMemMu.Lock()
		installSizeCache[cacheKey] = size
		bazaarMemMu.Unlock()
	}
	pkg.HInstallSize = humanize.BytesCustomCeil(uint64(pkg.InstallSize), 2)
	pkg.InstallSource = getPackageInstallSource(pkgType, pkg.Name)
	pkg.InstallSourceURL = getPackageInstallSourceURL(pkgType, pkg.Name)
	pkg.InstallIntegrity = getPackageInstallIntegrity(pkgType, pkg.Name)

	return true
}

// Add marketplace package config item `minAppVersion` https://github.com/lonelyor/SourceFlow/issues/8330
...
type PackageInfo struct {
	...
	InstallSourceURL string `json:"installSourceURL,omitempty"`
	InstallIntegrity string `json:"installIntegrity,omitempty"`
}
...
// SetPackageInstallRecord 记录集市包来源与摘要，供已安装列表展示。
func SetPackageInstallRecord(pkgType, pkgName string, installTime time.Time, source, sourceURL, integrity string) {
	getBazaarInfo()
	...
	p.InstallTime = installTime.UnixMilli()
	p.InstallSource = strings.TrimSpace(source)
	p.InstallSourceURL = strings.TrimSpace(sourceURL)
	p.InstallIntegrity = strings.TrimSpace(integrity)
	saveBazaarInfo()
}
...
func getPackageInstallSource(pkgType, pkgName string) string {
	getBazaarInfo()
	bazaarInfoCacheLock.RLock()
...
func getPackageInstallSourceURL(pkgType, pkgName string) string {
	getBazaarInfo()
	bazaarInfoCacheLock.RLock()
	defer bazaarInfoCacheLock.RUnlock()
	if bazaarInfoCache == nil || bazaarInfoCache.Packages[pkgType] == nil {
	...
	if p := bazaarInfoCache.Packages[pkgType][pkgName]; p != nil {
		return p.InstallSourceURL
	}
	return ""
}

func getPackageInstallIntegrity(pkgType, pkgName string) string {
	getBazaarInfo()
	bazaarInfoCacheLock.RLock()
	defer bazaarInfoCacheLock.RUnlock()
	if bazaarInfoCache == nil || bazaarInfoCache.Packages[pkgType] == nil {
