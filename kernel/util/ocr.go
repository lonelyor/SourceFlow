...
var (
	...
	assetsTextsLoadLock = sync.Mutex{}
	assetsTextsLoaded   = atomic.Bool{}
...
func EnsureAssetsTextsLoaded() {
	if assetsTextsLoaded.Load() {
		return
	}

	assetsTextsLoadLock.Lock()
	defer assetsTextsLoadLock.Unlock()
	if assetsTextsLoaded.Load() {
		return
	}

	loadAssetsTexts0()
	assetsTextsLoaded.Store(true)
}
...
// tesseractOCRLock 用于 Tesseract OCR 加锁串行执行提升稳定性 https://github.com/lonelyor/SourceFlow/issues/7265
...
func Tesseract(imgAbsPath string) (ret []map[string]interface{}) {
	...
	timeout := 7000
	timeoutEnv := GetEnv(TesseractTimeoutEnv)
	if "" != timeoutEnv {
...
func InitTesseract() {
	...
	maxSizeVal := GetEnv(TesseractMaxSizeEnv)
	if "" != maxSizeVal {
	...
	// Supports via environment var `SOURCEFLOW_TESSERACT_ENABLED=false` to close OCR https://github.com/lonelyor/SourceFlow/issues/9619
	enabled := GetEnv(TesseractEnabledEnv)
	if "" != enabled {
...
func filterTesseractLangs(langs []string) (ret []string) {
	...
	envLangsVal := GetEnv(TesseractLangsEnv)
	if "" != envLangsVal {
...
func getTesseractVer() (ret string) {
	...
	if err != nil {
		...
		if strings.Contains(errMsg, "executable file not found") || strings.Contains(errMsg, "no such file or directory") {
			// macOS 端 Tesseract OCR 安装后不识别 https://github.com/lonelyor/SourceFlow/issues/7107
