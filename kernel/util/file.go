...
func FilterUploadEmojiFileName(name string) string {
	if strings.HasPrefix(name, "api/icon/") {
		// 忽略动态图标 https://github.com/lonelyor/SourceFlow/issues/15139
		return name
	}
...
func FilterUploadFileName(name string) string {
	...
	// 插入资源文件时去除 `[`、`(` 等符号 https://github.com/lonelyor/SourceFlow/issues/6708
	ret = strings.ReplaceAll(ret, "~", "")
	//ret = strings.ReplaceAll(ret, "_", "") // 插入资源文件时允许下划线 https://github.com/lonelyor/SourceFlow/issues/3534
	ret = strings.ReplaceAll(ret, "[", "")
	ret = strings.ReplaceAll(ret, "]", "")
	ret = strings.ReplaceAll(ret, "(", "")
	ret = strings.ReplaceAll(ret, ")", "")
...
func TruncateLenFileName(name string) (ret string) {
	// 插入资源文件时文件名长度最大限制 189 字节 https://github.com/lonelyor/SourceFlow/issues/7099
	...
	if ".png" == ext {
		...
		// https://github.com/lonelyor/SourceFlow/pull/16714#issuecomment-3737987302
...
func FilterFileName(name string) string {
	...
	name = RemoveInvalid(name) // Remove invisible characters from file names when uploading assets https://github.com/lonelyor/SourceFlow/issues/11683
	return name
}
...
func IsReservedFilename(baseName string) bool {
	return "assets" == baseName || "templates" == baseName || "widgets" == baseName || "emojis" == baseName ||
		HiddenDataDirName == baseName || strings.HasPrefix(baseName, ".")
}
