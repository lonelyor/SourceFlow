...
func InitPandoc() {
	...
	if "" != PandocBinPath && IsValidPandocBin(PandocBinPath) {
		return
	}
...
func initPandoc() {
	...
	if confPath := filepath.Join(ConfDir, "conf.json"); gulu.File.IsExist(confPath) {
		// Workspace built-in Pandoc is no longer initialized after customizing Pandoc path https://github.com/lonelyor/SourceFlow/issues/8377
