...
func NewStdLute() (ret *lute.Lute) {
	...
	ret.SetFootnotes(false)
	ret.SetToC(false)
	ret.SetIndentCodeBlock(true) // 导入 Markdown 时支持缩进代码块语法 Support indented code block syntax when importing Markdown https://github.com/lonelyor/SourceFlow/issues/14429
	ret.SetAutoSpace(false)
	ret.SetHeadingID(false)
	ret.SetSetext(false)
	ret.SetYamlFrontMatter(false)
	ret.SetLinkRef(false)
	ret.SetGFMAutoLink(false) // 导入 Markdown 时不自动转换超链接 https://github.com/lonelyor/SourceFlow/issues/7682
	ret.SetImgPathAllowSpace(true)
	ret.SetInlineMathAllowDigitAfterOpenMarker(true) // Formula parsing supports $ followed by numbers when importing Markdown https://github.com/lonelyor/SourceFlow/issues/8362

	// 导入 Markdown 时遵循编辑器 Markdown 语法设置
	// Follow editor Markdown syntax settings when importing Markdown https://github.com/lonelyor/SourceFlow/issues/14731
	ret.SetInlineAsterisk(MarkdownSettings.InlineAsterisk)
	ret.SetInlineUnderscore(MarkdownSettings.InlineUnderscore)
	ret.SetSup(MarkdownSettings.InlineSup)
	ret.SetSub(MarkdownSettings.InlineSub)
...
func LinkTarget(htmlStr, linkBase string) (ret string) {
	...
	doc.Find("a").Each(func(i int, selection *goquery.Selection) {
		if href, ok := selection.Attr("href"); ok {
			...
			// The hyperlink in the marketplace package README fails to jump to the browser to open https://github.com/lonelyor/SourceFlow/issues/8452
