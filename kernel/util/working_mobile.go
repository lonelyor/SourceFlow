...
func BootMobile(container, appDir, workspaceBaseDir, lang string) {
	...
	userHomeConfDir := filepath.Join(HomeDir, ".config", "sourceflow")
	...
	defaultWorkspaceDir := filepath.Join(workspaceBaseDir, "sourceflow")
...
func initWorkspaceDirMobile(workspaceBaseDir string) {
	if gulu.File.IsDir(workspaceBaseDir) {
		...
		if err != nil {
			...
			// 旧版 iOS 端会在 workspaceBaseDir 下直接创建工作空间，这里需要将数据迁移到 workspaceBaseDir/sourceflow/ 文件夹下
			...
			if oldConf && oldData && oldTemp {
				for _, entry := range entries {
					if "home" == entry.Name() || "sourceflow" == entry.Name() || LegacyWorkspaceDirName() == entry.Name() {
						continue
					}

					from := filepath.Join(workspaceBaseDir, entry.Name())
					to := filepath.Join(workspaceBaseDir, "sourceflow", entry.Name())
