...
func GetChildDocDepth(treeAbsPath string) (ret int) {
	dir := strings.TrimSuffix(treeAbsPath, ".sf")
...
func FilterMoveDocFromPaths(fromPaths []string, toPath string) (ret []string) {
	...
	for _, fromPath := range tmp {
		fromDir := strings.TrimSuffix(fromPath, ".sf")
...
func FilterSelfChildDocs(paths []string) (ret []string) {
	...
	for _, fromPath := range paths {
		dir := strings.TrimSuffix(fromPath, ".sf")
		existParent := false
		for d := range dirs {
...
func FileURLToLocalPath(fileURL string) string {
	...
	if gulu.OS.IsWindows() && strings.Contains(p, ":") {
		// Windows 支持 file:// 后跟多个斜杠 https://github.com/lonelyor/SourceFlow/issues/11885
		p = strings.TrimLeft(p, "/")
	}
	if strings.Contains(p, "?") {
		// 去除查询参数 https://github.com/lonelyor/SourceFlow/issues/13600
		p = p[:strings.Index(p, "?")]
	}
	if unescaped, err := url.PathUnescape(p); err == nil && unescaped != p {
		// `Convert network images/assets to local` supports URL-encoded local file names https://github.com/lonelyor/SourceFlow/issues/9929
		p = unescaped
	}
	return p
}
...
var (
	SourceFlowAssetsImage = []string{".apng", ".ico", ".cur", ".jpg", ".jpe", ".jpeg", ".jfif", ".pjp", ".pjpeg", ".png", ".gif", ".webp", ".bmp", ".svg", ".avif"}
	SourceFlowAssetsAudio = []string{".mp3", ".wav", ".ogg", ".m4a", ".flac"}
	SourceFlowAssetsVideo = []string{".mov", ".weba", ".mkv", ".mp4", ".webm"}
)
...
func IsPossiblyImage(assetPath string) bool {
	...
	if "" != ext {
		return gulu.Str.Contains(ext, SourceFlowAssetsImage)
	}
	...
	if filePath := FileURLToLocalPath(assetPath); filePath != "" {
		...
		return gulu.Str.Contains(m.Extension(), SourceFlowAssetsImage)
	}
...
func IsDisplayableAsset(p string) bool {
	...
	if gulu.Str.Contains(ext, SourceFlowAssetsImage) {
		return true
	}
	if gulu.Str.Contains(ext, SourceFlowAssetsAudio) {
		return true
	}
	if gulu.Str.Contains(ext, SourceFlowAssetsVideo) {
		return true
	}
	return false
}
