...
func (value *Value) IsEdited() bool {
	...
	if KeyTypeCheckbox == value.Type {
		// 复选框不会为空，即使复选框未勾选，也不算是空，所以不能用下面的 IsEmpty 判断，这里使用更新时间判断是否编辑过 https://github.com/lonelyor/SourceFlow/issues/11016
	...
	for _, blockID := range relationVal.Relation.BlockIDs {
		...
		if nil == destVal {
			if KeyTypeCheckbox == destKey.Type {
				// 没有编辑过复选框的时候没有值，没有值等同于未选中，所以这里补一个未选中的值 https://github.com/lonelyor/SourceFlow/issues/15858
