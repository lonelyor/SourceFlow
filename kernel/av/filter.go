	...
	case KeyTypeDate:
		...
		if nil != value.Date {
			if !value.Date.IsNotEmpty {
				// 空值不进行比较，直接排除
				// Database date filter excludes empty values https://github.com/lonelyor/SourceFlow/issues/11061
				return false
			}
...
func (filter *ViewFilter) GetAffectValue(key *Key, addingBlockID string) (ret *Value) {
	...
	if FilterOperatorIsEmpty != filter.Operator && FilterOperatorIsNotEmpty != filter.Operator {
		if filter.Value.IsEmpty() && nil == filter.RelativeDate {
			// 在不是过滤空值和非空值的情况下，空值不设置默认值 https://github.com/lonelyor/SourceFlow/issues/11297
			return nil
		}
	}
