...
type Created struct {
	IncludeTime bool `json:"includeTime"` // 是否填充具体时间 Add `Include time` switch to database creation time field and update time field https://github.com/lonelyor/SourceFlow/issues/12091
}
...
type Date struct {
	AutoFillNow      bool `json:"autoFillNow"`      // 是否自动填充当前时间 The database date field supports filling the current time by default https://github.com/lonelyor/SourceFlow/issues/10823
	FillSpecificTime bool `json:"fillSpecificTime"` // 是否填充具体时间 Add `Default fill specific time` switch to database date field https://github.com/lonelyor/SourceFlow/issues/12089
}
...
const (
	...
	NodeAttrView           = "custom-sf-av-view"   // 用于标记块所属的属性视图视图 view id Database block support specified view https://github.com/lonelyor/SourceFlow/issues/10443
	NodeAttrViewStaticText = "custom-sf-av-s-text" // 用于标记块所属的属性视图静态文本 Database-bound block primary key supports setting static anchor text https://github.com/lonelyor/SourceFlow/issues/10049

	NodeAttrViewNames = "av-names" // 用于临时标记块所属的属性视图名称，空格分隔
)

func GetNodeAttrView(node *ast.Node) string {
	if nil == node {
		return ""
	}
	return node.IALAttr(NodeAttrView)
}

func GetNodeAttrViewFromAttrs(attrs map[string]string) string {
	if nil == attrs {
		return ""
	}
	return attrs[NodeAttrView]
}

func SetNodeAttrView(attrs map[string]string, viewID string) {
	if nil == attrs {
		return
	}
	if "" == viewID {
		delete(attrs, NodeAttrView)
		return
	}
	attrs[NodeAttrView] = viewID
}

func NodeAttrViewStaticTextKey(avID string) string {
	return NodeAttrViewStaticText + "-" + avID
}

func GetNodeAttrViewStaticText(node *ast.Node, avID string) string {
	if nil == node {
		return ""
	}
	return node.IALAttr(NodeAttrViewStaticTextKey(avID))
}

func GetNodeAttrViewStaticTextFromAttrs(attrs map[string]string, avID string) string {
	if nil == attrs {
		return ""
	}
	return attrs[NodeAttrViewStaticTextKey(avID)]
}

func SetNodeAttrViewStaticText(attrs map[string]string, avID, text string) {
	if nil == attrs {
		return
	}
	primaryKey := NodeAttrViewStaticTextKey(avID)
	if "" == text {
		delete(attrs, primaryKey)
		return
	}
	attrs[primaryKey] = text
}

func IsNodeAttrViewStaticTextKey(key string) bool {
	return strings.HasPrefix(key, NodeAttrViewStaticText+"-")
}
