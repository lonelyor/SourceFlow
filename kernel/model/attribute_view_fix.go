...
func checkAttrView(attrView *av.AttributeView, view *av.View) {
	...
	for i, v := range attrView.Views {
		if av.LayoutTypeGallery == v.LayoutType && nil == v.Gallery {
			// 切换为卡片视图时可能没有初始化卡片实例 https://github.com/lonelyor/SourceFlow/issues/15122
			if nil != v.Table {
	...
	// 截断超长的数据库标题 Limit the database title to 512 characters https://github.com/lonelyor/SourceFlow/issues/15459
