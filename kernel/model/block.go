...
func CheckBlockRef(ids []string) bool {
	...
	// TODO 还需要考虑容器块的子块引用计数 https://github.com/lonelyor/SourceFlow/issues/13396
...
func getBlockKramdown0(tree *parse.Tree, id, mode string, luteEngine *lute.Lute) (ret string) {
	...
	if "md" == mode {
		// `/api/block/getBlockKramdown` link/image URLs are no longer encoded with spaces https://github.com/lonelyor/SourceFlow/issues/15611
...
func getEmbeddedBlock(trees map[string]*parse.Tree, sqlBlock *sql.Block, headingMode int, breadcrumb bool) (block *Block, blockPaths []*BlockPath) {
	...
	// 嵌入块查询结果中显示块引用计数 https://github.com/lonelyor/SourceFlow/issues/7191
