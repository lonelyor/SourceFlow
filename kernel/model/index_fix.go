...
// resetDuplicateBlocksOnFileSys 重置重复 ID 的块。 https://github.com/lonelyor/SourceFlow/issues/7357
func resetDuplicateBlocksOnFileSys() {
	...
	for _, box := range boxes {
		...
		historyPath := util.HiddenDataPath(filepath.Join(util.DataDir, box.ID), "history")
		if gulu.File.IsDir(historyPath) {
			if removeErr := os.RemoveAll(historyPath); nil != removeErr {
				logging.LogErrorf("remove legacy history failed: %s", removeErr)
			} else {
				logging.LogInfof("removed history [%s]", historyPath)
			}
		}
		...
		filelock.Walk(boxPath, func(path string, d fs.DirEntry, err error) error {
			...
			if filepath.Ext(path) != ".sf" || strings.Contains(filepath.ToSlash(path), "/assets/") {
				return nil
			}
...
func recreateTree(tree *parse.Tree, absPath string) {
	...
	treenode.RemoveBlockTreesByPathPrefix(strings.TrimSuffix(tree.Path, ".sf"))
	...
	if gulu.File.IsDir(strings.TrimSuffix(absPath, ".sf")) {
		// 重命名子文档文件夹
		from := strings.TrimSuffix(absPath, ".sf")
