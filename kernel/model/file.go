...
import (
	...
	util2 "github.com/lonelyor/sourceflow/third_party/go/lute/util"
...
func SearchDocs(keyword string, flashcard bool, excludeIDs []string) (ret []map[string]string) {
	...
	cacheKey := buildSearchDocsCacheKey(keyword, flashcard, excludeIDs)
	if cached, ok := getCachedSearchDocs(cacheKey); ok {
		return cached
	}
			...
			for _, box := range boxes {
				if gulu.Str.Contains(box.Name, keywords) {
					if flashcard {
						...
						if 0 < flashcardCount {
							ret = append(ret, map[string]string{"path": "/", "hPath": box.Name + "/", "box": box.ID, "boxIcon": box.Icon, "rootID": "", "newFlashcardCount": strconv.Itoa(newFlashcardCount), "dueFlashcardCount": strconv.Itoa(dueFlashcardCount), "flashcardCount": strconv.Itoa(flashcardCount)})
						}
					} else {
						ret = append(ret, map[string]string{"path": "/", "hPath": box.Name + "/", "box": box.ID, "boxIcon": box.Icon, "rootID": ""})
					}
				}
			}
	...
	for _, rootBlock := range rootBlocks {
		...
		if flashcard {
			...
			if 0 < flashcardCount {
				ret = append(ret, map[string]string{"path": rootBlock.Path, "hPath": hPath, "box": rootBlock.Box, "boxIcon": b.Icon, "rootID": rootBlock.ID, "newFlashcardCount": strconv.Itoa(newFlashcardCount), "dueFlashcardCount": strconv.Itoa(dueFlashcardCount), "flashcardCount": strconv.Itoa(flashcardCount)})
			}
		} else {
			ret = append(ret, map[string]string{"path": rootBlock.Path, "hPath": hPath, "box": rootBlock.Box, "boxIcon": b.Icon, "rootID": rootBlock.ID})
		}
	}
	...
	setCachedSearchDocs(cacheKey, ret)
	return
}
...
func ListDocTree(boxID, listPath string, sortMode int, flashcard, showHidden bool, maxListCount int) (ret []*File, totals int, err error) {
	...
	for _, file := range files {
		if file.isdir {
			...
			parentDocPath := strings.TrimSuffix(file.path, "/") + ".sf"
			...
			if strings.HasSuffix(file.name, ".sf") && !ast.IsNodeIDPattern(strings.TrimSuffix(file.name, ".sf")) {
				// 不以块 ID 命名的 .sf 文件不应该被加载到源流中 https://github.com/lonelyor/SourceFlow/issues/16089
				continue
			}
		}

		subFolder := filepath.Join(boxLocalPath, strings.TrimSuffix(file.path, ".sf"))
...
func GetDoc(startID, endID, id string, index int, query string, queryTypes map[string]bool, queryMethod, mode int, size int, isBacklink bool, originalRefBlockIDs map[string]string, highlight bool) (
	...
	if nil == node {
		// Unable to open the doc when the block pointed by the scroll position does not exist https://github.com/lonelyor/SourceFlow/issues/9030
	...
	var nodes []*ast.Node
	if isBacklink {
		// 引用计数浮窗请求，需要按照反链逻辑组装 https://github.com/lonelyor/SourceFlow/issues/6853
	...
	for _, n := range nodes {
		...
		ast.Walk(n, func(n *ast.Node, entering bool) ast.WalkStatus {
			...
			if "1" == n.IALAttr("heading-fold") {
				// 折叠标题下被引用的块无法悬浮查看
				// The referenced block under the folded heading cannot be hovered to view https://github.com/lonelyor/SourceFlow/issues/9582
			...
			if avs := n.IALAttr(av.NodeAttrNameAvs); "" != avs {
				// 填充属性视图角标 Display the database title on the block superscript https://github.com/lonelyor/SourceFlow/issues/10545
			...
			if highlight && existKeywords {
				...
				if hitBlock {
					if ast.NodeCodeBlockCode == n.Type && !treenode.IsChartCodeBlockCode(n) {
						// 支持代码块搜索定位 https://github.com/lonelyor/SourceFlow/issues/5520
			...
			if existKeywords && id == n.ID {
				...
				for _, inline := range inlines {
					if inline.IsTextMarkType("inline-memo") && util.ContainsSubStr(inline.TextMarkInlineMemoContent, keywords) {
						// 支持行级备注搜索定位 https://github.com/lonelyor/SourceFlow/issues/13465
...
func loadNodesByMode(node *ast.Node, inputIndex, mode, size int, isDoc, isHeading bool) (nodes []*ast.Node, eof bool) {
	if 2 == mode /* 向下 */ {
		...
		if ast.NodeHeading == node.Type && "1" == node.IALAttr("fold") {
			// 标题展开时进行动态加载导致重复内容 https://github.com/lonelyor/SourceFlow/issues/4671
			// 这里要考虑折叠标题是最后一个块的情况
		...
		if isDoc {
			...
			for n := node.Next; nil != n; n = n.Next {
				if treenode.IsInFoldedHeading(n, node) {
					// 大纲点击折叠标题跳转聚焦 https://github.com/lonelyor/SourceFlow/issues/4920
					// 多级标题折叠后上级块引浮窗中未折叠 https://github.com/lonelyor/SourceFlow/issues/4997
					continue
				}
...
func DuplicateDoc(tree *parse.Tree) {
	...
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		// 复制为副本时移除数据库绑定状态 https://github.com/lonelyor/SourceFlow/issues/12294
		n.RemoveIALAttr(av.NodeAttrNameAvs)
		n.RemoveIALAttr(av.NodeAttrViewNames)
		n.RemoveIALAttrsByPrefix(av.NodeAttrViewStaticText)

		// 复制为副本时移除闪卡相关属性 https://github.com/lonelyor/SourceFlow/issues/13987
...
func CreateWithMarkdown(tags, boxID, hPath, md, parentID, id string, withMath bool, clippingHref string) (retID string, err error) {
	...
	if strings.HasPrefix(clippingHref, util.LegacyCommunityArticlePrefix()) || strings.HasPrefix(clippingHref, util.LegacyCloudArticlePrefix()) {
...
func CreateDailyNote(boxID string) (p string, existed bool, err error) {
	...
	if "" != templateDom {
		...
		if err == nil {
			...
			// Creating a dailynote template supports doc attributes https://github.com/lonelyor/SourceFlow/issues/10698
...
func MoveDocs(fromPaths []string, toBoxID, toPath string, callback interface{}) (err error) {
	...
	if 1 == len(fromPaths) {
		...
		if fromBox := pathsBoxes[fromPaths[0]]; nil != fromBox && fromBox.ID == toBoxID {
			...
			if ("/" == toPath && "/" == parentDir) || (parentDir+".sf" == toPath) {
				return
			}
		}
	}
	...
	// A progress layer appears when moving more than 64 documents at once https://github.com/lonelyor/SourceFlow/issues/9356
	subDocsCount := 0
	for fromPath, fromBox := range pathsBoxes {
...
func moveDoc(fromBox *Box, fromPath string, toBox *Box, toPath string, luteEngine *lute.Lute, callback interface{}) (newPath string, err error) {
	...
	newPath = path.Join(toFolder, tree.ID+".sf")
	...
	if needMoveSubDocs {
		// 将其所有子文档的移动事件推送到前端 https://github.com/lonelyor/SourceFlow/issues/11661
		subDocsFolder := path.Join(toFolder, tree.ID)
		sfFiles := listSfFiles(path.Join(toBox.ID, subDocsFolder))
		for _, sfFile := range sfFiles {
			relPath := strings.TrimPrefix(sfFile, "/"+path.Join(toBox.ID, toFolder))
...
func RenameDoc(boxID, p, title string) (err error) {
	...
	if 512 < utf8.RuneCountInString(title) {
		// 限制笔记本名和文档名最大长度为 `512` https://github.com/lonelyor/SourceFlow/issues/6299
	...
	isTitleEmpty := IsNodeTitleEmpty(tree.Root)
	if isTitleEmpty != isEmpty {
		SetNodeTitleEmpty(tree.Root, isEmpty)
...
func createDoc(boxID, p, title, dom string) (tree *parse.Tree, err error) {
	...
	if isEmpty {
		SetNodeTitleEmpty(tree.Root, true)
	}
	...
	// Convert mp3 and mp4 hyperlinks to audio and video when moving cloud inbox to docs https://github.com/lonelyor/SourceFlow/issues/9778
	var unlinks []*ast.Node
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		if !entering {
			return ast.WalkContinue
...
func normalizeDocTitle(title string) string {
	...
	// 不要踢掉 零宽连字符，否则有的 Emoji 会变形 https://github.com/lonelyor/SourceFlow/issues/11480
...
func moveSorts(rootID, fromBox, toBox string) {
	...
	fromConfPath := util.HiddenDataPath(filepath.Join(util.DataDir, fromBox), "sort.json")
	...
	toConfPath := util.HiddenDataPath(filepath.Join(util.DataDir, toBox), "sort.json")
...
func ChangeFileTreeSort(boxID string, paths []string) {
	...
	for _, f := range files {
		...
		id := strings.TrimSuffix(f.Name(), ".sf")
		val := sortIDs[id]
		if 0 == val {
			val = max
			max++
	...
	confDir, err := util.EnsureHiddenDataDir(boxDir)
	if err != nil {
...
func (box *Box) fillSort(files *[]*File) {
	confPath := util.HiddenDataPath(filepath.Join(util.DataDir, box.ID), "sort.json")
	if !filelock.IsExist(confPath) {
		return
	}
