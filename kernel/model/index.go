...
func UpsertIndexes(paths []string) {
	...
	for _, p := range paths {
		if strings.HasSuffix(p, "/") {
			sfFiles = append(sfFiles, listSfFiles(p)...)
			continue
		}
	...
	sfFiles = gulu.Str.RemoveDuplicatedElem(sfFiles)
	upsertIndexes(sfFiles)
}
...
func RemoveIndexes(paths []string) {
	...
	sfFiles = gulu.Str.RemoveDuplicatedElem(sfFiles)
	removeIndexes(sfFiles)
}

func listSfFiles(dir string) (ret []string) {
...
func indexBox(boxID string) {
	...
	for _, file := range files {
		if file.isdir || !strings.HasSuffix(file.name, ".sf") {
			continue
		}

		if !ast.IsNodeIDPattern(strings.TrimSuffix(file.name, ".sf")) {
...
// IndexEmbedBlockJob 嵌入块支持搜索 https://github.com/lonelyor/SourceFlow/issues/7112
...
func autoIndexEmbedBlock() {
	...
	for i, embedBlock := range embedBlocks {
		...
		if strings.HasPrefix(stmt, "//!js") {
			// https://github.com/lonelyor/SourceFlow/issues/9648
			// js 嵌入块不支持自动索引，由前端主动调用 /api/search/updateEmbedBlock 接口更新内容 https://github.com/lonelyor/SourceFlow/issues/9736
			continue
		}
...
func updateEmbedBlockContent(embedBlockID string, queryResultBlocks []*EmbedBlock) {
	...
	embedBlock.Content = "" // 嵌入块每查询一次多一个结果 https://github.com/lonelyor/SourceFlow/issues/7196
	for _, block := range queryResultBlocks {
