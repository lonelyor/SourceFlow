	...
	for _, pkg := range installedPackages {
		...
		case "plugins":
			...
			if nil != petal {
				...
				pkg.Enabled = &enabled
				pkg.DisabledReason = petal.DisabledReason
			}
		case "themes":
...
func backupInstalledPackage(installPath string) (string, error) {
	if !gulu.File.IsExist(installPath) {
		return "", nil
	}
	backupPath := installPath + ".sourceflow-backup"
...
func restoreInstalledPackageBackup(installPath, backupPath string) error {
	if "" == backupPath || !gulu.File.IsExist(backupPath) {
		return nil
	}
...
func cleanupInstalledPackageBackup(backupPath string) {
	if "" == backupPath {
		return
	}
...
func InstallBazaarPackage(pkgType, repoURL, repoHash, packageName string, themeMode int) error {
	...
	backupPath := ""
	if pkgType == "plugins" && update {
		backupPath, err = backupInstalledPackage(installPath)
		if err != nil {
			return fmt.Errorf("backup existing plugin failed: %w", err)
		}
		defer cleanupInstalledPackageBackup(backupPath)
	}

	err = bazaar.InstallPackage(repoURL, repoHash, installPath, Conf.System.ID, pkgType, packageName)
	if err != nil {
		if pkgType == "plugins" && "" != backupPath {
			_ = restoreInstalledPackageBackup(installPath, backupPath)
		}
		return fmt.Errorf(Conf.Language(46), packageName, err)
	}

	if pkgType == "plugins" {
		if _, err = LoadPluginManifest(installPath); err != nil {
			_ = os.RemoveAll(installPath)
			if "" != backupPath {
				_ = restoreInstalledPackageBackup(installPath, backupPath)
			}
			return fmt.Errorf("validate installed plugin failed: %w", err)
		}
	}
	...
	case "themes":
		if !update {
			// 更新主题后不需要切换到该主题 https://github.com/lonelyor/SourceFlow/issues/4966
			if 0 == themeMode {
