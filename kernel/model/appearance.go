...
func InitAppearance() {
	...
	if err := os.MkdirAll(util.AppearancePath, 0755); err != nil && !os.IsExist(err) {
		logging.LogErrorf("create appearance folder [%s] failed: %s", util.AppearancePath, err)
		util.ReportFileSysFatalError(err)
		return
	}

	if shouldRefreshAppearanceResources() {
		...
		if err := markAppearanceResourcesReady(); nil != err {
			logging.LogWarnf("write appearance version marker failed: %s", err)
		}
	}
...
func shouldRefreshAppearanceResources() bool {
	if "dev" == util.Mode {
		return true
	}

	requiredPaths := []string{
		filepath.Join(util.AppearancePath, "themes"),
		filepath.Join(util.AppearancePath, "icons"),
		filepath.Join(util.AppearancePath, "emojis", "conf.json"),
	}
	for _, requiredPath := range requiredPaths {
		if !gulu.File.IsExist(requiredPath) {
			return true
		}
	}

	versionMarkerPath := filepath.Join(util.AppearancePath, ".version")
	data, err := os.ReadFile(versionMarkerPath)
	if nil != err {
		return true
	}
	return strings.TrimSpace(string(data)) != util.Ver
}

func markAppearanceResourcesReady() error {
	versionMarkerPath := filepath.Join(util.AppearancePath, ".version")
	return os.WriteFile(versionMarkerPath, []byte(util.Ver), 0644)
}
