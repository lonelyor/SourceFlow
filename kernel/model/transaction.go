...
func (tx *Transaction) doMove(operation *Operation) (ret *TxErr) {
	...
	// 生成文档历史 https://github.com/lonelyor/SourceFlow/issues/14359
	...
	if isMovingFoldHeading := ast.NodeHeading == srcNode.Type && "1" == srcNode.IALAttr("fold"); isMovingFoldHeading {
		...
		// Blocks below other non-folded headings are no longer moved when moving a folded heading https://github.com/lonelyor/SourceFlow/issues/8321
	...
	if "" != targetPreviousID {
		...
		if 0 < len(headingChildren) {
			...
			// Nested lists cause hang after collapsing headings https://github.com/lonelyor/SourceFlow/issues/15943
...
func isMovingFoldHeadingIntoSelf(targetNode *ast.Node, headingChildren []*ast.Node) bool {
	for _, headingChild := range headingChildren {
		if headingChild.ID == targetNode.ID {
			// 不能将折叠标题移动到自己下方节点的前或后 https://github.com/lonelyor/SourceFlow/issues/7163
			return true
		}
	}
	return false
...
func (tx *Transaction) doAppendInsert(operation *Operation) (ret *TxErr) {
	...
	for _, toInsert := range toInserts {
		if isContainer {
			if ast.NodeList == node.Type {
				// 列表下只能挂列表项，所以这里需要分情况处理 https://github.com/lonelyor/SourceFlow/issues/9955
				if ast.NodeList == toInsert.Type {
					var childLis []*ast.Node
					for childLi := toInsert.FirstChild; nil != childLi; childLi = childLi.Next {
						childLis = append(childLis, childLi)
...
func (tx *Transaction) doDelete0(operation *Operation, tree *parse.Tree) {
	...
	if nil == node {
		return // move 以后的情况，列表项移动导致的状态异常 https://github.com/lonelyor/SourceFlow/issues/961
	}
	...
	if nil != node.Next && ast.NodeKramdownBlockIAL == node.Next.Type && bytes.Contains(node.Next.Tokens, []byte(node.ID)) {
		// 列表块撤销状态异常 https://github.com/lonelyor/SourceFlow/issues/3985
		node.Next.Unlink()
	}
	...
	// List item disconnection no longer affects database binding blocks https://github.com/lonelyor/SourceFlow/issues/12235
...
func (tx *Transaction) doInsert0(operation *Operation, tree *parse.Tree) (ret *TxErr) {
	...
	if "" != nextID {
		...
		if ast.NodeSuperBlock == node.Type {
			...
			if ast.NodeList == insertedNode.Type && nil != insertedNode.FirstChild && operation.ID == insertedNode.FirstChild.ID && operation.ID != insertedNode.ID {
				// 将一个列表项移动到另一个列表的第一项时 https://github.com/lonelyor/SourceFlow/issues/2341
				insertedNode = insertedNode.FirstChild
			}
	...
	if ast.NodeAttributeView == insertedNode.Type {
		...
		// 比如剪切操作时，会先进行 delete 数据库解绑块，这里需要重新绑定 https://github.com/lonelyor/SourceFlow/issues/13031
		...
		if nil == parseErr {
			...
			// 设置视图 https://github.com/lonelyor/SourceFlow/issues/15279
			...
			if nil != v {
				...
				if "" == av.GetNodeAttrViewFromAttrs(attrs) {
					av.SetNodeAttrView(attrs, v.ID)
...
func (tx *Transaction) processGlobalAssets(tree *parse.Tree) {
	...
	// 本地资源文件需要移动到用户手动建立的 assets 下 https://github.com/lonelyor/SourceFlow/issues/2410
...
func (tx *Transaction) doUpdate(operation *Operation) (ret *TxErr) {
		...
		if ast.NodeTextMark == n.Type {
			if n.IsTextMarkType("inline-math") {
				...
				if "d" == n.TextMarkBlockRefSubtype {
					// 偶发编辑文档标题后引用处的动态锚文本不更新 https://github.com/lonelyor/SourceFlow/issues/5891
	...
	if ast.NodeHTMLBlock == updatedNode.Type {
		...
		// 剔除连续的空行（包括空行内包含空格的情况） https://github.com/lonelyor/SourceFlow/issues/15377
		var newLines []string
	...
	if avNames := getAvNames(updatedNode.IALAttr(av.NodeAttrNameAvs)); "" != avNames {
		// updateBlock 会清空数据库角标 https://github.com/lonelyor/SourceFlow/issues/16549
		go func() {
...
func createdUpdated(node *ast.Node) {
	// 补全子节点的更新时间 Improve block update time filling https://github.com/lonelyor/SourceFlow/issues/12182
	ast.Walk(node, func(n *ast.Node, entering bool) ast.WalkStatus {
...
type Operation struct {
	...
	ID         string      `json:"id"`
	RootID     string      `json:"rootID"` // 源流内部暂时没有用到 https://github.com/lonelyor/SourceFlow/issues/17179#issuecomment-4051604916
...
func checkUpsertInUserGuide(tree *parse.Tree) {
	// In production mode, data reset warning pops up when editing data in the user guide https://github.com/lonelyor/SourceFlow/issues/9757
