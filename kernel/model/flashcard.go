...
func SetFlashcardsDueTime(cardDues []*SetFlashcardDueTime) (err error) {
	// Add internal kernel API `/api/riff/batchSetRiffCardsDueTime` https://github.com/lonelyor/SourceFlow/issues/10423
	EnsureFlashcardsLoaded()
...
func ResetFlashcards(typ, id, deckID string, blockIDs []string) {
	// Support resetting the learning progress of flashcards https://github.com/lonelyor/SourceFlow/issues/9564
	EnsureFlashcardsLoaded()
...
var (
	...
	deckLock           = sync.Mutex{}
	flashcardsLoadLock = sync.Mutex{}
	flashcardsLoaded   = atomic.Bool{}
)
...
func GetNotebookFlashcards(boxID string, page, pageSize int) (blocks []*Block, total, pageCount int) {
	...
	for _, entry := range entries {
		...
		rootIDs = append(rootIDs, strings.TrimSuffix(entry.Name(), ".sf"))
	}
...
func getCardsBlocks(cards []riff.Card, page, pageSize int) (blocks []*Block, total, pageCount int) {
	// sort by due date asc https://github.com/lonelyor/SourceFlow/pull/9673
	sort.Slice(cards, func(i, j int) bool {
		due1 := cards[i].(*riff.FSRSCard).C.Due
		due2 := cards[j].(*riff.FSRSCard).C.Due
		if due1.IsZero() || due2.IsZero() {
			// Improve flashcard management sorting https://github.com/lonelyor/SourceFlow/issues/14686
			cid1 := cards[i].ID()
			cid2 := cards[j].ID()
			return cid1 < cid2
		}
...
func GetTreeDueFlashcards(rootID string, reviewedCardIDs []string) (ret []*Flashcard, unreviewedCount, unreviewedNewCardCount, unreviewedOldCardCount int, err error) {
	...
	// 文档级新卡/复习卡上限控制 Document-level new card/review card limit control https://github.com/lonelyor/SourceFlow/issues/9365
...
func getTreeSubTreeChildBlocks(rootID string) (treeBlockIDsMap map[string]bool, treeBlockIDs []string) {
	...
	bts := treenode.GetBlockTreesByPathPrefix(strings.TrimSuffix(root.Path, ".sf"))
	for _, bt := range bts {
...
func getAllDueFlashcards(reviewedCardIDs []string) (ret []*Flashcard, unreviewedCount, unreviewedNewCardCount, unreviewedOldCardCount int) {
	...
	for _, deck := range Decks {
		if deck.ID != builtinDeckID {
			...
			// Alt+0 flashcard review entry no longer returns to card deck flashcards https://github.com/lonelyor/SourceFlow/issues/10635
			continue
		}
...
func (tx *Transaction) doRemoveFlashcards(operation *Operation) (ret *TxErr) {
	...
	if "" == deckID { // 支持在 All 卡包中移除闪卡 https://github.com/lonelyor/SourceFlow/issues/7425
		for _, deck := range Decks {
...
func (tx *Transaction) doAddFlashcards(operation *Operation) (ret *TxErr) {
	...
	for _, blockID := range blockIDs {
		...
		if 0 < len(cards) {
			// 一个块只能添加生成一张闪卡 https://github.com/lonelyor/SourceFlow/issues/7476
			continue
		}
...
func EnsureFlashcardsLoaded() {
	if flashcardsLoaded.Load() {
		return
	}

	flashcardsLoadLock.Lock()
	defer flashcardsLoadLock.Unlock()
	if flashcardsLoaded.Load() {
		return
	}

	loadFlashcards0()
	flashcardsLoaded.Store(true)
}
