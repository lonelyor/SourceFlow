...
import (
	...
	"github.com/lonelyor/sourceflow/third_party/go/epub"
...
func FullTextSearchAssetContent(query string, types map[string]bool, method, orderBy, page, pageSize int) (ret []*AssetContent, matchedAssetCount, pageCount int) {
	...
	cacheKey := buildAssetContentSearchCacheKey(query, types, method, orderBy, page, pageSize)
	if cachedRet, cachedMatchedAssetCount, cachedPageCount, ok := getCachedAssetContentSearch(cacheKey); ok {
		return cachedRet, cachedMatchedAssetCount, cachedPageCount
	}
	...
	setCachedAssetContentSearch(cacheKey, ret, matchedAssetCount, pageCount)
	return
}
...
func (parser *TxtAssetParser) Parse(absPath string) (ret *AssetParseResult) {
	...
	if !utf8.Valid(data) {
		// Non-UTF-8 encoded text files are not included in asset file content searching https://github.com/lonelyor/SourceFlow/issues/9052
...
func (parser *PdfAssetParser) Parse(absPath string) (ret *AssetParseResult) {
	...
	if PDFAssetContentMaxPage < pc.PageCount {
		// PDF files longer than 1024 pages are not included in asset file content searching https://github.com/lonelyor/SourceFlow/issues/9053
		logging.LogWarnf("ignore large PDF asset [%s] with [%d] pages", absPath, pc.PageCount)
		return
	}

	maxSizeVal := util.GetEnv(util.PDFAssetContentIndexMaxSizeEnv)
	if "" != maxSizeVal {
		if maxSize, parseErr := strconv.ParseUint(maxSizeVal, 10, 64); nil == parseErr {
			...
			logging.LogWarnf("invalid env [SOURCEFLOW_PDF_ASSET_CONTENT_INDEX_MAX_SIZE]: [%s], parsing failed: %s", maxSizeVal, parseErr)
		}
	}

	if PDFAssetContentMaxSize < uint64(len(pdfData)) {
		// PDF files larger than 128MB are not included in asset file content searching https://github.com/lonelyor/SourceFlow/issues/9500
