	...
	ast.Walk(heading, func(n *ast.Node, entering bool) ast.WalkStatus {
		if !entering {
			...
			case ast.NodeHeading:
				// Show heading block appearance style in the Outline Panel https://github.com/lonelyor/SourceFlow/issues/7872
		...
		if style := n.IALAttr("style"); "" != style {
			if strings.Contains(style, "font-size") { // 大纲字号不应该跟随字体设置 https://github.com/lonelyor/SourceFlow/issues/7202
		...
		case ast.NodeText, ast.NodeLinkText, ast.NodeCodeBlockCode, ast.NodeMathBlockContent:
			...
			tokens = bytes.ReplaceAll(tokens, []byte(" "), []byte("&nbsp;")) // 大纲面板条目中无法显示多个空格 https://github.com/lonelyor/SourceFlow/issues/4370
			buf.Write(tokens)
		...
		case ast.NodeImage:
			if title := n.ChildByType(ast.NodeLinkTitle); nil != title {
				// 标题后直接跟图片时图片的提示文本不再渲染到大纲中 https://github.com/lonelyor/SourceFlow/issues/6278
				title.Unlink()
			}
			dom := luteEngine.RenderNodeBlockDOM(n)
			buf.WriteString(dom)
...
func renderBlockText(node *ast.Node, excludeTypes []string, removeLineBreak bool) (ret string) {
	...
	ret = strings.TrimSpace(ret)
	if "" == ret {
		// 复制内容为空的块作为块引用时粘贴无效 https://github.com/lonelyor/SourceFlow/issues/4962
		buf := bytes.Buffer{}
			...
			for _, sqlBlock := range sqlBlocks {
				...
				if "d" == sqlBlock.Type {
					if 0 == blockEmbedMode {
						...
						// Improve export of heading levels in embedded blocks https://github.com/lonelyor/SourceFlow/issues/12233 https://github.com/lonelyor/SourceFlow/issues/12741
						embedTopLevel := 0
						...
						if "1" != h.IALAttr("fold") {
							...
							for _, c := range children {
								if "1" == c.IALAttr("heading-fold") {
									// 嵌入块包含折叠标题时不应该显示其下方块 https://github.com/lonelyor/SourceFlow/issues/4765
									continue
								}
