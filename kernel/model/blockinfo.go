...
func GetDocInfo(blockID string) (ret *BlockInfo) {
	...
	subFiles, err := os.ReadDir(filepath.Join(boxLocalPath, strings.TrimSuffix(tree.Path, ".sf")))
	if err == nil {
...
func GetBlockRefText(id string) string {
	...
	ast.Walk(node, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		if n.IsTextMarkType("inline-memo") {
			// Block ref anchor text no longer contains contents of inline-level memos https://github.com/lonelyor/SourceFlow/issues/9363
...
func getNodeAvBlockText(node *ast.Node, avID string) (icon, content string) {
	...
	if "" != avID {
		if staticText := av.GetNodeAttrViewStaticText(node, avID); "" != staticText {
			content = staticText
		}
	}
	if "" == content {
...
func buildBlockBreadcrumb(node *ast.Node, excludeTypes []string, isEmbedBlock bool, headingMode ...int) (ret []*BlockPath) {
	...
	for parent := node; nil != parent; parent = parent.Parent {
		...
		if ast.NodeList == parent.Type || ast.NodeSuperBlock == parent.Type || ast.NodeBlockquote == parent.Type || ast.NodeCallout == parent.Type {
			add = false
			if parent == node {
				// https://github.com/lonelyor/SourceFlow/issues/13141#issuecomment-2476789553
				add = true
			}
		}
		...
		if !isEmbedBlock {
			...
			if ast.NodeDocument != parent.Type {
				...
				if 2 == mode && ast.NodeHeading == parent.Type && parent == node {
					...
					// 在嵌入块中隐藏最后一个非文档路径的面包屑中的文本 Hide text in breadcrumb of last non-document path in embed block https://github.com/lonelyor/SourceFlow/issues/13866
					name = ""
				}
			}
		}
		...
		for prev := parent.Previous; nil != prev; prev = prev.Previous {
			...
			if ast.NodeSuperBlock == prev.Type {
				// 超级块中包含标题块时下方块面包屑计算不正确 https://github.com/lonelyor/SourceFlow/issues/6675
				b = treenode.SuperBlockLastHeading(prev)
				if nil == b {
					// 超级块下方块被作为嵌入块时设置显示面包屑后不渲染 https://github.com/lonelyor/SourceFlow/issues/6690
					b = prev
				}
			}
			...
			if ast.NodeHeading == b.Type && headingLevel > b.HeadingLevel {
				if b.ParentIs(ast.NodeListItem) {
					// 标题在列表下时不显示 https://github.com/lonelyor/SourceFlow/issues/13008
					continue
				}
