...
func BuildBookmark() (ret *Bookmarks) {
	...
	for _, block := range blocks {
		if "" != block.Name {
			// Blocks in the bookmark panel display their name instead of content https://github.com/lonelyor/SourceFlow/issues/8514
			block.Content = block.Name
		} else if "NodeAttributeView" == block.Type {
			// Display database title in bookmark panel https://github.com/lonelyor/SourceFlow/issues/11666
			avID := gulu.Str.SubStringBetween(block.Markdown, "av-id=\"", "\"")
			block.Content, _ = av.GetAttributeViewName(avID)
		} else {
			// Improve bookmark panel rendering https://github.com/lonelyor/SourceFlow/issues/9361
