...
func getRecentDocs(sortBy string) (ret []*RecentDoc, err error) {
	...
	for _, doc := range recentDocs {
		...
		// 文档块可能已经转换成标题块 https://github.com/lonelyor/SourceFlow/pull/16727#issuecomment-3810081850
		...
		if merged, ok := mergedDocs[bt.RootID]; !ok {
			doc.Title = path.Base(bt.HPath) // Recent docs not updated after renaming https://github.com/lonelyor/SourceFlow/issues/7827
...
func getLocalStorage() (ret map[string]interface{}) {
	// When local.json is corrupted, clear the file to avoid being unable to enter the main interface https://github.com/lonelyor/SourceFlow/issues/7911
