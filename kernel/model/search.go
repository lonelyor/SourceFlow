...
import (
	...
	"github.com/lonelyor/sourceflow/third_party/go/vitess-sqlparser/sqlparser"
					...
					if entering {
						...
						if ast.NodeTextMark == n.Type {
							if n.IsTextMarkType("a") {
								if util.IsBlockProtocolLink(n.TextMarkAHref) {
...
func buildEmbedBlock(embedBlockID string, excludeIDs []string, headingMode int, breadcrumb bool, sqlBlocks []*sql.Block) (ret []*EmbedBlock) {
	...
	for _, b := range sqlBlocks {
		if "query_embed" == b.Type { // 嵌入块不再嵌入
			// 嵌入块支持搜索 https://github.com/lonelyor/SourceFlow/issues/7112
...
func SearchRefBlock(id, rootID, keyword string, beforeLen int, isSquareBrackets, isDatabase bool) (ret []*Block, newDoc bool) {
	...
	if "" == keyword {
		// 查询为空时默认的块引排序规则按最近使用优先 https://github.com/lonelyor/SourceFlow/issues/3218
	...
	for _, b := range ret {
		...
		if b.IsContainerBlock() && "NodeDocument" != b.Type {
			// `((` 引用候选中排除当前块的父块 https://github.com/lonelyor/SourceFlow/issues/4538
			tree = cachedTrees[b.RootID]
			if nil == tree {
				tree, _ = loadTreeByBlockTree(bts[b.RootID])
				cachedTrees[b.RootID] = tree
	...
	} else { // 如果是数据库中搜索绑定块，则允许新建重名文档 https://github.com/lonelyor/SourceFlow/issues/11713
		newDoc = true
	}
...
func filterSelfHPath(blocks []*Block) {
	// 简化搜索结果列表中的文档块路径 Simplify document block paths in search results https://github.com/lonelyor/SourceFlow/issues/13364
	// 文档块不显示自己的路径（最后一层）
...
func prependNotebookNameInHPath(blocks []*Block) {
	// 在 hPath 中加入笔记本名 Show notebooks in hpath of block ref search list results https://github.com/lonelyor/SourceFlow/issues/9378
...
func FindReplace(keyword, replacement string, replaceTypes map[string]bool, ids []string, paths, boxes []string, types map[string]bool, method, orderBy, groupBy int) (err error) {
	...
	if 1 == method {
		...
		// Find and replace supports query syntax https://github.com/lonelyor/SourceFlow/issues/14937
		method = 0
	}

	if 0 != groupBy {
		// 按文档分组后不支持替换 Need to be reminded that replacement operations are not supported after grouping by doc https://github.com/lonelyor/SourceFlow/issues/10161
		// 因为分组条件传入以后搜索只能命中文档块，会导致 全部替换 失效
		err = errors.New(Conf.Language(221))
		return
	}

	// No longer trim spaces for the keyword and replacement https://github.com/lonelyor/SourceFlow/issues/9229
	...
	if 1 > len(ids) {
		// `Replace All` is no longer affected by pagination https://github.com/lonelyor/SourceFlow/issues/8265
			...
			ast.Walk(node, func(n *ast.Node, entering bool) ast.WalkStatus {
				...
				case ast.NodeTextMark:
					if n.IsTextMarkType("code") {
						...
						// Search and replace fails in some cases https://github.com/lonelyor/SourceFlow/issues/10016
						if !replaceTypes["text"] {
							return ast.WalkContinue
						}
...
// Supports replacing text elements with other elements https://github.com/lonelyor/SourceFlow/issues/11058
	...
	if 0 == method {
		...
		if Conf.Search.CaseSensitive {
			...
			if "" != strings.TrimSpace(keyword) {
				...
				// Replace fails when search results contain mixed case in text elements https://github.com/lonelyor/SourceFlow/issues/9171
...
func FullTextSearchBlock(query string, boxes, paths []string, types map[string]bool, method, orderBy, groupBy, page, pageSize int) (ret []*Block, matchedBlockCount, matchedRootCount, pageCount int, docMode bool) {
	...
	if ignoreLines := getSearchIgnoreLines(); 0 < len(ignoreLines) {
		// Support ignore search results https://github.com/lonelyor/SourceFlow/issues/10089
		buf := bytes.Buffer{}
	...
	default: // 关键字
		...
		if ast.IsNodeIDPattern(query) {
			...
			if 2 > len(strings.Split(strings.TrimSpace(query), " ")) {
				...
				docMode = true // 文档全文搜索模式 https://github.com/lonelyor/SourceFlow/issues/10584
...
func buildOrderBy(query string, method, orderBy int) string {
	...
	case 6:
		if 0 != method && 1 != method {
			// 只有关键字搜索和查询语法搜索才支持按相关度升序 https://github.com/lonelyor/SourceFlow/issues/7861
...
func searchBySQL(stmt string, beforeLen, page, pageSize int) (ret []*Block, matchedBlockCount, matchedRootCount int) {
	...
	if stdQuery {
		if strings.HasPrefix(stmt, "select a.* ") { // 多个搜索关键字匹配文档 https://github.com/lonelyor/SourceFlow/issues/7350
...
func extractID(content string) (ret string) {
	// Improve block ref search ID extraction https://github.com/lonelyor/SourceFlow/issues/10848
...
func fullTextSearchByFTS(query, boxFilter, pathFilter, typeFilter, ignoreFilter, orderBy string, beforeLen, page, pageSize int) (ret []*Block, matchedBlockCount, matchedRootCount int) {
	...
	projections := "id, parent_id, root_id, hash, box, path, " +
		// Search result content snippet returns more text https://github.com/lonelyor/SourceFlow/issues/10707
...
func fromSQLBlock(sqlBlock *sql.Block, terms string, beforeLen int) (block *Block) {
	...
	fContent := util.EscapeHTML(sqlBlock.FContent) // fContent 会用于和 content 对比，在反链计算时用于判断是否是列表项下第一个子块，所以也需要转义 https://github.com/lonelyor/SourceFlow/issues/11001
	block = &Block{
		Box:      sqlBlock.Box,
		Path:     sqlBlock.Path,
		ID:       id,
...
func markReplaceSpan(n *ast.Node, unlinks *[]*ast.Node, keywords []string, markSpanDataType string, luteEngine *lute.Lute) bool {
	if ast.NodeText == n.Type {
		...
		// 搜索结果高亮支持大部分行级元素 https://github.com/lonelyor/SourceFlow/issues/6745
...
func markReplaceSpanWithSplit(text string, keywords []string, replacementStart, replacementEnd string) (ret string) {
	// 虚拟引用和反链提及关键字按最长匹配优先 https://github.com/lonelyor/SourceFlow/issues/7465
...
func getSearchIgnoreLines() (ret []string) {
	...
	searchIgnorePath := util.HiddenDataPath(util.DataDir, "searchignore")
...
func getRefSearchIgnoreLines() (ret []string) {
	...
	searchIgnorePath := util.HiddenDataPath(util.DataDir, "refsearchignore")
	err := os.MkdirAll(filepath.Dir(searchIgnorePath), 0755)
	if err != nil {
		return
	}
