...
import (
	...
	"github.com/lonelyor/sourceflow/third_party/go/lute/editor"
	"github.com/lonelyor/sourceflow/third_party/go/lute/html"
	"github.com/lonelyor/sourceflow/third_party/go/lute/lex"
	"github.com/lonelyor/sourceflow/third_party/go/lute/parse"
	"github.com/lonelyor/sourceflow/third_party/go/lute/render"
	"github.com/lonelyor/sourceflow/third_party/go/pdfcpu/pkg/api"
	"github.com/lonelyor/sourceflow/third_party/go/pdfcpu/pkg/font"
	"github.com/lonelyor/sourceflow/third_party/go/pdfcpu/pkg/pdfcpu"
	"github.com/lonelyor/sourceflow/third_party/go/pdfcpu/pkg/pdfcpu/model"
	"github.com/lonelyor/sourceflow/third_party/go/pdfcpu/pkg/pdfcpu/types"
	"github.com/lonelyor/sourceflow/third_party/go/riff"
)

// Keep export-only download names space-free without changing global filename rules.
func filterExportFileName(name string) string {
	return stripExportWhitespace(util.FilterFileName(name))
}

func filterExportFilePath(p string) string {
	return stripExportWhitespace(util.FilterFilePath(p))
}

func stripExportWhitespace(s string) string {
...
func ExportCodeBlock(blockID string) (filePath string, err error) {
	// Supports exporting a code block as a file https://github.com/lonelyor/SourceFlow/pull/16774
	...
	name = filterExportFileName(name)
...
func ExportAv2CSV(avID, blockID string) (zipPath string, err error) {
	// Database block supports export as CSV https://github.com/lonelyor/SourceFlow/issues/10072
	...
	name := filterExportFileName(getAttrViewName(attrView))
	table := getAttrViewTable(attrView, view, "")

	// 遵循视图过滤和排序规则 Use filtering and sorting of current view settings when exporting database blocks https://github.com/lonelyor/SourceFlow/issues/10474
...
func ExportSystemLog() (zipPath string) {
	...
	appLog := filepath.Join(util.GetUserConfDir(), "app.log")
	...
	kernelLog := filepath.Join(util.GetUserConfDir(), "kernel.log")
	...
	sourceflowLog := filepath.Join(util.TempDir, "sourceflow.log")
	if gulu.File.IsExist(sourceflowLog) {
		to := filepath.Join(exportFolder, "sourceflow.log")
		if err := filelock.Copy(sourceflowLog, to); err != nil {
			logging.LogErrorf("copy kernel log from [%s] to [%s] failed: %s", err, sourceflowLog, to)
		}
	}
...
func ExportSYs(ids []string) (zipPath string) {
	...
	for _, bt := range bts {
		...
		if Conf.Export.IncludeSubDocs {
			docFiles := box.ListFiles(strings.TrimSuffix(bt.Path, ".sf"))
...
func ExportData() (zipPath string, err error) {
	...
	name := filterExportFileName(util.WorkspaceName) + "-" + util.CurrentTimeSecondsStr()
...
func ExportPreview(id string, fillCSSVar bool) (retStdHTML string) {
	...
	tree = exportTree(tree, false, false, true,
		...
		"#", "#", // 这里固定使用 # 包裹标签，否则无法正确解析标签 https://github.com/lonelyor/SourceFlow/issues/13857
	...
	// 移除超级块的属性列表 https://github.com/lonelyor/SourceFlow/issues/13451
	...
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		if ast.NodeFootnotesRef == n.Type && nil != n.Next {
			// https://github.com/lonelyor/SourceFlow/issues/15654
	...
	// 使用实际主题样式值替换样式变量 Use real theme style value replace var in preview mode https://github.com/lonelyor/SourceFlow/issues/11458
	if fillCSSVar {
...
func ExportMarkdownHTML(id, savePath string, docx, merge bool) (name, dom string) {
	...
	name = filterExportFileName(name) // 导出 PDF、HTML 和 Word 时未移除不支持的文件名符号 https://github.com/lonelyor/SourceFlow/issues/5614
	...
	if util.IsSymlinkPath(util.AppearancePath) {
		// Support for symlinked theme folder when exporting HTML https://github.com/lonelyor/SourceFlow/issues/9173
		var readErr error
	...
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		if ast.NodeEmojiImg == n.Type {
			...
			if nil != n.Next && ast.NodeText == n.Next.Type {
				// 行级代码导出 word 之后会有多余的零宽空格 https://github.com/lonelyor/SourceFlow/issues/14825
...
func ExportHTML(id, savePath string, pdf, keepFold, merge bool) (name, dom string, node *ast.Node) {
	...
	// 使用属性 `data-export-html` 导出时 `<style></style>` 标签丢失 https://github.com/lonelyor/SourceFlow/issues/6228
...
func processIFrame(tree *parse.Tree) {
	// 导出 PDF/Word 时 IFrame 块使用超链接 https://github.com/lonelyor/SourceFlow/issues/4035
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
...
func processPDFWatermark(pdfCtx *model.Context, watermark bool) {
	// Support adding the watermark on export PDF https://github.com/lonelyor/SourceFlow/issues/9961
	...
	wm.OnTop = true // Export PDF and add watermarks no longer covered by images https://github.com/lonelyor/SourceFlow/issues/10818
...
// 导出 PDF 时支持将资源文件作为附件嵌入 https://github.com/lonelyor/SourceFlow/issues/7414
func processPDFLinkEmbedAssets(pdfCtx *model.Context, assetDests []string, removeAssets bool) {
	...
	for _, link := range assetLinks {
		...
		link.URI = strings.ReplaceAll(link.URI, "http://"+util.LocalHost+":"+util.ServerPort+"/", "") // Exporting PDF embedded asset files as attachments fails https://github.com/lonelyor/SourceFlow/issues/7414#issuecomment-1704573557
		...
		fileSpecDict, newErr := pdfCtx.XRefTable.NewFileSpecDict(fn, fn, "attached by SourceFlow", *ir)
		if nil != newErr {
	...
	if 4 == Conf.Export.BlockRefMode { // 脚注+锚点哈希
		...
		ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
			...
			if treenode.IsBlockLink(n) {
				defID = util.TrimBlockProtocol(n.TextMarkAHref)
...
func yfm(docIAL map[string]string) string {
	// 导出 Markdown 文件时开头附上一些元数据 https://github.com/lonelyor/SourceFlow/issues/6880
...
func exportSYZip(boxID, rootDirPath, baseFolderName string, docPaths []string) (zipPath string) {
	...
	if strings.HasSuffix(name, "..") {
		// 文档标题以 `..` 结尾时无法导出 Markdown https://github.com/lonelyor/SourceFlow/issues/4698
	...
	for treeID, tree := range refTrees {
		...
		writePath := strings.TrimPrefix(tree.Path, rootDirPath)
		writePath = filepath.Join(exportDir, treeID+".sf")
		if writeErr := os.WriteFile(writePath, data, 0644); nil != writeErr {
			logging.LogErrorf("write export file [%s] failed: %s", writePath, writeErr)
			continue
		}
	...
	for _, tree := range trees {
		...
		titleImgPath := treenode.GetDocTitleImgPath(tree.Root) // Export .sf.zip doc title image is not exported https://github.com/lonelyor/SourceFlow/issues/8748
		if "" != titleImgPath {
		...
		for _, asset := range assets {
			...
			if !gulu.File.IsDir(srcPath) && strings.HasSuffix(strings.ToLower(srcPath), ".pdf") {
				...
				if filelock.IsExist(sya) {
					// Related PDF annotation information is not exported when exporting .sf.zip https://github.com/lonelyor/SourceFlow/issues/7836
	...
	// 导出数据库 Attribute View export https://github.com/lonelyor/SourceFlow/issues/8710
	...
	// 导出闪卡 Export related flashcard data when exporting .sf.zip https://github.com/lonelyor/SourceFlow/issues/9372
	...
	// 导出自定义排序
	sortPath := util.HiddenDataPath(filepath.Join(util.DataDir, box.ID), "sort.json")
	...
	if filelock.IsExist(sortPath) {
		...
		if 0 < len(sortIDs) {
			...
			if 0 < len(sortData) {
				confDir := util.HiddenDataDir(exportDir)
	...
	zipPath = exportDir + ".sf.zip"
	zip, err := gulu.Zip.Create(zipPath)
	if err != nil {
		logging.LogErrorf("create export .sf.zip [%s] failed: %s", exportDir, err)
		return ""
	}
	...
	if err = zip.Close(); err != nil {
		logging.LogErrorf("close export .sf.zip failed: %s", err)
	}
...
func exportAv(avID, exportStorageAvDir, exportFolder string, assetPathMap map[string]string) {
	...
	for _, keyValues := range attrView.KeyValues {
		...
		case av.KeyTypeMAsset: // 导出资源文件列 https://github.com/lonelyor/SourceFlow/issues/9919
...
func exportMarkdownContent(rootID, ext string, exportRefMode int, defBlockIDs []string, singleFile bool) (tree *parse.Tree, exportedMd string, isEmpty bool) {
	...
	if Conf.Export.MarkdownYFM {
		// 导出 Markdown 时在文档头添加 YFM 开关 https://github.com/lonelyor/SourceFlow/issues/7727
		exportedMd = yfm(docIAL) + exportedMd
	}
	return
}
...
func exportTree(tree *parse.Tree, wysiwyg, keepFold, avHiddenCol bool,
	...
	ast.Walk(ret.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		case ast.NodeMathBlockContent:
			n.Tokens = bytes.TrimSpace(n.Tokens) // 导出 Markdown 时去除公式内容中的首尾空格 https://github.com/lonelyor/SourceFlow/issues/4666
			return ast.WalkContinue
		...
		switch blockRefMode {
		case 2: // 锚文本块链
			blockRefLink := &ast.Node{Type: ast.NodeTextMark, TextMarkTextContent: linkText, TextMarkAHref: util.BuildBlockProtocol(defID)}
			blockRefLink.KramdownIAL = n.KramdownIAL
		...
		case 4: // 脚注+锚点哈希
			if currentTreeNodeIDs[defID] {
				// 当前文档内不转换脚注，直接使用锚点哈希 https://github.com/lonelyor/SourceFlow/issues/13283
	...
	if 4 == blockRefMode { // 脚注+锚点哈希
		...
		if nil != footnotesDefBlock {
			...
			// Improve focus export conversion of block refs to footnotes https://github.com/lonelyor/SourceFlow/issues/10647
	...
	// 导出时支持导出题头图 https://github.com/lonelyor/SourceFlow/issues/4372
	...
	ast.Walk(ret.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		// 支持按照现有折叠状态导出 PDF https://github.com/lonelyor/SourceFlow/issues/5941
		if !keepFold {
			// 块折叠以后导出 HTML/PDF 固定展开 https://github.com/lonelyor/SourceFlow/issues/4064
		...
		// 导出时去掉内容块闪卡样式 https://github.com/lonelyor/SourceFlow/issues/7374
		if n.IsBlock() {
		...
		case ast.NodeParagraph:
			if nil == n.FirstChild {
				// 空的段落块需要补全文本展位，否则后续格式化后再解析树会语义不一致 https://github.com/lonelyor/SourceFlow/issues/5806
				emptyParagraphs = append(emptyParagraphs, n)
			}
		case ast.NodeWidget:
			// 挂件块导出 https://github.com/lonelyor/SourceFlow/issues/3834 https://github.com/lonelyor/SourceFlow/issues/6188
			...
			exportMdVal = html.UnescapeString(exportMdVal) // 导出 `data-export-md` 时未解析代码块与行内代码内的转义字符 https://github.com/lonelyor/SourceFlow/issues/4180
			if "" != exportMdVal {
				luteEngine0 := util.NewLute()
				luteEngine0.SetYamlFrontMatter(true) // 挂件导出属性 `data-export-md` 支持 YFM https://github.com/lonelyor/SourceFlow/issues/7752
		...
		// Shift+Enter 换行在导出为 Markdown 时使用硬换行 https://github.com/lonelyor/SourceFlow/issues/3458
	...
	unlinks = nil
	// Attribute View export https://github.com/lonelyor/SourceFlow/issues/8710
	ast.Walk(ret.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		if !entering {
			return ast.WalkContinue
		}
		...
		for _, col := range table.Columns {
			if avHiddenCol && col.Hidden {
				// 按需跳过隐藏列 Improve database table view exporting https://github.com/lonelyor/SourceFlow/issues/12232
				continue
			}
	...
	for _, defID := range *refFootnoteOrder {
		...
		if ast.NodeHeading == defNode.Type {
			...
			if currentTree.ID != docID {
				// 同文档块引转脚注缩略定义考虑容器块和标题块 https://github.com/lonelyor/SourceFlow/issues/5917
		...
		for _, node := range newNodes {
			ast.Walk(node, func(n *ast.Node, entering bool) ast.WalkStatus {
				...
				if currentTree.ID == docID {
					// 同文档块引转脚注缩略定义 https://github.com/lonelyor/SourceFlow/issues/3299
...
func blockLink2Ref(currentTree *parse.Tree) {
	ast.Walk(currentTree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		if treenode.IsBlockLink(n) {
			...
			n.TextMarkBlockRefID = util.TrimBlockProtocol(n.TextMarkAHref)
...
func processFileAnnotationRef(refID string, n *ast.Node, fileAnnotationRefMode int) ast.WalkStatus {
	...
	dest := p + "#page=" + pageStr // https://github.com/lonelyor/SourceFlow/issues/11780
...
func exportPandocConvertZip(baseFolderName string, docPaths, defBlockIDs []string, pandocFrom, pandocTo, ext string) (zipPath string) {
	...
	for i, p := range docPaths {
		...
		dir = filterExportFilePath(dir) // 导出文档时未移除不支持的文件名符号 https://github.com/lonelyor/SourceFlow/issues/4590
		name = filterExportFileName(name)
		hPath = path.Join(dir, name)
		p = hPath + ext
		if 1 == len(docPaths) {
			// 如果仅导出单个文档则使用文档标题作为文件名，不使用父路径 https://github.com/lonelyor/SourceFlow/issues/13635#issuecomment-3794560233
			p = name + ext
		}
		...
		if isEmpty {
			entries, readErr := os.ReadDir(filepath.Join(util.DataDir, tree.Box, strings.TrimSuffix(tree.Path, ".sf")))
			if nil == readErr && 0 < len(entries) {
				// 如果文档内容为空并且存在子文档则仅导出文件夹
				// Improve export of empty documents with subdocuments https://github.com/lonelyor/SourceFlow/issues/15009
				continue
			}
		}
		...
		for _, newAsset := range newAssets {
			...
			// Improve export of Markdown hyperlink spaces https://github.com/lonelyor/SourceFlow/issues/9792
			// No assets were exported when exporting Markdown https://github.com/lonelyor/SourceFlow/issues/17046
	...
	// 导出 Markdown zip 包内不带文件夹 https://github.com/lonelyor/SourceFlow/issues/6869
...
func getExportBlockRefLinkText(blockRef *ast.Node, blockRefTextLeft, blockRefTextRight string) (defID, linkText string) {
	...
	linkText = util.UnescapeHTML(linkText) // 块引锚文本导出时 `&` 变为实体 `&amp;` https://github.com/lonelyor/SourceFlow/issues/7659
...
func prepareExportTrees(docPaths []string) (defBlockIDs []string, relatedDocPaths []string) {
	...
	defBlockIDs = []string{}
	for i, p := range docPaths {
		rootID := strings.TrimSuffix(path.Base(p), ".sf")
	...
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		if treenode.IsBlockRef(n) {
			...
			*defBlockIDs = append(*defBlockIDs, defID)
			...
			// Export the binding block docs when exporting the doc where the database is located https://github.com/lonelyor/SourceFlow/issues/11486
...
// Export preview mode supports focus use https://github.com/lonelyor/SourceFlow/issues/15340
