...
func ImportSY(zipPath, boxID, toPath string) (err error) {
	...
	filelock.Walk(unzipPath, func(path string, d fs.DirEntry, err error) error {
		...
		if !d.IsDir() && strings.HasSuffix(d.Name(), ".sf") {
			sfPaths = append(sfPaths, path)
		}
		return nil
	})
	...
	if 1 != len(entries) {
		logging.LogErrorf("invalid .sf.zip [%v]", entries)
	...
	if strings.HasPrefix(name, "data-20") && len("data-20230321175442") == len(name) {
		logging.LogErrorf("invalid .sf.zip [unzipRootPath=%s, baseName=%s]", unzipRootPath, name)
		return errors.New(Conf.Language(199))
	}
	...
	// 重新生成块 ID
	for i, sfPath := range sfPaths {
		data, readErr := os.ReadFile(sfPath)
		if nil != readErr {
			logging.LogErrorf("read .sf [%s] failed: %s", sfPath, readErr)
			err = readErr
			return
		}
		tree, _, parseErr := dataparser.ParseJSON(data, luteEngine.ParseOptions)
		if nil != parseErr {
			logging.LogErrorf("parse .sf [%s] failed: %s", sfPath, parseErr)
			err = parseErr
			return
		}
		ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
			...
			// Keep original creation time when importing .sf.zip https://github.com/lonelyor/SourceFlow/issues/9923
		...
		tree.Path = filepath.ToSlash(strings.TrimPrefix(sfPath, unzipRootPath))
		trees[tree.ID] = tree
		util.PushEndlessProgress(Conf.language(73) + " " + fmt.Sprintf(Conf.language(70), fmt.Sprintf("%d/%d", i+1, len(sfPaths))))
	}
	...
	for _, tree := range trees {
		...
		ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
			...
			} else if ast.NodeTextMark == n.Type && n.IsTextMarkType("a") && util.IsBlockProtocolLink(n.TextMarkAHref) {
				// Block hyperlinks do not point to regenerated block IDs when importing .sf.zip https://github.com/lonelyor/SourceFlow/issues/9083
				defID := util.TrimBlockProtocol(n.TextMarkAHref)
				newDefID := blockIDs[defID]
				if "" != newDefID {
					n.TextMarkAHref = util.BuildBlockProtocol(newDefID)
				}
			} else if ast.NodeBlockQueryEmbedScript == n.Type {
				for oldID, newID := range blockIDs {
					// 导入 `.sf.zip` 后查询嵌入块失效 https://github.com/lonelyor/SourceFlow/issues/5316
	...
	if gulu.File.IsExist(storageAvDir) {
		...
		filelock.Walk(storageAvDir, func(path string, d fs.DirEntry, err error) error {
			...
			if ".json" == d.Name() { // https://github.com/lonelyor/SourceFlow/issues/16637
		...
		// 插入关联关系 https://github.com/lonelyor/SourceFlow/issues/11628
	...
	if 1 > len(avIDs) { // 如果本次没有导入数据库，则清理掉文档中的数据库属性 https://github.com/lonelyor/SourceFlow/issues/13011
		for _, tree := range trees {
			ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
	...
	for _, tree := range trees {
		util.PushEndlessProgress(Conf.language(73) + " " + fmt.Sprintf(Conf.language(70), tree.Root.IALAttr("title")))
		sfPath := filepath.Join(unzipRootPath, tree.Path)
		...
		if err = os.WriteFile(sfPath, data, 0644); err != nil {
			logging.LogErrorf("write .sf [%s] failed: %s", sfPath, err)
			return
		}
		newSfPath := filepath.Join(filepath.Dir(sfPath), tree.ID+".sf")
		if err = filelock.Rename(sfPath, newSfPath); err != nil {
			logging.LogErrorf("rename .sf from [%s] to [%s] failed: %s", sfPath, newSfPath, err)
			return
		}
	}
	...
	var sortData []byte
	var sortErr error
	sortPath := util.HiddenDataPath(unzipRootPath, "sort.json")
	if filelock.IsExist(sortPath) {
		...
		boxSortPath := util.HiddenDataPath(filepath.Join(util.DataDir, boxID), "sort.json")
	...
	filelock.Walk(unzipRootPath, func(path string, d fs.DirEntry, err error) error {
		...
		if !strings.HasSuffix(d.Name(), ".sf") {
			return nil
		}
...
func ImportData(zipPath string) (err error) {
	...
	files, err := filepath.Glob(filepath.Join(unzipPath, "*/*.sf"))
	if err != nil {
...
func ImportFromLocalPath(boxID, localPath string, toPath string) (err error) {
	...
	if gulu.File.IsDir(localPath) { // 导入文件夹
		...
		filelock.Walk(localPath, func(currentPath string, d fs.DirEntry, err error) error {
			...
			if !d.IsDir() && (!strings.HasSuffix(currentPath, ".md") && !strings.HasSuffix(currentPath, ".markdown") ||
				...
				// 非 Markdown 文件作为资源文件处理 https://github.com/lonelyor/SourceFlow/issues/13817
			...
			targetPath = strings.ReplaceAll(targetPath, ".sf/", "/")
			targetPath += ".sf"
			...
			if d.IsDir() {
				if "assets" == d.Name() {
					// 如果是 assets 文件夹则跳过，里面的 Markdown 文件算作资源文件 https://github.com/lonelyor/SourceFlow/issues/13817
					return nil
				}

				if subMdFiles := util.GetFilePathsByExts(currentPath, []string{".md", ".markdown"}); 1 > len(subMdFiles) {
					// 如果该文件夹中不包含 Markdown 文件则不处理 https://github.com/lonelyor/SourceFlow/issues/11567
					return nil
				}

				// 如果当前文件夹路径下包含同名的 Markdown 文件，则不创建空文档 https://github.com/lonelyor/SourceFlow/issues/13149
			...
			targetPath = strings.ReplaceAll(targetPath, fname, id+".sf")
			targetPaths[curRelPath] = targetPath
			...
			tree.Box = boxID
			targetPath = path.Join(path.Dir(targetPath), tree.Root.ID+".sf")
			...
			ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
				...
				if strings.HasSuffix(absolutePath, ".md") || strings.HasSuffix(absolutePath, ".markdown") {
					if !strings.Contains(filepath.ToSlash(absolutePath), "/assets/") {
						...
						// Supports converting relative path hyperlinks into document block references after importing Markdown https://github.com/lonelyor/SourceFlow/issues/13817
						return ast.WalkContinue
					}
				}
		...
		targetPath := strings.TrimSuffix(toPath, ".sf")
		id := ast.NewNodeID()
		targetPath = path.Join(targetPath, id+".sf")
		var data []byte
	...
	if 0 < len(importTrees) {
		...
		// 按照路径排序 Improve sort when importing markdown files https://github.com/lonelyor/SourceFlow/issues/11390
		var hPaths []string
...
func parseStdMd(markdown []byte) (ret *parse.Tree, yfmRootID, yfmTitle, yfmUpdated string) {
	...
	luteEngine.SetYamlFrontMatter(true) // 解析 YAML Front Matter https://github.com/lonelyor/SourceFlow/issues/10878
...
func reassignIDUpdated(tree *parse.Tree, rootID, updated string) {
	...
	tree.ID = tree.Root.ID
	tree.Path = path.Join(path.Dir(tree.Path), tree.ID+".sf")
	tree.Root.SetIALAttr("id", tree.Root.ID)
}
