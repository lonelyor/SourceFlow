...
func (tx *Transaction) doFoldHeading(operation *Operation) (ret *TxErr) {
	...
	childrenIDs := []string{} // 这里不能用 nil，否则折叠下方没内容的标题时会内核中断 https://github.com/lonelyor/SourceFlow/issues/3643
...
func (tx *Transaction) doUnfoldHeading(operation *Operation) (ret *TxErr) {
	...
	// 展开折叠的标题后显示块引用计数 Display reference counts after unfolding headings https://github.com/lonelyor/SourceFlow/issues/13618
...
func Doc2Heading(srcID, targetID string, after bool) (srcTreeBox, srcTreePath string, err error) {
	...
	subDir := filepath.Join(util.DataDir, srcTree.Box, strings.TrimSuffix(srcTree.Path, ".sf"))
	...
	// 移动前先删除引用 https://github.com/lonelyor/SourceFlow/issues/7819
	...
	srcTree.Root.RemoveIALAttr("scroll") // Remove `scroll` attribute when converting the document to a heading https://github.com/lonelyor/SourceFlow/issues/9297
	...
	if "" != tagIAL && 0 < len(tags) {
		// 带标签的文档块转换为标题块时将标签移动到标题块下方 https://github.com/lonelyor/SourceFlow/issues/6550
...
func Heading2Doc(srcHeadingID, targetBoxID, targetPath, previousPath string) (srcRootBlockID, newTargetPath string, err error) {
	...
	newTargetPath = path.Join(toFolder, srcHeadingID+".sf")
	...
	// 折叠标题转换为文档时需要自动展开下方块 https://github.com/lonelyor/SourceFlow/issues/2947
