...
var (
	...
	BasicAuthHeaderValue = "Basic realm=\"SourceFlow Authorization Required\", charset=\"UTF-8\""
)
...
func LoginAuth(c *gin.Context) {
	...
	if rememberMe, ok := arg["rememberMe"].(bool); ok && rememberMe {
		// Add a 'Remember me' checkbox when logging in to save a session https://github.com/lonelyor/SourceFlow/pull/14964
...
func CheckReadonly(c *gin.Context) {
	...
	if IsBootSyncReadonlyGuardActive() && !IsBootSyncReadonlyGuardAllowed(path) {
		result := util.NewResult()
		result.Code = -1
		result.Msg = Conf.Language(17)
...
func CheckAuth(c *gin.Context) {
	...
	// 未设置访问授权码
	if "" == Conf.AccessAuthCode {
		// Skip the empty access authorization code check https://github.com/lonelyor/SourceFlow/issues/9709
		if util.SourceFlowAccessAuthCodeBypass {
			c.Set(RoleContextKey, RoleAdministrator)
			c.Next()
			return
		}

		// Authenticate requests with the Origin header other than 127.0.0.1 https://github.com/lonelyor/SourceFlow/issues/9180
		clientIP := c.ClientIP()
	...
	if workspaceSession.AccessAuthCode != Conf.AccessAuthCode {
		...
		if util.HasDesktopUserAgentPrefix(userAgentHeader) || strings.HasPrefix(userAgentHeader, "Mozilla/") {
...
var timingAPIs = map[string]int{
	"/api/search/fullTextSearchBlock": 200, // Monitor the search performance and suggest solutions https://github.com/lonelyor/SourceFlow/issues/7873
}
...
func Timing(c *gin.Context) {
	...
	timing := 15 * 1000
	if timingEnv := util.GetEnv(util.PerformanceTimingEnv); "" != timingEnv {
...
func ControlConcurrency(c *gin.Context) {
	...
	// Improve the concurrency of the kernel data reading interfaces https://github.com/lonelyor/SourceFlow/issues/10149
