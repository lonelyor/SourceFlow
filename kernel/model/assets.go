...
func DocImageAssets(rootID string) (ret []string, err error) {
	...
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		if ast.NodeImage == n.Type {
			...
			if 1 > len(dest) { // 双击打开图片不对 https://github.com/lonelyor/SourceFlow/issues/5876
				return ast.WalkContinue
			}
		...
		for _, dest := range dests {
			...
			if strings.HasPrefix(strings.ToLower(dest), "https://") || strings.HasPrefix(strings.ToLower(dest), "http://") || strings.HasPrefix(dest, "//") {
				if strings.HasPrefix(dest, "//") {
					// `Convert network images to local` supports `//` https://github.com/lonelyor/SourceFlow/issues/10598
					dest = "https:" + dest
				}

				u := dest
				if strings.Contains(u, "qpic.cn") {
					// 改进 `网络图片转换为本地图片` 微信图片拉取 https://github.com/lonelyor/SourceFlow/issues/5052
					if strings.Contains(u, "http://") {
						u = strings.Replace(u, "http://", "https://", 1)
					}

					// 改进 `网络图片转换为本地图片` 微信图片拉取 https://github.com/lonelyor/SourceFlow/issues/6431
				...
				if "" != originalURL {
					request.SetHeader("Referer", originalURL) // 改进浏览器剪藏扩展转换本地图片成功率 https://github.com/lonelyor/SourceFlow/issues/7464
				}
				...
				if strings.Contains(strings.ToLower(resp.GetContentType()), "text/html") {
					// 忽略超链接网页 `Convert network assets to local` no longer process webpage https://github.com/lonelyor/SourceFlow/issues/9965
					continue
				}
...
func RenameAsset(oldPath, newName string) (newPath string, err error) {
	...
	if filelock.IsExist(filepath.Join(util.DataDir, oldPath+".sya")) {
		// Rename the .sya annotation file when renaming a PDF asset https://github.com/lonelyor/SourceFlow/issues/9390
	...
	if ocrText := util.GetAssetText(oldPath); "" != ocrText {
		// 图片重命名后 ocr-texts.json 需要更新 https://github.com/lonelyor/SourceFlow/issues/12974
...
func UnusedAssets(sorted bool) (ret []*UnusedItem) {
	...
	for _, notebook := range notebooks {
		...
		// 分页加载，优化清理未引用资源内存占用 https://github.com/lonelyor/SourceFlow/issues/5200
		pages := pagedPaths(filepath.Join(util.DataDir, notebook.ID), 32)
		for _, paths := range pages {
		...
		for dest := range dests {
			...
			if idx := strings.Index(dest, "?"); 0 < idx {
				// `pdf?page` 资源文件链接会被判定为未引用资源 https://github.com/lonelyor/SourceFlow/issues/5649
				dest = dest[:idx]
			}
...
func MissingAssets() (ret []*UnusedItem) {
	...
	for _, notebook := range notebooks {
		...
		for dest := range dests {
			...
			if strings.Contains(strings.ToLower(dest), ".pdf/") {
				if idx := strings.LastIndex(dest, "/"); -1 < idx {
					if ast.IsNodeIDPattern(dest[idx+1:]) {
						// PDF 标注不计入 https://github.com/lonelyor/SourceFlow/issues/13891
						continue
					}
				}
			}
			...
			if "" == assetsPathMap[dest] {
				if strings.HasPrefix(dest, "assets/.") {
					// Assets starting with `.` should not be considered missing assets https://github.com/lonelyor/SourceFlow/issues/8821
...
func getQueryEmbedNodesAssetsLinkDests(node *ast.Node) (ret []string) {
	// The images in the embed blocks are not uploaded to the community hosting https://github.com/lonelyor/SourceFlow/issues/10042
...
func getAssetsLinkDests(node *ast.Node, includeServePath bool) (ret []string) {
	...
	ast.Walk(node, func(n *ast.Node, entering bool) ast.WalkStatus {
		if n.IsBlock() {
			...
			// Ignore assets associated with the `custom-data-assets` block attribute when cleaning unreferenced assets https://github.com/lonelyor/SourceFlow/issues/12574
		...
		if ast.NodeLinkDest == n.Type {
			...
			if ast.NodeWidget == n.Type {
				...
				if "" == dataAssets {
					// 兼容两种属性名 custom-data-assets 和 data-assets https://github.com/lonelyor/SourceFlow/issues/4122#issuecomment-1154796568
...
// allAssetAbsPaths 返回 asset 相对路径（assets/xxx）到绝对路径（F:\SourceFlow\data\assets\xxx）的映射。
	...
	for _, notebook := range notebooks {
		...
		filelock.Walk(notebookAbsPath, func(path string, d fs.DirEntry, err error) error {
			...
			if filelock.IsHidden(path) {
				// 清理资源文件时忽略隐藏文件 Ignore hidden files when cleaning unused assets https://github.com/lonelyor/SourceFlow/issues/12172
				return nil
			}
