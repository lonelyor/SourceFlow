...
func BuildTreeGraph(id, query string) (boxID string, nodes []*GraphNode, links []*GraphLink) {
	...
	if "" != rootID {
		// 局部关系图中添加文档链接关系 https://github.com/lonelyor/SourceFlow/issues/4996
		...
		if nil != rootBlock {
			...
			// 关系图日记过滤失效 https://github.com/lonelyor/SourceFlow/issues/7547
...
func filterDailyNote(blocks []*Block, local bool) (ret []*Block) {
	// Graph dailynote filtering not working https://github.com/lonelyor/SourceFlow/issues/16463
