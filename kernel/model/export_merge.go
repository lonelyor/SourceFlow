...
func mergeSubDocs(rootTree *parse.Tree) (ret *parse.Tree, err error) {
	...
	// 导出空文档 Word 和 PDF 时合并子文档失败 https://github.com/lonelyor/SourceFlow/issues/7429
	if nil == insertPoint {
	...
	if ast.NodeParagraph == insertPoint.Type && nil == insertPoint.FirstChild {
		// 删除空段落
		// Ignore the last empty paragraph block when exporting merged sub-documents https://github.com/lonelyor/SourceFlow/issues/15028
		insertPoint.Unlink()
	}
	return
}
