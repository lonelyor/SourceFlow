...
func createDocsByHPath(boxID, hPath, content, parentID, id string) (retID string, err error) {
	...
	hPath = strings.TrimSuffix(hPath, ".sf")
	hPath = util.TrimSpaceInPath(hPath)
	if "" != parentID {
		// The save path is incorrect when creating a sub-doc by ref in a doc with the same name https://github.com/lonelyor/SourceFlow/issues/8138
		...
		if nil != preferredParent && preferredParent.RootID == parentID {
			...
			p := strings.TrimSuffix(preferredParent.Path, ".sf") + "/" + id + ".sf"
	...
	// The subdoc creation path is unstable when a parent doc with the same name exists https://github.com/lonelyor/SourceFlow/issues/9322
		...
		for _, c := range root.Children {
			if "NodeListItem" == c.Type {
				...
				if nil == li || nil == li.FirstChild {
					// 反链面板拖拽到文档以后可能会出现这种情况 https://github.com/lonelyor/SourceFlow/issues/5363
					continue
				}
