...
// 改进打开虚拟引用后加载文档的性能 https://github.com/lonelyor/SourceFlow/issues/7378
...
func processVirtualRef(n *ast.Node, unlinks *[]*ast.Node, virtualBlockRefKeywords []string, refCount map[string]int, luteEngine *lute.Lute) bool {
	...
	if 0 < refCount[parentBlock.ID] {
		...
		// Referenced blocks support rendering virtual references https://github.com/lonelyor/SourceFlow/issues/10960
	...
	if content != newContent {
		// 虚拟引用排除命中自身块命名和别名的情况 https://github.com/lonelyor/SourceFlow/issues/3185
...
func getVirtualRefKeywords(root *ast.Node) (ret []string) {
	...
	// 虚拟引用排除当前文档名 https://github.com/lonelyor/SourceFlow/issues/4537
	// Virtual references exclude the name and aliases from the current document https://github.com/lonelyor/SourceFlow/issues/9204
...
func prepareMarkKeywords(keywords []string) (ret []string) {
	...
	var tmp []string
	for _, k := range ret {
		if "" != k && "*" != k { // 提及和虚引排除 * Ignore `*` back mentions and virtual references https://github.com/lonelyor/SourceFlow/issues/10873
			tmp = append(tmp, k)
		}
	}
	ret = tmp
