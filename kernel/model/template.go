...
func DocSaveAsTemplate(id, name string, overwrite bool) (code int, err error) {
	...
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		// https://github.com/lonelyor/SourceFlow/issues/9649
		// https://github.com/lonelyor/SourceFlow/issues/13701
		switch n.Type {
	...
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		if ast.NodeCodeBlockFenceInfoMarker == n.Type {
			if lang := string(n.CodeBlockInfo); "sourceflow-template" == lang || "template" == lang {
				// 将模板代码转换为段落文本 https://github.com/lonelyor/SourceFlow/pull/15345
...
func RenderTemplate(p, id string, preview bool) (tree *parse.Tree, dom string, err error) {
	...
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		if "" != n.ID {
			...
			// Blocks created via template update time earlier than creation time https://github.com/lonelyor/SourceFlow/issues/8607
			refreshUpdated(n)
		}
		...
		if n.IsTextMarkType("block-ref") {
			...
			if n.ParentIs(ast.NodeTableCell) {
				// 表格中的公式中带有管道符时使用 HTML 实体替换管道符 Improve the handling of inline-math containing `|` in the table https://github.com/lonelyor/SourceFlow/issues/9227
	...
	// 折叠标题导出为模板后使用会出现内容重复 https://github.com/lonelyor/SourceFlow/issues/4488
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		if !entering {
			return ast.WalkContinue
		}
	...
	if "" != icon {
		// 动态图标需要反转义 https://github.com/lonelyor/SourceFlow/issues/13211
...
func addBlockIALNodes(tree *parse.Tree, removeUpdated bool) {
	...
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		if ast.NodeBlockQueryEmbed == n.Type {
			...
			// 使用 <div> 包裹，否则后续解析时会识别为行级 HTML https://github.com/lonelyor/SourceFlow/issues/4244
