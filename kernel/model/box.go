...
func StatJob() {
	...
	// 桌面端检查磁盘可用空间 https://github.com/lonelyor/SourceFlow/issues/6873
...
func ListNotebooks() (ret []*Box, err error) {
	...
	for _, dir := range dirs {
		...
		if !isExistConf {
			// Automatically create notebook conf.json if not found it https://github.com/lonelyor/SourceFlow/issues/9647
...
func (box *Box) GetConf() (ret *conf.BoxConf) {
	...
	confPath := util.HiddenDataPath(filepath.Join(util.DataDir, box.ID), "conf.json")
...
func (box *Box) saveConf0(data []byte) {
	confPath := util.HiddenDataPath(filepath.Join(util.DataDir, box.ID), "conf.json")
	if err := os.MkdirAll(util.HiddenDataDir(filepath.Join(util.DataDir, box.ID)), 0755); err != nil {
...
func (box *Box) GetInfo() (ret *BoxInfo) {
	...
	for _, fileInfo := range fileInfos {
		...
		if !strings.HasSuffix(fileInfo.path, ".sf") {
			continue
		}

		id := strings.TrimSuffix(fileInfo.name, ".sf")
		if !ast.IsNodeIDPattern(id) {
			continue
		}
...
func (box *Box) renameSubTrees(tree *parse.Tree) {
	...
	for _, subFile := range subFiles {
		if !strings.HasSuffix(subFile.path, ".sf") {
			continue
		}
...
func normalizeTree(tree *parse.Tree) (yfmRootID, yfmTitle, yfmUpdated string) {
	...
	ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		if ast.NodeLinkTitle == n.Type {
			// 避免重复转义图片标题内容 Repeat the escaped content of the image title https://github.com/lonelyor/SourceFlow/issues/11681
			n.Tokens = html.UnescapeBytes(n.Tokens)
		}

		if ast.NodeYamlFrontMatterContent == n.Type {
			// Parsing YAML Front Matter as document custom attributes when importing Markdown files https://github.com/lonelyor/SourceFlow/issues/10878
			...
			for attrK, attrV := range attrs {
				// Improve parsing of YAML Front Matter when importing Markdown https://github.com/lonelyor/SourceFlow/issues/12962
				if "title" == attrK {
			...
			// Import the YAML at the beginning of the Markdown as a code block https://github.com/lonelyor/SourceFlow/issues/16488
...
func VacuumDataIndex() {
	...
	logging.LogInfof("vacuum database [sourceflow.db: %s -> %s, history.db: %s -> %s, asset_content.db: %s -> %s]",
