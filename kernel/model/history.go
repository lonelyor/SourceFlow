...
func GenerateFileHistory() {
	...
	for _, box := range Conf.GetBoxes() {
		historyDir = util.HiddenDataPath(filepath.Join(util.DataDir, box.ID), "history")
		os.RemoveAll(historyDir)
	}
	historyDir = util.HiddenDataPath(filepath.Join(util.DataDir, "assets"), "history")
	os.RemoveAll(historyDir)
	historyDir = util.HiddenDataPath(util.DataDir, "history")
	os.RemoveAll(historyDir)
}
...
func ClearWorkspaceHistory() (err error) {
	...
	for _, notebook := range notebooks {
		boxID := notebook.ID
		historyDir := util.HiddenDataPath(filepath.Join(util.DataDir, boxID), "history")
		if !gulu.File.IsDir(historyDir) {
			continue
		}
...
func GetDocHistoryContent(historyPath, keyword string, highlight bool) (id, rootID, content string, isLargeDoc bool, err error) {
	...
	if !isLargeDoc {
		...
		ast.Walk(historyTree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
			...
			// 数据历史浏览时忽略内容块折叠状态 https://github.com/lonelyor/SourceFlow/issues/5778
	...
	// 禁止文档历史内容可编辑 https://github.com/lonelyor/SourceFlow/issues/6580
...
func RollbackDocHistory(boxID, historyPath string) (err error) {
	...
	// 重置重复的块 ID https://github.com/lonelyor/SourceFlow/issues/14358
	...
	for _, nodeID := range duplicatedIDs {
		...
		if ast.NodeDocument == node.Type {
			tree.ID = node.ID
			tree.Path = tree.Path[:strings.LastIndex(tree.Path, "/")] + "/" + node.ID + ".sf"
		}
	}

	// 仅重新索引该文档，不进行全量索引
	// Reindex only the current document after rolling back the document https://github.com/lonelyor/SourceFlow/issues/12320
	...
	go func() {
		...
		evt.Data = map[string]interface{}{
			...
			"empty":   IsNodeTitleEmpty(tree.Root),
			"refText": refText,
		}
		util.PushEvent(evt)
...
func getRollbackDockPath(boxID, historyPath string, workingDoc *treenode.BlockTree) (destPath, parentHPath string, err error) {
	...
	if nil != parentWorkingDoc {
		// 父路径如果是文档，则恢复到父路径下
		parentDir := strings.TrimSuffix(parentWorkingDoc.Path, ".sf")
...
func buildSearchHistoryQueryFilter(query, op, box, table string, typ int) (stmt string) {
	...
	if HistoryTypeDocName == typ || HistoryTypeDoc == typ || HistoryTypeDocID == typ {
		if HistoryTypeDocName == typ || HistoryTypeDoc == typ {
			stmt += " AND path LIKE '%/" + box + "/%' AND path LIKE '%.sf'"
		}
...
func GetNotebookHistory() (ret []*History, err error) {
	...
	historyNotebookConfs, err := filepath.Glob(historyDir + "/*-delete/*/" + util.HiddenDataDirName + "/conf.json")
	if err != nil {
