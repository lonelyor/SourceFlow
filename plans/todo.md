...
- [x] 编辑器跨块复制粘贴安全强化设计：明确文本选择和块选择分离，普通跨块文本选区不写 SourceFlow 内部 Block DOM MIME，inline 粘贴不得用单块事务处理跨块 selection。
- [ ] 编辑器跨块复制粘贴安全闸落地：新增 selection scope helper，复制内部 MIME 写入条件收紧，`insertHTML()` 跨块 inline 粘贴失败关闭，并补目标回归。
- [ ] 编辑器跨块复制粘贴代码评审与验证：覆盖长无换行文本、手动换行文本、显式块选择、单块行内格式、表格/AV/代码块和撤销重做。
