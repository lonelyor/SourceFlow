...
- 2026-06-05 编辑器跨块复制粘贴安全强化设计完成：风险路径定位为前端普通文本 selection、SourceFlow 内部剪贴板 MIME 与 `insertHTML()` 单块事务边界不一致；已输出 `plans/20260605-编辑器跨块粘贴安全强化设计.md`。设计结论为文本选择和显式块选择严格分离，普通跨块文本选区不得写内部 Block DOM MIME，inline 粘贴在完整多块事务前必须对跨块 selection 失败关闭。
