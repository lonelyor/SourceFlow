...
- 2026-06-08 主页生产级审计补丁完成：主页绑定笔记可读性检查区分有效、索引中、删除/不可读和请求异常；只有明确删除/不可读或绑定非根文档时才清空主页绑定，网络/非 JSON/坏 JSON 等瞬时请求异常只降级显示“主页暂时无法打开”并保留绑定，避免移动端空状态挂载出现未处理 Promise 或瞬时故障破坏用户配置。主页模块测试已覆盖这些分支。
...
- 2026-06-08 Attribute View 大文件拆分阶段开始：仅做 `kernel/model/attribute_view.go` 同包纯拆分，移动完整函数/类型/常量块到职责文件，不改变 Attribute View 行为、接口、事务边界或存储格式。
- 2026-06-08 Attribute View 大文件纯拆分阶段完成：`attribute_view.go` 降至约 600 行，新增同包 group/layout/query/grouping/column_relation/view/block/key/column/cell/option 职责文件，所有拆出文件均低于 800 行；非空声明行与拆分前逐行一致。验证通过：`go test -vet=off ./model -run '^$' -count=1`、`go test -vet=off ./api -run '^$' -count=1`、`go test ./conf/ -v`、`pnpm --dir app run typecheck:app`、`pnpm --dir app run lint`、两个编辑器结构提示回归。
- 2026-06-09 用户明确 AI 旧配置不要兼容、只保留新的：旧 `Conf.AI.OpenAI` 不再作为 AI 助手配置来源、迁移来源或兼容读取来源；旧 `/api/ai/chatGPT*` 与旧 AI 写作弹窗链路应删除，现有 AI 入口走 Assistant Dock/skill 与 `ai_profiles`。
- 2026-06-09 AI 旧配置兼容删除已完成：删除 AI 助手启动时从旧 `Conf.AI.OpenAI` 自动迁移 profile 的逻辑，删除旧 `/api/ai/chatGPT*` 路由、旧 `model.ChatGPT` 调用栈和前端旧 AI 写作弹窗；`/` 菜单 AI 写作与快捷键改走现有 `AIActions`/Assistant skill 链路。新增回归确保旧 OpenAI 配置不会自动生成 AI Profile。验证通过：`go test -vet=off ./model -run TestAssistantAI -count=1`、`go test -vet=off ./api -run '^$' -count=1`、`go test ./conf/ -v`、`pnpm --dir app run typecheck:app`、`pnpm --dir app run lint`。
- 2026-06-09 AI Dock 输入失焦根因定位：Dock 内 `selectionchange`/click/focus 跟随当前笔记逻辑会在聊天输入框聚焦或输入时触发上下文刷新，刷新后 `renderAssistantAIDock` 用 `innerHTML` 重建整块 Dock，导致 textarea 被销毁并失焦；同类触发点还包括目标笔记搜索和 `@` 提及搜索完成后的 Dock 重绘。修复口径为不改变 AI 对话/来源/搜索语义，只过滤 Dock 内部事件触发的上下文跟随刷新，并在必要重绘后恢复可编辑输入的焦点与光标。
- 2026-06-09 AI 输入稳定性修复已完成：AI Dock 忽略 Dock 内部事件触发的当前笔记跟随刷新；Dock 重绘时恢复聊天输入框与目标笔记搜索框的焦点、光标选区和滚动位置；来源创作笔记搜索框在异步搜索重绘后同样恢复焦点。验证通过：`pnpm --dir app run test:ai-dock-runtime`、`pnpm --dir app run test:assistant-source-save`、`pnpm --dir app run typecheck:app`、`pnpm --dir app run lint`、`go test ./conf/ -v`、两个编辑器结构提示回归和 `git diff --check`；lint 仍为既有 3904 warnings、0 errors。
- 2026-06-09 AI 输入稳定性护栏已完成：新增 `assistant/common/inputStability.ts` 作为 AI 可输入区域焦点恢复与容器事件边界的公共 helper，Dock 与来源创作移除本地重复实现并复用该 helper；新增 `test:assistant-input-stability` 并接入 `typecheck:app`，固定输入不丢焦点、不跳光标、不被 Dock 内部事件误触发上下文跟随。验证通过：`pnpm --dir app run test:assistant-input-stability`、`pnpm --dir app run test:ai-dock-runtime`、`pnpm --dir app run test:assistant-source-save`、`pnpm --dir app run typecheck:app`、`pnpm --dir app run lint`、`go test ./conf/ -v`、两个编辑器结构提示回归和 `git diff --check`；lint 仍为既有 3904 warnings、0 errors。
- 2026-06-09 产品级发布就绪门禁已完成：新增 `test:product-readiness` 静态审计和 `lint:check` 入口，发布前可自动检查版本号一致、发布说明存在、plans 当前任务无未完成项、关键前端回归仍接入 `typecheck`、lint 检查入口不会自动改源码；质量文档已把 `test:product-readiness`、`lint:check` 和 `python 编译.py --stability-gate-only` 作为发布前门禁组合。验证通过：`pnpm --dir app run test:product-readiness`、根目录 `pnpm run test:product-readiness`、`pnpm --dir app run lint:check`、`pnpm --dir app run lint`、`pnpm --dir app run typecheck:app`、`go test ./conf/ -v`、两个编辑器结构提示回归、`python 编译.py --stability-gate-only` 和 `git diff --check`；lint 仍为既有 3904 warnings、0 errors。
- 2026-06-09 产品级韧性矩阵已完成：新增 `test:lint-budget`，将当前 3904 个历史 lint warning 固定为预算并要求 0 error，避免 warning 继续恶化；新增 `test:product-resilience` 并接入 `typecheck:app`，覆盖多窗口启动边界、真实桌面/内核烟测入口、构建内核 E2E 持久化烟测入口、慢请求取消、网络异常错误可读性、大文档裁剪和大量笔记/上传上限。验证通过：`pnpm --dir app run test:product-resilience`、根目录 `pnpm run test:product-resilience`、`pnpm --dir app run test:lint-budget`、根目录 `pnpm run test:lint-budget`、`pnpm --dir app run test:product-readiness`、`pnpm --dir app run lint:check`、`pnpm --dir app run lint`、`pnpm --dir app run typecheck:app`、`go test ./conf/ -v`、两个编辑器结构提示回归、`python 编译.py --stability-gate-only` 和 `git diff --check`；lint 仍为既有 3904 warnings、0 errors。
- 2026-06-09 lint warning 安全清理规划已确认：当前 3904 warnings / 0 errors 继续由 `test:lint-budget` 锁为上限，后续清理不与功能改动混做，不通过关闭规则或扩大 ignore 消除数字；按 Workbench、Protyle menu/gutter、Protyle wysiwyg/editorEvents/keydown、boot/globalEvent 四类风险分批推进，每批必须先快照、最小补丁、跑目标回归和全量 lint，再下调预算并独立提交。
- 2026-06-09 Windows 编译失败根因定位：`python 编译.py` 已通过质量门、前端 bundle 和 Go kernel 构建，但在 installer 阶段 electron-builder 扫描 pnpm hoisted 依赖时命中损坏的 `@esbuild/aix-ppc64` junction；既有 `clean_broken_pnpm_links()` 只在 portable 阶段调用且不能识别 Windows junction reparse point，导致 installer 阶段清理过晚。修复口径为只增强构建脚本断链清理时机和 junction 识别，不改变应用运行语义。
- 2026-06-09 Windows 编译失败修复已完成：`clean_broken_pnpm_links()` 改为识别 Windows junction/reparse point，并在 installer 调用 electron-builder 前执行；断链清理只删除目标不存在的 pnpm hoisted 链接。验证通过：`python -m py_compile 编译.py`、单独调用 `clean_broken_pnpm_links()`、完整 `python 编译.py`；本次完整编译生成并验证 Windows installer 与 portable 产物。
...
- 2026-06-08 AI 安全第 4 层扩展已落地：`AISecurityPermissionRequest` 新增可信入口来源、session/Agent/operation 元数据；patch apply 和 history revert/reapply 标记可信 assistant 来源；patch 安全检查会把同会话 AI 操作历史写入目标计入累计影响范围；未标记可信入口的 AI 写风险请求拒绝；同会话低风险写入累计到 3 个目标时转人工确认并返回 affectedItems。绕过检测逻辑拆入 `assistant_security_bypass.go`，避免 `assistant_security.go` 贴近 800 行上限。验证已通过：`go test -vet=off ./model -run Test.*AISecurity -count=1`、`go test -vet=off ./model -run TestAssistantPatch -count=1`、`go test -vet=off ./model -run TestAssistantOperationHistory -count=1`、`go test -vet=off ./model -run TestCountAssistantOperationHistorySessionWriteTargets -count=1`、`go test -vet=off ./api -run '^$' -count=1`、`pnpm --dir app run typecheck:app`、变更文件范围 eslint。
- 2026-06-08 发布后 24 小时检查已按 `docs/OPERATIONS.md` 完成：本地 v0.1.7 installer/portable/release-assets 与 `latest.yml` 版本一致；`app/build/sourceflow-portable` 存在 `.sf-portable`、`SourceFlow.exe` 和 `resources`；`python 诊断包.py` 生成 `.tmp/diagnostics/sourceflow-diagnostics-20260608-105838.zip`；诊断日志尾部显示 2026-06-08 10:56 同步主流程成功且无 SourceFlow CrashDumps，现存同步相关异常为本地备份 `refs/tags` 缺失导致的 snapshot protection 警告；插件集市 version/stage 网络拉取成功，plugins stage repo_count=2；GitHub `v0.1.7` Release 可访问且 4 个资产存在，开放 issue 查询返回 1 条无标签 issue，未发现阻断性回归标签。
...
- 2026-06-05 AI 写入可逆事务终审小修完成：Agent/历史面板状态改为可读标签，风险显示复用 L1-L4 标签；旧 localStorage 历史只有后端记录或确认具备低风险回滚路径时才显示“撤回”，`reverted` 的“取消撤回”只对后端持久历史开放，避免用户点到不可执行操作。
- 2026-06-05 编辑器跨块复制粘贴安全强化设计完成并按用户最新反馈修正：风险路径定位为前端普通文本 selection、SourceFlow 内部剪贴板 MIME 与 `insertHTML()` 单块事务边界不一致；已输出 `plans/20260605-编辑器跨块粘贴安全强化设计.md`。设计结论为文本选择和显式块选择严格分离，普通跨块文本选区不得写内部 Block DOM MIME，inline 粘贴不得用单块事务处理跨块 selection；跨块 CV 必须自动生成多块文本事务，无法证明结构安全时自动降级为纯文本流事务，不向用户暴露块边界、确认框或失败提示。
- 2026-06-05 编辑器跨块复制粘贴无感安全闸阶段 1 落地完成：新增 `selectionScope.ts` 统一判断 collapsed/single-block/multi-block/explicit-block/table/AV/code selection；复制和剪切仅在显式块选择、表格/AV 专用选择或单块纯行内片段时写 `SOURCEFLOW_HTML_CLIPBOARD_MIME`；粘贴时普通文本选区会丢弃包含 Block DOM 的内部 MIME；`insertHTML()` 在任何 `range.deleteContents()` 前先调用 `multiBlockPaste.ts`，同父级普通段落/标题跨块选区生成原子多块事务，复杂结构自动折叠到起始安全落点并走普通插入，不弹失败提示。
- 2026-06-05 本轮编辑器跨块 CV 验证已通过：`pnpm --dir app run test:protyle-paste-selection-safety`、`pnpm --dir app run typecheck:app`、`pnpm --dir app run test:editor-structure-guide`、`pnpm --dir app run test:editor-structure-guide-bugfix`、`go test ./conf/ -v`、变更文件范围 eslint 和 `git diff --check`。全量 `pnpm --dir app run lint` 仍被既有 `app/build-linux-portable/...` 打包产物阻断，本轮变更文件无 eslint error。
- 2026-06-05 细节审计追加修复完成：普通文本选区在丢弃 SourceFlow 内部 Block DOM MIME 后，会剥离 `text/html` 中的 SourceFlow 剪贴板注释并重新走标准 HTML 清洗，且 `processPasteCode` 只在该降级清洗之后执行；AI 操作历史撤回/取消撤回失败会可靠写回 `revert-failed` / `reapply-failed` 与错误原因，失败状态写回失败时不再被吞掉。
- 2026-06-05 本轮细节审计验证已通过：`pnpm --dir app run test:protyle-paste-selection-safety`、`pnpm --dir app run typecheck:app`、变更文件范围 `pnpm --dir app exec eslint src/protyle/util/paste.ts scripts/testProtylePasteSelectionSafety.js --cache`、`go test -vet=off ./model -run TestAssistantOperationHistory -count=1`、`go test ./conf/ -v`、两个编辑器结构提示回归和 `git diff --check`。全量 `pnpm --dir app run lint` 仍被既有 `app/build-linux-portable/...` 打包产物阻断，本轮变更文件无 eslint error。
...
- 2026-06-04 AI Profile 真相源收敛口径确认：`ai_profiles` 为 AI 助手生产配置唯一真相源；AI Profile 保存/删除不再反向同步旧配置。旧 `Conf.AI.OpenAI` 一次性迁移读取和非 AI 助手兼容调用边界已在 2026-06-09 按用户最新确认删除，不再作为当前规则。
