...
- AI 原生笔记助手必须支持 `@` 引用当前笔记、选区、笔记、文件夹、附件、搜索结果和工作台视图；文件夹和大量笔记引用必须先构建有界上下文包，禁止无界发送全文。
...
- AI 权限内核分为默认权限、自动审查和完全访问权限；能力分为可读、可写、可执行；黑白名单按笔记本、文件夹、笔记、标签、附件类型和工具类型限制，黑名单优先级最高。
- AI 安全内核包含硬禁止规则：禁止 AI 删除整个工作空间、删除整个笔记本、清空所有笔记、清空某个笔记本全部笔记、重置工作空间数据，或通过导入/同步/覆盖/移动清空等方式变相完成这些操作。
- AI 单次任务触达 `>= 10` 篇笔记的修改、删除、移动、重命名或属性批改，必须进入人工确认；阈值按同一会话、同一 Agent 任务和同一用户目标累计计算，禁止拆分批次绕过。
- 当 AI 试图关闭安全检查、静默执行、规避批量阈值、组合低风险工具达成高风险目标、直接调用底层文件 API，或用模糊大范围目标执行破坏性操作时，必须暂停并进入安全拦截；硬禁止操作不能授权继续。
