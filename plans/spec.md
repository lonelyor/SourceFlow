...
- 主页只保留一种产品语义：主页是一篇普通笔记的快捷入口，不再是独立页面、默认模板页或自定义 HTML/Markdown 模板页。
- 主页状态只绑定具体笔记 ID；未绑定、绑定为空、目标笔记删除或无法读取时，主页入口显示空态提示“尚未创建主页”，不得自动回退到默认模板。
- 点击左侧“主页”按钮时，如果已绑定有效主页笔记，应直接打开该笔记编辑器；不得打开只读渲染页、模板页或第二套主页 Tab。
- 主页空态允许创建普通笔记并自动设为主页，也允许将当前笔记设为主页；创建完成后直接打开该普通笔记。
- 文档树右键和当前笔记标题菜单继续提供“设为主页”，该操作只更新主页笔记 ID，不改变笔记内容、路径或数据结构。
- 主页笔记与普通笔记共享同一套编辑器、块、链接、引用、AI、历史、搜索、同步和审计能力，不引入单独主页渲染器或模板脚本运行时。
- 主页笔记允许承载手动点击的本地路径快捷入口：本地文件、目录、应用快捷方式和网页链接均通过普通笔记链接触发，复用现有桌面端 `shell.openPath` / `showItemInFolder` 路径。
- 外部打开必须由用户显式点击触发；禁止主页加载时自动打开，禁止任意命令行、shell 脚本、带参数执行、模板 JS 调本地能力或 AI 自动触发打开应用。
- 应用类路径只作为本地文件/快捷方式打开处理，不提供命令参数执行能力；后续如增加首次确认或白名单，应作为普通本地链接安全策略统一落地，而不是主页专属权限。
- 主页空态应降低用户心智负担：无绑定时提供创建主页笔记入口；有当前普通笔记时提供“设当前笔记为主页”；后续选择已有笔记也应通过搜索选择器完成，不要求用户输入笔记 ID。
- 笔记内快捷入口作为普通链接增强落地：通过编辑器插入菜单和 `/快捷入口` 命令生成普通链接，支持选择本地文件、文件夹和输入网页地址；该能力只写入笔记内容，不在插入、渲染或主页加载时打开目标。
- 快捷入口第一阶段不新增独立数据表、模板块、HTML/JS 运行时、命令参数、脚本执行、自动打开或 AI 触发能力；路径选择优先复用系统文件选择器，链接打开继续复用既有普通链接点击链路。
...
- AI 助手采用 Kilo 式单 Dock 交互：`问 AI`、连续对话和 Agent 都复用现有 AI Dock、同一来源面板、同一消息流、同一安全内核和同一审计体系；禁止为“单次问答”新增独立浮层、独立消息存储或第二套上下文真相源。
- Dock 模式分为 `ask`、`chat` 和 `agent`：`ask` 是只读问答模式，默认禁用工具写入，仅围绕当前来源解释、总结和分析；`chat` 是普通连续对话；`agent` 仍通过现有 Agent 面板处理批量任务和 patch 审阅。
- 普通 AI 对话和编辑重生成必须支持用户主动停止生成：前端保存当前请求 `AbortController`，停止时中止流式请求，后端通过 request context 释放 provider 请求；用户主动停止不视为失败，保留已生成部分并标记为已停止。
- 编辑器右键、选区浮条、文档树和标题菜单的 `问 AI` 入口应打开现有 AI Dock，把目标转换为来源面板来源，切到 `ask` 模式并聚焦输入框；`加入当前对话` 只追加来源不自动发送，`开启新对话` 创建/选择新会话后追加来源。
- AI 入口复用是硬约束：所有需要对话历史、来源面板、停止生成、模型/Profile 选择、工具权限、Agent 任务、patch 审阅或审计历史的入口，都必须落到现有 AI Dock；入口模块只负责收集触发目标并转换为 Dock options/source，不得自行维护 prompt、消息、来源、工具或请求状态。
- 仅允许高频轻量内联 AI 保留独立交互，例如当前选区翻译、改写、续写和补全；这类能力只能处理当前编辑器局部结果，仍必须复用统一上下文、模型调用、安全策略、patch 审阅和审计边界。内联流程一旦需要多轮追问、引用多个来源、持久历史或任务队列，应转入 AI Dock。
- 禁止新增与 AI Dock 平级的问答浮层、独立聊天 dialog、独立 Agent 聊天、独立来源面板、独立 AI Profile 选择器或独立停止生成实现；如某入口需要差异化体验，应通过 Dock mode、来源快照、面板状态和 composer draft 参数表达。
...
- Agent 队列以后端持久状态为唯一生产真相源，存放在 `storage/assistant_agent_tasks.json`；前端不得再把 `localStorage` 作为 Agent 队列兼容来源或迁移来源。旧本地队列直接废弃，只保留新的后端队列/API 路径。
- Agent 后端 API 负责创建任务、列出任务、更新任务状态、更新任务项、取消剩余项、获取/释放执行锁；任务状态与任务项 patch/context/error/retryCount 必须通过后端安全写入并持久化。
- Agent 执行锁采用后端租约 token：运行任务前获取任务级 lease，lease 绑定 taskId、持有人和过期时间；未持有有效 lease 的前端不得执行该任务，暂停/取消/完成后释放 lease，过期 lease 可被新运行请求接管。
- Agent 面板创建批量任务时复用 Dock 当前输入作为任务列表，任务项必须保存待审阅 patch 与最小笔记上下文；暂停/取消需要中止当前执行请求并更新后端任务状态，恢复/重试必须重新获取后端执行锁再进入同一执行器。
- AI 操作历史升级为后端持久可逆事务模型，记录 patch、session/profile、目标、状态、应用结果、失败原因和最小可逆快照；撤回和取消撤回以后端历史记录为真相源，旧前端 localStorage 历史仅作为兼容展示来源。
- 前端 AI 操作历史不再读写旧 localStorage 历史，也不再提供旧本地低风险回滚兜底；历史面板只展示后端 `/api/assistant/history/list` 返回的记录，撤回和取消撤回只调用后端历史接口。
...
- AI Dock 输入区、目标笔记搜索框、`@` 提及输入和来源创作的笔记搜索框属于可编辑焦点区；这些输入聚焦期间，Dock 内部点击、光标变化和输入选择变化不得触发“跟随当前笔记”的全量上下文刷新。若 Dock/来源创作因异步上下文、提及搜索或目标搜索需要重绘，必须通过 `assistant/common/inputStability.ts` 统一恢复原输入框焦点、光标选区和滚动位置，避免输入过程中 textarea/input 被整块重建后失焦；新增或修改 AI 可输入区域时，必须补充或通过 `test:assistant-input-stability` 回归约束。
...
- 旧 localStorage AI 历史不再作为生产级撤回入口或兼容展示入口；前端历史面板应以后端列表为准，不再通过普通块/文件树删除接口执行旧本地历史回滚。
...
- 第 4 层绕过检测在后端安全内核集中落地为人工确认/拒绝策略：写入目标为空且目标类型为 note/folder/notebook 时视为范围不明确；写风险操作累计影响数低于但接近批量阈值时视为疑似分批规避；同一会话/Agent/工具在短窗口内连续低风险写入多个不同目标时按累计影响范围确认；AI 写入必须通过 assistant patch/tool 安全入口标记来源，疑似直接底层写 API 绕过的请求拒绝；影响范围优先归一化到 root note ID 并返回可审计 affectedItems。
...
- 后端大文件拆分属于维护性强化，不改变外部 API、导出符号、数据结构、存储格式、错误语义或事务边界。
- `kernel/model/attribute_view.go` 拆分只能按同一 `model` package 内的完整函数、类型或常量块移动；调用方仍通过原有 `model` 包符号访问 Attribute View 能力。
- Attribute View 拆分阶段不得混入业务逻辑修复、性能策略调整、默认值调整、兼容路径删除或格式化无关文件。
...
- 工作台主查询是可用性硬依赖，失败时显示可重试错误页；关联块搜索是增强层，失败时只能局部降级并显示提示，不得拖垮主工作台结果。
- 工作台前端数据入口必须归一化后端可空集合字段，至少包括 `items/allItems[].tags`；后端 Go nil slice 可能序列化为 JSON `null`，渲染层不得直接对未归一化字段读取 `.length`。
- 工作台第一屏应优先呈现核心路径：视图切换、搜索、常用快速筛选、选中项批量操作、结果列表和局部错误提示；低频的仪表盘、视图模板、结果层级、排序、分组、导出、AI 摘要和嵌入入口放入渐进展开区，不改变既有查询语义、DOM 控件 ID 和事件 `data-action` 契约。
...
- 发布前必须运行产品韧性矩阵：慢请求/取消、网络异常、大文档、大量笔记、多窗口启动边界和真实桌面/内核烟测入口都必须有自动化约束；前端 lint warning 当前作为历史预算处理，不得超过记录基线，清零必须分批独立推进。
...
- 退出时同步失败详情若命中 `tree not found` / `block tree not found` 等本地笔记树索引缺失特征，应在失败提示中明确引导用户到“设置 - 关于 - 重建索引”；网络、权限、密钥、云端限流等同步失败不得误提示重建索引。
...
详细设计见 `plans/20260605-编辑器跨块粘贴安全强化设计.md`。
