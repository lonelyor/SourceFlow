...
- AI 助手生产级加固继续遵循“AI 写入先预览、后审阅、再提交”的单一写入语义：前端 patch 接受不应直接调用普通块/文件树写接口，应通过后端 `/api/assistant/patch/apply` 入口完成安全检查、目标归一化、正式写入和结果审计。
...
- AI Profile 当前生产配置源为 `ai_profiles` 表；旧 `Conf.AI.OpenAI` 只应作为一次性迁移来源或非 AI 助手兼容边界。彻底移除反向同步属于兼容性变更，应独立阶段处理并同步调用方。
