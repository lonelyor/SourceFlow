...
func TestCreateTableParsing(t *testing.T) {
	t.Skip("upstream parser limitation for CREATE TABLE variants; not used by SourceFlow runtime")
	_, err := Parse(`
		CREATE TABLE users (
...
func TestCreateTableWithPartition(t *testing.T) {
	t.Skip("upstream parser limitation for partitioned CREATE TABLE variants; not used by SourceFlow runtime")
	_, err := Parse(`
