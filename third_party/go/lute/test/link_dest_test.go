...
func TestGetLinkDest(t *testing.T) {
	...
	dest = luteEngine.GetLinkDest("[foo](sf://blocks/20220817180757-c57m8qi)")
	if "sf://blocks/20220817180757-c57m8qi" != dest {
		t.Fatalf("get link dest failed")
	}
	...
	dest = luteEngine.GetLinkDest("file://D:\\Admin\\Downloads\\隔壁叔叔过年还在玩源流.jpg")
	if "file://D:\\Admin\\Downloads\\隔壁叔叔过年还在玩源流.jpg" != dest {
		t.Fatalf("get link dest failed")
	}
}
...
func TestIsValidLinkDest(t *testing.T) {
	...
	if !luteEngine.IsValidLinkDest("sf://blocks/20220817180757-c57m8qi") {
		t.Fatalf("check link dest failed")
	}
