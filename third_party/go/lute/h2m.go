...
import (
	...
	"github.com/lonelyor/sourceflow/third_party/go/lute/util"
)
...
func (lute *Lute) genASTByDOM(n *html.Node, tree *parse.Tree) {
	...
	if strings.HasPrefix(class, "line-number") &&
		!strings.HasPrefix(class, "line-numbers" /* 简书代码块 https://github.com/lonelyor/SourceFlow/issues/4361 */) {
		return
	}

	if strings.Contains(class, "mw-editsection") {
		// 忽略 Wikipedia [编辑] Do not clip the `Edit` element next to Wikipedia headings https://github.com/lonelyor/SourceFlow/issues/11600
		return
	}

	if strings.Contains(class, "citation-comment") {
		// 忽略 Wikipedia 引用中的注释 https://github.com/lonelyor/SourceFlow/issues/11640
		return
	}
	...
	case 0:
		...
		if "fn__space5" == class {
			// 链滴剪藏图片时多了长宽显示 https://github.com/lonelyor/SourceFlow/issues/10987
			return
		}
		...
		node.Tokens = bytes.ReplaceAll(node.Tokens, []byte("\n"), []byte{' '}) // 将 \n 转换为空格 https://github.com/lonelyor/SourceFlow/issues/6052
		...
		if ast.NodeParagraph == tree.Context.Tip.Type && bytes.HasPrefix(node.Tokens, []byte("$$")) && bytes.HasSuffix(node.Tokens, []byte("$$")) && 2 == bytes.Count(node.Tokens, []byte("$$")) {
			// 将其转换为公式块 https://github.com/lonelyor/SourceFlow/issues/14360
		...
		if nil != n.Parent && atom.Span == n.Parent.DataAtom && 0 == len(n.Parent.Attr) {
			...
			if lute.ParseOptions.ProtyleWYSIWYG {
				if ast.NodeLink != tree.Context.Tip.Type { // a 标签锚文本中的标记符不进行转义 https://github.com/lonelyor/SourceFlow/issues/14733
		...
		if ast.NodeListItem == tree.Context.Tip.Type || (nil != tree.Context.Tip.Parent && ast.NodeListItem == tree.Context.Tip.Parent.Type) ||
			...
			// 去掉文本中开头的列表项标记符 https://github.com/lonelyor/SourceFlow/issues/14329
	...
	case atom.P, atom.Div, atom.Section, atom.Dt, atom.Dd:
		...
		if atom.Div == n.DataAtom || atom.Section == n.DataAtom {
			...
			// The browser extension supports CSDN formula https://github.com/lonelyor/SourceFlow/issues/5624
			...
			// The browser extension supports Wikipedia formula clipping https://github.com/lonelyor/SourceFlow/issues/11583
		...
		if "" == strings.TrimSpace(util.DomText(n)) {
			for { // 这里用 for 是为了简化实现
				...
				if tex := util.GetFormula(n); "" != tex {
					// https://github.com/lonelyor/SourceFlow/issues/15457
					appendMathBlock(tree, tex)
					return
				}
	...
	case atom.Pre:
		...
		if strings.Contains(class, "iconfont") {
			...
			// https://github.com/lonelyor/SourceFlow/issues/15314
			firstc = firstc.NextSibling
			n.FirstChild.Unlink()
		}

		if atom.Em == firstc.DataAtom && nil != firstc.NextSibling && atom.Em == firstc.NextSibling.DataAtom {
			// pre.em,em,code 的情况，这两个 em 是“复制代码”和“隐藏代码” https://github.com/lonelyor/SourceFlow/issues/13026
		...
		if atom.Div == firstc.DataAtom {
			if nil == firstc.NextSibling {
				if 1 == len(codes) {
					code := codes[0]
					// pre 下只有一个 div，且 div 下只有一个 code，那么将 pre.div 替换为 pre.code https://github.com/lonelyor/SourceFlow/issues/11131
					code.Unlink()
					n.AppendChild(code)
					firstc.Unlink()
					firstc = n.FirstChild
				...
				// pre 下全是 div，每个 div 为一行代码 https://github.com/lonelyor/SourceFlow/issues/14195
		...
		// 改进两种 pre.ol.li 的代码块解析 https://github.com/lonelyor/SourceFlow/issues/11296
		...
		if html.TextNode == firstc.Type || atom.Span == firstc.DataAtom || atom.Code == firstc.DataAtom || atom.Section == firstc.DataAtom || atom.Pre == firstc.DataAtom || atom.A == firstc.DataAtom || atom.Strong == firstc.DataAtom || atom.B == firstc.DataAtom || atom.I == firstc.DataAtom || atom.P == firstc.DataAtom {
			...
			if atom.Code == firstc.DataAtom {
				...
				if nil != n.LastChild && atom.Ul == n.LastChild.DataAtom {
					...
					n.LastChild.Unlink() // 去掉最后一个代码行号子块 https://github.com/lonelyor/SourceFlow/issues/5564
				}
			}
			...
			if tree.Context.Tip.ParentIs(ast.NodeTable) {
				...
				// Improve HTML parsing code blocks https://github.com/lonelyor/SourceFlow/issues/11068
	...
	case atom.Code:
		...
		for c := n.FirstChild; nil != c; c = c.NextSibling {
			...
			if atom.Em == c.DataAtom || atom.Strong == c.DataAtom {
				// https://github.com/lonelyor/SourceFlow/issues/11682
				continue
			}
		...
		if allSpan {
			// 如果全部都是 span 子节点，那么直接使用 span 的内容 https://github.com/lonelyor/SourceFlow/issues/11281
	...
	case atom.A:
		...
		if "" == text && nil != n.Parent && lute.parentIs(n, atom.H1, atom.H2, atom.H3, atom.H4, atom.H5, atom.H6, atom.Div, atom.Section) && nil == util.DomChildrenByType(n, atom.Img) {
			...
			// https://github.com/lonelyor/SourceFlow/issues/11445
			return
		}
		if "" == text && nil == n.FirstChild {
			// 剪藏时过滤空的超链接 https://github.com/lonelyor/SourceFlow/issues/5686
			return
		}
	...
	case atom.Img:
		...
		if "emoji" == imgClass {
			if e := parse.EmojiUnicodeAlias[imgAlt]; "" != e {
				// 直接使用 alt 值（即 emoji 字符）https://github.com/lonelyor/SourceFlow/issues/13342
			...
			// 处理使用 data-original 属性的情况 https://github.com/lonelyor/SourceFlow/issues/11826
			...
			// Wikipedia 使用图片原图 https://github.com/lonelyor/SourceFlow/issues/11640
	...
	case atom.Table:
		...
		if nil != tree.Context.Tip.LastChild && !tree.Context.Tip.LastChild.IsBlock() {
			// 如果上一个节点不是块级元素，那么需要先添加一个换行节点 Improve HTML table clipping https://github.com/lonelyor/SourceFlow/issues/15307
	...
	case atom.Span:
		...
		if "tip" == class {
			// 转换为行级备注 https://github.com/lonelyor/SourceFlow/issues/13998
		...
		// Improve inline elements pasting https://github.com/lonelyor/SourceFlow/issues/11740
		...
		// The browser extension supports Zhihu formula https://github.com/lonelyor/SourceFlow/issues/5599
		if tex := strings.TrimSpace(util.DomAttrValue(n, "data-tex")); "" != tex {
			...
			if nil != tree.Context.Tip.LastChild && ast.NodeImage == tree.Context.Tip.LastChild.Type {
				if closeParen := tree.Context.Tip.LastChild.ChildByType(ast.NodeCloseParen); nil != closeParen {
					if nil != n.FirstChild && atom.Span == n.FirstChild.DataAtom {
						// 单独处理 <figcaption> 中的 <span leaf> 标签的情况 https://github.com/lonelyor/SourceFlow/issues/14507
