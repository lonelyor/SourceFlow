...
func (t *Tree) parseGFMAutoLink0(node *ast.Node) {
	...
	for i < length {
		...
		if 10 <= tmpLen /* www.xxx.xx */ && 'w' == tokens[i] && 'w' == tokens[i+1] && 'w' == tokens[i+2] && '.' == tokens[i+3] {
			...
			// 自定义协议均认为是有效的 https://github.com/lonelyor/SourceFlow/issues/5865
		...
		if bytes.HasPrefix(linkText, []byte("https://github.com/")) {
			if bytes.Contains(linkText, []byte("/issues/")) {
				// 优化 GitHub Issues 自动链接文本 https://github.com/lonelyor/sourceflow/third_party/go/lute/issues/161
				...
				// 优化 GitHub Pull Requests 自动链接文本 https://github.com/lonelyor/sourceflow/third_party/go/lute/issues/208
				repo := linkText[len("https://github.com/"):]
