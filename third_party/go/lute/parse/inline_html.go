...
func SetTextMarkNode(node *ast.Node, n *html.Node, options *Options) {
	...
	for _, typ := range types {
		...
		case "inline-memo":
			...
			node.TextMarkInlineMemoContent = strings.ReplaceAll(node.TextMarkInlineMemoContent, "\"", "&quot;")
		default:
			if !isInlineMath { // 带有字体样式的公式复制之后内容不正确 https://github.com/lonelyor/SourceFlow/issues/6799
				...
				if node.ContainTextMarkTypes("strong", "em", "s", "mark", "sup", "sub") {
					// Improve some inline elements Markdown editing https://github.com/lonelyor/SourceFlow/issues/9999
					...
					if !options.KeepEscaped && !node.ContainTextMarkTypes("code") {
						// Improve the unescaping of copied block contents https://github.com/lonelyor/SourceFlow/issues/16136
				...
				if node.ParentIs(ast.NodeTableCell) && node.IsTextMarkType("code") {
					// 表格中的代码中带有管道符时使用 HTML 实体替换管道符 Improve the handling of inline-code containing `|` in the table https://github.com/lonelyor/SourceFlow/issues/9252
					node.TextMarkTextContent = strings.ReplaceAll(node.TextMarkTextContent, "|", "&#124;")
				}

				if "u" == node.TextMarkType {
					// 下划线中支持包含 Markdown 语法 Improve underline element parsing https://github.com/lonelyor/SourceFlow/issues/13768
