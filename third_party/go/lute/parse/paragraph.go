					...
					if (3 == len(tokens) && tokens[0] == lex.ItemOpenBracket && tokens[2] == lex.ItemCloseBracket && tokens[1] != lex.ItemCloseBracket) ||
						...
						if 0 < len(p.Tokens) {
							...
							if ast.NodeParagraph != subBlock.Type {
								if !context.ParseOption.ProtyleWYSIWYG {
									// Protyle `Optimize typography` exception in case of task list and heading https://github.com/lonelyor/SourceFlow/issues/9035
								...
								// Incomplete data when pasting task list nested list https://github.com/lonelyor/SourceFlow/issues/9239
								var last *ast.Node
								var blocks []*ast.Node
