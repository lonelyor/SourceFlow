		...
		if !node.IsTextMarkType("a") && !node.IsTextMarkType("inline-memo") && !node.IsTextMarkType("block-ref") && !node.IsTextMarkType("file-annotation-ref") && !node.IsTextMarkType("inline-math") {
			...
			if node.IsTextMarkType("code") {
				...
				if node.ParentIs(ast.NodeTableCell) {
					// 多加一个转义符 Improve the handling of inline-code containing `|` in the table https://github.com/lonelyor/SourceFlow/issues/9252
			...
			if isStrongEm {
				...
				// https://github.com/lonelyor/SourceFlow/issues/6472
				// https://github.com/lonelyor/SourceFlow/issues/9542
	...
	if "a" == typ || "inline-memo" == typ || "block-ref" == typ || "file-annotation-ref" == typ || "inline-math" == typ {
		...
		if entering {
			...
			case "inline-math":
				...
				if node.ParentIs(ast.NodeTableCell) {
					// Improve the handling of inline-math containing `|` in the table https://github.com/lonelyor/SourceFlow/issues/9227
		...
		for i, typ := range types {
			...
			if entering {
				if "" != code && len(types)-2 == i { // 最内层是 code 时，需要在渲染 code 前添加零宽空格，然后再渲染 code 标记符
					ret += editor.Zwsp // Improve exporting inline code markdown element https://github.com/lonelyor/SourceFlow/issues/10988
				}
			}
...
func (r *ProtyleExportMdRenderer) renderLinkTitle(node *ast.Node, entering bool) ast.WalkStatus {
	if entering {
		r.WriteByte(lex.ItemDoublequote)
		// 这里不进行转义输出，仅转义双引号 https://github.com/lonelyor/SourceFlow/issues/15023
		tokens := node.Tokens
		tokens = bytes.ReplaceAll(tokens, []byte("\""), []byte("&quot;"))
		r.Write(tokens)
		r.WriteByte(lex.ItemDoublequote)
...
func (r *ProtyleExportMdRenderer) renderText(node *ast.Node, entering bool) ast.WalkStatus {
	if entering {
		// 去掉开头的零宽空格 Exported Markdown inline code no longer contains zero-width spaces after it https://github.com/lonelyor/SourceFlow/issues/15328
		// When exporting inline code to Markdown, try to remove the trailing zero-width space https://github.com/lonelyor/SourceFlow/issues/15854
		...
		// 导出 Markdown 时去掉图片节点左右的零宽空格 Remove zero-width spaces around image nodes when exporting Markdown https://github.com/lonelyor/SourceFlow/issues/14263
...
func (r *ProtyleExportMdRenderer) renderListItem(node *ast.Node, entering bool) ast.WalkStatus {
	if entering {
		...
		if bytes.HasPrefix(buf, []byte("* ")) {
			// 说明该列表项为空 https://github.com/lonelyor/SourceFlow/issues/6206
			buf = append([]byte(" \n\n"), buf...)
		}
		lines := bytes.Split(buf, []byte{lex.ItemNewline})
		for _, line := range lines {
