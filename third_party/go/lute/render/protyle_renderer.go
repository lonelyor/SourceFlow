	...
	if entering {
		...
		if node.IsTextMarkType("code") {
			if r.Options.Spellcheck {
				// Spell check should be disabled inside inline and block code https://github.com/lonelyor/SourceFlow/issues/9672
...
func (r *ProtyleRenderer) renderTextMarkAttrs(node *ast.Node) (attrs [][]string) {
	...
	for _, typ := range types {
		if "block-ref" == typ {
			...
			// 超链接元素地址中存在 `"` 字符时粘贴无法正常解析 https://github.com/lonelyor/SourceFlow/issues/11385
			href = strings.ReplaceAll(href, "\"", "&amp;quot;")

			attrs = append(attrs, []string{"data-href", href})
			if "" != node.TextMarkATitle {
				// 超链接元素标题中存在 `"` 字符时粘贴无法正常解析 https://github.com/lonelyor/SourceFlow/issues/5974
			...
			content = strings.ReplaceAll(content, editor.IALValEscNewLine, "\n")
			// Improve inline formulas input https://github.com/lonelyor/SourceFlow/issues/8972
