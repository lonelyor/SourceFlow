...
func (lute *Lute) adjustTag(n *html.Node) {
	...
	if html.ElementNode == n.Type && 0 == n.DataAtom {
		...
		if lute.ParseOptions.ProtyleWYSIWYG {
			// 将非表格内的文本节点中的 <br> 转换为 \n https://github.com/lonelyor/SourceFlow/issues/15373
...
func (lute *Lute) adjustTableCode(n *html.Node) {
	if atom.Table == n.DataAtom {
		// 表格类型的代码块进行预处理 https://github.com/lonelyor/SourceFlow/issues/11540
		// 移除 <td class="gutter">
		...
		for _, td := range tds {
			...
			// 移除 <span class="lnt"> 格式的行号 https://github.com/lonelyor/SourceFlow/issues/13242
...
func (lute *Lute) searchEmptyNodes(n *html.Node, emptyNodes *[]*html.Node) {
	...
	case 0:
		if lute.isInline(n.PrevSibling) || lute.isInline(n.NextSibling) || lute.isInline(n.Parent) {
			...
			if html.TextNode == n.Type && nil != n.PrevSibling && atom.Strong == n.PrevSibling.DataAtom {
				// 前节点是加粗节点的话去掉 ZWSP https://github.com/lonelyor/SourceFlow/issues/16896
		...
		if html.TextNode == n.Type {
			...
			for strings.Contains(data, "\n\n") {
				// 浏览器剪藏扩展列表下方段落缩进成为子块 https://github.com/lonelyor/SourceFlow/issues/6289
	...
	case atom.Span:
		if lc := n.LastChild; nil != lc && atom.Br == lc.DataAtom {
			...
			// 表格内多个连续的超链接无法换行显示 https://github.com/lonelyor/SourceFlow/issues/5966
			n.InsertAfter(lc)
		}
...
func (lute *Lute) adjustVditorDOMListItemInP(n *html.Node) {
	...
	case atom.Li:
		...
		if nil != n.PrevSibling && nil != n.FirstChild {
			id := util.DomAttrValue(n.FirstChild, "data-node-id") // id 为空的话是行级节点，列表项行级排版自动换行问题 https://github.com/lonelyor/SourceFlow/issues/379
...
func (lute *Lute) adjustVditorDOMCodeA(n *html.Node) {
	// https://github.com/lonelyor/SourceFlow/issues/11370
