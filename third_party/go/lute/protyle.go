...
func (lute *Lute) SpinBlockDOM(ivHTML string) (ovHTML string) {
	...
	// 使用 Markdown 标记符嵌套行级元素后被还原为纯文本 https://github.com/lonelyor/SourceFlow/issues/7637
...
func (lute *Lute) BlockDOM2Tree(htmlStr string) (ret *parse.Tree) {
	...
	// Improve `inline code` markdown editing https://github.com/lonelyor/SourceFlow/issues/9978
...
func (lute *Lute) genASTByBlockDOM(n *html.Node, tree *parse.Tree) {
	...
	// Custom dom, which will be omitted when build tree https://github.com/lonelyor/sourceflow/third_party/go/lute/issues/206
	...
	case ast.NodeBlockQueryEmbed:
		...
		// 嵌入块中存在换行 SQL 语句时会被转换为段落文本 https://github.com/lonelyor/SourceFlow/issues/5728
	...
	case ast.NodeHeading:
		...
		if strings.HasPrefix(tmp, "#") {
			// Allow changing headings with `#` https://github.com/lonelyor/SourceFlow/issues/7924
...
func (lute *Lute) genASTContenteditable(n *html.Node, tree *parse.Tree) {
	...
	case 0:
		...
		if lute.parentIs(n, atom.Table) {
			...
			if (nil == n.NextSibling && !strings.Contains(content, "\n")) /* 外部内容粘贴到表格中后编辑导致换行丢失  https://github.com/lonelyor/SourceFlow/issues/7501 */ ||
				(nil != n.NextSibling && atom.Br == n.NextSibling.DataAtom && strings.HasPrefix(content, "\n")) /* 表格内存在行级公式时编辑会产生换行 https://github.com/lonelyor/SourceFlow/issues/2279 */ {
				content = strings.ReplaceAll(content, "\n", "")
			}

			if strings.Contains(content, "\\") {
				// After entering `\` in the table, the next column is merged incorrectly https://github.com/lonelyor/SourceFlow/issues/7817
		...
		if ast.NodeCodeSpan == tree.Context.Tip.Type || ast.NodeInlineMath == tree.Context.Tip.Type {
			if nil != tree.Context.Tip.Previous && tree.Context.Tip.Type == tree.Context.Tip.Previous.Type { // 合并相邻的代码
				...
				if nil != tree.Context.Tip.FirstChild.Next.Next && ast.NodeBackslash == tree.Context.Tip.FirstChild.Next.Next.Type {
					// 表格单元格中使用代码和 `|` 的问题 https://github.com/lonelyor/SourceFlow/issues/4717
		...
		if ast.NodeKbd == tree.Context.Tip.Type {
			// `<kbd>` 中反斜杠转义问题 https://github.com/lonelyor/SourceFlow/issues/2242
			node.Tokens = bytes.ReplaceAll(node.Tokens, []byte("\\\\"), []byte("\\"))
			node.Tokens = bytes.ReplaceAll(node.Tokens, []byte("\\"), []byte("\\\\"))

			if bytes.Equal(node.Tokens, editor.CaretTokens) {
				// `<kbd>` 无法删除 https://github.com/lonelyor/SourceFlow/issues/4162
	...
	case atom.Span:
		...
		if strings.Contains(dataType, "img") {
			// 给文字和图片同时设置字体格式后图片丢失 https://github.com/lonelyor/SourceFlow/issues/6297
			dataType = "img"
		}

		if nil != tree.Context.Tip && nil != tree.Context.Tip.LastChild {
			// 行级元素前输入转义符 `\` 导致异常 https://github.com/lonelyor/SourceFlow/issues/6237
		...
		if "tag" == dataType {
			...
			if title := util.DomAttrValue(img, "title"); "" != title {
				...
				// < 和 > 符号不用转义，可以符合 Markdown 规范 https://github.com/lonelyor/SourceFlow/issues/15023
			...
			if n.FirstChild == n.LastChild && nil != n.FirstChild.FirstChild {
				// 转义字符加行级样式后继续输入会出现标记符 https://github.com/lonelyor/SourceFlow/issues/6134
				return
			}
	...
	case atom.Br:
		...
		if ast.NodeParagraph == tree.Context.Tip.Type {
			// 将硬换行转换为软换行 https://github.com/lonelyor/SourceFlow/issues/14481
			node.Type = ast.NodeText
			node.Tokens = []byte("\n")
			tree.Context.Tip.AppendChild(node)
			return
	...
	case atom.Strong, atom.B:
		...
		if nil != tree.Context.Tip.LastChild {
			// 转义符导致的行级元素样式属性暴露 https://github.com/lonelyor/SourceFlow/issues/2969
