// HttpClient - HTTP client for SourceFlow.
...
var (
	...
	sourceflowUserAgent = "SourceFlow/0.0.0"
)
...
func SetUserAgent(sourceflowUA string) {
	sourceflowUserAgent = sourceflowUA
}
...
func newCloudFileClient2m() {
	cloudFileClientTimeout2Min = req.C().
		EnableForceHTTP1(). // 强制使用 HTTP/1.1，避免有些服务器并发请求时报错 https://github.com/lonelyor/SourceFlow/issues/6948
		SetCommonHeader("Cache-Control", "no-cache, no-store, must-revalidate").
		SetCommonHeader("Pragma", "no-cache").
		SetCommonHeader("Expires", "0").
		SetUserAgent(sourceflowUserAgent).
