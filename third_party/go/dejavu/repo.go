...
import (
	...
	"github.com/lonelyor/sourceflow/third_party/go/dejavu/util"
...
// Repo 描述了逮虾户数据仓库。
type Repo struct {
	DataPath    string   // 数据文件夹的绝对路径，如：F:\\SourceFlow\\data\\
	Path        string   // 仓库的绝对路径，如：F:\\SourceFlow\\repo\\
	HistoryPath string   // 数据历史文件夹的绝对路径，如：F:\\SourceFlow\\history\\
	TempPath    string   // 临时文件夹的绝对路径，如：F:\\SourceFlow\\temp\\
...
var (
	...
	// Improve data snapshot and sync robustness https://github.com/lonelyor/SourceFlow/issues/9941
...
// Support manual purge of unreferenced data snapshots in the S3/WebDAV cloud storage https://github.com/lonelyor/SourceFlow/issues/10081
func (repo *Repo) PurgeCloud() (ret *entity.PurgeStat, err error) {
	...
	// 删除所有遗留的校验索引
	// S3/WebDAV 不上传校验索引 S3/WebDAV data sync no longer uploads check index https://github.com/lonelyor/SourceFlow/issues/10180
...
func (repo *Repo) index0(memo string, checkChunks bool, context map[string]interface{}) (ret *entity.Index, err error) {
	...
	err = filelock.Walk(repo.DataPath, func(path string, d fs.DirEntry, err error) error {
		if nil != err {
			if isNoSuchFileOrDirErr(err) {
				// An error `Failed to create data snapshot` is occasionally reported during automatic data sync https://github.com/lonelyor/SourceFlow/issues/8998
	...
	//}
	if 1 > len(files) {
		// 如果没有文件，则不创建快照 Abandon snapshot if file does not exist when creating snapshot https://github.com/lonelyor/SourceFlow/issues/9948
		err = ErrEmptyIndex
	...
	if nil != fullLatest {
		...
		if !init {
			...
			p, _ := ants.NewPoolWithFunc(4, func(arg interface{}) {
				...
				if checkChunks { // 仅在非移动端校验，因为移动端私有数据空间不会存在外部操作导致分块损坏的情况 https://github.com/lonelyor/SourceFlow/issues/13216
					// Check local data chunk integrity before data synchronization https://github.com/lonelyor/SourceFlow/issues/8853
					for _, chunk := range file.Chunks {
						...
						if errors.Is(statErr, os.ErrPermission) {
							...
							// Improve checking local data chunk integrity before data sync https://github.com/lonelyor/SourceFlow/issues/9688
...
func (repo *Repo) builtInIgnore(info os.FileInfo, absPath string) (ignored bool, err error) {
	...
	if info.IsDir() {
		...
		if "filesys_status_check" == name {
			// 数据同步忽略用于文件系统检查的文件 https://github.com/lonelyor/SourceFlow/issues/7744
			return true, filepath.SkipDir
		}
		return true, nil
	} else {
		...
		if strings.HasSuffix(slashAbsPath, "data/storage/local.json") {
			// localStorage 不再支持同步 https://github.com/lonelyor/SourceFlow/issues/6964
			return true, nil
		}
		if strings.HasSuffix(slashAbsPath, "data/storage/recent-doc.json") {
			// 数据同步忽略最近文档存储 https://github.com/lonelyor/SourceFlow/issues/7246
			return true, nil
		}
	}
...
func (repo *Repo) checkoutFiles(files []*entity.File, context map[string]interface{}) (err error) {
	...
	var hiddenDataFiles, assets, emojis, storage, plugins, widgets, templates, public, others, all []*entity.File
	for _, file := range files {
		if strings.Contains(file.Path, ".sourceflow") {
			hiddenDataFiles = append(hiddenDataFiles, file)
	...
	sort.Slice(hiddenDataFiles, func(i, j int) bool {
		return strings.Count(hiddenDataFiles[i].Path, "/") < strings.Count(hiddenDataFiles[j].Path, "/")
	})
	all = append(all, hiddenDataFiles...)
	hiddenDataFiles = nil
...
func (repo *Repo) isCloudSourceFlow() bool {
	if nil == repo.cloud {
		return false
	}

	switch repo.cloud.(type) {
	case *cloud.SourceFlow:
		return true
	default:
		return false
	}
