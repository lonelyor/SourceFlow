...
func (webdav *WebDAV) UploadBytes(filePath string, data []byte, overwrite bool) (length int64, err error) {
	...
	key := path.Join(webdav.Dir, "sourceflow", "repo", filePath)
	folder := path.Dir(key)
...
func (webdav *WebDAV) GetIndexes(page int) (ret []*entity.Index, pageCount, totalCount int, err error) {
	...
	repoKey := path.Join(webdav.Dir, "sourceflow", "repo")
	for i := start; i < end; i++ {
		...
		index.Files = nil // Optimize the performance of obtaining cloud snapshots https://github.com/lonelyor/SourceFlow/issues/8387
...
func (webdav *WebDAV) GetChunks(checkChunkIDs []string) (chunkIDs []string, err error) {
	repoObjects := path.Join(webdav.Dir, "sourceflow", "repo", "objects")
	var keys []string
...
func (webdav *WebDAV) ListObjects(pathPrefix string) (ret map[string]*entity.ObjectInfo, err error) {
	...
	pathPrefix = path.Join(webdav.Dir, "sourceflow", "repo", pathPrefix)
	if endWithSlash {
		pathPrefix += "/"
	}
...
func (webdav *WebDAV) listRepoRefs(refPrefix string) (ret []*Ref, err error) {
	keyPath := path.Join(webdav.Dir, "sourceflow", "repo", "refs", refPrefix)
