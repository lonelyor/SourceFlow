...
type MergeResult struct {
	...
	UpsertPetals []string // storage/petal/petals.json 中变更的插件，在源流中计算并填充
	RemovePetals []string // storage/petal/petals.json 中删除的插件，在源流中计算并填充
}
...
func (repo *Repo) sync0(context map[string]interface{},
	...
	// 避免旧的本地数据覆盖云端数据 https://github.com/lonelyor/SourceFlow/issues/7403
	...
	for _, upsert := range localUpserts {
		if "/.sourceflow/syncignore" == upsert.Path {
	...
	for _, cloudUpsert := range cloudUpserts {
		if "/.sourceflow/syncignore" == cloudUpsert.Path {
		...
		if nil == repo.getFile(localRemoves, cloudUpsert) {
			if strings.HasSuffix(cloudUpsert.Path, ".tmp") {
				// 数据仓库不迁出 `.tmp` 临时文件 https://github.com/lonelyor/SourceFlow/issues/7087
	...
	// 云端如果更新了忽略文件则使用其规则过滤 remove，避免后面误删本地文件 https://github.com/lonelyor/SourceFlow/issues/5497
...
func (repo *Repo) ignoreLocalUpsert(localUpsert *entity.File, latestSyncFiles []*entity.File, now string, context map[string]interface{}) bool {
	if !strings.HasSuffix(localUpsert.Path, ".sf") {
		return false // 非 .sf 文件目前不做内容对比，直接认为本地 upsert 是最新的
	}
...
func (repo *Repo) updateCloudIndexes(latest *entity.Index, trafficStat *TrafficStat, context map[string]interface{}) (err error) {
	...
	go func() {
		...
		// 上传索引和更新 refs/latest 两个操作需要保证顺序，否则可能会导致云端索引 和 refs/latest 不一致 https://github.com/lonelyor/SourceFlow/issues/10111
	...
	isS3OrSourceFlow := repo.isCloudS3() || repo.isCloudSourceFlow()
	if isS3OrSourceFlow {
		// 上传最新索引列表 https://github.com/lonelyor/SourceFlow/issues/12991
...
// filterLocalUpserts 避免旧的本地数据覆盖云端数据 https://github.com/lonelyor/SourceFlow/issues/7403
...
func (repo *Repo) uploadCloudMissingObjects(trafficStat *TrafficStat, context map[string]interface{}) {
	...
	if _, ok := repo.cloud.(*cloud.SourceFlow); !ok {
		return
	}
...
func (repo *Repo) updateCloudIndexesV2(latest *entity.Index, context map[string]interface{}) (downloadBytes, uploadBytes int64, err error) {
	...
	if 0 < len(data) {
		...
		// Deduplication when uploading cloud snapshot indexes https://github.com/lonelyor/SourceFlow/issues/8424
		found := false
		tmp := &cloud.Indexes{}
...
func (repo *Repo) downloadCloudLatest(context map[string]interface{}) (downloadBytes int64, index *entity.Index, err error) {
	...
	go func() {
		...
		if isS3OrSourceFlow {
			// 确认下载到的是最新索引 https://github.com/lonelyor/SourceFlow/issues/12991
			seqNumLatestID, _, _ = repo.getSeqNumLatest()
		}
	}()
	waitGroup.Wait()

	if isS3OrSourceFlow && ("" != seqNumLatestID && "" != index.ID && latestID != seqNumLatestID) {
